...
internal object SourceParser {
    fun parse(dir: File): CdkSourceModel {
        ...
        val builderMap = sourceClasses.filter { it.className.simpleName == "Builder" }.associateBy { it.className }
    ...
    private fun processType(type: TypeDeclaration<*>): List<CdkSourceClass> {
        ...
        list.addAll(type.members.filterIsInstance<TypeDeclaration<*>>().filter { !it.name.asString().contains("Jsii") }.flatMap {
            processType(it)
        })
        return list
    }
    ...
    private fun convertTypeDeclaration(type: TypeDeclaration<*>): CdkSourceClass {
        val methods = when (type.name.identifier) {
            "Builder" -> type.methods.filter {
                ...
                val comment = it.comment.getOrNull()?.let { convertJavadocComment(it.content) }
                CdkSourceMethod(name = it.name.identifier, type = type, comment = comment)
            }
    ...
    private val TypeDeclaration<*>.className: ClassName
        get() {
            ...
            return ClassName(findCompilationUnit().get().packageDeclaration.get().name.asString(), names)
        }

    private fun convertJavadocComment(comment: String): String {
        return comment.lines().asSequence().map { it.trim() }.map { it.removePrefix("* ") }.dropWhile { it.isBlank() }.map { it.trim() }.filter { it !in javaDocLinesToRemove }
            .map { it.replace("This parameter is required.", "") }.map {
                it.replace(seeAlsoRegex) {
                    val (url, label) = it.destructured
                    "[Documentation]($url)"
                }
            }.map { it.replace("<code>", "`").replace("</code>", "`") }.map { it.replace("<strong>", "**").replace("</strong>", "**") }.map { it.replace("<em>", "*").replace("</em>", "*") }
            .map { it.replace(aHrefRegex) { "[${it.groupValues[2]}](${it.groupValues[1]})" } }.map { it.replace("<p>", "") }.map { it.replace("<li>", "* ") }.map { it.replace("</li>", "") }
            .map { it.replace(linkRegex) { "[${it.groupValues[1]}]" } }.map { it.replace("<blockquote>", "") }.map { it.replace("</blockquote>", "") }.map { it.replace("<pre>", "```") }
            .map{ it.replace("/*", "/ *")}
            .map { it.replace("</pre>", "```") }.map { it.replace("<ul>", "") }.map { it.replace("</ul>", "") }.filter { !it.startsWith("Sets the value of ") }.joinToString("\n")
    }
    ...
    private val javaDocLinesToRemove = setOf("@return {@code this}", "<ul>", "</ul>", "<blockquote>", "</blockquote>", "@deprecated <ul>")
}