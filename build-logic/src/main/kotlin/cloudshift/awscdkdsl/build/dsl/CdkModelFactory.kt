...
internal object CdkModelFactory {
    fun createModel(cdkClasses: List<CdkClass>): CdkModel {
        ...
        val builderClasses =
            cdkClasses.filter { cdkClass -> cdkClass.isBuilder() }.sortedBy { it.className }

        // map of builder classname to buildable class
        val buildersToBuildables =
            builderClasses.associate { builderClass ->
                val buildMethod =
                    builderClass.publicMemberFunctions.single {
                        it.name == "build" && it.parameters.isEmpty()
                    }
                val returnType = buildMethod.returnType
                builderClass.className to
                    (classMap[returnType] ?: error("Unable to resolve $returnType"))
            }

        // map of buildable classname to builder class
        val buildablesToBuilders =
            builderClasses.associateBy { builderClass ->
                val buildMethod =
                    builderClass.publicMemberFunctions.single {
                        it.name == "build" && it.parameters.isEmpty()
                    }
        ...
        val builders =
            builderClasses.map { builderClass ->
                val buildableClass =
                    buildersToBuildables[builderClass.className]
                        ?: error("Cannot find buildable for ${builderClass.className}")
                val builderFactoryFunction =
                    findBuilderFactoryFunction(builderClass, buildableClass)
        ...
        return CdkModel(
            classes = cdkClasses,
            builders = builders,
            buildablesToBuilders = buildablesToBuilders
        )
    }
}

private fun findBuilderFactoryFunction(
    builderClass: CdkClass,
    buildableClass: CdkClass
...
private val builderFactoryFunctionLocators =
    listOf(
...
private fun findBuilderProperties(
    ...
    val candidates =
        builderClass.publicMemberFunctions.filter {
            it.name !in excludedMembers && it.parameters.isNotEmpty()
        }

    require(candidates.all { it.parameters.size == 1 }) {
        "Expected builder methods with a single parameter"
    }
...
internal fun ClassName.dslClassName(): ClassName {
    val newName =
        "${
        simpleNames.filterNot { it == "Builder" }
            .joinToString("")
    }Dsl"
