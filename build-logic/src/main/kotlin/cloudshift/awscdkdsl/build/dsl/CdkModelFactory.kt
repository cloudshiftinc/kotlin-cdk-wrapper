...
internal object CdkModelFactory {
    fun createModel(cdkClasses: List<CdkClass>): CdkModel {
        ...
        val builderClasses = cdkClasses.filter { cdkClass -> cdkClass.isBuilder() }
        ...
        val builders = builderClasses.map { builderClass ->
            val buildableClass = buildersToBuildables[builderClass.className] ?: error("Cannot find buildable for ${builderClass.className}")
...
private fun findBuilderFactoryFunction(builderClass: CdkClass, buildableClass: CdkClass): BuilderFactoryFunction {
    return builderFactoryFunctionLocators.firstNotNullOfOrNull {
        it(builderClass, buildableClass)
...
private val builderFactoryFunctionLocators = listOf(
    ...
    { _: CdkClass, buildableClass: CdkClass ->
        buildableClass.publicStaticFunctions.filter { it.name == "builder" }
            .map {
    ...
    { builderClass: CdkClass, _: CdkClass ->
        builderClass.publicStaticFunctions.filter { it.name == "create" }
            .map {
                BuilderFactoryFunction(
...
private fun findBuilderProperties(builderClass: CdkClass, buildersForBuildable: Map<ClassName, CdkClass>): List<BuilderProperty> {
    val candidates = builderClass.publicMemberFunctions.filter { it.name !in excludedMembers && it.parameters.isNotEmpty() }
