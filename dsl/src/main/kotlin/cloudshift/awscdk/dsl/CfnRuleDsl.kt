 ...
 * users can use the template and which values they can specify for parameters that are declared in
 * the AWS CloudFormation template used to create the product they are attempting to use.
 *
 * Rules are useful for preventing end users from inadvertently specifying an incorrect value. For
 * example, you can add a rule to verify whether end users specified a valid subnet in a given VPC
 * or used m1.small instance types for test environments. AWS CloudFormation uses rules to validate
 ...
 * Assertions property. You define a rule condition and assertions by using rule-specific intrinsic
 * functions.
 *
 * Example:
 * ```
