...
public class CfnUpdatePolicyDsl {
     ...
     *   it contains are replaced during an update. During replacement, AWS CloudFormation retains
     *   the old group until it finishes creating the new one. If the update fails, AWS
     *   CloudFormation can roll back to the old Auto Scaling group and delete the new Auto Scaling
     *   group.
     */
     ...
     *   an Auto Scaling group, use the AutoScalingRollingUpdate policy. Rolling updates enable you
     *   to specify whether AWS CloudFormation updates instances that are in an Auto Scaling group
     ...
     *   MinSize, MaxSize, and DesiredCapacity properties when the
     ...
     *   changes on an AWS::Lambda::Alias resource, use the CodeDeployLambdaAliasUpdate update
     *   policy.
     */
     ...
     *   shards, rather than replacing the entire AWS::ElastiCache::ReplicationGroup resource, use
     *   the UseOnlineResharding update policy.
     */
