...
public class CfnAutoScalingRollingUpdateDsl {
     ...
     *   rolling update that must signal success for an update to succeed. You can specify a value
     *   from 0 to 100. AWS CloudFormation rounds to the nearest tenth of a percent. For example, if
     *   you update five instances with a minimum successful percentage of 50, three instances must
     *   signal success.
     *
     ...
     *   batch of instances to give those instances time to start software applications. For
     *   example, you might need to specify PauseTime when scaling up the number of instances in an
     *   Auto Scaling group.
     *
     ...
     *   For example, you can suspend alarming so that Auto Scaling doesn't execute scaling policies
     *   associated with an alarm. For valid values, see the ScalingProcesses.member.N parameter for
     *   the SuspendProcesses action in the Auto Scaling API Reference.
     */
     ...
     *   new instances during an update. Use this property to ensure that instances have completed
     *   installing and configuring applications before the Auto Scaling group update proceeds. AWS
     *   CloudFormation suspends the update of an Auto Scaling group after new EC2 instances are
     *   launched into the group. AWS CloudFormation must receive a signal from each new instance
     *   within the specified PauseTime before continuing the update. To signal the Auto Scaling
     *   group, use the cfn-signal helper script or SignalResource API.
     *
