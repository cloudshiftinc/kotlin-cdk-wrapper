...
public class FunctionAttributesDsl {
     ...
     *   the same environment as the stack. This affects certain behaviours such as, whether this
     *   function's permission can be modified. When not configured, the CDK attempts to
     *   auto-determine this. For environment agnostic stacks, i.e., stacks where the account is not
     *   specified with the `env` property, this is determined to be false.
     *
    ...
    /**
     * @param securityGroup The security group of this Lambda, if in a VPC. This needs to be given
     *   in order to support allowing connections to this Lambda.
     */
     ...
     *   ALREADY HAS the necessary permissions for what you are trying to do. When not configured,
     *   the CDK attempts to auto-determine whether or not additional permissions are necessary on
     *   the function when grant APIs are used. If the CDK tried to add permissions on an imported
     *   lambda, it will fail.
     *
