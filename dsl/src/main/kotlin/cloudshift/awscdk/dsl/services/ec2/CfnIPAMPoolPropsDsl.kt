...
public class CfnIPAMPoolPropsDsl {
     ...
     *   this IPAM pool to be compliant. The maximum netmask length must be greater than the minimum
     ...
     *   this IPAM pool to be compliant. The minimum netmask length must be less than the maximum
     *   netmask length. Possible netmask lengths for IPv4 addresses are 0 - 32. Possible netmask
     *   lengths for IPv6 addresses are 0 - 128.
     */
    ...
    /**
     * @param awsService Limits which service in AWS that the pool can be used in. "ec2", for
    ...
    /**
     * @param locale The locale of the IPAM pool. In IPAM, the locale is the AWS Region where you
     *   want to make an IPAM pool available for allocations. Only resources in the same Region as
     *   the locale of the pool can get IP address allocations from the pool. You can only allocate
     *   a CIDR for a VPC, for example, from an IPAM pool that shares a locale with the VPC’s
     *   Region. Note that once you choose a Locale for a pool, you cannot modify it. If you choose
     *   an AWS Region for locale that has not been configured as an operating Region for the IPAM,
     *   you'll get an error.
     */
    ...
    /**
     * @param publiclyAdvertisable Determines if a pool is publicly advertisable. This option is not
    ...
    /**
     * @param sourceIpamPoolId The ID of the source IPAM pool. You can use this option to create an
