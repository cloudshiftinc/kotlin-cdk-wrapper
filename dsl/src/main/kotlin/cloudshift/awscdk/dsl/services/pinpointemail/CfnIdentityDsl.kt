 ...
 * Before you can use Amazon Pinpoint to send an email from an identity, you first have to verify
 * it. By verifying an identity, you demonstrate that you're the owner of the address or domain, and
 * that you've given Amazon Pinpoint permission to send email from that identity.
 *
 ...
 * domain is verified when Amazon Pinpoint detects these records in the DNS configuration for your
 * domain. It usually takes around 72 hours to complete the domain verification process.
 *
 * When you use CloudFormation to specify an identity, CloudFormation might indicate that the
 * identity was created successfully. However, you have to verify the identity before you can use it
 * to send email.
 *
 * Example:
 * ```
...
public class CfnIdentityDsl(
     ...
     * both the DKIM keys for your domain, as well as the keys for the `amazonses.com` domain. If
     * the value is `false` , then the messages that you send are only signed using the DKIM keys
     * for the `amazonses.com` domain.
     *
