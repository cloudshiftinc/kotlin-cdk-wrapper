...
public class CfnByteMatchSetByteMatchTuplePropertyDsl {
   ...
   * example, in the query string, if any), specify where you want AWS WAF to search. 
   ...
   * * `TargetString` is in the middle of the specified part of the web request and is preceded and
   ...
   * * `QUERY_STRING` : The value that you want AWS WAF to search for in the query string, which is
   * the part of a URL that appears after a `?` character.
   * * `URI` : The value that you want AWS WAF to search for in the part of a URL that identifies a
