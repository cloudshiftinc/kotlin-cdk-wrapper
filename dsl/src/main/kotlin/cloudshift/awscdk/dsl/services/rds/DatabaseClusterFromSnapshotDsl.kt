...
public class DatabaseClusterFromSnapshotDsl(
     ...
     * Backup settings.
     *
     * Default: - Backup retention period for automated backups is 1 day. Backup preferred window is
     * set to a 30-minute window selected at random from an 8-hour block of time for each AWS
     ...
     *
     * Note - using this prop only works with `Credentials.fromPassword()` with the username of the
     * snapshot, `Credentials.fromUsername()` with the username and password of the snapshot or
     * `Credentials.fromSecret()` with a secret containing the username and password of the
     * snapshot.
     *
     * Default: - A username of 'admin' (or 'postgres' for PostgreSQL) and SecretsManager-generated
     * password that **will not be applied** to the cluster, use `snapshotCredentials` for the
     * correct behavior.
     *
     ...
     * deletion protection. To disable deletion protection after it has been enabled, you must
     * explicitly set this value to `false`.
     *
     ...
     *
     * Default: - clusterIdentifier is used with the word "Instance" appended. If clusterIdentifier
