...
public object simspaceweaver {
     ...
     * configured with the proper permissions, and the name of an existing Amazon S3 bucket. Your
     * account must have permissions to read the Amazon S3 bucket. The Amazon S3 bucket must contain
     * a valid schema. The schema must refer to simulation assets that are already uploaded to the
     * AWS Cloud . For more information, see the
     * [detailed tutorial](https://docs.aws.amazon.com/simspaceweaver/latest/userguide/getting-started_detailed.html)
     * in the *AWS SimSpace Weaver User Guide* .
     *
     * Specify a `SnapshotS3Location` to start a simulation from a snapshot instead of from a
     * schema. When you start a simulation from a snapshot, SimSpace Weaver initializes the entity
     * data in the State Fabric with data saved in the snapshot, starts the spatial and service apps
     * that were running when the snapshot was created, and restores the clock to the appropriate
     * tick. Your app zip files must be in the same location in Amazon S3 as they were in for the
     * original simulation. You must start any custom apps separately. For more information about
     * snapshots, see
     ...
     * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-simspaceweaver-simulation.html)
     */
    public inline fun cfnSimulationProps(
        block: CfnSimulationPropsDsl.() -> Unit = {}
    ): CfnSimulationProps {
     ...
     * simulation data, such as your app .zip files and schema file. For more information about
     * Amazon S3 , see the
     ...
     */
    public inline fun cfnSimulationS3LocationProperty(
