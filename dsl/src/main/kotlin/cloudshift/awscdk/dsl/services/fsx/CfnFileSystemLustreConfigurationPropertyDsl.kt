...
public class CfnFileSystemLustreConfigurationPropertyDsl {
   ...
   * appear as file and directory listings.
   * Use this property to choose how Amazon FSx keeps your file and directory listings up to date as
   ...
   * listings of any new objects added to the S3 bucket, any existing objects that are changed in the
   * S3 bucket, and any objects that were deleted in the S3 bucket.
   *
   ...
   * Setting this property to `0` disables automatic backups. You can retain automatic backups for a
   ...
   * system, regardless of this value. Only valid for use with `PERSISTENT_1` deployment types.
   */
   ...
   * For example, `05:00` specifies 5 AM daily.
   */
   ...
   * need temporary storage and shorter-term processing of data.
   ...
   * `PerUnitStorageThroughput` (up to 1000 MB/s/TiB). `PERSISTENT_2` is available in a limited number
   * of AWS Regions . For more information, and an up-to-date list of AWS Regions in which
   * `PERSISTENT_2` is available, see [File system deployment options for FSx for
   ...
   * This parameter is required when storage type is HDD. Set this property to `READ` to improve the
   * performance for frequently accessed files by caching up to 20% of the total storage capacity of
   * the file system.
   *
  ...
  /**
   * @param exportPath (Optional) Specifies the path in the Amazon S3 bucket where the root of your
   * Amazon FSx file system is exported.
   ...
   * that you're using as the data repository for your Amazon FSx for Lustre file system.
   * The root of your FSx for Lustre file system will be mapped to the root of the Amazon S3 bucket
   * you select. An example is `s3://import-bucket/optional-prefix` . If you specify a prefix after the
   * Amazon S3 bucket name, only object keys with that prefix are loaded into the file system.
   *
   *
   * This parameter is not supported for Lustre file systems with a data repository association.
   */
   ...
   * types, provisions the amount of read and write throughput for each 1 tebibyte (TiB) of file system
   * storage capacity, in MB/s/TiB.
   * File system throughput capacity is calculated by multiplying ﬁle system storage capacity (TiB)
   * by the `PerUnitStorageThroughput` (MB/s/TiB). For a 2.4-TiB ﬁle system, provisioning 50 MB/s/TiB
   * of `PerUnitStorageThroughput` yields 120 MB/s of ﬁle system throughput. You pay for the amount of
   ...
   * `D` is the day of the week, for which 1 represents Monday and 7 represents Sunday. For further
   * details, see [the ISO-8601 spec as described on
