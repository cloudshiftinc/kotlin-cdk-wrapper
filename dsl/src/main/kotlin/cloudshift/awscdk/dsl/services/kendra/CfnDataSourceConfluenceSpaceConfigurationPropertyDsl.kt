...
public class CfnDataSourceConfluenceSpaceConfigurationPropertyDsl {
   ...
   * If you include a key, the blogs, documents, and attachments in the space are not indexed. If a
   * space is in both the `ExcludeSpaces` and the `IncludeSpaces` list, the space is excluded.
   */
   ...
   * logs an error when the data source is synchronized. If a space is in both the `IncludeSpaces` and
   * the `ExcludeSpaces` list, the space is excluded.
   */
  ...
  /**
   * @param spaceFieldMappings Maps attributes or field names of Confluence spaces to Amazon Kendra
   * index field names.
