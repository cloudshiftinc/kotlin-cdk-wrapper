...
public class CfnTargetGroupTargetDescriptionPropertyDsl {
    ...
    /**
     * @param availabilityZone An Availability Zone or `all` . This determines whether the target
     *   receives traffic from the load balancer nodes in the specified Availability Zone or from
     ...
     *   specify an instance ID. If the target type is `ip` , specify an IP address. If the target
     *   type is `lambda` , specify the ARN of the Lambda function. If the target type is `alb` ,
    ...
    /**
     * @param port The port on which the target is listening. If the target group protocol is
     *   GENEVE, the supported port is 6081. If the target type is `alb` , the targeted Application
     *   Load Balancer must have at least one listener whose port matches the target group port.
