 ...
 * an exception to it. For example, if you set the `CacheBehavior` to `cache` , you can use a
 * per-path cache behavior to specify a directory, file, or file type that your distribution will
 * cache. If you don’t want your distribution to cache a specified directory, file, or file type,
 * set the per-path cache behavior to `dont-cache` .
 *
 * Example:
 * ```
...
public class CfnDistributionCacheBehaviorPerPathPropertyDsl {
    ...
    /**
     * @param path The path to a directory or file to cache, or not cache. Use an asterisk symbol to
