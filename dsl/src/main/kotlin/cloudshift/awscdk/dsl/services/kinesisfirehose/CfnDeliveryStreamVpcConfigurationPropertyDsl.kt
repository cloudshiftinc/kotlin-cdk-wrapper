...
public class CfnDeliveryStreamVpcConfigurationPropertyDsl {
   ...
   * You can use your existing Kinesis Data Firehose delivery role or you can specify a new role. In
   * either case, make sure that the role trusts the Kinesis Data Firehose service principal and that
   ...
   * use when it creates ENIs in the VPC of the Amazon ES destination. 
   ...
   * HTTPS traffic from the security groups specified here. If you use the same security group for both
   * your delivery stream and the Amazon ES domain, make sure the security group inbound rule allows
   * HTTPS traffic.
   */
   ...
   * VPC of the Amazon ES destination. 
   * Make sure that the routing tables and inbound and outbound rules allow traffic to flow from the
   * subnets whose IDs are specified here to the subnets that have the destination Amazon ES endpoints.
   * Kinesis Data Firehose creates at least one ENI in each of the subnets that are specified here. Do
   * not delete or modify these ENIs.
   *
   * The number of ENIs that Kinesis Data Firehose creates in the subnets specified here scales up
   * and down automatically based on throughput. To enable Kinesis Data Firehose to scale up the number
   * of ENIs to match throughput, ensure that you have sufficient quota. To help you calculate the
   * quota you need, assume that Kinesis Data Firehose can create up to three ENIs for this delivery
   * stream for each of the subnets specified here.
   */
