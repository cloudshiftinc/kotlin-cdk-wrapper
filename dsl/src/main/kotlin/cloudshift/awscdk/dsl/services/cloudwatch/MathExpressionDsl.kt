 ...
 * expose.
 *
 * MathExpression can also be used for search expressions. In this case, it also optionally accepts
 * a searchRegion and searchAccount property for cross-environment search expressions.
 *
 * This class does not represent a resource, so hence is not a construct. Instead, MathExpression is
 * an abstraction that makes it easy to specify metrics for use in both alarms and graphs.
 *
 * Example:
 * ```
 * Function fn;
...
public class MathExpressionDsl {
     ...
     * combination of the expression label and the individual metric label. Specify the empty string
     * (`''`) to suppress the expression label and only keep the metric label.
     *
     * You can use
     ...
     *
     * Specifying a searchAccount has no effect to the account used for metrics within the
     * expression (passed via usingMetrics).
     *
     ...
     *
     * Specifying a searchRegion has no effect to the region used for metrics within the expression
     * (passed via usingMetrics).
     *
     ...
     *
     * The key is the identifier that represents the given metric in the expression, and the value
     * is the actual Metric object.
     *
     * Default: - Empty map.
     *
