...
public class DnsValidatedCertificatePropsDsl {
  ...
  /**
   * @param cleanupRoute53Records When set to true, when the DnsValidatedCertificate is deleted, the
   * associated Route53 validation records are removed.
  ...
  /**
   * @param route53Endpoint An endpoint of Route53 service, which is not necessary as AWS SDK could
   * figure out the right endpoints for most regions, but for some regions such as those in aws-cn
   * partition, the default endpoint is not working now, hence the right endpoint need to be specified
   * through this prop.
