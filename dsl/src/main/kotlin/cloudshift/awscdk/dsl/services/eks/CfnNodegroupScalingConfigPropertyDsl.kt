...
public class CfnNodegroupScalingConfigPropertyDsl {
   ...
   *
   * If you use Cluster Autoscaler, you shouldn't change the desiredSize value directly, as this can
   * cause the Cluster Autoscaler to suddenly scale up or scale down.
   *
   *
   * Whenever this parameter changes, the number of worker nodes in the node group is updated to the
   * specified size. If this parameter is given a value that is smaller than the current number of
   * running worker nodes, the necessary number of worker nodes are terminated to match the given
   ...
   * hosts for testing. This parameter can also be different when you want to start with an estimated
   * number of needed hosts, but let Cluster Autoscaler reduce the number if there are too many. When
   * Cluster Autoscaler is used, the desiredSize parameter is altered by Cluster Autoscaler (but can be
   * out-of-date for short periods of time). Cluster Autoscaler doesn't scale a managed node group
