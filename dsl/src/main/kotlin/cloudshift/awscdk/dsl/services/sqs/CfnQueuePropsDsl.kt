...
public class CfnQueuePropsDsl {
     ...
     *   enable content-based deduplication. During the deduplication interval, Amazon SQS treats
     *   messages that are sent with identical content as duplicates and delivers only one copy of
     *   the message. For more information, see the `ContentBasedDeduplication` attribute for the
     ...
     *   deduplication occurs at the message group or queue level. Valid values are `messageGroup`
     *   and `queue` .
     *
     ...
     *   is delayed. You can specify an integer value of `0` to `900` (15 minutes). The default
     *   value is `0` .
     */
    ...
    /**
     * @param fifoQueue If set to true, creates a FIFO queue. If you don't specify this property,
     *   Amazon SQS creates a standard queue. For more information, see
     ...
     *   queue throughput quota applies to the entire queue or per message group. Valid values are
     ...
     *   reuse a data key to encrypt or decrypt messages before calling AWS KMS again. The value
     *   must be an integer between 60 (1 minute) and 86,400 (24 hours). The default is 300 (5
     *   minutes).
     *
     ...
     *   custom KMS. To use the AWS managed KMS for Amazon SQS , specify a (default) alias ARN,
     *   alias name (e.g. `alias/aws/sqs` ), key ARN, or key ID. For more information, see the
     *   following:
     * * [Encryption at
     ...
     *   Amazon SQS rejects it. You can specify an integer value from `1,024` bytes (1 KiB) to
    ...
    /**
     * @param messageRetentionPeriod The number of seconds that Amazon SQS retains a message. You
     *   can specify an integer value from `60` seconds (1 minute) to `1,209,600` seconds (14 days).
    ...
    /**
     * @param queueName A name for the queue. To create a FIFO queue, the name of your FIFO queue
     *   must end with the `.fifo` suffix. For more information, see
     *   [FIFO queues](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/FIFO-queues.html)
     *   in the *Amazon SQS Developer Guide* .
     *
     * If you don't specify a name, AWS CloudFormation generates a unique physical ID and uses that
     * ID for the queue name. For more information, see
     ...
     *   ReceiveMessage action call waits until a message is in the queue in order to include it in
     *   the response, rather than returning an empty response if a message isn't yet available. You
     *   can specify an integer from 1 to 20. Short polling is used as the default or when you
     ...
     */
    public fun redriveAllowPolicy(redriveAllowPolicy: MapBuilder.() -> Unit = {}) {
     ...
     *   keys. Only one server-side encryption option is supported per queue (for example,
    ...
    /**
     * @param tags The tags that you attach to this queue. For more information, see
     ...
     *   a message is delivered from the queue. This blocks other components from receiving the same
     *   message and gives the initial component time to process and delete the message from the
     *   queue.
     *
