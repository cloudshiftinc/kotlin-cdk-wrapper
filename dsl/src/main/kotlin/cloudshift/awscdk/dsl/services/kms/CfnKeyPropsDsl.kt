...
public class CfnKeyPropsDsl {
     ...
     *   distinguish this KMS key from others in the account, such as its intended use.
     */
     ...
     *   key. By default, automatic key rotation is not enabled.
     *
     ...
     *   cryptographic operations. When `Enabled` is `true` , the *key state* of the KMS key is
     *   `Enabled` . When `Enabled` is `false` , the key state of the KMS key is `Disabled` . The
     ...
     *   decryption. In China Regions, `SYMMETRIC_DEFAULT` creates a 128-bit symmetric key that uses
     *   SM4 encryption. You can't change the `KeySpec` value after the KMS key is created. For help
     ...
     *   Regions . You can't change the `MultiRegion` value after the KMS key is created.
     *
     ...
     *   deletes a KMS key that has been removed from a CloudFormation stack. Enter a value between
     *   7 and 30 days. The default value is 30 days.
     *
