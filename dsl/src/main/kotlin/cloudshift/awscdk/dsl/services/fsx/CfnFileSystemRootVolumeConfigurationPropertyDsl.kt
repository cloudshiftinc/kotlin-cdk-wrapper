...
public class CfnFileSystemRootVolumeConfigurationPropertyDsl {
     ...
     *   copied to snapshots of the volume. This value defaults to `false` . If it's set to `true` ,
     *   all tags for the volume are copied to snapshots where the user doesn't specify tags. If
     *   this value is `true` and you specify one or more tags, only the specified tags are copied
     *   to snapshots. If you specify one or more tags when creating the snapshot, no tags are
    ...
    /**
     * @param readOnly A Boolean value indicating whether the volume is read-only. Setting this
     *   value to `true` can be useful after you have completed changes to a volume and no longer
