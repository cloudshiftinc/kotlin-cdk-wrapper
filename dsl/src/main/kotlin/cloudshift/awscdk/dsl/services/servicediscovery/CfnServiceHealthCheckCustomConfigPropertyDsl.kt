 ...
 *   available over the internet. For example, you can use a custom health check when the instance
 *   is in an Amazon VPC. (To check the health of resources in a VPC, the health checker must also
 *   be in the VPC.)
 ...
 * * If another `UpdateInstanceCustomHealthStatus` request doesn't arrive during that time to change
 *   the status back to healthy, AWS Cloud Map stops routing traffic to the resource.
 *
 * Example:
 * ```
...
public class CfnServiceHealthCheckCustomConfigPropertyDsl {
    ...
    /**
     * @param failureThreshold This parameter is no longer supported and is always set to 1. AWS
