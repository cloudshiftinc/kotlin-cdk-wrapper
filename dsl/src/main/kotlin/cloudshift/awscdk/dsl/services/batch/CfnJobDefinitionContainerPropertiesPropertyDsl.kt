...
public class CfnJobDefinitionContainerPropertiesPropertyDsl {
    ...
    /**
     * @param ephemeralStorage The amount of ephemeral storage to allocate for the task. This
     ...
     *   can assume. For jobs that run on Fargate resources, you must provide an execution role. For
     ...
     *   Fargate resources. Jobs that are running on EC2 resources must not specify this parameter.
     */
    ...
    public fun fargatePlatformConfiguration(
        fargatePlatformConfiguration: CfnJobDefinition.FargatePlatformConfigurationProperty
    ) {
        cdkBuilder.fargatePlatformConfiguration(fargatePlatformConfiguration)
    }
    ...
    /**
     * @param instanceType The instance type to use for a multi-node parallel job. All node groups
     ...
     *   resources. For jobs that run on EC2 resources, it specifies the memory hard limit (in MiB)
     *   for a container. If your container attempts to exceed the specified number, it's
     *   terminated. You must specify at least 4 MiB of memory for a job using this parameter. The
     *   memory hard limit can be specified in several places. It must be specified for each node at
     *   least once.
     */
     ...
     *   resources. Jobs that are running on EC2 resources must not specify this parameter.
     */
    ...
    public fun networkConfiguration(
        networkConfiguration: CfnJobDefinition.NetworkConfigurationProperty
    ) {
        cdkBuilder.networkConfiguration(networkConfiguration)
    }
    ...
    /**
     * @param resourceRequirements The type and amount of resources to assign to a container. The
     ...
     *   requirements for the job definition. It's not supported for jobs running on Fargate
     *   resources. For jobs running on EC2 resources, it specifies the number of vCPUs reserved for
     *   the job.
     *
     ...
     * [docker run](https://docs.aws.amazon.com/https://docs.docker.com/engine/reference/run/) . The
     * number of vCPUs must be specified but can be specified in several places. You must specify it
