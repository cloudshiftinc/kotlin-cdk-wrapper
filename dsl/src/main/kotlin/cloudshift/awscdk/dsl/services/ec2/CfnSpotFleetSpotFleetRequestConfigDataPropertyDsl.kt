...
public class CfnSpotFleetSpotFleetRequestConfigDataPropertyDsl {
     ...
     *   capacity availability for the number of instances that are launching. This means that we
     *   will request Spot Instances from the pools that we believe have the lowest chance of
     *   interruption in the near term. Spot Fleet then requests Spot Instances from the lowest
     ...
     *   interruption in the near term. To give certain instance types a higher chance of launching
     *   first, use `capacityOptimizedPrioritized` . Set a priority for each instance type by using
     *   the `Priority` parameter for `LaunchTemplateOverrides` . You can assign the same priority
     *   to different `LaunchTemplateOverrides` . EC2 implements the priorities on a best-effort
     *   basis, but optimizes for capacity first. `capacityOptimizedPrioritized` is supported only
     *   if your Spot Fleet uses a launch template. Note that if the `OnDemandAllocationStrategy` is
     *   set to `prioritized` , the same priority is applied when fulfilling On-Demand capacity.
     ...
     *   desired capacity is met, you might receive Spot Instances from several pools. Because this
     *   strategy only considers instance price and not capacity availability, it might lead to high
     *   interruption rates.
     *
     * Default: `lowestPrice`
     */
     ...
     *   instances on your behalf. For more information, see
    ...
    /**
     * @param instanceInterruptionBehavior The behavior when a Spot Instance is interrupted. The
     ...
     *   Spot capacity. Valid only when Spot *AllocationStrategy* is set to `lowest-price` . Spot
     *   Fleet selects the cheapest Spot pools and evenly allocates your target Spot capacity across
     ...
     * from more than the number of pools that you specified. Similarly, if most of the pools have
     * no Spot capacity, you might receive your full target capacity from fewer than the number of
    ...
    /**
     * @param launchSpecifications The launch specifications for the Spot Fleet request. If you
     *   specify `LaunchSpecifications` , you can't specify `LaunchTemplateConfigs` .
     */
     ...
     *   the Spot Fleet request. Spot Fleet registers the running Spot Instances with the specified
     ...
     *   fulfilling On-Demand capacity. If you specify `lowestPrice` , Spot Fleet uses price to
     *   determine the order, launching the lowest price first. If you specify `prioritized` , Spot
     *   Fleet uses the priority that you assign to each Spot Fleet launch template override,
     *   launching the highest priority first. If you do not specify a value, Spot Fleet defaults to
     *   `lowestPrice` .
     */
     ...
     *   willing to pay. You can use the `onDemandMaxTotalPrice` parameter, the `spotMaxTotalPrice`
    ...
    /**
     * @param onDemandTargetCapacity The number of On-Demand units to request. You can choose to set
     *   the target capacity in terms of instances or a performance characteristic that is important
     *   to your application workload, such as vCPUs, memory, or I/O. If the request type is
    ...
    /**
     * @param spotMaintenanceStrategies The strategies for managing your Spot Instances that are at
    ...
    public fun spotMaintenanceStrategies(
        spotMaintenanceStrategies: CfnSpotFleet.SpotMaintenanceStrategiesProperty
    ) {
        cdkBuilder.spotMaintenanceStrategies(spotMaintenanceStrategies)
    }
    ...
    /**
     * @param targetCapacity The number of units to request for the Spot Fleet. You can choose to
     *   set the target capacity in terms of instances or a performance characteristic that is
     *   important to your application workload, such as vCPUs, memory, or I/O. If the request type
    ...
    /**
     * @param type The type of request. Indicates whether the Spot Fleet only requests the target
     *   capacity or also attempts to maintain it. When this value is `request` , the Spot Fleet
     *   only places the required requests. It does not attempt to replenish Spot Instances if
     *   capacity is diminished, nor does it submit requests in alternative Spot pools if capacity
     *   is not available. When this value is `maintain` , the Spot Fleet maintains the target
     *   capacity. The Spot Fleet places the required requests to meet capacity and automatically
     *   replenishes any interrupted instances. Default: `maintain` . `instant` is listed but is not
     *   used by Spot Fleet.
     */
    ...
    /**
     * @param validFrom The start date and time of the request, in UTC format ( *YYYY* - *MM* - *DD*
     *   T *HH* : *MM* : *SS* Z). By default, Amazon EC2 starts fulfilling the request immediately.
     */
    ...
    /**
     * @param validUntil The end date and time of the request, in UTC format ( *YYYY* - *MM* - *DD*
     *   T *HH* : *MM* : *SS* Z). After the end date and time, no new Spot Instance requests are
     *   placed or able to fulfill the request. If no value is specified, the Spot Fleet request
    ...
    public fun build(): CfnSpotFleet.SpotFleetRequestConfigDataProperty {
        if (_launchSpecifications.isNotEmpty())
            cdkBuilder.launchSpecifications(_launchSpecifications)
        if (_launchTemplateConfigs.isNotEmpty())
            cdkBuilder.launchTemplateConfigs(_launchTemplateConfigs)
