...
public class CfnDocumentClassifierDocumentClassifierInputDataConfigPropertyDsl {
   ...
   */
  public
      fun documentReaderConfig(documentReaderConfig: CfnDocumentClassifier.DocumentReaderConfigProperty) {
    cdkBuilder.documentReaderConfig(documentReaderConfig)
  }
   ...
   * training documents use a delimiter other than the default or the delimiter you specify, the labels
   * on that line will be combined to make a single unique label, such as LABELLABELLABEL.
   */
   ...
   * The S3 bucket must be in the same Region as the API endpoint that you are calling. The URI can
   * point to a single input file or it can provide the prefix for a collection of input files.
   *
  ...
  /**
   * @param testS3Uri This specifies the Amazon S3 location where the test annotations for an entity
