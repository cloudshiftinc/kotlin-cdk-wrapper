...
public class CfnLoadBalancerPropsDsl {
     ...
     *   a VPC, specify `Subnets` instead. Update requires replacement if you did not previously
     *   specify an Availability Zone or if you are removing all Availability Zones. Otherwise,
     ...
     *   instances. Update requires replacement if you did not previously specify health check
     *   settings or if you are removing the health check settings. Otherwise, update requires no
     *   interruption.
     */
     ...
     *   per port. If you update the properties for a listener, AWS CloudFormation deletes the
     *   existing listener and creates a new one with the specified properties. While the new
     *   listener is being created, clients cannot connect to the load balancer.
     */
    ...
    /**
     * @param loadBalancerName The name of the load balancer. This name must be unique within your
     *   set of load balancers for the region.
     *
    ...
    /**
     * @param policies The policies defined for your Classic Load Balancer. Specify only back-end
     *   server policies.
     */
    ...
    /**
     * @param scheme The type of load balancer. Valid only for load balancers in a VPC. If `Scheme`
     *   is `internet-facing` , the load balancer has a public DNS name that resolves to a public IP
     *   address.
     *
     ...
     *   subnet per Availability Zone. Update requires replacement if you did not previously specify
     *   a subnet or if you are removing all subnets. Otherwise, update requires no interruption. To
     *   update to a different subnet in the current Availability Zone, you must first update to a
     *   subnet in a different Availability Zone, then update to the new subnet in the original
     *   Availability Zone.
     */
