...
public class CfnAutomationRuleMapFilterPropertyDsl {
    ...
    /**
     * @param comparison The condition to apply to the key value when querying for findings with a
     *   map filter. To search for values that exactly match the filter value, use `EQUALS` . For
     *   example, for the `ResourceTags` field, the filter `Department EQUALS Security` matches
     *   findings that have the value `Security` for the tag `Department` .
     *
     * To search for values other than the filter value, use `NOT_EQUALS` . For example, for the
     * `ResourceTags` field, the filter `Department NOT_EQUALS Finance` matches findings that do not
     * have the value `Finance` for the tag `Department` .
     *
    ...
    /**
     * @param key The key of the map filter. For example, for `ResourceTags` , `Key` identifies the
    ...
    /**
     * @param value The value for the key in the map filter. Filter values are case sensitive. For
