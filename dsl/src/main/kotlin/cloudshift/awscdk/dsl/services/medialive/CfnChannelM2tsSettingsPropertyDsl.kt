...
public class CfnChannelM2tsSettingsPropertyDsl {
  ...
  /**
   * @param absentInputAudioBehavior When set to drop, the output audio streams are removed from the
   * program if the selected input audio stream is removed from the input.
   ...
   * You can enter the value as a decimal or hexadecimal value. Valid values are 32 (or 0x20)..8182
   * (or 0x1ff6).
   */
  ...
  /**
   * @param audioStreamType When set to atsc, uses stream type = 0x81 for AC3 and stream type = 0x87
   * for EAC3.
   ...
   * Multiple values are accepted, and can be entered in ranges and/or by comma separation. You can
   * enter the value as a decimal or hexadecimal value. Each PID specified must be in the range of 32
   * (or 0x20)..8182 (or 0x1ff6).
   */
  ...
  /**
   * @param ebpAudioInterval When videoAndFixedIntervals is selected, audio EBP markers are added to
   * partitions 3 and 4.
   * The interval between these additional markers is fixed, and is slightly shorter than the video
   * EBP marker interval. This is only available when EBP Cablelabs segmentation markers are selected.
   ...
   * If set to videoAndAudioPids, EBP markers are placed on the video PID and all audio PIDs. If set
   * to videoPid, EBP markers are placed on only the video PID.
   */
   ...
   * for every Packetized Elementary Stream (PES) header.
   * This parameter is effective only when the PCR PID is the same as the video or audio elementary
   * stream.
   */
   ...
   * raiSegstart sets the Random Access Indicator bit in the adaptation field. raiAdapt sets the RAI
   * bit and adds the current timecode in the private data bytes. psiSegstart inserts PAT and PMT
  ...
  /**
   * @param segmentationStyle The segmentation style parameter controls how segmentation markers are
   * inserted into the transport stream.
   * With avails, it is possible that segments might be truncated, which can influence where future
