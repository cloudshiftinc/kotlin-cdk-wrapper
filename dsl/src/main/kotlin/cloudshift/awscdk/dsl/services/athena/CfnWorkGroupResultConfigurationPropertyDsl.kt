...
public class CfnWorkGroupResultConfigurationPropertyDsl {
     ...
     *   ownership of stored query results. Currently the only supported canned ACL is
     *   `BUCKET_OWNER_FULL_CONTROL` . This is a client-side setting. If workgroup settings override
     *   client-side settings, then the query uses the ACL configuration that is specified for the
     *   workgroup, and also uses the location for storing query results specified in the workgroup.
     *   See `EnforceWorkGroupConfiguration` .
     */
     ...
     *   encryption option used (for example, `SSE_KMS` or `CSE_KMS` ) and key information. This is
     *   a client-side setting. If workgroup settings override client-side settings, then the query
     ...
     *   If the `ExpectedBucketOwner` account ID does not match the actual owner of the Amazon S3
     ...
     * query uses the `ExpectedBucketOwner` setting that is specified for the workgroup, and also
     ...
     *   `s3://path/to/query/bucket/` . To run a query, you must specify the query results location
     *   using either a client-side setting for individual queries or a location specified by the
     *   workgroup. If workgroup settings override client-side settings, then the query uses the
     *   location specified for the workgroup. If no query location is set, Athena issues an error.
