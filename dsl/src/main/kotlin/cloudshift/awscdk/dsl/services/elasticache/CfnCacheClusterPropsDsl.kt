...
public class CfnCacheClusterPropsDsl {
     ...
     *   their equivalent previous generation counterparts. Changing the CacheNodeType of a
     *   Memcached instance is currently not supported. If you need to scale using Memcached, we
     *   recommend forcing a replacement update by changing the `LogicalResourceId` of the resource.
     * * General purpose:
     * * Current generation:
     *
     ...
     *   is used. You cannot use any parameter group which has `cluster-enabled='yes'` when creating
     *   a cluster.
     */
    ...
    /**
     * @param cacheSubnetGroupName The name of the subnet group to be used for the cluster. Use this
     *   parameter only when you are creating a cluster in an Amazon Virtual Private Cloud (Amazon
     *   VPC).
     *
    ...
    /**
     * @param engine The name of the cache engine to be used for this cluster. Valid values for this
     ...
     * engine version, you must delete the existing cluster or replication group and create it anew
     ...
     *   engine version 1.6.6 on all instances built on the
     ...
     *   group (shard). The snapshot status changes to `restoring` while the new node group (shard)
     *   is being created.
     *
     * This parameter is only valid if the `Engine` parameter is `redis` .
     */
    ...
    /**
     * @param vpcSecurityGroupIds One or more VPC security groups associated with the cluster. Use
