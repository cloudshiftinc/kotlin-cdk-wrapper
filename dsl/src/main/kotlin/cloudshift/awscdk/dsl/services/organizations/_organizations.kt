...
public object organizations {
   ...
   * Because `CreateAccount` operates asynchronously, it can return a successful completion message
   * even though account initialization might still be in progress. You might need to wait a few
   ...
   * assume the role. AWS Organizations clones the company name and address information for the new
   ...
   * on each account resource type so that the accounts are created sequentially. If you create
   * multiple accounts at the same time, Organizations returns an error and the stack operation fails.
   ...
   * AWS CLI commands, we don't automatically collect the information required for the account to
   * operate as a standalone account. That includes collecting the payment method and signing the end
   * user license agreement (EULA). If you must remove an account from your organization later, you can
   * do so only after you provide the missing information. Follow the steps at [To leave an
   ...
   * still initializing, wait one hour and then try again. If the error persists, contact [AWS
   ...
   * organization. If you created some of your member accounts using AWS Organizations , you might be
   * blocked from removing those accounts. You can remove a member account only if it has all the
   * information that's required to operate as a standalone AWS account. For more information about how
   * to provide that information and then remove the account, see [Leaving an organization as a member
   ...
   * until it is finally closed. This can take up to 90 days and can prevent you from deleting the
   ...
   * account](https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_org_delete.html)
   * in the *AWS Organizations User Guide* .
   *
   *
   * Example:
   ...
   * Units](https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_ous.html) in the
   ...
   * management for AWS Organizations to specified member accounts to perform policy actions that are
   * by default available only to the management account.
   *
