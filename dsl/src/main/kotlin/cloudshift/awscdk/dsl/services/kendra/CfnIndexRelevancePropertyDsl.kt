...
public class CfnIndexRelevancePropertyDsl {
   ...
   * boost drops off. The higher the importance, the faster the effect drops off. If you don't specify
   * a value, the default is 3 months. The value of the field is a numeric string followed by the
   ...
   * For example, if document 1 was created on November 5, and document 2 was created on October 31,
   * document 1 is "fresher" than document 2. You can only set the `Freshness` field on one `DATE` type
   ...
   * When the `RankOrder` field is `ASCENDING` , higher numbers are better. For example, a document
   * with a rating score of 10 is higher ranking than a document with a rating score of 1.
   *
   ...
   * For example, if you want to boost query terms that match the "department" field in the result,
   * query terms that match this field are boosted in the result. You can add entries from the
