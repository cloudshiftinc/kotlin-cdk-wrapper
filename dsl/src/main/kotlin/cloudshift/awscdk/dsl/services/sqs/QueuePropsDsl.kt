...
public class QueuePropsDsl {
     ...
     *   the message.
     *
     * If you don't enable content-based deduplication and you want to deduplicate messages, provide
     * an explicit deduplication ID in your SendMessage() call.
     *
     ...
     *   again. The value must be an integer between 60 (1 minute) and 86,400 (24 hours). The
     ...
     *   deduplication occurs at the message group or queue level. (Only applies to FIFO queues.)
     */
    ...
    /**
     * @param encryptionMasterKey External KMS key to use for queue encryption. Individual messages
     *   will be encrypted using data keys. The data keys in turn will be encrypted using this key,
     *   and reused for a maximum of `dataKeyReuseSecs` seconds.
     *
     ...
     *   queue throughput quota applies to the entire queue or per message group. (Only applies to
     *   FIFO queues.)
     */
     ...
     *   Amazon SQS rejects it. You can specify an integer value from 1024 bytes (1 KiB) to 262144
    ...
    /**
     * @param queueName A name for the queue. If specified and this is a FIFO queue, must end in the
     *   string '.fifo'.
     */
    ...
    /**
     * @param receiveMessageWaitTime Default wait time for ReceiveMessage calls. Does not wait if
     *   set to 0, otherwise waits this amount of seconds by default for messages to arrive.
     *
    ...
    /**
     * @param removalPolicy Policy to apply when the queue is removed from the stack. Even though
     *   queues are technically stateful, their contents are transient and it is common to add and
     *   remove Queues while rearchitecting your application. The default is therefore `DESTROY`.
     *   Change it to `RETAIN` if the messages are so valuable that accidentally losing them would
     *   be unacceptable.
     */
    ...
    /**
     * @param retentionPeriod The number of seconds that Amazon SQS retains a message. You can
     *   specify an integer value from 60 seconds (1 minute) to 1209600 seconds (14 days). The
     ...
     *   becomes visible again for dequeueing by another processor.
     *
