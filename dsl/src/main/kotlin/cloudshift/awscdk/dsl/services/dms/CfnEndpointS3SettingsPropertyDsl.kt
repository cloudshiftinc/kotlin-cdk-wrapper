...
public class CfnEndpointS3SettingsPropertyDsl {
  ...
  /**
   * @param addColumnName An optional parameter that, when set to `true` or `y` , you can use to add
  ...
  /**
   * @param cannedAclForObjects A value that enables AWS DMS to specify a predefined (canned) access
   * control list (ACL) for objects created in an Amazon S3 bucket as .csv or .parquet files. For more
   ...
   * setting is `false` , but when `CdcInsertsAndUpdates` is set to `true` or `y` , only INSERTs and
   ...
   * the `IncludeOpForFullLoad` parameter. If `IncludeOpForFullLoad` is set to `true` , the first field
   * of every CDC record is set to either `I` or `U` to indicate INSERT and UPDATE operations at the
   * source. But if `IncludeOpForFullLoad` is set to `false` , CDC records are written without an
   * indication of INSERT or UPDATE operations at the source. For more information about how these
   ...
   *
   * `CdcInsertsOnly` and `CdcInsertsAndUpdates` can't both be set to `true` for the same endpoint.
   * Set either `CdcInsertsOnly` or `CdcInsertsAndUpdates` to `true` for the same endpoint, but not
   * both.
   */
   ...
   * setting), the first field in a .csv or .parquet record contains the letter I (INSERT), U (UPDATE),
   * or D (DELETE). These values indicate whether the row was inserted, updated, or deleted at the
   ...
   * depends on the value of `IncludeOpForFullLoad` . If `IncludeOpForFullLoad` is set to `true` , the
   * first field of every CDC record is set to I to indicate the INSERT operation at the source. If
   * `IncludeOpForFullLoad` is set to `false` , every CDC record is written without a first field to
   * indicate the INSERT operation at the source. For more information about how these settings work
   ...
   * When `CdcMaxBatchInterval` and `CdcMinFileSize` are both specified, the file write is triggered
   * by whichever parameter condition is met first within an AWS DMS CloudFormation template.
   *
   ...
   * Amazon S3.
   * When `CdcMinFileSize` and `CdcMaxBatchInterval` are both specified, the file write is triggered
   * by whichever parameter condition is met first within an AWS DMS CloudFormation template.
   *
  ...
  /**
   * @param csvDelimiter The delimiter used to separate columns in the .csv file for both source and
   ...
   * While handling the null value, you can use this parameter to pass a user-defined string as null
   * when writing to the target. For example, when target columns are not nullable, you can use this
   * option to differentiate between the empty string value and the null value. So, if you set this
   * parameter value to the empty string ("" or ''), AWS DMS treats the empty string as the null value
   * instead of `NULL` .
   *
  ...
  /**
   * @param csvRowDelimiter The delimiter used to separate rows in the .csv file for both source and
   * target.
  ...
  /**
   * @param dataFormat The format of the data that you want to use for output. You can choose one of
   * the following:.
   ...
   * based on transaction commit dates.
   ...
   * This parameter defaults to 1024 * 1024 bytes (1 MiB), the maximum size of a dictionary page before
   * it reverts to `PLAIN` encoding. This size is used for .parquet file format only.
   */
   ...
   * Choose `true` to enable statistics, `false` to disable. Statistics include `NULL` , `DISTINCT`
   * , `MAX` , and `MIN` values. This parameter defaults to `true` . This value is used for .parquet
   * file format only.
   */
  ...
  /**
   * @param ignoreHeaderRows When this value is set to 1, AWS DMS ignores the first row header in a
   * .csv file. A value of 1 turns on the feature; a value of 0 turns off the feature.
   * The default is 0.
   */
  ...
  /**
   * @param includeOpForFullLoad A value that enables a full load to write INSERT operations to the
   * comma-separated value (.csv) output files only to indicate how the rows were added to the source
   * database.
   *
   ...
   * source database. If `IncludeOpForFullLoad` is set to `true` or `y` , the INSERT is recorded as an
   * I annotation in the first field of the .csv file. This allows the format of your target records
   * from a full load to be consistent with the target records from a CDC load.
   *
   *
   * This setting works together with the `CdcInsertsOnly` and the `CdcInsertsAndUpdates` parameters
   ...
   * values. Set this parameter to `true` for S3 endpoint object files that are .parquet formatted only
  ...
  /**
   * @param rfc4180 For an S3 source, when this value is set to `true` or `y` , each leading double
   ...
   * literals are copied to the target as is. In this case, a delimiter (row or column) signals the end
   * of the field. Thus, you can't use a delimiter as part of the string, because it signals the end of
   * the value.
   *
   ...
   * encloses the entire column with an additional pair of double quotation marks ("). Every quotation
   ...
   * The key that you use needs an attached policy that enables IAM user permissions and allows use
   * of the key.
   *
  ...
  /**
   * @param serviceAccessRoleArn A required parameter that specifies the Amazon Resource Name (ARN)
   * used by the service to access the IAM role.
   ...
   * the precision of this value is in microseconds. For a CDC load, the rounding of the precision
   ...
   * capture (CDC) load are written in .csv format. If this setting is set to `true` for columns not
   ...
   *
   * When `useTaskStartTimeForFullLoadTimestamp` is set to `false` , the full load timestamp in the
   * timestamp column increments with the time data arrives at the target.
   */
