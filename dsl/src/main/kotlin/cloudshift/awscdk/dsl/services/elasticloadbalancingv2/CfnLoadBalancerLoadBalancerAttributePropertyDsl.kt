...
public class CfnLoadBalancerLoadBalancerAttributePropertyDsl {
   ...
   * Load Balancers is `false` . The default for Application Load Balancers is `true` , and cannot be
   * changed.
   *
   ...
   * * `routing.http.preserve_host_header.enabled` - Indicates whether the Application Load Balancer
   * should preserve the `Host` header in the HTTP request and send it to the target without any
   * change. The possible values are `true` and `false` . The default is `false` .
   ...
   * request to the target. The possible values are `append` , `preserve` , and `remove` . The default
   * is `append` .
   ...
   * * If the value is `remove` , the Application Load Balancer removes the `X-Forwarded-For` header
   * in the HTTP request before it sends it to targets.
   * * `routing.http2.enabled` - Indicates whether HTTP/2 is enabled. The possible values are `true`
   * and `false` . The default is `true` . Elastic Load Balancing requires that message header names
