...
public class CfnEndpointOracleSettingsPropertyDsl {
  ...
  /**
   * @param accessAlternateDirectly Set this attribute to `false` in order to use the Binary Reader
   * to capture change data for an Amazon RDS for Oracle as the source.
  ...
  /**
   * @param addSupplementalLogging Set this attribute to set up table-level supplemental logging for
   * the Oracle database.
   * This attribute enables PRIMARY KEY supplemental logging on all tables selected for a migration
   * task.
   *
   ...
   * standby setup.
   * This attribute is useful in the case of a switchover. In this case, AWS DMS needs to know which
   * destination to get archive redo logs from to read changes. This need arises because the previous
   * primary instance is now a standby instance after switchover.
   *
   * Although AWS DMS supports the use of the Oracle `RESETLOGS` option to open the database, never
   * use `RESETLOGS` unless necessary. For additional information about `RESETLOGS` , see [RMAN Data
   * Repair
   ...
   * This value should be the same as a number in the dest_id column of the v$archived_log view. If
   * you work with an additional redo log destination, use the `AdditionalArchivedLogDestId` option to
  ...
  /**
   * @param archivedLogsOnly When this field is set to `Y` , AWS DMS only accesses the archived redo
   * logs.
   ...
   * (ASM) password.
   * You can set this value from the `*asm_user_password*` value. You set this value as part of the
   * comma-separated value that you set to the `Password` request parameter when you create the
   * endpoint to access transaction logs using Binary Reader. For more information, see [Configuration
   * for change data capture (CDC) on an Oracle source
   ...
   * connection attribute string to access an Oracle server with Binary Reader that uses ASM. For more
  ...
  /**
   * @param charLengthSemantics Specifies whether the length of a character column is in bytes or in
   * characters.
  ...
  /**
   * @param directPathNoLog When set to `true` , this attribute helps to increase the commit rate on
   * the Oracle target database by writing directly to tables and not writing a trail to database logs.
   */
   ...
   * This attribute also only applies when you use the AWS DMS parallel load feature. Note that the
   ...
   * archived redo logs.
   * These IDs are the values of the `dest_id` column in the `v$archived_log` view. Use this setting
   * with the `archivedLogDestId` extra connection attribute in a primary-to-single setup or a
   * primary-to-multiple-standby setup.
   *
   * This setting is useful in a switchover when you use an Oracle Data Guard database as a source.
   * In this case, AWS DMS needs information about what destination to get archive redo logs from to
   * read changes. AWS DMS needs this because after the switchover the previous primary is a standby
   * instance. For example, in a primary-to-single standby setup you might apply the following
   * settings.
   *
   ...
   *
   * Although AWS DMS supports the use of the Oracle `RESETLOGS` option to open the database, never
   * use `RESETLOGS` unless it's necessary. For more information about `RESETLOGS` , see [RMAN Data
   * Repair
   * Concepts](https://docs.aws.amazon.com/https://docs.oracle.com/en/database/oracle/oracle-database/19/bradv/rman-data-repair-concepts.html#GUID-1805CCF7-4AF2-482D-B65A-998192F89C2B)
   * in the *Oracle Database Backup and Recovery User's Guide* .
   */
   ...
   * the actual size of an LOB column is greater than the specified `LobMaxSize` .
   ...
   * configures to perform a change data capture (CDC) load using Oracle Automatic Storage Management
   * (ASM).
  ...
  /**
   * @param secretsManagerOracleAsmAccessRoleArn Required only if your Oracle endpoint uses Advanced
   * Storage Manager (ASM).
   ...
   *
   * You can specify one of two sets of values for these permissions. You can specify the values for
   * this setting and `SecretsManagerOracleAsmSecretId` . Or you can specify clear-text values for
   ...
   * Storage Manager (ASM).
   * The full ARN, partial ARN, or display name of the `SecretsManagerOracleAsmSecret` that contains
   ...
   * transparent data encryption (TDE) of the columns and tablespaces in an Oracle source database that
   * is encrypted using TDE.
   ...
   * setting the `securityDbEncryptionName` extra connection attribute in [Supported encryption methods
   * for using Oracle as a source for AWS
   * DMS](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Source.Oracle.html#CHAP_Source.Oracle.Encryption)
   * in the *AWS Database Migration Service User Guide* .
   */
   ...
   *
   * In AWS DMS , you can create an Oracle CDC task that uses an Active Data Guard standby instance
   * as a source for replicating ongoing changes. Doing this eliminates the need to connect to an
   * active database that might be in production.
   */
   ...
   * load.
   * Specify this value to use the direct path protocol in the Oracle Call Interface (OCI). By using
   * this OCI protocol, you can bulk-load Oracle target tables during a full load.
   */
   ...
   * `UseLogminerReader` to N, also set `UseBfile` to Y. For more information on this setting and using
   * Oracle ASM, see [Using Oracle LogMiner or AWS DMS Binary Reader for
   * CDC](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Source.Oracle.html#CHAP_Source.Oracle.CDC)
   ...
   * Reader to capture change data for an Amazon RDS for Oracle as the source.
   * This value specifies the path prefix used to replace the default Oracle root to access the redo
   * logs.
   */
