 ...
 * Starts a job run.
 *
 * A job is a unit of work that you submit to Amazon EMR on EKS for execution. The work performed by
 * the job can be defined by a Spark jar, PySpark script, or SparkSQL query. A job run is an
 * execution of the job on the virtual cluster.
 *
 * Example:
 * ```
...
public class EmrContainersStartJobRunDsl(
     ...
     *
     * If `virtualClusterId` is from a JSON input path, an execution role must be provided. If an
     * execution role is provided, follow the documentation to update the role trust policy.
     *
