...
public class DatabaseClusterFromSnapshotPropsDsl {
    ...
    /**
     * @param credentials Credentials for the administrative user. Note - using this prop only works
     *   with `Credentials.fromPassword()` with the username of the snapshot,
     *   `Credentials.fromUsername()` with the username and password of the snapshot or
    ...
    /**
     * @param snapshotCredentials Master user credentials. Note - It is not possible to change the
     *   master username for a snapshot; however, it is possible to provide (or generate) a new
     *   password.
     */
     ...
     *   to restore from. You can use either the name or the Amazon Resource Name (ARN) to specify a
     *   DB cluster snapshot. However, you can use only the ARN to specify a DB instance snapshot.
     */
    ...
    public fun build(): DatabaseClusterFromSnapshotProps {
