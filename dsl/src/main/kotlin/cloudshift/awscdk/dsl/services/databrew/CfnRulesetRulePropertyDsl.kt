...
public class CfnRulesetRulePropertyDsl {
     ...
     *   followed by variable references, possibly grouped and combined with other conditions. For
     *   example, `(:col1 starts_with :prefix1 or :col1 starts_with :prefix2) and (:col1 ends_with
     *   :suffix1 or :col1 ends_with :suffix2)` . Column and value references are substitution
     *   variables that should start with the ':' symbol. Depending on the context, substitution
     *   variables' values can be either an actual value or a column name. These values are defined
     *   in the SubstitutionMap. If a CheckExpression starts with a column reference, then
     *   ColumnSelectors in the rule should be null. If ColumnSelectors has been defined, then there
     *   should be no columnn reference in the left side of a condition, for example, `is_between
     *   :val1 and :val2` .
     */
    ...
    /**
     * @param columnSelectors List of column selectors. Selectors can be used to select columns
     *   using a name or regular expression from the dataset. Rule will be applied to selected
     *   columns.
     */
    ...
    /**
     * @param disabled A value that specifies whether the rule is disabled. Once a rule is disabled,
     ...
     *   expression. Variable names should start with a ':' (colon). Variable values can either be
     *   actual values or column names. To differentiate between the two, column names should be
    ...
    /**
     * @param threshold The threshold used with a non-aggregate check expression. Non-aggregate
     *   check expressions will be applied to each row in a specific column, and the threshold will
