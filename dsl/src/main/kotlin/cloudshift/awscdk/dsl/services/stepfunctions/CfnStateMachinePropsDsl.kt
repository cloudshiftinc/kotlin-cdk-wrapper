...
public class CfnStateMachinePropsDsl {
    ...
    /**
     * @param definition The Amazon States Language definition of the state machine. The state
     *   machine definition must be in JSON or YAML, and the format of the object must match the
     *   format of your CloudFormation template file. See
    ...
    /**
     * @param definitionString The Amazon States Language definition of the state machine. The state
     *   machine definition must be in JSON. See
     *   [Amazon States Language](https://docs.aws.amazon.com/step-functions/latest/dg/concepts-amazon-states-language.html)
     *   .
     */
     ...
     *   placeholder variables in the state machine definition. This enables the customer to inject
     *   values obtained at runtime, for example from intrinsic functions, in the state machine
     *   definition. Variables can be template parameter names, resource logical IDs, resource
    ...
    /**
     * @param stateMachineName The name of the state machine. A name must *not* contain:
     * * white space
     * * brackets `&lt; &gt; { } [ ]`
     * * wildcard characters `? *`
     * * special characters `" # % \ ^ | ~ ` $ &amp; , ; : /`
     ...
     *   created. The default is `STANDARD` . You cannot update the `type` of a state machine once
     *   it has been created. For more information on `STANDARD` and `EXPRESS` workflows, see
