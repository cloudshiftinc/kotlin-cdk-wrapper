...
public class CfnChannelH265SettingsPropertyDsl {
   ...
   * If afdSignaling is "auto", the system will try to preserve the input AFD value (in cases where
   * multiple AFD values are valid). If set to "fixed", the AFD value will be the value configured in
  ...
  /**
   * @param flickerAq If set to enabled, adjust quantization within each frame to reduce flicker or
   * 'pop' on I-frames.
   */
   ...
   * Defaults to 5 if multiplex rate control is used. Enforces separation between repeated (cadence)
   ...
   * your assets to devices that cannot handle variable bitrates. Multiplex: This rate control mode
   * is only supported (and is required) when the video is being
   ...
   * Must be less than or equal to the number of macroblock rows for progressive pictures, and less
   * than or equal to half the number of macroblock rows for interlaced pictures.
   * This field is optional; when no value is specified the encoder will choose the number of slices
