...
public class CfnVolumeOntapConfigurationPropertyDsl {
     ...
     *   copied to backups. This value defaults to false. If it's set to true, all tags for the
     *   volume are copied to all automatic and user-initiated backups where the user doesn't
    ...
    /**
     * @param securityStyle Specifies the security style for the volume. If a volume's security
     *   style is not specified, it is automatically set to the root volume's security style. The
     *   security style determines the type of permissions that FSx for ONTAP uses to control data
    ...
    /**
     * @param tieringPolicy Describes the data tiering policy for an ONTAP volume. When enabled,
     *   Amazon FSx for ONTAP's intelligent tiering automatically transitions a volume's data
     *   between the file system's primary storage and capacity pool storage based on your access
     *   patterns.
     *
