...
public class CfnRotationScheduleHostedRotationLambdaPropertyDsl {
   ...
   * `aws/secretsmanager` doesn't yet exist, then Secrets Manager creates it for you automatically the
   ...
   * the [Alternating users rotation
   ...
   * that the Lambda rotation function can find it. CloudFormation does not hardcode secret ARNs in the
   * Lambda rotation function, so you can use the function to rotate multiple secrets. For more
   ...
   * .
   *
   * You can specify `MasterSecretArn` or `SuperuserSecretArn` but not both. They represent the same
   * superuser secret.
   */
   ...
   * superuser secret, if you use the [alternating users
   * strategy](https://docs.aws.amazon.com/secretsmanager/latest/userguide/rotating-secrets_strategies.html#rotating-secrets-two-users)
   * and the superuser secret is encrypted with a customer managed key. You don't need to specify this
   * property if the superuser secret is encrypted using the key `aws/secretsmanager` . CloudFormation
   * grants the execution role for the Lambda rotation function `Decrypt` , `DescribeKey` , and
   * `GenerateDataKey` permission to the key in this property. For more information, see [Lambda
   * rotation function execution role permissions for Secrets
   * Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/rotating-secrets-required-permissions-function.html)
   * .
   ...
   *
   * If you only do one of the steps, your rotation function will be incompatible with the binaries.
   * For more information, see [Why did my Lambda rotation function fail with a "pg module not found"
  ...
  /**
   * @param superuserSecretArn The ARN of the secret that contains superuser credentials, if you use
   * the [Alternating users rotation
   * strategy](https://docs.aws.amazon.com/secretsmanager/latest/userguide/rotating-secrets_strategies.html#rotating-secrets-two-users)
   * . CloudFormation grants the execution role for the Lambda rotation function `GetSecretValue`
   * permission to the secret in this property. For more information, see [Lambda rotation function
  ...
  /**
   * @param superuserSecretKmsKeyArn The ARN of the KMS key that Secrets Manager used to encrypt the
   * superuser secret, if you use the [alternating users
   * strategy](https://docs.aws.amazon.com/secretsmanager/latest/userguide/rotating-secrets_strategies.html#rotating-secrets-two-users)
   * and the superuser secret is encrypted with a customer managed key. You don't need to specify this
   * property if the superuser secret is encrypted using the key `aws/secretsmanager` . CloudFormation
