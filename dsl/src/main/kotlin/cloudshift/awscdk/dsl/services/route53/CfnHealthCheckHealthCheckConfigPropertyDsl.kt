...
public class CfnHealthCheckHealthCheckConfigPropertyDsl {
   ...
   * Route 53 health checkers to use to determine whether the specified health check is healthy.
   */
   ...
   * `ChildHealthCheck` element for each health check that you want to associate with a `CALCULATED`
   * health check.
   */
   ...
   * handshake_failure` . A health check can also have that status for other reasons. If SNI is enabled
   * and you're still getting the error, check the SSL/TLS configuration on your endpoint and confirm
   ...
   * fail for Amazon Route 53 to change the current status of the endpoint from unhealthy to healthy or
   * vice versa.
  ...
  /**
   * @param fullyQualifiedDomainName Amazon Route 53 behavior depends on whether you specify a value
   * for `IPAddress` .
   ...
   * `CALCULATED` health check that Amazon Route 53 must consider healthy for the `CALCULATED` health
   * check to be considered healthy.
   * To specify the child health checks that you want to associate with a `CALCULATED` health check,
   * use the
  ...
  /**
   * @param regions A complex type that contains one `Region` element for each region from which you
  ...
  /**
   * @param resourcePath The path, if any, that you want Amazon Route 53 to request when performing
   * health checks.
   * The path can be any value for which your endpoint will return an HTTP status code of 2xx or 3xx
   * when the endpoint is healthy, for example, the file /docs/route53-health-check.html. You can also
  ...
  /**
   * @param searchString If the value of Type is `HTTP_STR_MATCH` or `HTTPS_STR_MATCH` , the string
  ...
  /**
   * @param type The type of health check that you want to create, which indicates how Amazon Route
   ...
   * submits an `HTTPS` request and searches the first 5,120 bytes of the response body for the string
   * that you specify in `SearchString` .
   * * *TCP* : Route 53 tries to establish a TCP connection.
   * * *CLOUDWATCH_METRIC* : The health check is associated with a CloudWatch alarm. If the state of
   * the alarm is `OK` , the health check is considered healthy. If the state is `ALARM` , the health
   * check is considered unhealthy. If CloudWatch doesn't have sufficient data to determine whether the
   ...
   * Controller routing control. If the routing control state is `ON` , the health check is considered
   * healthy. If the state is `OFF` , the health check is considered unhealthy.
   *
