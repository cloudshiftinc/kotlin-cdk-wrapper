...
public class CfnBucketReplicationRulePropertyDsl {
    ...
    /**
     * @param deleteMarkerReplication Specifies whether Amazon S3 replicates delete markers. If you
     *   specify a `Filter` in your replication configuration, you must also include a
    ...
    public fun deleteMarkerReplication(
        deleteMarkerReplication: CfnBucket.DeleteMarkerReplicationProperty
    ) {
        cdkBuilder.deleteMarkerReplication(deleteMarkerReplication)
    }
     ...
     *   applies. A `Filter` must specify exactly one `Prefix` , `TagFilter` , or an `And` child
     *   element. The use of the filter field indicates that this is a V2 replication configuration.
     *   This field isn't supported in a V1 replication configuration.
     *
    ...
    /**
     * @param id A unique identifier for the rule. The maximum value is 255 characters. If you don't
     *   specify a value, AWS CloudFormation generates a random ID. When using a V2 replication
     *   configuration this property is capitalized as "ID".
     */
     ...
     *   rule applies. The maximum prefix length is 1,024 characters. To include all objects in a
     *   bucket, specify an empty string. To filter using a V1 replication configuration, add the
     ...
     *   replication rules conflict. Amazon S3 will attempt to replicate objects according to all
     *   replication rules. However, if there are two or more rules with the same destination
     ...
     *   the source objects that you want to replicate. You can choose to enable or disable the
    ...
    public fun sourceSelectionCriteria(
        sourceSelectionCriteria: CfnBucket.SourceSelectionCriteriaProperty
    ) {
        cdkBuilder.sourceSelectionCriteria(sourceSelectionCriteria)
    }
