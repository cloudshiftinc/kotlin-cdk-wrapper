...
public class CfnEventDataStoreAdvancedFieldSelectorPropertyDsl {
  ...
  /**
   * @param equalTo An operator that includes events that match the exact value of the event record
   ...
   *
   * * *`readOnly`* - Optional. Can be set to `Equals` a value of `true` or `false` . If you do not
   * add this field, CloudTrail logs both `read` and `write` events. A value of `true` logs only `read`
   ...
   * `kms.amazonaws.com` .
   * * *`eventName`* - Can use any operator. You can use it to ﬁlter in or ﬁlter out any data event
   * logged to CloudTrail, such as `PutBucket` or `GetSnapshotBlock` . You can have multiple values for
   ...
   *
   * When resources.type equals `AWS::CloudTrail::Channel` , and the operator is set to `Equals` or
   * `NotEquals` , the ARN must be in the following format:
   *
   * *
   * `arn:&lt;partition&gt;:cloudtrail:&lt;region&gt;:&lt;account_ID&gt;:channel/&lt;channel_UUID&gt;`
   *
   * When resources.type equals `AWS::CodeWhisperer::Profile` , and the operator is set to `Equals`
   ...
   *
   * When `resources.type` equals `AWS::EMRWAL::Workspace` , and the operator is set to `Equals` or
   * `NotEquals` , the ARN must be in the following format:
   *
   * * `arn:&lt;partition&gt;:emrwal:&lt;region&gt;::workspace/&lt;workspace_name&gt;`
   *
   * When `resources.type` equals `AWS::FinSpace::Environment` , and the operator is set to `Equals`
   * or `NotEquals` , the ARN must be in the following format:
   *
   * *
   ...
   * `NotEquals` , the ARN must be in one of the following formats. To log events on all objects in an
   * S3 access point, we recommend that you use only the access point ARN, don’t include the object
