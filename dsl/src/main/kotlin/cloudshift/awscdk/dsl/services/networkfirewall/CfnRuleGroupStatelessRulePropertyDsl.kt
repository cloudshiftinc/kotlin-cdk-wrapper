...
public class CfnRuleGroupStatelessRulePropertyDsl {
  ...
  /**
   * @param priority Indicates the order in which to run this rule relative to all of the rules that
   * are defined for a stateless rule group. 
   * Network Firewall evaluates the rules in a rule group starting with the lowest priority setting.
   ...
   * rules later, number them so there's a wide range in between, for example use 100, 200, and so on.
   */
