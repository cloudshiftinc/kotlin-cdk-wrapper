...
public class CfnInstancePropsDsl {
    ...
    /**
     * @param additionalInfo This property is reserved for internal use. If you use it, the stack
     *   fails with this error: `Bad property set: [Testing this property] (Service: AmazonEC2;
     *   Status Code: 400; Error Code: InvalidParameterCombination; Request ID:
    ...
    /**
     * @param affinity Indicates whether the instance is associated with a dedicated host. If you
     *   want the instance to always restart on the same host on which it was launched, specify
     ...
     *   Availability Zone will be automatically chosen for you based on the load balancing criteria
     *   for the Region.
     *
     ...
     *   attach to the instance at launch. By default, the block devices specified in the block
     *   device mapping for the AMI are used. You can override the AMI block device mapping using
     *   the instance block device mapping. For the root volume, you can override only the volume
     *   size, volume type, volume encryption settings, and the `DeleteOnTermination` setting.
     *
    ...
    /**
     * @param iamInstanceProfile The name of an IAM instance profile. To create a new IAM instance
     *   profile, use the
    ...
    /**
     * @param imageId The ID of the AMI. An AMI ID is required to launch an instance and must be
     ...
     *   in the *Amazon EC2 User Guide* . When you change your EBS-backed instance type, instance
     *   restart or replacement behavior depends on the instance type compatibility between the old
     *   and new types. An instance that's backed by an instance store volume is always replaced.
     *   For more information, see
     ...
     *   you specify in the AWS CloudFormation template override the same parameters in the launch
    ...
    /**
     * @param monitoring Specifies whether detailed monitoring is enabled for the instance. Specify
     *   `true` to enable detailed monitoring. Otherwise, basic monitoring is enabled. For more
     *   information about detailed monitoring, see
     ...
     *   to all of the volumes attached to the instance at launch. If you specify `true` and you
     *   assign tags to the instance, those tags are automatically assigned to all of the volumes
     *   that you attach to the instance at launch. If you specify `false` , those tags are not
     *   assigned to the attached volumes.
     */
    ...
    /**
     * @param ramdiskId The ID of the RAM disk to select. Some kernels require additional drivers at
     *   launch. Check the kernel requirements for information about whether you need to specify a
     *   RAM disk. To find kernel requirements, go to the AWS Resource Center and search for the
     *   kernel ID.
     *
     * We recommend that you use PV-GRUB instead of kernels and RAM disks. For more information, see
     * [PV-GRUB](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/UserProvidedkernels.html) in
     ...
     *   you must use security group IDs instead. You cannot specify this option and the network
     *   interfaces option in the same request. The list can contain both the name of existing
     *   Amazon EC2 security groups or references to AWS::EC2::SecurityGroup resources created in
     *   the template.
     *
    ...
    /**
     * @param subnetId The ID of the subnet to launch the instance into. If you specify a network
    ...
    /**
     * @param tags The tags to add to the instance. These tags are not applied to the EBS volumes,
     *   such as the root volume, unless
