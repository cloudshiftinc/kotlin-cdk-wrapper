...
public class CfnServiceDeploymentConfigurationPropertyDsl {
   ...
   * completed successfully. For more information, see [Rolling
   ...
   * `maximumPercent` parameter represents an upper limit on the number of your service's tasks that
   ...
   * This parameter enables you to define the deployment batch size. For example, if your service is
   * using the `REPLICA` service scheduler and has a `desiredCount` of four tasks and a
   ...
   * service use the Fargate launch type, the maximum percent value is not used, although it is
   ...
   * type, the `minimumHealthyPercent` represents a lower limit on the number of your service's tasks
   * that must remain in the `RUNNING` state during a deployment, as a percentage of the `desiredCount`
   * (rounded up to the nearest integer).
   * This parameter enables you to deploy without using additional cluster capacity. For example, if
   * your service has a `desiredCount` of four tasks and a `minimumHealthyPercent` of 50%, the service
   ...
   * wait for both the task to reach a healthy status and the load balancer target group health check
   * to return a healthy status before counting the task towards the minimum healthy percent total.
   *
   * If a service is using either the blue/green ( `CODE_DEPLOY` ) or `EXTERNAL` deployment types
   * and is running tasks that use the EC2 launch type, the *minimum healthy percent* value is set to
   * the default value and is used to define the lower limit on the number of the tasks in the service
   * that remain in the `RUNNING` state while the container instances are in the `DRAINING` state. If a
   * service is using either the blue/green ( `CODE_DEPLOY` ) or `EXTERNAL` deployment types and is
   * running tasks that use the Fargate launch type, the minimum healthy percent value is not used,
