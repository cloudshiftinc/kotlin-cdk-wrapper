...
public class CfnProfilingGroupPropsDsl {
    ...
    /**
     * @param agentPermissions The agent permissions attached to this profiling group. This action
     *   group grants `ConfigureAgent` and `PostAgentProfile` permissions to perform actions
    ...
    public fun anomalyDetectionNotificationConfiguration(
        ...
        _anomalyDetectionNotificationConfiguration.addAll(
            listOf(*anomalyDetectionNotificationConfiguration)
        )
    }
     ...
     */
    public fun anomalyDetectionNotificationConfiguration(
        anomalyDetectionNotificationConfiguration: Collection<Any>
    ) {
     ...
     *   application runs on AWS Lambda. Use `Default` if your application runs on a compute
     *   platform that is not AWS Lambda , such an Amazon EC2 instance, an on-premises server, or a
     *   different platform. If not specified, `Default` is used. This property is immutable.
     */
    ...
    public fun build(): CfnProfilingGroupProps {
        if (_anomalyDetectionNotificationConfiguration.isNotEmpty())
            cdkBuilder.anomalyDetectionNotificationConfiguration(
                _anomalyDetectionNotificationConfiguration
            )
