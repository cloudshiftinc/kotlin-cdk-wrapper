...
public class CfnSecurityProfileBehaviorCriteriaPropertyDsl {
   ...
   * longer in violation of the behavior for the specified number of consecutive datapoints, the alarm
   * is cleared.
   * If not specified, the default is 1.
   */
   ...
   * For a `statisticalThreshhold` metric comparison, measurements from all devices are accumulated
   * over this time duration before being used to calculate percentiles, and later, measurements from
   * an individual device are also accumulated over this time duration before being given a percentile
   ...
   */
  public
      fun mlDetectionConfig(mlDetectionConfig: CfnSecurityProfile.MachineLearningDetectionConfigProperty) {
    cdkBuilder.mlDetectionConfig(mlDetectionConfig)
  }

  /**
   * @param statisticalThreshold A statistical ranking (percentile)that indicates a threshold value
   * by which a behavior is determined to be in compliance or in violation of the behavior.
   */
   ...
   */
  public
      fun statisticalThreshold(statisticalThreshold: CfnSecurityProfile.StatisticalThresholdProperty) {
    cdkBuilder.statisticalThreshold(statisticalThreshold)
  }
