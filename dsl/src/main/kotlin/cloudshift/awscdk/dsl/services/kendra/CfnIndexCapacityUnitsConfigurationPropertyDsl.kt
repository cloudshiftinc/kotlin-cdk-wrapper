...
public class CfnIndexCapacityUnitsConfigurationPropertyDsl {
     ...
     *   capacity. A single extra capacity unit for an index provides 0.1 queries per second or
     ...
     * the base capacity of 2.5 calls per second, whichever is higher. For example, the base
     * capacity for an index is 0.1 queries per second, and `GetQuerySuggestions` capacity has a
     * base of 2.5 calls per second. If you add another 0.1 queries per second to total 0.2 queries
     * per second for an index, the `GetQuerySuggestions` capacity is 2.5 calls per second (higher
     * than five times 0.2 queries per second).
     */
    ...
    /**
     * @param storageCapacityUnits The amount of extra storage capacity for an index. A single
     *   capacity unit provides 30 GB of storage space or 100,000 documents, whichever is reached
