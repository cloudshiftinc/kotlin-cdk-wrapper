 ...
 *
 * The log group is created if it doesn't already exist. The policy is removed when `retentionDays`
 * is `undefined` or equal to `Infinity`. Log group can be created in the region that is different
 * from stack region by specifying `logGroupRegion`
 *
 * Example:
 * ```
