...
public class CfnCanaryCodePropertyDsl {
   ...
   * For canaries that use the `syn-python-selenium-1.0` runtime or a `syn-nodejs.puppeteer` runtime
   * earlier than `syn-nodejs.puppeteer-3.4` , the handler must be specified as `*fileName* .handler` .
   * For `syn-python-selenium-1.1` , `syn-nodejs.puppeteer-3.4` , and later runtimes, the handler can
   * be specified as `*fileName* . *functionName*` , or you can specify a folder where canary scripts
  ...
  /**
   * @param script If you input your canary script directly into the canary instead of referring to
   * an S3 location, the value of this parameter is the script in plain text.
  ...
  /**
   * @param sourceLocationArn The ARN of the Lambda layer where Synthetics stores the canary script
   * code.
   */
