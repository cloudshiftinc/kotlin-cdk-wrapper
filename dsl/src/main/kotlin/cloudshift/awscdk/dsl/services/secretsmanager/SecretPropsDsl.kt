...
public class SecretPropsDsl {
    ...
    /**
     * @param generateSecretString Configuration for how to generate a secret value. Only one of
     *   `secretString` and `generateSecretString` can be provided.
     */
    ...
    /**
     * @param secretName A name for the secret. Note that deleting secrets from SecretsManager does
     *   not happen immediately, but after a 7 to 30 days blackout period. During that period, it is
     *   not possible to create another secret that shares the same name.
     */
    ...
    /**
     * @param secretObjectValue Initial value for a JSON secret. **NOTE:** *It is **highly**
     *   encouraged to leave this field undefined and allow SecretsManager to create the secret
     *   value. The secret object -- if provided -- will be included in the output of the cdk as
     *   part of synthesis, and will appear in the CloudFormation template in the console. This can
     *   be secure(-ish) if that value is merely reference to another resource (or one of its
     *   attributes), but if the value is a plaintext string, it will be visible to anyone with
     *   access to the CloudFormation template (via the AWS Console, SDKs, or CLI).
     *
    ...
    /**
     * @param secretStringBeta1 Initial value for the secret. **NOTE:** *It is **highly** encouraged
     *   to leave this field undefined and allow SecretsManager to create the secret value. The
     *   secret string -- if provided -- will be included in the output of the cdk as part of
     *   synthesis, and will appear in the CloudFormation template in the console. This can be
     *   secure(-ish) if that value is merely reference to another resource (or one of its
     *   attributes), but if the value is a plaintext string, it will be visible to anyone with
     *   access to the CloudFormation template (via the AWS Console, SDKs, or CLI).
     *
    ...
    /**
     * @param secretStringValue Initial value for the secret. **NOTE:** *It is **highly** encouraged
     *   to leave this field undefined and allow SecretsManager to create the secret value. The
     *   secret string -- if provided -- will be included in the output of the cdk as part of
     *   synthesis, and will appear in the CloudFormation template in the console. This can be
     *   secure(-ish) if that value is merely reference to another resource (or one of its
