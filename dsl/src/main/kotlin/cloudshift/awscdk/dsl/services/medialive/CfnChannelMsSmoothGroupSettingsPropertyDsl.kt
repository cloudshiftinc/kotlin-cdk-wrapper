...
public class CfnChannelMsSmoothGroupSettingsPropertyDsl {
   ...
   * each message placed in the sparse track.
  ...
  /**
   * @param certificateMode If set to verifyAuthenticity, verifies the HTTPS certificate chain to a
  ...
  /**
   * @param connectionRetryInterval The number of seconds to wait before retrying the connection to
   * the IIS server if the connection is lost.
  ...
  /**
   * @param streamManifestBehavior When set to send, sends a stream manifest so that the publishing
   ...
   * * useEventStartDate: Use the date the channel was started as the offset - useConfiguredOffset:
