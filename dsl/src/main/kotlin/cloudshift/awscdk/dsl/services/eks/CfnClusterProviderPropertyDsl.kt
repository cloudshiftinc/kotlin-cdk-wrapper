...
public class CfnClusterProviderPropertyDsl {
   ...
   * The KMS key must be symmetric and created in the same AWS Region as the cluster. If the KMS key
   * was created in a different account, the [IAM
   * principal](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_terms-and-concepts.html) must
   * have access to the KMS key. For more information, see [Allowing users in other accounts to use a
   * KMS
