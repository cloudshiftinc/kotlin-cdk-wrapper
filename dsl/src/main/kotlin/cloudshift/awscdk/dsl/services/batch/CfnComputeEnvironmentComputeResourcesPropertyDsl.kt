...
public class CfnComputeEnvironmentComputeResourcesPropertyDsl {
  ...
  /**
   * @param allocationStrategy The allocation strategy to use for the compute resource if not enough
   * instances of the best fitting instance type can be allocated.
   * This might be because of availability of the instance type in the Region or [Amazon EC2 service
   * limits](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-resource-limits.html) . For more
   * information, see [Allocation
   * strategies](https://docs.aws.amazon.com/batch/latest/userguide/allocation-strategies.html) in the
   ...
   * jobs with a preference for the lowest-cost instance type. If additional instances of the selected
   * instance type aren't available, AWS Batch waits for the additional instances to be available. If
   * there aren't enough instances available, or if the user is reaching [Amazon EC2 service
   * limits](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-resource-limits.html) then
   * additional jobs aren't run until the currently running jobs have completed. This allocation
   ...
   * with a lower cost per unit vCPU. If additional instances of the previously selected instance types
   * aren't available, AWS Batch will select new instance types.
   * * **SPOT_CAPACITY_OPTIMIZED** - AWS Batch will select one or more instance types that are large
   * enough to meet the requirements of the jobs in the queue, with a preference for instance types
   * that are less likely to be interrupted. This allocation strategy is only available for Spot
   * Instance compute resources.
   *
   * With both `BEST_FIT_PROGRESSIVE` and `SPOT_CAPACITY_OPTIMIZED` allocation strategies using
   * On-Demand or Spot Instances, and the `BEST_FIT` strategy using Spot Instances, AWS Batch might
   * need to go above `maxvCpus` to meet your capacity requirements. In this event, AWS Batch never
   ...
   * current On-Demand price for that Amazon EC2 instance. You always pay the lowest (market) price and
   * never more than your maximum percentage. For most use cases, we recommend leaving this field
   * empty.
   *
   ...
   * EC2 instances in the compute environment.
   * If `Ec2Configuration` isn't specified, the default is `ECS_AL2` .
   *
   * When updating a compute environment, changing this setting requires an infrastructure update of
   * the compute environment. For more information, see [Updating compute
   * environments](https://docs.aws.amazon.com/batch/latest/userguide/updating-compute-environments.html)
   * in the *AWS Batch User Guide* . To remove the EC2 configuration and any custom AMI ID specified in
   * `imageIdOverride` , set this value to an empty string.
   *
   ...
   * compute environment.
   * You can specify the short name or full Amazon Resource Name (ARN) of an instance profile. For
   * example, `*ecsInstanceRole*` or `arn:aws:iam:: *&lt;aws_account_id&gt;* :instance-profile/
   * *ecsInstanceRole*` . For more information, see [Amazon ECS instance
   * role](https://docs.aws.amazon.com/batch/latest/userguide/instance_IAM_role.html) in the *AWS Batch
   * User Guide* .
   *
   * When updating a compute environment, changing this setting requires an infrastructure update of
   * the compute environment. For more information, see [Updating compute
   ...
   *
   * This parameter isn't applicable to jobs that are running on Fargate resources. Don't specify
   * it. &gt; When you create a compute environment, the instance types that you select for the compute
   * environment must share the same architecture. For example, you can't mix x86 and ARM instances in
   * the same compute environment. &gt; Currently, `optimal` uses instance types from the C4, M4, and
   * R4 instance families. In Regions that don't have instance types from those instance families,
   ...
   * launch template ID or launch template name in the request, but not both. For more information, see
   * [Launch Template
   ...
   * With both `BEST_FIT_PROGRESSIVE` and `SPOT_CAPACITY_OPTIMIZED` allocation strategies using
   * On-Demand or Spot Instances, and the `BEST_FIT` strategy using Spot Instances, AWS Batch might
   * need to exceed `maxvCpus` to meet your capacity requirements. In this event, AWS Batch never
   * exceeds `maxvCpus` by more than a single instance. That is, no more than a single instance from
   ...
   * keeps your multi-node parallel job on a logical grouping of instances within a single Availability
   * Zone with high network flow potential. For more information, see [Placement
   ...
   * groups. For Fargate compute resources, providing an empty list is handled as if this parameter
   * wasn't specified and no change is made. For EC2 compute resources, providing an empty list removes
   * the security groups from the compute resource.
   *
   ...
   * made. For EC2 compute resources, providing an empty list removes the VPC subnets from the compute
   * resource. For more information, see [VPCs and
   * subnets](https://docs.aws.amazon.com/vpc/latest/userguide/VPC_Subnets.html) in the *Amazon VPC
   * User Guide* .
   *
   * When updating a compute environment, changing the VPC subnets requires an infrastructure update
   * of the compute environment. For more information, see [Updating compute
   * environments](https://docs.aws.amazon.com/batch/latest/userguide/updating-compute-environments.html)
   * in the *AWS Batch User Guide* .
   *
   *
   * AWS Batch on Amazon EC2 and AWS Batch on Amazon EKS support Local Zones. For more information,
   * see [Local
   ...
   * Zones](https://docs.aws.amazon.com/eks/latest/userguide/local-zones.html) in the *Amazon EKS User
   * Guide* and [Amazon ECS clusters in Local Zones, Wavelength Zones, and AWS
  ...
  /**
   * @param tags Key-value pair tags to be applied to EC2 resources that are launched in the compute
   * environment.
   * For AWS Batch , these take the form of `"String1": "String2"` , where `String1` is the tag key
   * and `String2` is the tag value-for example, `{ "Name": "Batch Instance - C4OnDemand" }` . This is
   * helpful for recognizing your AWS Batch instances in the Amazon EC2 console. These tags aren't seen
   * when using the AWS Batch `ListTagsForResource` API operation.
   *
   * When updating a compute environment, changing this setting requires an infrastructure update of
   * the compute environment. For more information, see [Updating compute
   * environments](https://docs.aws.amazon.com/batch/latest/userguide/updating-compute-environments.html)
   ...
   * determined by the launch template that's specified in the `launchTemplate` parameter. If an AMI ID
   * is specified any of these ways, this parameter is ignored. For more information about to update
   * AMI IDs during an infrastructure update, see [Updating the AMI
