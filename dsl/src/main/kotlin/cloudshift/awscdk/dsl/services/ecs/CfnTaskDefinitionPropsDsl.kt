...
public class CfnTaskDefinitionPropsDsl {
   ...
   * The valid values are `host` or `task` . If `host` is specified, then all containers within the
   * tasks that specified the `host` PID mode on the same container instance share the same process
   * namespace with the host Amazon EC2 instance. If `task` is specified, all containers within the
   * specified task share the same process namespace. If no value is specified, the default is a
   * private namespace. For more information, see [PID
   ...
   * The valid values are `EC2` , `FARGATE` , and `git diff` . For more information, see [Amazon ECS
