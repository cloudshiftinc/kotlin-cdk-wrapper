 ...
 *   `sni-only` . This is recommended. Most browsers and clients support SNI. (In CloudFormation,
 *   the field name is `SslSupportMethod` . Note the different capitalization.)
 ...
 *   charges from CloudFront. (In CloudFormation, the field name is `SslSupportMethod` . Note the
 *   different capitalization.)
 * * The minimum SSL/TLS protocol version that the distribution can use to communicate with viewers.
 *   To specify a minimum version, choose a value for `MinimumProtocolVersion` . For more
 *   information, see
...
public class CfnDistributionViewerCertificatePropertyDsl {
     ...
     *   different capitalization. If the distribution uses `Aliases` (alternate domain names or
     ...
     *   different capitalization. If the distribution uses `Aliases` (alternate domain names or
     *   CNAMEs), specify which viewers the distribution accepts HTTPS connections from.
     ...
     *   feature by the CloudFront team. If you have a use case that requires static IP addresses
     *   for a distribution, contact CloudFront through the
