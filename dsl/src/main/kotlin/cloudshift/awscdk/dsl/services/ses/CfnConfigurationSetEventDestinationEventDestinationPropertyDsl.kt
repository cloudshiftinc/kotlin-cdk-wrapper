...
public class CfnConfigurationSetEventDestinationEventDestinationPropertyDsl {
  ...
  /**
   * @param cloudWatchDestination An object that contains the names, default values, and sources of
   * the dimensions associated with an Amazon CloudWatch event destination.
   */
   ...
   * email with the associated configuration set.
   * Set to `true` to enable publishing to this destination; set to `false` to prevent publishing to
   ...
   * * `renderingFailure` - The email wasn't sent because of a template rendering issue. This event
   * type can occur when template data is missing, or when there is a mismatch between template
