...
public class CfnInstancePortPropertyDsl {
     ...
     *   (0.0.0.0/0)` , or `Custom` if a specific IP address or range of IP addresses is allowed.
     */
     ...
     *   addresses. The only alias currently supported is `lightsail-connect` , which allows IP
    ...
    /**
     * @param fromPort The first port in a range of open ports on an instance. Allowed ports:
     ...
     *   allowed to connect to an instance through the ports, and the protocol. Only devices with an
     *   IPv6 address can connect to an instance through IPv6; otherwise, IPv4 should be used.
     *
     ...
     *   network. If you have an application that doesn't require reliable data stream service, use
     *   UDP instead.
     ...
     *   not required to set up transmission channels or data paths. Applications that don't require
     ...
     *   operational information indicating success or failure when communicating with an instance.
     *   For example, an error is indicated when an instance could not be reached. When you specify
     *   `icmp` as the `protocol` , you must specify the ICMP type using the `fromPort` parameter,
