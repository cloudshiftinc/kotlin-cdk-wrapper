...
public class NodejsFunctionDsl(
     ...
     *
     * Default: - the path is found by walking up parent directories searching for a `yarn.lock`,
     * `pnpm-lock.yaml` or `package-lock.json` file
     *
     ...
     * `NodejsFunction` is defined in `stack.ts` with `my-handler` as id (`new NodejsFunction(this,
     * 'my-handler')`), the construct will look at `stack.my-handler.ts` and `stack.my-handler.js`.
     *
     ...
     *
     * The handler is prefixed with `index.` unless the specified handler value contains a `.`, in
     * which case it is used as-is.
     *
     * Default: handler
     *
