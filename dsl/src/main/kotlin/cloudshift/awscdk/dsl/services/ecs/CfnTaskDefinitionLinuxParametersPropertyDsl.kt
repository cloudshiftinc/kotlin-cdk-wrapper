...
public class CfnTaskDefinitionLinuxParametersPropertyDsl {
  ...
  /**
   * @param capabilities The Linux capabilities for the container that are added to or dropped from
  ...
  /**
   * @param initProcessEnabled Run an `init` process inside the container that forwards signals and
   * reaps processes.
   ...
   * `swappiness` value of `100` will cause pages to be swapped very aggressively. Accepted values are
   * whole numbers between `0` and `100` . If the `swappiness` parameter is not specified, a default
   * value of `60` is used. If a value is not specified for `maxSwap` then this parameter is ignored.
