...
public class CfnBudgetBudgetDataPropertyDsl {
     ...
     *   coverage budgets. You can't use `BudgetLimit` with `PlannedBudgetLimits` for `CreateBudget`
     ...
     *   `BudgetName` in the template, Billing and Cost Management assigns your budget a randomly
     *   generated name.
     */
     ...
     *   future limits. `PlannedBudgetLimits` is available for cost or usage budget and supports
     ...
     * the next three quarters. The `key` is the start of the quarter, `UTC` in epoch seconds.
     *
     ...
     * to use the last limit as the `BudgetLimit` . At that point, the planned budget provides the
    ...
    /**
     * @param timePeriod The period of time that is covered by a budget. The period has a start date
     *   and an end date. The start date must come before the end date. There are no restrictions on
     *   the end date.
     *
     ...
     * For example, if you create your budget on January 24, 2019, choose `MONTHLY` , and don't set
     * a start date, the start date defaults to `01/01/19 00:00 UTC` . The defaults are the same for
