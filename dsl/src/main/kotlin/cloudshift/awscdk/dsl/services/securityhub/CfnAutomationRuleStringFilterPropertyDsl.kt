...
public class CfnAutomationRuleStringFilterPropertyDsl {
  ...
  /**
   * @param comparison The condition to apply to a string value when querying for findings. 
   ...
   * that starts with `AwsIam` . Findings with a resource type of `AwsIamPolicy` , `AwsIamRole` , or
   * `AwsIamUser` would all match.
   *
   * `EQUALS` and `PREFIX` filters on the same field are joined by `OR` . A finding matches if it
   ...
   * resource type that does not start with `AwsIam` . Findings with a resource type of `AwsIamPolicy`
   * , `AwsIamRole` , or `AwsIamUser` would all be excluded from the results.
   *
   * `NOT_EQUALS` and `PREFIX_NOT_EQUALS` filters on the same field are joined by `AND` . A finding
   * matches only if it matches all of those filters.
   *
   * For filters on the same field, you cannot provide both an `EQUALS` filter and a `NOT_EQUALS` or
   * `PREFIX_NOT_EQUALS` filter. Combining filters in this way always returns an error, even if the
   * provided filter values would return valid results.
   *
   * You can combine `PREFIX` filters with `NOT_EQUALS` or `PREFIX_NOT_EQUALS` filters for the same
   * field. Security Hub first processes the `PREFIX` filters, then the `NOT_EQUALS` or
   * `PREFIX_NOT_EQUALS` filters.
   *
   * For example, for the following filter, Security Hub first identifies findings that have
   * resource types that start with either `AwsIAM` or `AwsEc2` . It then excludes findings that have a
   * resource type of `AwsIamPolicy` and findings that have a resource type of `AwsEc2NetworkInterface`
   * .
   *
   ...
   * `Security Hub` . If you provide `security hub` as the filter text, then there is no match.
   */
