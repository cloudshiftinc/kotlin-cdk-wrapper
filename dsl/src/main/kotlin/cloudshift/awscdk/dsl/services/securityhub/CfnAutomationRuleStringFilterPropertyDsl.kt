...
public class CfnAutomationRuleStringFilterPropertyDsl {
    ...
    /**
     * @param comparison The condition to apply to a string value when querying for findings. To
     *   search for values that contain the filter criteria value, use one of the following
     ...
     * `AwsIamPolicy` , `AwsIamRole` , or `AwsIamUser` would all be excluded from the results.
     *
     ...
     *
     * For filters on the same field, you cannot provide both an `EQUALS` filter and a `NOT_EQUALS`
     * or `PREFIX_NOT_EQUALS` filter. Combining filters in this way always returns an error, even if
     * the provided filter values would return valid results.
     *
     * You can combine `PREFIX` filters with `NOT_EQUALS` or `PREFIX_NOT_EQUALS` filters for the
     * same field. Security Hub first processes the `PREFIX` filters, then the `NOT_EQUALS` or
    ...
    /**
     * @param value The string filter value. Filter values are case sensitive. For example, the
     *   product name for control-based findings is `Security Hub` . If you provide `security hub`
