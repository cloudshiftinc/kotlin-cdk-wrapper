...
public class CfnRecordSetAliasTargetPropertyDsl {
  ...
  /**
   * @param dnsName *Alias records only:* The value that you specify depends on where you want to
   * route queries:. 
   * * **Amazon API Gateway custom regional APIs and edge-optimized APIs** - Specify the applicable
   * domain name for your API. You can get the applicable value using the AWS CLI command
   * [get-domain-names](https://docs.aws.amazon.com/cli/latest/reference/apigateway/get-domain-names.html)
   * :
   * * For regional APIs, specify the value of `regionalDomainName` .
   * * For edge-optimized APIs, specify the value of `distributionDomainName` . This is the name of
   * the associated CloudFront distribution, such as `da1b2c3d4e5.cloudfront.net` .
   *
   *
   * The name of the record that you're creating must match a custom domain name for your API, such
   * as `api.example.com` .
   *
   *
   ...
   * record. For example, if the name of the record is *acme.example.com* , your CloudFront
   ...
   * and secondary records. A distribution must include an alternate domain name that matches the name
   * of the record. However, the primary and secondary records have the same name, and you can't
   * include the same alternate domain name in more than one distribution.
   *
   *
   * * **Elastic Beanstalk environment** - If the domain name for your Elastic Beanstalk environment
   * includes the region that you deployed the environment in, you can create an alias record that
   * routes traffic to the environment. For example, the domain name `my-environment. *us-west-2*
   ...
   * region. To route traffic to these environments, you must create a CNAME record instead of an alias
   * record. Note that you can't create a CNAME record for the root domain name. For example, if your
   * domain name is example.com, you can create a record that routes traffic for acme.example.com to
   * your Elastic Beanstalk environment, but you can't create a record that routes traffic for
   * example.com to your Elastic Beanstalk environment.
   *
   *
   ...
   * Beanstalk](https://docs.aws.amazon.com/elasticbeanstalk/latest/dg/customdomains.html) in the *AWS
   ...
   * .
   * * **Amazon S3 bucket that is configured as a static website** - Specify the domain name of the
   ...
   * 53](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/getting-started.html) in the *Amazon
   ...
   * apex), you can't specify the domain name for a record for which the value of `Type` is `CNAME` .
   * This is because the alias record must have the same type as the record that you're routing traffic
  ...
  /**
   * @param evaluateTargetHealth *Applies only to alias, failover alias, geolocation alias, latency
   * alias, and weighted alias resource record sets:* When `EvaluateTargetHealth` is `true` , an alias
   * resource record set inherits the health of the referenced AWS resource, such as an ELB load
   ...
   * Elastic Load Balancing routes queries only to the healthy Amazon EC2 instances that are registered
   * with the load balancer. (An environment automatically contains an ELB load balancer if it includes
   * more than one Amazon EC2 instance.) If you set `EvaluateTargetHealth` to `true` and either no
   * Amazon EC2 instances are healthy or the load balancer itself is unhealthy, Route 53 routes queries
   ...
   * * *Classic Load Balancers* : If you specify an ELB Classic Load Balancer in `DNSName` , Elastic
   * Load Balancing routes queries only to the healthy Amazon EC2 instances that are registered with
   * the load balancer. If you set `EvaluateTargetHealth` to `true` and either no EC2 instances are
   * healthy or the load balancer itself is unhealthy, Route 53 routes queries to other resources.
   * * *Application and Network Load Balancers* : If you specify an ELB Application or Network Load
   * Balancer and you set `EvaluateTargetHealth` to `true` , Route 53 routes queries to the load
   * balancer based on the health of the target groups that are associated with the load balancer:
   * * For an Application or Network Load Balancer to be considered healthy, every target group that
   * contains targets must contain at least one healthy target. If any target group contains only
   * unhealthy targets, the load balancer is considered unhealthy, and Route 53 routes queries to other
   * resources.
   ...
   * * **Other records in the same hosted zone** - If the AWS resource that you specify in `DNSName`
   * is a record or a group of records (for example, a group of weighted records) but is not another
   * alias record, we recommend that you associate a health check with all of the records in the alias
  ...
  /**
   * @param hostedZoneId *Alias resource records sets only* : The value used depends on where you
   * want to route traffic:. 
   * * **Amazon API Gateway custom regional APIs and edge-optimized APIs** - Specify the hosted zone
   * ID for your API. You can get the applicable value using the AWS CLI command
   * [get-domain-names](https://docs.aws.amazon.com/cli/latest/reference/apigateway/get-domain-names.html)
   * :
   * * For regional APIs, specify the value of `regionalHostedZoneId` .
   * * For edge-optimized APIs, specify the value of `distributionHostedZoneId` .
   * * **Amazon Virtual Private Cloud interface VPC endpoint** - Specify the hosted zone ID for your
   * interface endpoint. You can get the value of `HostedZoneId` using the AWS CLI command
   * [describe-vpc-endpoints](https://docs.aws.amazon.com/cli/latest/reference/ec2/describe-vpc-endpoints.html)
   * .
   ...
   * .
   * * *AWS CLI* : Use `describe-load-balancers` to get the applicable value. For more information,
