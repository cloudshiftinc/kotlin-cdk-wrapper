...
public class CfnUserDsl(
     ...
     * path is made visible and `Target` is the actual Amazon S3 path. If you only specify a target,
     * it will be displayed as is. You will need to also make sure that your IAM role provides
     ...
     * ignored. As a workaround, you can use the Amazon S3 API to create 0 byte objects as place
     * holders for your directory. If using the CLI, use the `s3api` call instead of `s3` so you can
     * use the put-object operation. For example, you use the following: `AWS s3api put-object
     * --bucket bucketname --key path/to/folder/` . Make sure that the end of the key name ends in a
