...
public class CfnWebACLRulePropertyDsl {
   ...
   * * If the rule statement references a rule group, you must not set this action setting, because
   * the actions are already set on the rules inside the rule group. You must set the rule's override
   * action setting to indicate specifically whether to override the actions that are set on the rules
   * in the rule group.
   */
  ...
  /**
   * @param overrideAction The override action to apply to the rules in a rule group, instead of the
   * individual rule action settings.
   * This is used only for rules whose statements reference a rule group. Rule statements that
   * reference a rule group are `RuleGroupReferenceStatement` and `ManagedRuleGroupStatement` .
   *
   * Set the override action to none to leave the rule group rule actions in effect. Set it to count
   * to only count matches, regardless of the rule action settings.
   *
   * You must set either this `OverrideAction` setting or the `Action` setting, but not both:
   *
   * * If the rule statement references a rule group, you must set this override action setting and
   * you must not set the rule's action setting.
