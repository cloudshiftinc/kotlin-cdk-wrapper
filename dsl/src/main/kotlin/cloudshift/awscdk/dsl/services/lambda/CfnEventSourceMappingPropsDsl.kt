...
public class CfnEventSourceMappingPropsDsl {
  ...
  /**
   * @param batchSize The maximum number of records in each batch that Lambda pulls from your stream
   * or queue and sends to your function.
  ...
  /**
   * @param destinationConfig (Kinesis and DynamoDB Streams only) An Amazon SQS queue or Amazon SNS
  ...
  /**
   * @param enabled When true, the event source mapping is active. When false, Lambda pauses polling
   * and invocation.
   * Default: True
   */
  ...
  /**
   * @param functionResponseTypes (Streams and SQS) A list of current response type enums applied to
  ...
  /**
   * @param maximumRecordAgeInSeconds (Kinesis and DynamoDB Streams only) Discard records older than
   * the specified age.
   ...
   * processing window for DynamoDB and Kinesis Streams event sources.
