...
public class CfnReplicaKeyPropsDsl {
     ...
     * AWS Key Management Service does not synchronize this property.
     */
     ...
     *   in cryptographic operations. When `Enabled` is `true` , the *key state* of the KMS key is
     ...
     * operations. Also, while the replica key is being created, its key state is `Creating` . When
     * the process is complete, the key state of the replica key changes to `Enabled` .
     *
    ...
    /**
     * @param keyPolicy The key policy that authorizes use of the replica key. The key policy is not
     *   a shared property of multi-Region keys. You can specify the same key policy or a different
     *   key policy for each key in a set of related multi-Region keys. AWS KMS does not synchronize
     *   this property.
     *
     ...
     *   permission on the KMS key. This reduces the risk that the KMS key becomes unmanageable. For
     ...
     *   deletes a replica key that has been removed from a CloudFormation stack. Enter a value
     ...
     * determines the length of waiting period. During the waiting period, the key state of replica
     * key is `Pending Deletion` , which prevents it from being used in cryptographic operations.
     * When the waiting period expires, AWS KMS permanently deletes the replica key.
     *
     ...
     * your template that is based on the same primary key as the original replica key,
    ...
    /**
     * @param primaryKeyArn Specifies the multi-Region primary key to replicate. The primary key
     *   must be in a different AWS Region of the same AWS partition. You can create only one
     ...
     * and costs aggregated by tags. Tags can also be used to control access to a KMS key. For
     * details, see
