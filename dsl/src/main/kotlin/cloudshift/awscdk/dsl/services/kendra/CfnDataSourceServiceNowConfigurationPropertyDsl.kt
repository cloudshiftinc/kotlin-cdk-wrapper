...
public class CfnDataSourceServiceNowConfigurationPropertyDsl {
     ...
     *   instance. If you choose `HTTP_BASIC` , Amazon Kendra is authenticated using the user name
     *   and password provided in the AWS Secrets Manager secret in the `SecretArn` field. If you
     *   choose `OAUTH2` , Amazon Kendra is authenticated using the credentials of client ID, client
     *   secret, user name and password.
     *
     * When you use `OAUTH2` authentication, you must generate a token and a client secret using the
     * ServiceNow console. For more information, see
    ...
    /**
     * @param hostUrl The ServiceNow instance that the data source connects to. The host endpoint
    ...
    public fun knowledgeArticleConfiguration(
        knowledgeArticleConfiguration: CfnDataSource.ServiceNowKnowledgeArticleConfigurationProperty
    ) {
        cdkBuilder.knowledgeArticleConfiguration(knowledgeArticleConfiguration)
    }
     ...
     *   contains the user name and password required to connect to the ServiceNow instance. You can
     *   also provide OAuth authentication credentials of user name, password, client ID, and client
    ...
    public fun serviceCatalogConfiguration(
        serviceCatalogConfiguration: CfnDataSource.ServiceNowServiceCatalogConfigurationProperty
    ) {
        cdkBuilder.serviceCatalogConfiguration(serviceCatalogConfiguration)
    }
     ...
     *   running. If the host is not running the `LONDON` release, use `OTHERS` .
     */
