...
public class CfnResponseHeadersPolicyServerTimingHeadersConfigPropertyDsl {
  ...
  /**
   * @param enabled A Boolean that determines whether CloudFront adds the `Server-Timing` header to
   * HTTP responses that it sends in response to requests that match a cache behavior that's associated
  ...
  /**
   * @param samplingRate A number 0–100 (inclusive) that specifies the percentage of responses that
   * you want CloudFront to add the `Server-Timing` header to.
   ...
   * requests that match the cache behavior. You can set the sampling rate to any number 0–100 with up
