...
public class CfnMethodIntegrationPropertyDsl {
     ...
     *   valid values for `cacheKeyParameters` , these parameters must also be specified for Method
     *   `requestParameters` .
     */
     ...
     *   uses the resource ID as the `cacheNamespace` . You can specify the same `cacheNamespace`
    ...
    /**
     * @param credentials Specifies the credentials required for the integration, if any. For AWS
    ...
    /**
     * @param integrationHttpMethod Specifies the integration's HTTP method type. For the Type
     *   property, if you specify `MOCK` , this property is optional. For Lambda integrations, you
     *   must set the integration method to `POST` . For all other types, you must specify this
     *   property.
     */
     ...
     *   will be passed through the integration request to the back end without transformation. A
     *   content type is unmapped if no mapping template is defined in the integration or the
     ...
     *   mapping templates defined in the integration request. `WHEN_NO_TEMPLATES` : passes the
     *   method request body through the integration request to the back end without transformation
     *   when no mapping template is defined in the integration request. If a template is defined
     *   when this option is selected, the method request of an unmapped content-type will be
     *   rejected with an HTTP 415 Unsupported Media Type response. `NEVER` : rejects the method
     *   request with an HTTP 415 Unsupported Media Type response when either the method request
     *   content type does not match any content type associated with the mapping templates defined
    ...
    /**
     * @param timeoutInMillis Custom timeout between 50 and 29,000 milliseconds. The default value
     ...
     * ports above 1024. An HTTP or HTTP proxy integration with a `connectionType` of `VPC_LINK` is
    ...
    /**
     * @param uri Specifies Uniform Resource Identifier (URI) of the integration endpoint. For
