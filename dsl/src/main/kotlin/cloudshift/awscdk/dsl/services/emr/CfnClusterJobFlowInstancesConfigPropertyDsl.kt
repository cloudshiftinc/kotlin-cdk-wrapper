...
public class CfnClusterJobFlowInstancesConfigPropertyDsl {
  ...
  /**
   * @param additionalSlaveSecurityGroups A list of additional Amazon EC2 security group IDs for the
   * core and task nodes.
   */
  ...
  /**
   * @param ec2KeyName The name of the Amazon EC2 key pair that can be used to connect to the master
   ...
   * maintained), "0.20.205" (no longer maintained), "1.0.3", "2.2.0", or "2.4.0". If you do not set
   * this value, the default of 0.18 is used, unless the `AmiVersion` parameter is set in the
  ...
  /**
   * @param keepJobFlowAliveWhenNoSteps Specifies whether the cluster should remain available after
   * completing all steps.
   * Defaults to `true` . For more information about configuring cluster termination, see [Control
   * Cluster
   * Termination](https://docs.aws.amazon.com/emr/latest/ManagementGuide/emr-plan-termination.html) in
  ...
  /**
   * @param taskInstanceFleets Describes the EC2 instances and instance configurations for the task
   * instance fleets when using clusters with the instance fleet configuration.
   * These task instance fleets are added to the cluster as part of the cluster launch. Each task
   * instance fleet must have a unique name specified so that CloudFormation can differentiate between
   ...
   * `TaskInstanceFleets` , Amazon EMR uses its default allocation strategy, which allows for a maximum
   * of five Amazon EC2 instance types.
   */
   ...
   * instance group must have a unique name specified so that CloudFormation can differentiate between
   * the task instance groups.
   *
   *
   * After creating the cluster, you can only modify the mutable properties of `InstanceGroupConfig`
   * , which are `AutoScalingPolicy` and `InstanceCount` . Modifying any other property results in
   * cluster replacement.
   */
