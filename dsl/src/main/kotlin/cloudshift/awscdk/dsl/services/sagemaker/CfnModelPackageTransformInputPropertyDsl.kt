...
public class CfnModelPackageTransformInputPropertyDsl {
    ...
    /**
     * @param contentType The multipurpose internet mail extension (MIME) type of the data. Amazon
     *   SageMaker uses the MIME type with each http call to transfer data to the transform job.
     */
     ...
     *   batches. Splitting is necessary when the total size of each object is too large to fit in a
     ...
     *   contents of an input object. Set the value of this parameter to `Line` to split records on
     *   a newline character boundary. `SplitType` also supports a number of record-oriented binary
     *   data formats. Currently, the supported record formats are:
     * * RecordIO
     * * TFRecord
     *
