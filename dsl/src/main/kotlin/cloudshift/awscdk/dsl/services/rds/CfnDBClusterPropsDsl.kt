...
public class CfnDBClusterPropsDsl {
     ...
     *   can be created. For information on AWS Regions and Availability Zones, see
    ...
    /**
     * @param databaseName The name of your database. If you don't provide a name, then Amazon RDS
     *   won't create a database in this DB cluster. For naming constraints, see
     ...
     *   available in all AWS Regions , or for all database engines. For the full list of DB
     ...
     *   `copy-on-write` , and don't specify a DB subnet group name, then the DB cluster is restored
     *   with a default DB subnet group.
     *
     * Valid for: Aurora DB clusters and Multi-AZ DB clusters
     */
     ...
     *   the Directory Service. Valid for: Aurora DB clusters only
     */
    ...
    /**
     * @param engine The name of the database engine to be used for this DB cluster. Valid Values:
     * * `aurora-mysql`
     * * `aurora-postgresql`
     * * `mysql`
     * * `postgres`
    ...
    /**
     * @param engineVersion The version number of the database engine to use. To list all of the
     *   available engine versions for Aurora MySQL version 2 (5.7-compatible) and version 3
     ...
     *   want your Aurora DB cluster to be a secondary member in the global database cluster,
     *   specify the global cluster ID of the global database cluster. To define the primary
     *   database cluster of the global cluster, use the
     ...
     *   enable the `StorageEncrypted` property but don't specify this property, the default KMS key
     *   is used. If you specify this property, you must set the `StorageEncrypted` property to
     *   `true` .
     *
     ...
     * `GlobalClusterIdentifier` property, don't specify this property. The value is inherited from
     * the source DB cluster, the snapshot, or the primary DB cluster for the global database
     ...
     *   send Enhanced Monitoring metrics to Amazon CloudWatch Logs. An example is
    ...
    /**
     * @param networkType The network type of the DB cluster. Valid values:
     * * `IPV4`
     * * `DUAL`
     *
     ...
     *   DB cluster. For more information, see
     ...
     *   data. Valid for Cluster Type: Multi-AZ DB clusters only
     *
     * Valid Values:
     * * `7`
    ...
    /**
     * @param publiclyAccessible Specifies whether the DB cluster is publicly accessible. When the
    ...
    /**
     * @param restoreToTime The date and time to restore the DB cluster to. Valid Values: Value must
     *   be a time in Universal Coordinated Time (UTC) format
     *
     * Constraints:
    ...
    public fun serverlessV2ScalingConfiguration(
        serverlessV2ScalingConfiguration: CfnDBCluster.ServerlessV2ScalingConfigurationProperty
    ) {
        cdkBuilder.serverlessV2ScalingConfiguration(serverlessV2ScalingConfiguration)
    }
     ...
     * property, an empty DB cluster is created, and the original DB cluster is deleted. If you
     * specify a property that is different from the previous snapshot restore property, a new DB
     * cluster is restored from the specified `SnapshotIdentifier` property, and the original DB
     * cluster is deleted.
     *
     ...
     *   identifier of the source DB cluster from which to restore. Constraints:
     ...
     *   cluster. For example, `us-east-1` . Valid for: Aurora DB clusters only
     */
     ...
     * use this property to specify that the restored DB cluster is encrypted. Specify the
     * `KmsKeyId` property for the KMS key to use for encryption. If you don't want the restored DB
     * cluster to be encrypted, then don't set this property or set it to `false` .
     *
     * Valid for: Aurora DB clusters and Multi-AZ DB clusters
     */
     ...
     *   storage types for Aurora DB clusters, see
    ...
    /**
     * @param tags An optional array of key-value pairs to apply to this DB cluster. Valid for:
     ...
     *   the latest restorable backup time. By default, the DB cluster is not restored to the latest
     ...
     *   cluster. If you plan to update the resource, don't specify VPC security groups in a shared
     *   VPC.
     *
     * Valid for: Aurora DB clusters and Multi-AZ DB clusters
     */
