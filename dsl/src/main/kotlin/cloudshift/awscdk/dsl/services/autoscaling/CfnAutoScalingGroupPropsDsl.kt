...
public class CfnAutoScalingGroupPropsDsl {
  ...
  /**
   * @param defaultInstanceWarmup The amount of time, in seconds, until a new instance is considered
   * to have finished initializing and resource consumption to become stable after it enters the
   * `InService` state.
   ...
   * waits before checking the health status of an EC2 instance that has come into service and marking
   * it unhealthy due to a failed health check.
  ...
  /**
   * @param serviceLinkedRoleArn The Amazon Resource Name (ARN) of the service-linked role that the
   * Auto Scaling group uses to call other AWS service on your behalf.
  ...
  /**
   * @param terminationPolicies A policy or a list of policies that are used to select the instance
   * to terminate.
  ...
  /**
   * @param vpcZoneIdentifier A list of subnet IDs for a virtual private cloud (VPC) where instances
