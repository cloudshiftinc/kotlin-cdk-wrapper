...
public class CfnLaunchConfigurationBlockDevicePropertyDsl {
   ...
   * Encrypted EBS volumes can only be attached to instances that support Amazon EBS encryption. For
  ...
  /**
   * @param iops The number of input/output (I/O) operations per second (IOPS) to provision for the
   * volume.
   * For `gp3` and `io1` volumes, this represents the number of IOPS that are provisioned for the
   * volume. For `gp2` volumes, this represents the baseline performance of the volume and the rate at
   ...
   * * `standard` : 1-1,024
   *
   * You must specify either a `SnapshotId` or a `VolumeSize` . If you specify both `SnapshotId` and
   * `VolumeSize` , the volume size must be equal or greater than the size of the snapshot.
   */
