...
public class CfnAuthorizerDsl(
     ...
     * the remaining substring in the URI should be treated as the path to the resource, including
     ...
     * the client. For example, if the token header name is `Auth` , the header mapping expression
     * is `method.request.header.Auth` . For the `REQUEST` authorizer, this is required when
     * authorization caching is enabled. The value is a comma-separated string of one or more
     * mapping expressions of the specified request parameters. For example, if an `Auth` header, a
     ...
     * when there is a match. Otherwise, it will return a 401 Unauthorized response without calling
     * the Lambda function. The validation expression does not apply to the `REQUEST` authorizer.
     *
