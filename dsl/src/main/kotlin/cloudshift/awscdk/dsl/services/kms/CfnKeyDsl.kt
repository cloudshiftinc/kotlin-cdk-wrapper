 ...
 * [KMS key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#kms_keys) in AWS
 * Key Management Service . You can use this resource to create symmetric encryption KMS keys,
 ...
 * property values. &gt; AWS KMS replaced the term *customer master key (CMK)* with *AWS KMS key*
 * and *KMS key* . The concept has not changed. To prevent breaking changes, AWS KMS is keeping some
 ...
 * encryption KMS key to encrypt data stored in AWS services that are
 ...
 * a `KeyUsage` value of `GENERATE_VERIFY_MAC` . For details, see
 * [HMAC keys in AWS KMS](https://docs.aws.amazon.com/kms/latest/developerguide/hmac.html) in the
...
public class CfnKeyDsl(
     ...
     * `SYMMETRIC_DEFAULT` ). For asymmetric KMS keys and HMAC KMS keys, omit the
     ...
     * copies the rotation status to all replica keys. For details, see
    ...
    /**
     * The key policy that authorizes use of the KMS key. The key policy must conform to the
     * following rules.
     * * The key policy must allow the caller to make a subsequent
     *   [PutKeyPolicy](https://docs.aws.amazon.com/kms/latest/APIReference/API_PutKeyPolicy.html)
     *   request on the KMS key. This reduces the risk that the KMS key becomes unmanageable. For
     ...
     *   principal in a key policy because the new principal might not be immediately visible to AWS
     ...
     *
     * If you are unsure of which policy to use, consider the *default key policy* . This is the key
     * policy that AWS KMS applies to KMS keys that are created by using the CreateKey API with no
     * specified key policy. It gives the AWS account that owns the key permission to perform all
     * operations on the key. It also allows you write IAM policies to authorize access to the key.
     * For details, see
     ...
     *
     * @param keyPolicy The key policy that authorizes use of the KMS key. The key policy must
     *   conform to the following rules.
     */
    public fun keyPolicy(keyPolicy: MapBuilder.() -> Unit = {}) {
        val builder = MapBuilder()
        builder.apply(keyPolicy)
     ...
     * encryption and decryption. In China Regions, `SYMMETRIC_DEFAULT` creates a 128-bit symmetric
     * key that uses SM4 encryption. You can't change the `KeySpec` value after the KMS key is
     * created. For help choosing a key spec for your KMS key, see
     ...
     * that the KMS key supports. To further restrict the algorithms that can be used with the KMS
     ...
     * fails, regardless of the value of the
     * [`UpdateReplacePolicy` attribute](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-updatereplacepolicy.html)
     ...
     * encryption with asymmetric KMS keys. For help determining whether a KMS key is asymmetric,
     * see
     ...
     * determines the length of waiting period. During the waiting period, the key state of KMS key
     * is `Pending Deletion` or `Pending Replica Deletion` , which prevents the KMS key from being
     * used in cryptographic operations. When the waiting period expires, AWS KMS permanently
     * deletes the KMS key.
     *
     ...
     * that has replica keys. If you remove a multi-Region primary key from a CloudFormation stack,
     * its key state changes to `PendingReplicaDeletion` so it cannot be replicated or used in
     * cryptographic operations. This state can persist indefinitely. When the last of its replica
     * keys is deleted, the key state of the primary key changes to `PendingDeletion` and the
     * waiting period specified by `PendingWindowInDays` begins. When this waiting period expires,
     * AWS KMS deletes the primary key. For details, see
     ...
     * it from the stack, regardless of the waiting period. If you specify a KMS key in your
     * template, even one with the same name, CloudFormation creates a new KMS key. To cancel
     ...
     * [Key state: Effect on your KMS key](https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html)
     * in the *AWS Key Management Service Developer Guide* . For more information about deleting KMS
     * keys, see the
     * [ScheduleKeyDeletion](https://docs.aws.amazon.com/kms/latest/APIReference/API_ScheduleKeyDeletion.html)
     ...
     * [Tagging keys](https://docs.aws.amazon.com/kms/latest/developerguide/tagging-keys.html) in
     * the *AWS Key Management Service Developer Guide* . For information about tags in
     * CloudFormation, see
