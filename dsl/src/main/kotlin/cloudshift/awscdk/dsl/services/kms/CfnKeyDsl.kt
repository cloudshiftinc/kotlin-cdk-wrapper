...
public class CfnKeyDsl(
   ...
   * `SYMMETRIC_DEFAULT` ). For asymmetric KMS keys and HMAC KMS keys, omit the `EnableKeyRotation`
   ...
   * refer to the scenario in the [Default key
   ...
   * operations on the key. It also allows you write IAM policies to authorize access to the key. For
   * details, see [Default key
   ...
   * @param keyPolicy The key policy that authorizes use of the KMS key. The key policy must conform
