...
public class CfnEndpointRedshiftSettingsPropertyDsl {
  ...
  /**
   * @param acceptAnyDate A value that indicates to allow any date format, including invalid formats
   * such as 00/00/00 00:00:00, to be loaded without generating an error.
  ...
  /**
   * @param bucketFolder An S3 folder where the comma-separated-value (.csv) files are stored before
   ...
   * *BucketFolder/TableID* path. AWS DMS uses the Redshift `COPY` command to upload the .csv files to
   * the target table. The files are deleted once the `COPY` operation has finished. For more
   * information, see [COPY](https://docs.aws.amazon.com/redshift/latest/dg/r_COPY.html) in the *Amazon
  ...
  /**
   * @param connectionTimeout A value that sets the amount of time to wait (in milliseconds) before
   ...
   * Valid values are `auto` (case-sensitive), your date format string enclosed in quotes, or NULL.
   * If this parameter is left unset (NULL), it defaults to a format of 'YYYY-MM-DD'. Using `auto`
  ...
  /**
   * @param emptyAsNull A value that specifies whether AWS DMS should migrate empty CHAR and VARCHAR
   * fields as NULL.
   ...
   * auto-generated values with explicit values loaded from the source data files used to populate the
  ...
  /**
   * @param loadTimeout The amount of time to wait (in milliseconds) before timing out of operations
   * performed by AWS DMS on a Redshift cluster, such as Redshift COPY, INSERT, DELETE, and UPDATE.
   */
  ...
  /**
   * @param maxFileSize The maximum size (in KB) of any .csv file used to load data on an S3 bucket
   ...
   * All characters within the quotation marks, including delimiters, are retained. Choose `true` to
   ...
   * If you are using `SSE_KMS` for the `EncryptionMode` , provide this key ID. The key that you use
   ...
   * number of characters, so that the data fits in the column.
  ...
  /**
   * @param writeBufferSize The size (in KB) of the in-memory file write buffer used when generating
   * .csv files on the local disk at the DMS replication instance. The default value is 1000 (buffer
   * size is 1000KB).
   */
