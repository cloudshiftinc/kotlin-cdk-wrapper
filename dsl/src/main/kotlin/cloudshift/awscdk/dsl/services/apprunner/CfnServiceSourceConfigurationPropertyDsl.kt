...
public class CfnServiceSourceConfigurationPropertyDsl {
   ...
   */
  public
      fun authenticationConfiguration(authenticationConfiguration: CfnService.AuthenticationConfigurationProperty) {
    cdkBuilder.authenticationConfiguration(authenticationConfiguration)
  }

  /**
   * @param autoDeploymentsEnabled If `true` , continuous integration from the source repository is
   * enabled for the App Runner service.
   * Each repository change (including any source code commit or new image version) starts a
   * deployment.
   *
   * Default: App Runner sets to `false` for a source image that uses an ECR Public repository or an
   * ECR repository that's in an AWS account other than the one that the service is in. App Runner sets
   * to `true` in all other cases (which currently include a source code repository or a source image
