...
public class CfnRecordSetDsl(
  ...
  private val cdkBuilder: CfnRecordSet.Builder = CfnRecordSet.Builder.create(scope, id)
   ...
   * @param aliasTarget *Alias resource record sets only:* Information about the AWS resource, such
   * as a CloudFront distribution or an Amazon S3 bucket, that you want to route traffic to. 
   */
   ...
   * @param healthCheckId If you want Amazon Route 53 to return this resource record set in response
   * to a DNS query only when the status of a health check is healthy, include the `HealthCheckId`
   ...
   * . You can't use an * for one of the middle labels, for example, `marketing.*.example.com` . In
   * addition, the * must replace the entire label; for example, you can't specify `prod*.example.com`
   * .
   *
   ...
   * @param resourceRecords One or more values that correspond with the value that you specified for
   * the `Type` property. 
   */
   ...
   * of name and type, such as multiple weighted resource record sets named acme.example.com that have
   * a type of A. In a group of resource record sets that have the same name and type, the value of
   ...
   * same combination of DNS name and type, a value that determines the proportion of DNS queries that
