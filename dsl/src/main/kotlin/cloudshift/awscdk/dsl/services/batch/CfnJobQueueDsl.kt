...
public class CfnJobQueueDsl(
     ...
     * job. Compute environments must be in the `VALID` state before you can associate them with a
     * job queue. You can associate up to three compute environments with a job queue. All of the
     ...
     * are evaluated first when associated with the same compute environment. Priority is determined
     * in descending order. For example, a job queue with a priority value of `10` is given
     * scheduling preference over a job queue with a priority value of `1` . All of the compute
