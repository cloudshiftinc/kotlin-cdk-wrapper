...
public class CfnTaskDefinitionPortMappingPropertyDsl {
   ...
   * This parameter only applies to Service Connect. We recommend that you set this parameter to be
   * consistent with the protocol that your application uses. If you set this parameter, Amazon ECS
   * adds protocol-specific connection handling to the Service Connect proxy. If you set this
   ...
   * and not a host port, your container automatically receives a host port in the ephemeral port
   * range. For more information, see `hostPort` . Port mappings that are automatically assigned in
   ...
   * default ephemeral port range from 49153 through 65535 is used. Do not attempt to specify a host
   * port in the ephemeral port range as these are reserved for automatic assignment. In general, ports
   ...
   * `serviceConnectConfiguration` of a service. The name can include up to 64 characters. The
