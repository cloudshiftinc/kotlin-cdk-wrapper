...
public class CfnContainerContainerPropertyDsl {
   ...
   * Container images that are sourced from (registered and stored on) your container service start
   * with a colon ( `:` ). For example, if your container service name is `container-service-1` , the
   * container image label is `mystaticsite` , and you want to use the third version ( `3` ) of the
   * registered container image, then you should specify `:container-service-1.mystaticsite.3` . To use
   * the latest version of a container image, specify `latest` instead of a version number (for
   * example, `:container-service-1.mystaticsite.latest` ). Your container service will automatically
