...
public class DashboardPropsDsl {
    ...
    /**
     * @param dashboardName Name of the dashboard. If set, must only contain alphanumerics, dash (-)
     *   and underscore (_)
     */
     ...
     *   dashboard loads. If you specify a value for end, you must also specify a value for start.
     ...
     *   loads. Specifying `Auto` causes the period of all graphs on the dashboard to automatically
     *   adapt to the time range of the dashboard. Specifying `Inherit` ensures that the period set
     *   for each graph is always obeyed.
     */
    ...
    /**
     * @param start The start of the time range to use for each widget on the dashboard. You can
     *   specify start without specifying end to specify a relative time range that ends with the
     *   current time. In this case, the value of start must begin with -P, and you can use M, H, D,
     *   W and M as abbreviations for minutes, hours, days, weeks and months. For example, -PT8H
    ...
    /**
     * @param widgets Initial set of widgets on the dashboard. One array represents a row of
     *   widgets.
     */
