...
public class CfnServiceDnsRecordPropertyDsl {
  ...
  /**
   * @param ttl The amount of time, in seconds, that you want DNS resolvers to cache the settings
   * for this record. 
   *
   * Alias records don't include a TTL because Route 53 uses the TTL for the AWS resource that an
   * alias record routes traffic to. If you include the `AWS_ALIAS_DNS_NAME` attribute when you submit
   * a [RegisterInstance](https://docs.aws.amazon.com/cloud-map/latest/api/API_RegisterInstance.html)
  ...
  /**
   * @param type The type of the resource, which indicates the type of value that Route 53 returns
   ...
   * * **SRV** - Route 53 returns the value for an `SRV` record. The value for an `SRV` record uses
