...
public class CfnDeliveryStreamRedshiftDestinationConfigurationPropertyDsl {
  ...
  /**
   * @param password The password for the Amazon Redshift user that you specified in the `Username`
   * property. 
   */
  ...
  /**
   * @param roleArn The ARN of the AWS Identity and Access Management (IAM) role that grants Kinesis
   ...
   * After the data is in the bucket, Kinesis Data Firehose uses the `COPY` command to load the data
   * into the Amazon Redshift cluster. For the Amazon S3 bucket's compression format, don't specify
   * `SNAPPY` or `ZIP` because the Amazon Redshift `COPY` command doesn't support them.
   */
