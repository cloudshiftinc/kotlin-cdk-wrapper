...
public class CfnEndpointKafkaSettingsPropertyDsl {
  ...
  /**
   * @param broker A comma-separated list of one or more broker locations in your Kafka cluster that
   ...
   * Migration Service](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_Target.Kafka.html) in the
  ...
  /**
   * @param saslPassword The secure password that you created when you first set up your Amazon MSK
   * cluster to validate a client identity and make an encrypted connection between server and client
  ...
  /**
   * @param securityProtocol Set secure connection to a Kafka target endpoint using Transport Layer
   * Security (TLS).
   * Options include `ssl-encryption` , `ssl-authentication` , and `sasl-ssl` . `sasl-ssl` requires
  ...
  /**
   * @param sslCaCertificateArn The Amazon Resource Name (ARN) for the private certificate authority
   * (CA) cert that AWS DMS uses to securely connect to your Kafka target endpoint.
   */
  ...
  /**
   * @param sslClientCertificateArn The Amazon Resource Name (ARN) of the client certificate used to
  ...
  /**
   * @param sslClientKeyPassword The password for the client private key used to securely connect to
