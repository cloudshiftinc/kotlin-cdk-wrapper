...
public class CfnTargetGroupTargetGroupAttributePropertyDsl {
    ...
    /**
     * @param key The name of the attribute. The following attributes are supported by all load
     *   balancers:
     ...
     *   enabled. The value is `true` , `false` or `use_load_balancer_configuration` . The default
     *   is `use_load_balancer_configuration` .
     * * `target_group_health.dns_failover.minimum_healthy_targets.count` - The minimum number of
     *   targets that must be healthy. If the number of healthy targets is below this value, mark
     *   the zone as unhealthy in DNS, so that traffic is routed only to healthy zones. The possible
     *   values are `off` or an integer from 1 to the maximum number of targets. The default is
     *   `off` .
     * * `target_group_health.dns_failover.minimum_healthy_targets.percentage` - The minimum
     *   percentage of targets that must be healthy. If the percentage of healthy targets is below
     *   this value, mark the zone as unhealthy in DNS, so that traffic is routed only to healthy
     *   zones. The possible values are `off` or an integer from 1 to 100. The default is `off` .
     ...
     *   value, send traffic to all targets, including unhealthy targets. The possible values are 1
     *   to the maximum number of targets. The default is 1.
     ...
     *   seconds (15 minutes). The default is 0 seconds (disabled).
     ...
     *   that are exchanged between the load balancer and the Lambda function include arrays of
     *   values or strings. The value is `true` or `false` . The default is `false` . If the value
     *   is `false` and the request contains a duplicate header field name or query parameter key,
     ...
     *   flows when a target is unhealthy. The possible values are `rebalance` and `no_rebalance` .
     *   The default is `no_rebalance` . The two attributes ( `target_failover.on_deregistration`
     *   and `target_failover.on_unhealthy` ) cannot be set independently. The value you set for
