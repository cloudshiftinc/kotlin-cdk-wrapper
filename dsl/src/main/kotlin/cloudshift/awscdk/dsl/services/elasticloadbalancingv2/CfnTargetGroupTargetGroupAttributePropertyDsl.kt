...
public class CfnTargetGroupTargetGroupAttributePropertyDsl {
   ...
   * . The range is 0-3600 seconds. The default value is 300 seconds. If the target is a Lambda
   * function, this attribute is not supported.
   * * `stickiness.enabled` - Indicates whether target stickiness is enabled. The value is `true` or
   ...
   * The value is `true` , `false` or `use_load_balancer_configuration` . The default is
   * `use_load_balancer_configuration` .
   * * `target_group_health.dns_failover.minimum_healthy_targets.count` - The minimum number of
   * targets that must be healthy. If the number of healthy targets is below this value, mark the zone
   * as unhealthy in DNS, so that traffic is routed only to healthy zones. The possible values are
   * `off` or an integer from 1 to the maximum number of targets. The default is `off` .
   ...
   * traffic to all targets, including unhealthy targets. The possible values are 1 to the maximum
   * number of targets. The default is 1.
   * * `target_group_health.unhealthy_state_routing.minimum_healthy_targets.percentage` - The
   * minimum percentage of targets that must be healthy. If the percentage of healthy targets is below
   * this value, send traffic to all targets, including unhealthy targets. The possible values are
   ...
   * target receives an increasing share of the traffic to the target group. After this time period
   * ends, the target receives its full share of traffic. The range is 30-900 seconds (15 minutes). The
   ...
   * * `stickiness.app_cookie.duration_seconds` - The time period, in seconds, during which requests
   * from a client should be routed to the same target. After this time period expires, the
   ...
   * strings. The value is `true` or `false` . The default is `false` . If the value is `false` and the
   * request contains a duplicate header field name or query parameter key, the load balancer uses the
   ...
   * * `preserve_client_ip.enabled` - Indicates whether client IP preservation is enabled. The value
   * is `true` or `false` . The default is disabled if the target group type is IP address and the
