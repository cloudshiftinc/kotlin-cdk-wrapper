...
public object neptune {
    /**
     * The `AWS::Neptune::DBCluster` resource creates an Amazon Neptune DB cluster. Neptune is a
     ...
     * behavior is that the entire volume will be deleted without a snapshot. To retain a backup of
     * the volume, the `DeletionPolicy` should be set to `Snapshot` . For more information about how
     * AWS CloudFormation deletes resources, see
     ...
     * `EngineVersion` property in the `DBCluster` . That `EngineVersion` needs to be compatible
     * with the value of the `Family` property in the `DBClusterParameterGroup` .
     *
     * Example:
     * ```
     * // The code below shows an example of how to instantiate this type.
     ...
     */
    public inline fun cfnDBClusterServerlessScalingConfigurationProperty(
     ...
     * the instance when the stack is deleted. For Neptune DB instances, you can choose to *retain*
     * the instance, to *delete* the instance, or to *create a snapshot* of the instance. The
     * default AWS CloudFormation behavior depends on the `DBClusterIdentifier` property:
     ...
     * * For `AWS::Neptune::DBInstance` resources that do specify the `DBClusterIdentifier`
     ...
     * creates a replacement DB instance, changes references from other dependent resources to point
     * to the replacement DB instance, and finally deletes the old DB instance.
     *
     ...
     *
     * Applying a parameter group to a DB instance might require the instance to reboot, resulting
     * in a database outage for the duration of the reboot. &gt; If you provide a custom
     * `DBParameterGroup` that you associate with `DBInstance` , it is best to specify an
     * `EngineVersion` property in `DBCluster` . That `EngineVersion` needs to be compatible with
     * the value of the `Family` property in the `DBParameterGroup` .
     *
     ...
     * allows Amazon Neptune to fully complete the create action before the parameter group is used
     * as the default for a new DB instance. This is especially important for parameters that are
     * critical when creating the default database for a DB instance, such as the character set for
     * the default database defined by the `character_set_database` parameter. You can use the
