 ...
 * [Job Definitions](https://docs.aws.amazon.com/batch/latest/userguide/job_definitions.html) in the
 * ** .
 *
 * Example:
 * ```
...
public class CfnJobDefinitionDsl(
     ...
     *
     * Valid values are `containerProperties` , `eksProperties` , and `nodeProperties` . Only one
     * can be specified.
     *
     ...
     * about specifying parameters, see
     ...
     * tasks when the tasks are created. For tags with the same name, job tags are given priority
     * over job definitions tags. If the total number of combined tags from the job and job
