 ...
 * a combination of a tag key and value, or a combination of one resource type and one resource ID.
 * If no scope is specified, evaluations for the rule are triggered when any resource in your
...
public class CfnFrameworkControlScopePropertyDsl {
     ...
     *   evaluation for a rule. A maximum of one key-value pair can be provided. The tag value is
     *   optional, but it cannot be an empty string. The structure to assign a tag is:
    ...
    public fun build(): CfnFramework.ControlScopeProperty {
        if (_complianceResourceIds.isNotEmpty())
            cdkBuilder.complianceResourceIds(_complianceResourceIds)
