...
public class CfnUserPoolUserPropsDsl {
   ...
   * custom workflow that is invoked by the *pre sign-up* trigger.
   * You create custom workflows by assigning AWS Lambda functions to user pool triggers. When you
   * create a `UserPoolUser` resource and include the `ClientMetadata` property, Amazon Cognito invokes
   * the function that is assigned to the *pre sign-up* trigger. When Amazon Cognito invokes this
   * function, it passes a JSON payload, which the function receives as input. This payload contains a
   ...
   * phone number. This can be done in your call to AdminCreateUser or in the *Users* tab of the Amazon
   ...
   * name will be sent. Required if the `email_verified` attribute is set to `True` , or if `"EMAIL"`
   ...
   * Must be unique within the user pool. Must be a UTF-8 string between 1 and 128 characters. After
   * the user is created, the username can't be changed.
   */
   ...
   * This is an array of name-value pairs that contain user attributes and attribute values that you
   * can use for custom validation, such as restricting the types of user accounts that can be
   * registered. For example, you might choose to allow or disallow user sign-up based on the user's
   * domain.
   *
