...
public class CfnTaskOptionsPropertyDsl {
     ...
     *   is, when the file was read or written to). If you set `Atime` to `BEST_EFFORT` , AWS
     *   DataSync attempts to preserve the original `Atime` attribute on all source files (that is,
     *   the version before the PREPARING phase). However, `Atime` 's behavior is not fully standard
     *   across platforms, so AWS DataSync can only do this on a best-effort basis.
     *
    ...
    /**
     * @param bytesPerSecond A value that limits the bandwidth used by AWS DataSync . For example,
     *   if you want AWS DataSync to use a maximum of 1 MB, set this value to `1048576`
     *   (=1024*1024).
     */
     ...
     *   was written to) before the PREPARING phase. This option is required for cases when you need
     ...
     *   specific purpose, such as reading, writing, or execution of the file. This option should be
     *   set only for Network File System (NFS), Amazon EFS, and Amazon S3 locations. For more
     ...
     *   don't exist in the source file system are preserved. This option can affect your storage
     *   costs. If your task deletes objects, you might incur minimum storage duration charges for
     ...
     *   with that device name and metadata on the destination. DataSync does not copy the contents
     *   of such devices, only the name and metadata.
     *
     ...
     *   security descriptor are copied from source to destination objects. This value is only used
     *   for transfers between SMB and Amazon FSx for Windows File Server locations, or between two
     *   Amazon FSx for Windows File Server locations. For more information about how DataSync
     *   handles metadata, see
     ...
     *   transfers all the content from the source, without comparing it to the destination
     *   location. `CHANGED` : DataSync copies only data or metadata that is new or different from
    ...
    /**
     * @param verifyMode A value that determines whether a data integrity verification is performed
     *   at the end of a task execution after all data and metadata have been transferred. For more
     *   information, see
