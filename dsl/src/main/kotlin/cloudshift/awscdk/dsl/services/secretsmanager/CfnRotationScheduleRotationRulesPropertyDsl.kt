...
public class CfnRotationScheduleRotationRulesPropertyDsl {
     ...
     *   secret. You can use this value to check that your secret meets your compliance guidelines
     ...
     *   window. Secrets Manager rotates your secret at any time during this window. The window must
     *   not extend into the next rotation window or the next UTC day. The window starts according
     *   to the `ScheduleExpression` . If you don't specify a `Duration` , for a
     *   `ScheduleExpression` in hours, the window automatically closes after one hour. For a
     *   `ScheduleExpression` in days, the window automatically closes at the end of the UTC day.
     ...
     *   rotating your secret. Secrets Manager rotation schedules use UTC time zone. Secrets Manager
     ...
     * rotate your secret, for example `rate(12 hours)` or `rate(10 days)` . You can rotate a secret
     * as often as every four hours. If you use a `rate()` expression, the rotation window starts at
     * midnight. For a rate in hours, the default rotation window closes after one hour. For a rate
     * in days, the default rotation window closes at the end of the day. You can set the `Duration`
     ...
     * rotation interval. For more information, including examples, see
     * [Schedule expressions in Secrets Manager rotation](https://docs.aws.amazon.com/secretsmanager/latest/userguide/rotate-secrets_schedule.html)
     ...
     * can set the `Duration` to change the rotation window. The rotation window must not extend
