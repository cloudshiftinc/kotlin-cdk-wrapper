...
public class CfnChannelHlsGroupSettingsPropertyDsl {
   ...
   * .m3u8 file. The partial URI prefix can be used if the base manifest is delivered from a different
   ...
   * One value per output group. This field is required only if you are completing Base URL content
   * A, and the downstream system has notified you that the media files for pipeline 1 of all outputs
   * are in a location different from the media files for pipeline 0.
   */
  ...
  /**
   * @param baseUrlManifest A partial URI prefix that will be prepended to each output in the media
   * .m3u8 file. The partial URI prefix can be used if the base manifest is delivered from a different
   * URL than the main .m3u8 file.
   */
   ...
   * One value per output group. Complete this field only if you are completing Base URL manifest A,
   * and the downstream system has notified you that the child manifest files for pipeline 1 of all
   * outputs are in a location different from the child manifest files for pipeline 0.
   */
   ...
   * Insert: Include CLOSED-CAPTIONS lines in the manifest. Specify at least one language in the CC1
   * Language Code field. One CLOSED-CAPTION line is added for each Language Code that you specify.
   * Make sure to specify the languages in the order in which they appear in the original source (if
   * the source is embedded format) or the order of the captions selectors (if the source is other than
   * embedded). Otherwise, languages in the manifest will not match properly with the output captions.
   * None: Include the CLOSED-CAPTIONS=NONE line in the manifest. Omit: Omit any CLOSED-CAPTIONS line
   * from the manifest.
   */
  ...
  /**
   * @param clientCache When set to "disabled," sets the #EXT-X-ALLOW-CACHE:no tag in the manifest,
  ...
  /**
   * @param discontinuityTags Specifies whether to insert EXT-X-DISCONTINUITY tags in the HLS child
   ...
   * specification) and serve an important purpose.
  ...
  /**
   * @param incompleteSegmentBehavior Specifies whether to include the final (incomplete) segment in
   * the media output when the pipeline stops producing output because of a channel stop, a channel
   ...
   * segments are removed from the media manifest. This number must be less than or equal to the Keep
   * Segments field.
   */
   ...
   * encrypting blocks. If set to "include," the IV is listed in the manifest. Otherwise, the IV is not
   * in the manifest.
   */
  ...
  /**
   * @param manifestDurationFormat Indicates whether the output manifest should use a floating point
   ...
   * the beginning of the channel. VOD mode uses HLS EXT-X-PLAYLIST-TYPE of EVENT while the channel is
   * running, converting it to a "VOD" type manifest on completion of the stream.
   */
   ...
   * files. The value is calculated as follows: Either the program date and time are initialized using
   * the input timecode source, or the time is initialized using the input timecode source and the date
   * is initialized using the timestampOffset.
   */
   ...
   * information about both pipelines: first its own media files, then the media files of the other
   * pipeline. This feature allows a playout device that supports stale manifest detection to switch
   * from one manifest to the other, when the current manifest seems to be stale. There are still two
   * destinations and two master manifests, but both master manifests reference the media files from
   * both pipelines. DISABLED: The master manifest (.m3u8 file) for each pipeline includes information
   * about its own pipeline only. For an HLS output group with MediaPackage as the destination, the
   * DISABLED behavior is always followed. MediaPackage regenerates the manifests it serves to players,
   * so a redundant manifest from MediaLive is irrelevant.
   */
  ...
  /**
   * @param segmentLength The length of the MPEG-2 Transport Stream segments to create, in seconds.
   ...
   * SINGLEFILE: Applies only if the Mode field is VOD. Emits the program as a single .ts media file.
   * The media manifest includes #EXT-X-BYTERANGE tags to index segments for playback. A typical use
   * for this value is when sending the output to AWS Elemental MediaConvert, which can accept only a
   * single media file. Playback while the channel is running is not guaranteed due to HTTP server
   * caching.
   */
