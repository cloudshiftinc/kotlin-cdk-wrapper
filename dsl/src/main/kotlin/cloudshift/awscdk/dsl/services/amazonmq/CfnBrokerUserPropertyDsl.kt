...
public class CfnBrokerUserPropertyDsl {
   ...
   * This value must be at least 12 characters long, must contain at least 4 unique characters, and
   ...
   * dashes, periods, underscores, and tildes (- . _ ~). For Amazon MQ for RabbitMQ brokers, this value
   * can contain only alphanumeric characters, dashes, periods, underscores (- . _). This value must
   * not contain a tilde (~) character. Amazon MQ prohibts using guest as a valid usename. This value
   * must be 2-100 characters long.
   *
   *
   * Do not add personally identifiable information (PII) or other confidential or sensitive
   * information in broker usernames. Broker usernames are accessible to other AWS services, including
