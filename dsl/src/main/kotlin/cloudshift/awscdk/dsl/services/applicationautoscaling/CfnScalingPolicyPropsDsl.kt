...
public class CfnScalingPolicyPropsDsl {
    ...
    /**
     * @param policyName The name of the scaling policy. Updates to the name of a target tracking
     *   scaling policy are not supported, unless you also update the metric used for scaling. To
     *   change only a target tracking scaling policy's name, first delete the policy by removing
     *   the existing `AWS::ApplicationAutoScaling::ScalingPolicy` resource from the template and
    ...
    /**
     * @param policyType The scaling policy type. The following policy types are supported:
     *
     ...
     *   scalable target. For more information about the ID, see the Return Value section of the
    ...
    public fun stepScalingPolicyConfiguration(
        stepScalingPolicyConfiguration: CfnScalingPolicy.StepScalingPolicyConfigurationProperty
    ) {
        cdkBuilder.stepScalingPolicyConfiguration(stepScalingPolicyConfiguration)
    }
