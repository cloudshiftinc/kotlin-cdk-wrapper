...
public class CfnCachePolicyParametersInCacheKeyAndForwardedToOriginPropertyDsl {
   ...
   * so, which cookies) are included in the cache key and in requests that CloudFront sends to the
   * origin. 
   */
   ...
   * header is included in the cache key and included in requests that CloudFront sends to the origin.
   * This field is related to the `EnableAcceptEncodingGzip` field. If one or both of these fields
   * is `true` *and* the viewer request includes the `Accept-Encoding` header, then CloudFront does the
   * following:
   *
