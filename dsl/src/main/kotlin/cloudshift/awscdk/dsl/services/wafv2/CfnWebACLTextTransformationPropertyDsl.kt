...
public class CfnWebACLTextTransformationPropertyDsl {
   ...
   * `syndata.html#characters` . This function uses up to two bytes in the decoding process, so it can
   * help to uncover ASCII characters that were encoded using CSS encoding that wouldn’t typically be
   ...
   *
   * *JS_DECODE* - Decode JavaScript escape sequences. If a `\` `u` `HHHH` code is in the full-width
   * ASCII code range of `FF01-FF5E` , then the higher byte is used to detect and adjust the lower
   * byte. If not, only the lower byte is used and the higher byte is zeroed, causing a possible loss
   * of information.
   *
   * *LOWERCASE* - Convert uppercase letters (A-Z) to lowercase (a-z).
   *
   * *MD5* - Calculate an MD5 hash from the data in the input. The computed hash is in a raw binary
   * form.
   *
   ...
   * single space. Multiple consecutive occurrences are not compressed. Unterminated comments are also
   * replaced with a space (ASCII 0x20). However, a standalone termination of a comment ( `* /` ) is
   * not acted upon.
   *
   ...
   * detect and adjust the lower byte. Otherwise, only the lower byte is used and the higher byte is
   * zeroed.
   *
