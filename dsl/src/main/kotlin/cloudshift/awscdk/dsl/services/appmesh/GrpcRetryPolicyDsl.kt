...
public class GrpcRetryPolicyDsl {
    ...
    /**
     * @param grpcRetryEvents gRPC events on which to retry. You must specify at least one value for
    ...
    /**
     * @param httpRetryEvents Specify HTTP events on which to retry. You must specify at least one
    ...
    /**
     * @param tcpRetryEvents TCP events on which to retry. The event occurs before any processing of
     *   a request has started and is encountered when the upstream is temporarily or permanently
     *   unavailable. You must specify at least one value for at least one types of retry events.
     */
