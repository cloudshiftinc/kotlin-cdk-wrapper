...
public class CfnEndpointPostgreSqlSettingsPropertyDsl {
   ...
   * actual size of a LOB column is greater than the specified `LobMaxSize` .
   ...
   * By doing this, it prevents idle logical replication slots from holding onto old WAL logs, which
   * can result in storage full situations on the source. This heartbeat keeps `restart_lsn` moving and
   ...
   *
   * For more information about setting the `CdcStartPosition` request parameter, see [Determining a
   * CDC native start
