...
public class CfnPipeBatchResourceRequirementPropertyDsl {
    ...
    /**
     * @param type The type of resource to assign to a container. The supported resources include
     *   `GPU` , `MEMORY` , and `VCPU` .
     */
    ...
    /**
     * @param value The quantity of the specified resource to reserve for the container. The values
     *   vary based on the `type` specified.
     * * **type="GPU"** - The number of physical GPUs to reserve for the container. Make sure that
     *   the number of GPUs reserved for all containers in a job doesn't exceed the number of
     ...
     *   is supported for jobs that are running on EC2 resources. If your container attempts to
     *   exceed the memory specified, the container is terminated. This parameter maps to `Memory`
     *   in the
     ...
     *   You must specify at least 4 MiB of memory for a job. This is required but can be specified
     *   in several places for multi-node parallel (MNP) jobs. It must be specified for each node at
     *   least once. This parameter maps to `Memory` in the
     *   [Create a container](https://docs.aws.amazon.com/https://docs.docker.com/engine/api/v1.23/#create-a-container)
     *   section of the
     *   [Docker Remote API](https://docs.aws.amazon.com/https://docs.docker.com/engine/api/v1.23/)
     *   and the `--memory` option to
     ...
     *   [docker run](https://docs.aws.amazon.com/https://docs.docker.com/engine/reference/run/) .
     *   Each vCPU is equivalent to 1,024 CPU shares. For EC2 resources, you must specify at least
     *   one vCPU. This is required but can be specified in several places; it must be specified for
