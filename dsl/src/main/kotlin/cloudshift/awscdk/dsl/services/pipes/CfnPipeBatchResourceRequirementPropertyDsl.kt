...
public class CfnPipeBatchResourceRequirementPropertyDsl {
   ...
   * * **type="GPU"** - The number of physical GPUs to reserve for the container. Make sure that the
   * number of GPUs reserved for all containers in a job doesn't exceed the number of available GPUs on
   * the compute resource that the job is launched on.
   *
   *
   ...
   * is supported for jobs that are running on EC2 resources. If your container attempts to exceed the
   * memory specified, the container is terminated. This parameter maps to `Memory` in the [Create a
   ...
   * . You must specify at least 4 MiB of memory for a job. This is required but can be specified in
   ...
   * possible for a particular instance type, see [Memory
   * management](https://docs.aws.amazon.com/batch/latest/userguide/memory-management.html) in the *AWS
   * Batch User Guide* .
   *
   *
   ...
   * option to [docker run](https://docs.aws.amazon.com/https://docs.docker.com/engine/reference/run/)
   * . Each vCPU is equivalent to 1,024 CPU shares. For EC2 resources, you must specify at least one
   * vCPU. This is required but can be specified in several places; it must be specified for each node
   * at least once.
   *
   ...
   * quotas](https://docs.aws.amazon.com/general/latest/gr/ecs-service.html#service-quotas-fargate) in
