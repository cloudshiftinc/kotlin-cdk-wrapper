...
public class CfnStateMachineAliasDeploymentPreferencePropertyDsl {
    ...
    /**
     * @param alarms A list of Amazon CloudWatch alarms to be monitored during the deployment. The
     *   deployment fails and rolls back if any of these alarms go into the `ALARM` state.
     */
     ...
     *   shifting is complete. While performing gradual deployments, you can only provide a single
     *   state machine version ARN. To explicitly set version weights in a CloudFormation template,
     *   use `RoutingConfiguration` instead.
     */
