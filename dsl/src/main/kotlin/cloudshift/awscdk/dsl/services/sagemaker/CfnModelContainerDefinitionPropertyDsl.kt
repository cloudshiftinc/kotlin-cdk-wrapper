...
public class CfnModelContainerDefinitionPropertyDsl {
   ...
   * `ContainerDefinition` in the pipeline. If you specify a value for the `ContainerHostName` for any
   * `ContainerDefinition` that is part of an inference pipeline, you must specify a value for the
   ...
   * This can be either in Amazon EC2 Container Registry or in a Docker registry that is accessible
   * from the same VPC that you configure for your endpoint. If you are using your own custom algorithm
   * instead of an algorithm provided by SageMaker, the inference code must meet SageMaker
   ...
   *
   *
   * The model artifacts must be in an S3 bucket that is in the same region as the model or endpoint
   * you are creating.
   *
   *
   * If you provide a value for this parameter, SageMaker uses AWS Security Token Service to
   * download model artifacts from the S3 path you provide. AWS STS is activated in your AWS account by
   * default. If you previously deactivated AWS STS for a region, you need to reactivate AWS STS for
   * that region. For more information, see [Activating and Deactivating AWS STS in an AWS
