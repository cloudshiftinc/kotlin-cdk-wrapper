...
public class CfnInstanceDsl(
   ...
   * routes traffic to an Elastic Load Balancing load balancer, specify the DNS name that is associated
   ...
   * * **AWS_INIT_HEALTH_STATUS** - If the service configuration includes `HealthCheckCustomConfig`
   * , when creating resources with a type of
   * [AWS::ServiceDiscovery::Instance](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-servicediscovery-instance.html)
   * you can optionally use `AWS_INIT_HEALTH_STATUS` to specify the initial status of the custom health
   * check, `HEALTHY` or `UNHEALTHY` . If you don't specify a value for `AWS_INIT_HEALTH_STATUS` , the
   * initial status is `HEALTHY` . This attribute can only be used when creating resources and will not
   * be seen on existing resources.
   ...
   * record.
   *
   * * **AWS_INSTANCE_IPV4** - If the service configuration includes an `A` record, the IPv4 address
   * that you want Route 53 to return in response to DNS queries, for example, `192.0.2.44` .
   *
   ...
   * @param instanceAttributes A string map that contains the following information for the service
   ...
   * DNS queries and any `Type` member of `DnsRecord` for the service contains `SRV` because the
