...
public class PolicyDsl(
     ...
     *
     * Unless set to `true`, this `Policy` construct will not materialize to an `AWS::IAM::Policy`
     * CloudFormation resource in case it would have no effect (for example, if it remains
     * unattached to an IAM identity or if it has no statements). This is generally desired
     * behavior, since it prevents creating invalid--and hence undeployable--CloudFormation
     * templates.
     *
     ...
     *
     * Default: - Uses the logical ID of the policy resource, which is ensured to be unique within
     * the stack.
     *
