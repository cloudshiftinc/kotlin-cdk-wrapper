...
public class CfnChannelH264SettingsPropertyDsl {
  ...
  /**
   * @param forceFieldPictures This setting applies only when scan type is "interlaced." It controls
   * whether coding is performed on a field basis or on a frame basis. (When the video is progressive,
   * the coding is always performed on a frame basis.) enabled: Force MediaLive to code on a field
   * basis, so that odd and even sets of fields are coded separately. disabled: Code the two sets of
   * fields separately (on a field basis) or together (on a frame basis using PAFF), depending on what
   ...
   * If you select "specified," the output video frame rate is determined by framerateNumerator and
   * framerateDenominator. If you select "initializeFromSource," the output video frame rate is set
  ...
  /**
   * @param gopBReference If enabled, uses reference B frames for GOP structures that have B frames
   * &gt; 1.
   */
   ...
   * A value of low can decrease latency and memory usage, while high can produce better quality for
   * certain content.
   */
   ...
   * I-frame, the GOP is shrunk or stretched to the scene change I-frame. GOP stretch requires enabling
   * lookahead as well as setting the I-interval. The normal cadence resumes for the next GOP. Note
   * that the maximum GOP stretch = GOP size + Min-I-interval - 1.
   */
   ...
   * and parDenominator. If "initializeFromSource" is selected, the output pixels aspect ratio will be
   * set equal to the input video pixel aspect ratio of the first input.
   */
   ...
   * maximum bitrate. We recommend this if you or your viewers pay for bandwidth. VBR: The quality and
   * bitrate vary, depending on the video complexity. We recommend this instead of QVBR if you want to
   * maintain a specific average bitrate over the duration of the channel. CBR: The quality varies,
   * depending on the video complexity. We recommend this only if you distribute your assets to devices
   * that can't handle variable bitrates.
   */
  ...
  /**
   * @param scanType Sets the scan type of the output to progressive or top-field-first interlaced.
   */
   ...
   * On: inserts I-frames when the scene change is detected. Off: does not force an I-frame when the
   ...
   * This field is optional. If you don't specify a value, MediaLive chooses the number of slices based
   * on the encode resolution.
   */
  ...
  /**
   * @param spatialAq If set to enabled, adjusts quantization within each frame based on the spatial
  ...
  /**
   * @param timecodeInsertion Determines how timecodes should be inserted into the video elementary
   * stream.
   * disabled: don't include timecodes. picTimingSei: pass through picture timing SEI messages from
   * the source specified in Timecode Config.
   */
