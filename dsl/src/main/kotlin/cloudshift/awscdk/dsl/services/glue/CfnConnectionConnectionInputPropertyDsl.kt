...
public class CfnConnectionConnectionInputPropertyDsl {
   ...
   * `SECRET_ID` .
   * * `CUSTOM` - Uses configuration settings contained in a custom connector to read from and write
   * to data stores that are not natively supported by AWS Glue .
   *
   * `SFTP` is not supported.
   *
   * For more information about how optional ConnectionProperties are used to configure features in
   * AWS Glue , consult [AWS Glue connection
   * properties](https://docs.aws.amazon.com/glue/latest/dg/connection-defining.html) .
   *
   * For more information about how optional ConnectionProperties are used to configure features in
   * AWS Glue Studio, consult [Using connectors and
