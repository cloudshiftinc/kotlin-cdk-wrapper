...
public class CfnProjectWebhookFilterPropertyDsl {
     ...
     *   events do not trigger a build. If true, then a webhook event that does not match the
     *   `pattern` triggers a build. If false, then a webhook event that matches the `pattern`
     *   triggers a build.
     */
     ...
     *   specifies one or more events. For example, the webhook filter `PUSH, PULL_REQUEST_CREATED,
    ...
    /**
     * @param type The type of webhook filter. There are six webhook filter types: `EVENT` ,
     *   `ACTOR_ACCOUNT_ID` , `HEAD_REF` , `BASE_REF` , `FILE_PATH` , and `COMMIT_MESSAGE` .
     ...
     *   `PULL_REQUEST_REOPENED` , and `PULL_REQUEST_MERGED` . The `EVENT` patterns are specified as
     *   a comma-separated string. For example, `PUSH, PULL_REQUEST_CREATED, PULL_REQUEST_UPDATED`
