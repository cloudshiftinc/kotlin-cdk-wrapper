 ...
 * data at rest using an AES-256 encryption algorithm. This does not require any action on your
 * part.
 *
 * For more control over the encryption of the contents of your repository, you can use server-side
 * encryption with AWS Key Management Service key stored in AWS Key Management Service ( AWS KMS )
...
public class CfnRepositoryEncryptionConfigurationPropertyDsl {
    ...
    /**
     * @param encryptionType The encryption type to use. If you use the `KMS` encryption type, the
     *   contents of the repository will be encrypted using server-side encryption with AWS Key
     *   Management Service key stored in AWS KMS . When you use AWS KMS to encrypt your data, you
     *   can either use the default AWS managed AWS KMS key for Amazon ECR, or specify your own AWS
     *   KMS key, which you already created. For more information, see
     ...
     * encryption algorithm. For more information, see
     ...
     *   must exist in the same Region as the repository. If no key is specified, the default AWS
