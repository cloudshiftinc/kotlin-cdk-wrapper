...
public class CfnUserPoolPropsDsl {
    ...
    /**
     * @param emailConfiguration The email configuration of your user pool. The email configuration
     *   type sets your preferred sending method, AWS Region, and sender for messages from your user
     *   pool.
     */
    ...
    /**
     * @param enabledMfas Enables MFA on a specified user pool. To disable all MFAs after it has
     *   been enabled, set MfaConfiguration to “OFF” and remove EnabledMfas. MFAs can only be all
     *   disabled if MfaConfiguration is OFF. Once SMS_MFA is enabled, SMS_MFA can only be disabled
     *   by setting MfaConfiguration to “OFF”. Can be one of the following values:
     ...
     *   Notification Service. To send SMS messages with Amazon SNS in the AWS Region that you want,
     *   the Amazon Cognito user pool uses an AWS Identity and Access Management (IAM) role in your
     *   AWS account .
     */
    ...
    /**
     * @param userAttributeUpdateSettings The settings for updates to user attributes. These
    ...
    /**
     * @param userPoolTags The tag keys and values to assign to the user pool. A tag is a label that
     ...
     *   specified as user names when a user signs up. Possible values: `phone_number` or `email` .
     *
     * This user pool property cannot be updated.
     */
     ...
     *   the selected sign-in option. For example, when this is set to `False` , users will be able
     *   to sign in using either "username" or "Username". This configuration is immutable once it
     *   has been set.
     */
