...
public class CfnWorkspaceNetworkAccessControlPropertyDsl {
   ...
   * A prefix list is a list of CIDR ranges of IP addresses. The IP addresses specified are allowed
   * to access your workspace. If the list is not included in the configuration (passed an empty array)
   * then no IP addresses are allowed to access the workspace. You create a prefix list using the
   * Amazon VPC console.
   *
   * Prefix list IDs have the format `pl- *1a2b3c4d*` .
   *
   * For more information about prefix lists, see [Group CIDR blocks using managed prefix
   * lists](https://docs.aws.amazon.com/vpc/latest/userguide/managed-prefix-lists.html) in the *Amazon
   ...
   * You can create VPC endpoints to your Amazon Managed Grafana workspace for access from within a
   * VPC. If a `NetworkAccessConfiguration` is specified then only VPC endpoints specified here are
