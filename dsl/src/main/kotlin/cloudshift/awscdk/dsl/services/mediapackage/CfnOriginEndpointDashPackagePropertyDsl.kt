...
public class CfnOriginEndpointDashPackagePropertyDsl {
  ...
  /**
   * @param minBufferTimeSeconds Minimum amount of content (measured in seconds) that a player must
   * keep available in the buffer.
   */
   ...
   * * `NUMBER_WITH_DURATION` - The `$Number$` variable is used in the `media` URL and a `duration`
   * attribute is added to the segment template. The `SegmentTimeline` object is removed from the
   * representation.
   * * `TIME_WITH_TIMELINE` - The `$Time$` variable is used in the `media` URL. The value of this
   * variable is the timestamp of when the segment starts. A full `SegmentTimeline` object is presented
  ...
  /**
   * @param suggestedPresentationDelaySeconds Amount of time (in seconds) that the player should be
   * from the live point at the end of the manifest.
   */
  ...
  /**
   * @param utcTimingUri Specifies the value attribute of the UTC timing field when utcTiming is set
