...
import io.cloudshiftdev.awscdkdsl.cloudassembly.schema.AssetManifestOptionsDsl
import io.cloudshiftdev.awscdkdsl.cloudassembly.schema.DockerImageDestinationDsl
import io.cloudshiftdev.awscdkdsl.cloudassembly.schema.FileDestinationDsl
import io.cloudshiftdev.awscdkdsl.cloudassembly.schema.MissingContextDsl
import kotlin.Any
 ...
 *
 * Derive the region from the stack, use the asset hash as the key, copy the file extension over,
 * and set the prefix.
 *
 * @param stack
 * @param asset
 ...
 *
 * Return the artifact id, which should be added to the `additionalDependencies` field of the stack
 * artifact.
 *
 * @param stack
 * @param session
 ...
 *
 * The synthesizer must rely on some out-of-band mechanism to make sure the given files are actually
 * placed in the returned location before the deployment happens. This can be by writing the
 * instructions to the asset manifest (for use by the `cdk-assets` tool), by relying on the CLI to
 * upload files (legacy behavior), or some other operator controlled mechanism.
 *
 * @param _asset
 */
public inline fun BootstraplessSynthesizer.addDockerImageAsset(
 ...
 * @param _asset
 */
public inline fun BootstraplessSynthesizer.addFileAsset(
 ...
 */
public inline fun CfnCodeDeployBlueGreenHook.setAdditionalOptions(
 ...
 * deployment.
 *
 * You can use the same function or a different one for deployment lifecycle events. Following
 * completion of the validation tests, the Lambda
 * `CfnCodeDeployBlueGreenLifecycleEventHooks.afterAllowTraffic` function calls back CodeDeploy and
 * delivers a result of 'Succeeded' or 'Failed'.
 *
 * Default: - no lifecycle event hooks
 */
public inline fun CfnCodeDeployBlueGreenHook.setLifecycleEventHooks(
 ...
 * time
 */
public inline fun CfnCodeDeployBlueGreenHook.setTrafficRoutingConfig(
 ...
 * CloudFormation, either because you've removed it from the CDK application or because you've made
 * a change that requires the resource to be replaced.
 *
 * The resource can be deleted (`RemovalPolicy.DESTROY`), or left in your AWS account for data
 * recovery and cleanup later (`RemovalPolicy.RETAIN`). In some cases, a snapshot can be taken of
 * the resource prior to deletion (`RemovalPolicy.SNAPSHOT`). A list of resources that support this
 * policy can be found in the following link:
 *
 ...
 * @param options
 */
public inline fun CfnResource.applyRemovalPolicy(
 ...
 * @param asset
 */
public inline fun CliCredentialsStackSynthesizer.addDockerImageAsset(
    block: DockerImageAssetSourceDsl.() -> Unit = {}
): DockerImageAssetLocation {
    val builder = DockerImageAssetSourceDsl()
    builder.apply(block)
 ...
 * @param asset
 */
public inline fun DefaultStackSynthesizer.addDockerImageAsset(
    block: DockerImageAssetSourceDsl.() -> Unit = {}
): DockerImageAssetLocation {
    val builder = DockerImageAssetSourceDsl()
    builder.apply(block)
 ...
 * @param asset
 */
public inline fun DefaultStackSynthesizer.addFileAsset(
    block: FileAssetSourceDsl.() -> Unit = {}
): FileAssetLocation {
    val builder = FileAssetSourceDsl()
    builder.apply(block)
 ...
 * locally.
 *
 * If the local bundler exists, and bundling was performed locally, return `true`. Otherwise, return
 * `false`.
 *
 * @param outputDir the directory where the bundled asset should be output.
 * @param options bundling options for this asset.
 */
public inline fun ILocalBundling.tryBundle(
    arg0: String,
    block: BundlingOptionsDsl.() -> Unit = {}
): Boolean {
 ...
 * @param options
 */
public inline fun IResolveContext.resolve(
    arg0: Any,
 ...
 * @param asset
 */
public inline fun IStackSynthesizer.addFileAsset(
    block: FileAssetSourceDsl.() -> Unit = {}
): FileAssetLocation {
    val builder = FileAssetSourceDsl()
    builder.apply(block)
 ...
 * @param asset
 */
public inline fun LegacyStackSynthesizer.addDockerImageAsset(
    block: DockerImageAssetSourceDsl.() -> Unit = {}
): DockerImageAssetLocation {
    val builder = DockerImageAssetSourceDsl()
    builder.apply(block)
 ...
 * @param asset
 */
public inline fun LegacyStackSynthesizer.addFileAsset(
    block: FileAssetSourceDsl.() -> Unit = {}
): FileAssetLocation {
    val builder = FileAssetSourceDsl()
    builder.apply(block)
 ...
 * @param asset
 */
public inline fun NestedStackSynthesizer.addDockerImageAsset(
    block: DockerImageAssetSourceDsl.() -> Unit = {}
): DockerImageAssetLocation {
    val builder = DockerImageAssetSourceDsl()
    builder.apply(block)
 ...
 * @param asset
 */
public inline fun NestedStackSynthesizer.addFileAsset(
    block: FileAssetSourceDsl.() -> Unit = {}
): FileAssetLocation {
    val builder = FileAssetSourceDsl()
    builder.apply(block)
 ...
 * Export. The export expression is automatically wrapped with an `Fn::Join` and the import value
 * with an `Fn::Split`, since CloudFormation can only export strings. You can control the name for
 * the export by passing the `name` option.
 *
 * If you don't supply a value for `name`, the value you're exporting must be a Resource attribute
 * (for example: `bucket.bucketName`) and it will be given the same name as the automatic
 * cross-stack reference that would be created if you used the attribute in another Stack.
 *
 * One of the uses for this method is to *remove* the relationship between two Stacks established by
 * automatic cross-stack references. It will temporarily ensure that the CloudFormation Export still
 * exists while you remove the reference from the consuming stack. After that, you can remove the
 * resource and the manual export.
 *
 ...
 * @param options
 */
public inline fun Stack.exportStringListValue(
    exportedValue: Any,
 ...
 *
 * Returns a string representing the corresponding `Fn.importValue()` expression for this Export.
 * You can control the name for the export by passing the `name` option.
 *
 * If you don't supply a value for `name`, the value you're exporting must be a Resource attribute
 * (for example: `bucket.bucketName`) and it will be given the same name as the automatic
 * cross-stack reference that would be created if you used the attribute in another Stack.
 ...
 * `consumerStack` (perhaps because an AWS Lambda Function writes into it, or something like that).
 *
 ...
 * * Make sure `consumerStack` no longer references `bucket.bucketName` (maybe the consumer stack
 *   now uses its own bucket, or it writes to an AWS DynamoDB table, or maybe you just remove the
 *   Lambda Function altogether).
 * * In the `ProducerStack` class, call `this.exportValue(this.bucket.bucketName)`. This will make
 *   sure the CloudFormation Export continues to exist while the relationship between the two stacks
 *   is being broken.
 ...
 *
 * If any component is the empty string, an empty string will be inserted into the generated ARN at
 * the location that component corresponds to.
 *
 ...
 * @param asset
 */
public inline fun StackSynthesizer.addFileAsset(
    block: FileAssetSourceDsl.() -> Unit = {}
): FileAssetLocation {
    val builder = FileAssetSourceDsl()
    builder.apply(block)
 ...
 *
 * Once an assembly has been synthesized, it cannot be modified. Subsequent calls will return the
 * same assembly.
 *
 * @param options
 */
