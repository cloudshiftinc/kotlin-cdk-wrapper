 ...
 * before adding the task instance fleet to the cluster. In order to add a task instance fleet to
 * the cluster as part of the cluster launch and minimize delays in provisioning task nodes, use the
 * `TaskInstanceFleets` subproperty for the
...
public class CfnInstanceFleetConfigDsl(
    ...
    public fun launchSpecifications(
        launchSpecifications: CfnInstanceFleetConfig.InstanceFleetProvisioningSpecificationsProperty
    ) {
        cdkBuilder.launchSpecifications(launchSpecifications)
    }
