...
public object macie {
     ...
     * when it inspects data sources for sensitive data. If data matches text or a text pattern in
     * an allow list, Macie doesn’t report the data in sensitive data findings or sensitive data
     * discovery results, even if the data matches the criteria of a custom data identifier or a
     * managed data identifier. You can create and use allow lists in all the AWS Regions where
     * Macie is currently available except the Asia Pacific (Osaka) Region.
     *
     * Macie supports two types of allow lists:
     * * *Predefined text* - For this type of list ( `S3WordsList` ), you create a line-delimited
     *   plaintext file that lists specific text to ignore, and you store the file in an Amazon
     *   Simple Storage Service ( Amazon S3 ) bucket. You then configure settings for Macie to
     *   access the list in the bucket.
     *
     * This type of list typically contains specific words, phrases, and other kinds of character
     * sequences that aren’t sensitive, aren't likely to change, and don’t necessarily adhere to a
     * common pattern. If you use this type of list, Macie doesn't report occurrences of text that
     * exactly match a complete entry in the list. Macie treats each entry in the list as a string
     ...
     *   that defines a text pattern to ignore. Unlike an allow list with predefined text, you store
     *   the regex and all other list settings in Macie .
     *
     * This type of list is helpful if you want to specify text that isn’t sensitive but varies or
     * is likely to change while also adhering to a common pattern. If you use this type of list,
     * Macie doesn't report occurrences of text that completely match the pattern defined by the
     * list.
     *
     ...
     * `AWS::Macie::AllowList` resource for the account. Use a
    ...
    /**
     * Specifies the criteria for an allow list, which is a list that defines specific text or a
     * text pattern to ignore when inspecting data sources for sensitive data.
     *
     * The criteria can be:
     ...
     * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-macie-allowlist.html)
     */
    public inline fun cfnAllowListProps(
        block: CfnAllowListPropsDsl.() -> Unit = {}
    ): CfnAllowListProps {
     ...
     *
     * A *custom data identifier* is a set of custom criteria for Amazon Macie to use when it
     * inspects data sources for sensitive data. The criteria consist of a regular expression (
     * *regex* ) that defines a text pattern to match and, optionally, character sequences and a
     * proximity rule that refine the results. The character sequences can be:
     ...
     * custom criteria that specifies which findings to include or exclude from the results of a
     * query for findings. The criteria can help you identify and focus on findings that have
     * specific characteristics, such as severity, type, or the name of an affected AWS resource.
     * You can also configure a findings filter to suppress (automatically archive) findings that
     * match the filter's criteria. For more information, see
     ...
     * specifies which findings to include or exclude from the results of a query for findings. You
     * can also configure a findings filter to suppress (automatically archive) findings that match
     * the filter's criteria. For more information, see
     * [Filtering findings](https://docs.aws.amazon.com/macie/latest/user/findings-filter-overview.html)
     * in the *Amazon Macie User Guide* .
     *
     * Example:
     ...
     * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-macie-findingsfilter.html)
     */
    public inline fun cfnFindingsFilterProps(
        block: CfnFindingsFilterPropsDsl.() -> Unit = {}
     ...
     *
     * It enables Macie to become operational for a specific account in a specific Region. An
     * account can have only one session in each Region.
     *
