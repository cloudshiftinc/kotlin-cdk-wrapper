...
public class RuleTargetConfigDsl {
    ...
    public fun deadLetterConfig(
        deadLetterConfig: CfnRuleDeadLetterConfigPropertyDsl.() -> Unit = {}
    ) {
     ...
     *   stream. If you don't include this parameter, eventId is used as the partition key.
     */
    public fun kinesisParameters(
        kinesisParameters: CfnRuleKinesisParametersPropertyDsl.() -> Unit = {}
    ) {
    ...
    public fun runCommandParameters(
        runCommandParameters: CfnRuleRunCommandParametersPropertyDsl.() -> Unit = {}
    ) {
     ...
     *   will actually have some action performed on it when used as a target (for example, start a
     *   build for a CodeBuild project). We need it to determine whether the rule belongs to a
     *   different account than the target - if so, we generate a more complex setup, including an
     *   additional stack containing the EventBusPolicy.
     */
