...
public class CfnCloudFormationProvisionedProductProvisioningPreferencesPropertyDsl {
     ...
     *   operation can fail before AWS Service Catalog stops the operation in that Region. If the
     *   operation is stopped in a Region, AWS Service Catalog doesn't attempt the operation in any
     *   subsequent Regions.
     *
     * Applicable only to a `CFN_STACKSET` provisioned product type.
     *
     ...
     *   this stack operation can fail before AWS Service Catalog stops the operation in that
     *   Region. If the operation is stopped in a Region, AWS Service Catalog doesn't attempt the
     ...
     *   operation at one time. This is dependent on the value of `StackSetFailureToleranceCount` .
     ...
     *   perform this operation at one time. When calculating the number of accounts based on the
     *   specified percentage, AWS Service Catalog rounds down to the next whole number. This is
     *   true except in cases where rounding down would result is zero. In this case, AWS Service
     ...
     *   set or a stack instance represented by the provisioned product. The default value is
     *   `UPDATE` if nothing is specified.
     *
     * Applicable only to a `CFN_STACKSET` provisioned product type.
     * * **CREATE** - Creates a new stack instance in the stack set represented by the provisioned
     *   product. In this case, only new stack instances are created based on accounts and Regions;
