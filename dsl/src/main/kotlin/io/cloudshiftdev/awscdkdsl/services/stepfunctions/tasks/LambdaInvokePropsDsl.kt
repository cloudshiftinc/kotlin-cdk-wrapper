...
public class LambdaInvokePropsDsl {
     ...
     *   without additional metadata. The `payloadResponseOnly` property cannot be used if
     *   `integrationPattern`, `invocationType`, `clientContext`, or `qualifier` are specified. It
     *   always uses the REQUEST_RESPONSE behavior.
     */
    ...
    /**
     * @param qualifier Version or alias to invoke a published version of the function. You only
     *   need to supply this if you want the version of the Lambda Function to depend on data in the
     *   state machine state. If not, you can pass the appropriate Alias or Version object directly
     *   as the `lambdaFunction` argument.
    ...
    /**
     * @param retryOnServiceExceptions Whether to retry on Lambda service exceptions. This handles
     *   `Lambda.ServiceException`, `Lambda.AWSLambdaException` and `Lambda.SdkClientException` with
     *   an interval of 2 seconds, a back-off rate of 2 and 6 maximum attempts.
     */
