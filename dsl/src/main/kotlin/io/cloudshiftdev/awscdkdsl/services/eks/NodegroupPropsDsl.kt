...
public class NodegroupPropsDsl {
    ...
    /**
     * @param amiType The AMI type for your node group. If you explicitly specify the launchTemplate
     *   with custom AMI, do not specify this property, or the node group deployment will fail. In
     *   other cases, you will need to specify correct amiType for the nodegroup.
     */
     ...
     *   to. This number must be greater than or equal to zero.
     */
    ...
    /**
     * @param nodeRole The IAM role to associate with your node group. The Amazon EKS worker node
     *   kubelet daemon makes calls to AWS APIs on your behalf. Worker nodes receive permissions for
     *   these API calls through an IAM instance profile and associated policies. Before you can
     *   launch worker nodes and register them into a cluster, you must create an IAM role for those
     *   worker nodes to use when they are launched.
     */
     ...
     *   group. By specifying the SubnetSelection, the selected subnets will automatically apply
     *   required tags i.e. `kubernetes.io/cluster/CLUSTER_NAME` with a value of `shared`, where
