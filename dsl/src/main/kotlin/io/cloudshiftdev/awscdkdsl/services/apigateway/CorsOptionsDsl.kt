...
public class CorsOptionsDsl {
     ...
     *   whether to expose the response to frontend JavaScript code when the request's credentials
     *   mode (Request.credentials) is "include". When a request's credentials mode
     ...
     *   methods allowed when accessing the resource in response to a preflight request. If `ANY` is
     *   specified, it will be expanded to `Cors.ALL_METHODS`.
     */
     ...
     *   headers can be exposed as part of the response by listing their names. If you want clients
     *   to be able to access other headers, you have to list them using the
     ...
     *   preflight request (that is the information contained in the Access-Control-Allow-Methods
     *   and Access-Control-Allow-Headers headers) can be cached. To disable caching altogether use
     *   `disableCache: true`.
     */
