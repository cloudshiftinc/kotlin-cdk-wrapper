 ...
 * IdP and your AWS account. This is useful when creating a mobile app or web application that
 * requires access to AWS resources, but you don't want to create custom sign-in code or manage your
 * own user identities.
 *
 * Example:
 * ```
...
public class OpenIdConnectProviderDsl(
     ...
     *
     * When a mobile or web app registers with an OpenID Connect provider, they establish a value
     * that identifies the application. (This is the value that's sent as the client_id parameter on
     * OAuth requests.)
     *
     ...
     *
     * Default: - If no thumbprints are specified (an empty array or `undefined`), the thumbprint of
     * the root certificate authority will be obtained from the provider's server as described in
