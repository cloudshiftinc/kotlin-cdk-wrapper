...
public class CfnMethodDsl(
     ...
     * invocation. The authorization works by matching the method scopes against the scopes parsed
     * from the access token in the incoming request. The method invocation is authorized if any
     * method scopes matches a claimed scope in the access token. Otherwise, the invocation is not
     ...
     * and `name` is a valid and unique parameter name. The value associated with the key is a
     * Boolean flag indicating whether the parameter is required ( `true` ) or optional ( `false` ).
