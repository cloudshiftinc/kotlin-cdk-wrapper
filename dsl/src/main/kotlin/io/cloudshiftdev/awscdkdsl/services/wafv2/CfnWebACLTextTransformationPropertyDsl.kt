...
public class CfnWebACLTextTransformationPropertyDsl {
    ...
    /**
     * @param type You can specify the following transformation types:. *BASE64_DECODE* - Decode a
     ...
     *
     * *CMD_LINE* - Command-line transformations. These are helpful in reducing effectiveness of
     ...
     * can help to uncover ASCII characters that were encoded using CSS encoding that wouldn’t
     * typically be encoded. It's also useful in countering evasion, which is a combination of a
     * backslash and non-hexadecimal characters. For example, `ja\vascript` for javascript.
     *
     ...
     *
     * *JS_DECODE* - Decode JavaScript escape sequences. If a `\` `u` `HHHH` code is in the
     * full-width ASCII code range of `FF01-FF5E` , then the higher byte is used to detect and
     * adjust the lower byte. If not, only the lower byte is used and the higher byte is zeroed,
     * causing a possible loss of information.
     *
     * *LOWERCASE* - Convert uppercase letters (A-Z) to lowercase (a-z).
     *
     * *MD5* - Calculate an MD5 hash from the data in the input. The computed hash is in a raw
     * binary form.
     *
     ...
     * single space. Multiple consecutive occurrences are not compressed. Unterminated comments are
     ...
     *
     * *URL_DECODE_UNI* - Like `URL_DECODE` , but with support for Microsoft-specific `%u` encoding.
     * If the code is in the full-width ASCII code range of `FF01-FF5E` , the higher byte is used to
     * detect and adjust the lower byte. Otherwise, only the lower byte is used and the higher byte
     * is zeroed.
     *
