...
public class CfnFormFileUploaderFieldConfigPropertyDsl {
     ...
     *   where they are stored. The valid values for this property are `private` , `protected` , or
     *   `public` . For detailed information about the permissions associated with each access
     *   level, see
     ...
     *   value is `false` . When `isResumable` is set to `true` , the file uploader uses a multipart
    ...
    /**
     * @param maxFileCount Specifies the maximum number of files that can be selected to upload. The
    ...
    /**
     * @param maxSize The maximum file size in bytes that the file uploader will accept. The default
    ...
    /**
     * @param showThumbnails Specifies whether to display or hide the image preview after selecting
     *   a file for upload. The default value is `true` to display the image preview.
     */
