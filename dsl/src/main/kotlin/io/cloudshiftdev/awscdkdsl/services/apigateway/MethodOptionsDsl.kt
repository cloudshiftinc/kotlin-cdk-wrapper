...
public class MethodOptionsDsl {
     ...
     *   available via the `Authorizer` class, such as `Authorizer#token()`, it is recommended that
     *   this option not be specified. The authorizer will take care of setting the correct
     ...
     *   authorizer resource. If specified, the value of `authorizationType` must be set to `Custom`
     */
    ...
    /**
     * @param operationName A friendly operation name for the method. For example, you can assign
     ...
     *   combined with `requestValidator` or `requestValidatorOptions`, the service will validate
     *   the API request payload before it reaches the API's Integration (including proxies).
     *   Specify `requestModels` as key-value pairs, with a content type (e.g. `'application/json'`)
     ...
     *   parameters as key-value pairs (string-to-Boolean mapping), with a source as the key and a
     *   Boolean as the value. The Boolean specifies whether a parameter is required. A source must
     *   match the format method.request.location.name, where the location is querystring, path, or
