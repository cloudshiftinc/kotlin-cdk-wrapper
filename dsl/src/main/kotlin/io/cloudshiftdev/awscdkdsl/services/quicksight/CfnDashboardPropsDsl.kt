...
public class CfnDashboardPropsDsl {
     ...
     *   `DISABLED` . When this is set to `DISABLED` , Amazon QuickSight disables the left filter
     *   pane on the published dashboard, which can be used for ad hoc (one-time) filtering. This
     *   option is `ENABLED` by default.
     * * `AvailabilityStatus` for `ExportToCSVOption` - This status can be either `ENABLED` or
     *   `DISABLED` . The visual option to export data to .CSV format isn't enabled when this is set
     *   to `DISABLED` . This option is `ENABLED` by default.
     ...
     *   `COLLAPSED` or `EXPANDED` . This option is `COLLAPSED` by default.
     */
    public fun dashboardPublishOptions(
        dashboardPublishOptions: CfnDashboard.DashboardPublishOptionsProperty
    ) {
        cdkBuilder.dashboardPublishOptions(dashboardPublishOptions)
    }
     ...
     *   In `SourceEntity` , you specify the type of object that you want to use. You can only
     *   create a dashboard from a template, so you use a `SourceTemplate` entity. If you need to
     *   create a dashboard from an analysis, first convert the analysis to a template by using the
     *   `CreateTemplate` API operation. For `SourceTemplate` , specify the Amazon Resource Name
     *   (ARN) of the source template. The `SourceTemplate` ARN can contain any AWS account; and any
