...
public class CfnServerPropsDsl {
    ...
    /**
     * @param backupRetentionCount The number of automated backups that you want to keep. Whenever a
     *   new backup is created, AWS OpsWorks CM deletes the oldest backups if this number is
    ...
    /**
     * @param disableAutomatedBackup Enable or disable scheduled backups. Valid values are `true` or
    ...
    /**
     * @param engine The configuration management engine to use. Valid values include `ChefAutomate`
     *   and `Puppet` .
     */
    ...
    /**
     * @param engineAttributes Optional engine attributes on a specified server. **Attributes
     ...
     *   server, the valid value for EngineVersion is currently `2` . For a Puppet server, valid
    ...
    /**
     * @param keyPair The Amazon EC2 key pair to set for the instance. This parameter is optional;
     ...
     *   AWS OpsWorks CM performs maintenance on the instance. Valid values must be specified in the
     *   following format: `DDD:HH:MM` . `MM` must be specified as `00` . The specified time is in
     ...
     *   you add this parameter, the specified security groups must be within the VPC that is
    ...
    /**
     * @param subnetIds The IDs of subnets in which to launch the server EC2 instance. Amazon
