...
public class CfnResourceDataSyncPropsDsl {
    ...
    /**
     * @param syncFormat A supported sync format. The following format is currently supported:
     *   JsonSerDe
     */
    ...
    /**
     * @param syncType The type of resource data sync. If `SyncType` is `SyncToDestination` , then
     *   the resource data sync synchronizes data to an S3 bucket. If the `SyncType` is
