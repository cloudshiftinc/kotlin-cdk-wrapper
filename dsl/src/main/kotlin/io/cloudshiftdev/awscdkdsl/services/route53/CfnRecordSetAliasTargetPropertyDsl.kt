...
public class CfnRecordSetAliasTargetPropertyDsl {
     ...
     *   latency alias, and weighted alias resource record sets:* When `EvaluateTargetHealth` is
     *   `true` , an alias resource record set inherits the health of the referenced AWS resource,
     *   such as an ELB load balancer or another resource record set in the hosted zone. Note the
     *   following:
     ...
     *   Elastic Beanstalk environment in `DNSName` and the environment contains an ELB load
     *   balancer, Elastic Load Balancing routes queries only to the healthy Amazon EC2 instances
     *   that are registered with the load balancer. (An environment automatically contains an ELB
     *   load balancer if it includes more than one Amazon EC2 instance.) If you set
     *   `EvaluateTargetHealth` to `true` and either no Amazon EC2 instances are healthy or the load
     *   balancer itself is unhealthy, Route 53 routes queries to other available resources that are
     *   healthy, if any.
     *
     ...
     *   no EC2 instances are healthy or the load balancer itself is unhealthy, Route 53 routes
     ...
     *   `DNSName` is a record or a group of records (for example, a group of weighted records) but
     *   is not another alias record, we recommend that you associate a health check with all of the
     *   records in the alias target. For more information, see
     ...
     *   created the environment in. The environment must have a regionalized subdomain. For a list
     *   of regions and the corresponding hosted zone IDs, see
     ...
     *   balancer in. Note that there are separate columns for Application and Classic Load
     ...
     *   region that you created the bucket in. For more information about valid values, see the
     *   table
     *   [Amazon S3 Website Endpoints](https://docs.aws.amazon.com/general/latest/gr/s3.html#s3_website_region_endpoints)
     *   in the *Amazon Web Services General Reference* .
