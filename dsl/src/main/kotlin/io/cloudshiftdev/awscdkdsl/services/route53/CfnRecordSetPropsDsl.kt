...
public class CfnRecordSetPropsDsl {
     ...
     *   to. If you're creating resource records sets for a private hosted zone, note the following:
     ...
     *   are linking a resource record set to a CIDR location. A `LocationName` with an asterisk “*”
     *   can be used to create a default CIDR record. `CollectionId` is still required for default
     *   record.
     */
     ...
     *   `Failover` element to two resource record sets. For one resource record set, you specify
     *   `PRIMARY` as the value for `Failover` ; for the other resource record set, you specify
     *   `SECONDARY` . In addition, you include the `HealthCheckId` element and specify the health
     *   check that you want Amazon Route 53 to perform for each resource record set.
     *
     ...
     *   applicable value from the secondary resource record set. This is true regardless of the
     ...
     *   query. For example, if you want all queries from Africa to be routed to a web server with
     ...
     *   `HealthCheckId` element and specify the ID of the applicable health check. Route 53
     *   determines whether a resource record set is healthy based on one of the following:
     ...
     * AWS CloudFormation cannot find a hosted zone with a matching domain name, or if there is more
     ...
     *   create, update, or delete. For `ListResourceRecordSets` responses, the name of a record in
     ...
     *   created the resource that this resource record set refers to. The resource typically is an
     *   AWS resource, such as an EC2 instance or an ELB load balancer, and is referred to by an IP
     ...
     *   for the `Type` property. For example, if you specified `A` for `Type` , you specify one or
     *   more IP addresses in IPv4 format for `ResourceRecords` . For information about the format
     *   of values for each record type, see
     ...
     *   acme.example.com that have a type of A. In a group of resource record sets that have the
     *   same name and type, the value of `SetIdentifier` must be unique for each resource record
     *   set. For information about routing policies, see
    ...
    /**
     * @param type The DNS record type. For information about different record types and how data is
     *   encoded for them, see
     *   [Supported DNS Resource Record Types](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/ResourceRecordTypes.html)
     *   in the *Amazon Route 53 Developer Guide* .
     *
     ...
     *   queries that Amazon Route 53 responds to using the current resource record set. Route 53
     ...
     *   a resource record set, Route 53 never responds to queries with the applicable value for
