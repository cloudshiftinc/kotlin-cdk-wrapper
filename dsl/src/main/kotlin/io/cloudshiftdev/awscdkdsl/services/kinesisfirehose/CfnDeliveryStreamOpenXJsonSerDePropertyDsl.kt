...
public class CfnDeliveryStreamOpenXJsonSerDePropertyDsl {
    ...
    /**
     * @param caseInsensitive When set to `true` , which is the default, Firehose converts JSON keys
     ...
     *   column names. This is useful when the JSON contains keys that are Hive keywords. For
     *   example, `timestamp` is a Hive keyword. If you have a JSON key named `timestamp` , set this
     ...
     *   the keys include dots and that you want Firehose to replace them with underscores. This is
     *   useful because Apache Hive does not allow dots in column names. For example, if the JSON
     *   contains a key whose name is "a.b", you can define the column name to be "a_b" when using
     *   this option.
     *
