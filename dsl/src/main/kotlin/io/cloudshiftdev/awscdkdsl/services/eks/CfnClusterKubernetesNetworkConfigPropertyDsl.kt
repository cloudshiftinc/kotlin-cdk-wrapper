...
public class CfnClusterKubernetesNetworkConfigPropertyDsl {
     ...
     *   addresses. If you don't specify a value, `ipv4` is used by default. You can only specify an
     *   IP family when you create a cluster and can't change this value once the cluster is
     *   created. If you specify `ipv6` , the VPC and subnets that you specify for cluster creation
     *   must have both `IPv4` and `IPv6` CIDR blocks assigned to them. You can't specify `ipv6` for
     ...
     * requirements listed in the considerations listed in
     * [Assigning IPv6 addresses to pods and services](https://docs.aws.amazon.com/eks/latest/userguide/cni-ipv6.html)
     * in the Amazon EKS User Guide. Kubernetes assigns services `IPv6` addresses from the unique
     * local address range `(fc00::/7)` . You can't specify a custom `IPv6` CIDR block. Pod
    ...
    /**
     * @param serviceIpv4Cidr Don't specify a value if you select `ipv6` for *ipFamily* . The CIDR
     *   block to assign Kubernetes service IP addresses from. If you don't specify a block,
     *   Kubernetes assigns addresses from either the `10.100.0.0/16` or `172.20.0.0/16` CIDR
