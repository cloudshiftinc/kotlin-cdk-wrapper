...
public class CfnDBInstancePropsDsl {
     ...
     * which corresponds to the minimum Iops value of 1,000. If you increase the `Iops` value (in
     * 1,000 IOPS increments), then you must also increase the `AllocatedStorage` value (in 100-GiB
     * increments).
     *
     * *Amazon Aurora*
     *
     ...
     *   the DB instance. For more info, see
    ...
    /**
     * @param caCertificateIdentifier The identifier of the CA certificate for this DB instance. For
     ...
     *   associated with the specified character set. *Amazon Aurora*
     *
     ...
     * managed by the DB cluster. Setting this value for an Aurora DB instance has no effect on the
     * DB cluster setting.
     */
     ...
     *   snapshot to restore from. For more information on Multi-AZ DB clusters, see
    ...
    /**
     * @param dbInstanceIdentifier A name for the DB instance. If you specify a name, AWS
     *   CloudFormation converts it to lowercase. If you don't specify a name, AWS CloudFormation
     *   generates a unique physical ID and uses that ID for the DB instance. For more information,
     *   see
     ...
     *   resource created in the template. To list all of the available DB parameter group names,
    ...
    /**
     * @param dbSecurityGroups A list of the DB security groups to assign to the DB instance. The
     *   list can include both the name of existing DB security groups or references to
     ...
     * and is no longer recommended for providing security information to an RDS DB instance.
     ...
     * want to submit other properties, such as `StorageType` , `StorageEncrypted` , or `KmsKeyId` .
     * If you're already using the `DBSecurityGroups` property, you can't use these other properties
     * by updating your DB instance to use a VPC security group. You must recreate the DB instance.
     */
     ...
     *   used to restore the DB instance. If you're restoring from a shared manual DB snapshot, you
     ...
     * the `DBSnapshotIdentifier` property is an empty string or the `AWS::RDS::DBInstance`
     * declaration has no `DBSnapshotIdentifier` property, AWS CloudFormation creates a new
     * database. If the property contains a value (other than an empty string), AWS CloudFormation
     * creates a database from the specified snapshot. If a snapshot with the specified name doesn't
     * exist, AWS CloudFormation can't create the database and it rolls back the stack.
     *
    ...
    /**
     * @param dbSubnetGroupName A DB subnet group to associate with the DB instance. If you update
     ...
     * deleted and can't be recovered. Manual DB cluster snapshots of the DB cluster are not
     * deleted.
     */
     ...
     *   default, deletion protection is disabled. For more information, see
    ...
    /**
     * @param domain The Active Directory directory ID to create the DB instance in. Currently, only
     *   Db2, MySQL, Microsoft SQL Server, Oracle, and PostgreSQL DB instances can be created in an
     ...
     *   domain controllers. Constraints:
     ...
     *   Directory Service. This setting doesn't apply to the following DB instances:
    ...
    /**
     * @param engine The name of the database engine that you want to use for this DB instance. Not
     *   every database engine is available in every AWS Region.
     *
    ...
    /**
     * @param engineVersion The version number of the database engine to use. For a list of valid
     ...
     * IOPS rate to the amount of storage that you allocate (IOPS to allocated storage). For
     * example, you can provision an Oracle database instance with 1000 IOPS and 200 GiB of storage
     * (a ratio of 5:1), or specify 2000 IOPS with 200 GiB of storage (a ratio of 10:1). For more
     * information, see
     ...
     *   `arn:aws:kms:us-east-1:012345678910:key/abcd1234-a123-456a-a12b-a123b4cd56ef` . If you
     *   enable the StorageEncrypted property but don't specify this property, AWS CloudFormation
     *   uses the default KMS key. If you specify this property, you must set the StorageEncrypted
     *   property to true.
     *
    ...
    /**
     * @param licenseModel License model information for this DB instance. Valid Values:
     ...
     *   printable ASCII character except "/", """, or "&#64;". *Amazon Aurora*
     *
     ...
     *   setting, including limitations that apply to it, see
     ...
     *   option group. Permanent options, such as the TDE option for Oracle Advanced Security TDE,
     *   can't be removed from an option group. Also, that option group can't be removed from a DB
     ...
     *   data. This setting doesn't apply to RDS Custom DB instances.
     *
     * Valid Values:
     * * `7`
    ...
    /**
     * @param port The port number on which the database accepts connections. *Amazon Aurora*
     *
     ...
     *   occur, in Universal Coordinated Time (UTC). Format: `ddd:hh24:mi-ddd:hh24:mi`
     *
     ...
     *   name, which resolves to a public IP address. If you specify false, AWS CloudFormation
    ...
    /**
     * @param replicaMode The open mode of an Oracle read replica. For more information, see
    ...
    /**
     * @param restoreTime The date and time to restore from. Constraints:
     ...
     *   `arn:aws:rds:us-east-1:123456789012:auto-backup:ab-L2IJCEXJP7XQ7HOJ4SIEXAMPLE` . This
     ...
     *   the ID of the source DB instance. Each DB instance can have a limited number of read
     ...
     * your stack, AWS CloudFormation promotes the Read Replica to a standalone DB instance.
     * * If you specify a source DB instance that uses VPC security groups, we recommend that you
     *   specify the `VPCSecurityGroups` property. If you don't specify the property, the read
     *   replica inherits the value of the `VPCSecurityGroups` property from the source DB when you
     *   create the replica. However, if you update the stack, AWS CloudFormation reverts the
     *   replica's `VPCSecurityGroups` property to the default value because it's not defined in the
     *   stack's template. This change might cause unexpected issues.
     ...
     *   , and `PreferredBackupWindow` properties. The database attributes are inherited from the
     *   source DB instance, and backups are disabled for read replicas.
     ...
     *   default, it isn't encrypted. If you specify the `KmsKeyId` property, then you must enable
     *   encryption.
     *
    ...
    /**
     * @param storageType Specifies the storage type to be associated with the DB instance. Valid
     ...
     *   uses its default processor features. This setting doesn't apply to RDS Custom DB instances.
     */
     ...
     *   backup time. By default, the DB instance isn't restored from the latest backup time.
     *
     * Constraints:
     ...
     *   because the DB instance update fails or because an update fails in another AWS
     ...
     *   recreate the DB instance. If you don't, AWS CloudFormation submits only the property values
