...
public class CfnJobDefinitionLinuxParametersPropertyDsl {
    ...
    /**
     * @param devices Any of the host devices to expose to the container. This parameter maps to
     *   `Devices` in the
     ...
     *   is translated to the `--memory-swap` option to
     *   [docker run](https://docs.aws.amazon.com/https://docs.docker.com/engine/reference/run/)
     ...
     * the swap configuration for the container instance that it's running on. A `maxSwap` value
     ...
     *   behavior. A `swappiness` value of `0` causes swapping to not occur unless absolutely
     *   necessary. A `swappiness` value of `100` causes pages to be swapped aggressively. Valid
     *   values are whole numbers between `0` and `100` . If the `swappiness` parameter isn't
     *   specified, a default value of `60` is used. If a value isn't specified for `maxSwap` , then
     *   this parameter is ignored. If `maxSwap` is set to 0, the container doesn't use swap. This
     ...
     * the instance to use this feature. For more information, see
     ...
     *   container has a default `swappiness` value of 60. Moreover, the total swap usage is limited
