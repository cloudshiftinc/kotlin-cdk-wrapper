...
public class CfnEndpointNeptuneSettingsPropertyDsl {
     ...
     *   of migrated graph data to the Neptune target database before raising an error. The default
     *   is 250.
     */
     ...
     *   parameter to `true` . Then attach the appropriate IAM policy document to your service role
     ...
     *   before AWS DMS bulk-loads the data to the Neptune target database. The default is 1,048,576
     *   KB. If the bulk load is successful, AWS DMS clears the bucket, ready to store the next
     ...
     *   data to the Neptune target database before raising an error. The default is 5.
     */
     ...
     *   migrated graph data in .csv files before bulk-loading it to the Neptune target database.
     *   AWS DMS maps the SQL source data to graph data before storing it in these .csv files.
     */
     ...
     *   created for the Neptune target endpoint. The role must allow the `iam:PassRole` action.
     *
