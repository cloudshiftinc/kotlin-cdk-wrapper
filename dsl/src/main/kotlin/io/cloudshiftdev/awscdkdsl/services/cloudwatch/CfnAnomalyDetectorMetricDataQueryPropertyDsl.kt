 ...
 * just retrieving a batch set of data for one metric, or is performing a Metrics Insights query or
 * a math expression. A single `GetMetricData` call can include up to 500 `MetricDataQuery`
 * structures.
 *
...
public class CfnAnomalyDetectorMetricDataQueryPropertyDsl {
    ...
    /**
     * @param accountId The ID of the account where the metrics are located. If you are performing a
     ...
     *   expression to be performed on the returned data. For more information about Metrics
     *   Insights queries, see
    ...
    /**
     * @param id A short name used to tie this object to the results in the response. This name must
     *   be unique within a single call to `GetMetricData` . If you are performing math expressions
     ...
     *   if this is an expression, so that you know what the value represents. If the metric or
    ...
    /**
     * @param metricStat The metric to be returned, along with statistics, period, and units. Use
     *   this parameter only if this object is retrieving a metric and not performing a math
    ...
    /**
     * @param returnData When used in `GetMetricData` , this option indicates whether to return the
     *   timestamps and raw data values of this metric. If you are performing this call just to do
     *   math expressions and do not also need the raw data returned, you can specify `false` . If
     *   you omit this, the default of `true` is used.
     *
