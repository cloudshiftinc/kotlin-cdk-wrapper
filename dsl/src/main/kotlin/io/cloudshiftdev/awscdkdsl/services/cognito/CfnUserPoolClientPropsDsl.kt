...
public class CfnUserPoolClientPropsDsl {
    ...
    /**
     * @param accessTokenValidity The access token time limit. After this limit expires, your user
    ...
    /**
     * @param callbackUrLs A list of allowed redirect (callback) URLs for the IdPs. A redirect URI
     *   must:
     * * Be an absolute URI.
    ...
    /**
     * @param defaultRedirectUri The default redirect URI. Must be in the `CallbackURLs` list. A
     *   redirect URI must:
     * * Be an absolute URI.
     * * Be registered with the authorization server.
     * * Not include a fragment component.
     ...
     *   context data. For more information about propagation of user context data, see
     ...
     *   their ID token. To specify the time unit for `IdTokenValidity` as `seconds` , `minutes` ,
    ...
    /**
     * @param refreshTokenValidity The refresh token time limit. After this limit expires, your user
     *   can't use their refresh token. To specify the time unit for `RefreshTokenValidity` as
     *   `seconds` , `minutes` , `hours` , or `days` , set a `TokenValidityUnits` value in your API
     *   request.
     *
    ...
    /**
     * @param tokenValidityUnits The units in which the validity times are represented. The default
    ...
    /**
     * @param writeAttributes The user pool attributes that the app client can write to. If your app
     *   client allows users to sign in through an IdP, this array must include all attributes that
     *   you have mapped to IdP attributes. Amazon Cognito updates mapped attributes when users sign
     *   in to your application through an IdP. If your app client does not have write access to a
     *   mapped attribute, Amazon Cognito throws an error when it tries to update the attribute. For
