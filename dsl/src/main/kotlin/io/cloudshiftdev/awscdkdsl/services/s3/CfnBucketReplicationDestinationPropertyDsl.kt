...
public class CfnBucketReplicationDestinationPropertyDsl {
     ...
     *   and destination bucket owners are not the same), and you want to change replica ownership
     *   to the AWS account that owns the destination bucket. If this is not specified in the
     *   replication configuration, the replicas are owned by same AWS account that owns the source
     *   object.
     */
    ...
    public fun accessControlTranslation(
        accessControlTranslation: CfnBucket.AccessControlTranslationProperty
    ) {
        cdkBuilder.accessControlTranslation(accessControlTranslation)
    }

    /**
     * @param account Destination bucket owner account ID. In a cross-account scenario, if you
     *   direct Amazon S3 to change replica ownership to the AWS account that owns the destination
     *   bucket by specifying the `AccessControlTranslation` property, this is the account ID of the
    ...
    public fun encryptionConfiguration(
        encryptionConfiguration: CfnBucket.EncryptionConfigurationProperty
    ) {
        cdkBuilder.encryptionConfiguration(encryptionConfiguration)
    }
     ...
     *   reduced redundancy. By default, Amazon S3 uses the storage class of the source object to
