...
public class CfnDBClusterPropsDsl {
     ...
     *   roles that are associated with the DB cluster. IAM roles that are associated with a DB
     *   cluster grant permission for the DB cluster to access other Amazon services on your behalf.
     */
     ...
     *   are retained. An update may require some interruption. See
    ...
    /**
     * @param dbClusterIdentifier Contains a user-supplied DB cluster identifier. This identifier is
    ...
    /**
     * @param dbClusterParameterGroupName Provides the name of the DB cluster parameter group. An
     *   update may require some interruption. See
     *   [ModifyDBInstance](https://docs.aws.amazon.com/neptune/latest/userguide/api-instances.html#ModifyDBInstance)
     *   in the Amazon Neptune User Guide for more information.
     */
     ...
     *   instances of the DB cluster. Used only in case of a major engine version upgrade request
     *
    ...
    /**
     * @param restoreToTime Creates a new DB cluster from a DB snapshot or DB cluster snapshot. If a
     *   DB snapshot is specified, the target DB cluster is created from the source DB snapshot with
    ...
    /**
     * @param restoreType Creates a new DB cluster from a DB snapshot or DB cluster snapshot. If a
     *   DB snapshot is specified, the target DB cluster is created from the source DB snapshot with
     *   a default configuration and default security group.
     *
     * If a DB cluster snapshot is specified, the target DB cluster is created from the source DB
    ...
    public fun serverlessScalingConfiguration(
        serverlessScalingConfiguration: CfnDBCluster.ServerlessScalingConfigurationProperty
    ) {
        cdkBuilder.serverlessScalingConfiguration(serverlessScalingConfiguration)
    }
     ...
     *   identifier of an existing snapshot. After you restore a DB cluster using a
     *   `SnapshotIdentifier` , you must specify the same `SnapshotIdentifier` for any future
     *   updates to the DB cluster. When you specify this property for an update, the DB cluster is
     ...
     *   snapshot. If a DB snapshot is specified, the target DB cluster is created from the source
     ...
     *   `DBClusterIdentifier` , `DBSnapshotIdentifier` , or `SourceDBInstanceIdentifier` property,
     *   don't specify this property. The value is inherited from the cluster, snapshot, or source
