...
public class CfnStageMethodSettingPropertyDsl {
    ...
    /**
     * @param httpMethod The HTTP method. To apply settings to multiple resources and methods,
     *   specify an asterisk ( `*` ) for the `HttpMethod` and `/ *` for the `ResourcePath` . This
     ...
     *   entries pushed to Amazon CloudWatch Logs. Valid values are `OFF` , `ERROR` , and `INFO` .
     *   Choose `ERROR` to write only error-level entries to CloudWatch Logs, or choose `INFO` to
     *   include all `ERROR` events as well as extra informational events.
     */
    ...
    /**
     * @param resourcePath The resource path for this method. Forward slashes ( `/` ) are encoded as
     *   `~1` and the initial slash must include a forward slash. For example, the path value
     *   `/resource/subresource` must be encoded as `/~1resource~1subresource` . To specify the root
     *   path, use only a slash ( `/` ). To apply settings to multiple resources and methods,
     *   specify an asterisk ( `*` ) for the `HttpMethod` and `/ *` for the `ResourcePath` . This
     *   parameter is required when you specify a `MethodSetting` .
     */
