 ...
 * connected home features. You can then select what you want the stream processor to detect, such
 * as people or pets.
 *
 * When the stream processor has started, one notification is sent for each object class specified.
 * For example, if packages and pets are selected, one SNS notification is published the first time
 * a package is detected and one SNS notification is published the first time a pet is detected. An
...
public class CfnStreamProcessorConnectedHomeSettingsPropertyDsl {
     ...
     *   pets. The current valid labels you can include in this list are: "PERSON", "PET",
     *   "PACKAGE", and "ALL".
     */
