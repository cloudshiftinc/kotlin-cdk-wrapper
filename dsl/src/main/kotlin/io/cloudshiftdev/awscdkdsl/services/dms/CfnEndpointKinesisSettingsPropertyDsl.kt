...
public class CfnEndpointKinesisSettingsPropertyDsl {
     ...
     *   definition, and table and column changes in the Kinesis message output. The default is
     *   `false` .
     */
     ...
     *   unless the partition type is `schema-table-type` . The default is `false` .
     */
     ...
     *   column type in hexadecimal format moving from an Oracle source to an Amazon Kinesis target.
     *   Use the `NoHexPrefix` endpoint setting to enable migration of RAW data type columns without
     ...
     *   Kinesis shards. For example, suppose that a SysBench schema has thousands of tables and
     *   each table has only limited range for a primary key. In this case, the same primary key is
     *   sent from thousands of tables to the same shard, which causes throttling. The default is
     *   `false` .
     */
     ...
     *   to write to the Kinesis data stream. The role must allow the `iam:PassRole` action.
     */
