...
public class CfnVolumeOpenZFSConfigurationPropertyDsl {
     ...
     *   copied to snapshots. This value defaults to `false` . If it's set to `true` , all tags for
     *   the volume are copied to snapshots where the user doesn't specify tags. If this value is
     ...
     *   kibibytes (KiB). Valid values are 4, 8, 16, 32, 64, 128, 256, 512, or 1024 KiB. The default
     *   is 128 KiB. We recommend using the default setting for the majority of use cases.
     ...
     *   streaming workloads (large record size). For additional guidance on when to set a custom
     *   record size, see
     ...
     *   volume. You can specify a quota that is larger than the storage on the parent volume. A
     *   volume quota limits the amount of storage that the volume can consume to the configured
     *   amount, but does not guarantee the space will be available on the parent volume. To
     *   guarantee quota space, you must also set `StorageCapacityReservationGiB` . To *not* specify
     ...
     *   reserve from the parent volume. Setting `StorageCapacityReservationGiB` guarantees that the
     *   specified amount of storage space on the parent volume will always be available for the
     *   volume. You can't reserve more storage than the parent volume has. To *not* specify a
     *   storage capacity reservation, set this to `0` or `-1` . For more information, see
     *   [Volume properties](https://docs.aws.amazon.com/fsx/latest/OpenZFSGuide/managing-volumes.html#volume-properties)
     *   in the *Amazon FSx for OpenZFS User Guide* .
     */
