...
public object refactorspaces {
     ...
     * environment, regardless of the account that creates the application. Refactor Spaces
     * provisions an Amazon API Gateway , API Gateway VPC link, and Network Load Balancer for the
     ...
     * application proxy to a service with a private URL endpoint. For more information, see
     ...
     */
    public inline fun cfnApplicationApiGatewayProxyInputProperty(
     ...
     * environment owner has cross-account visibility and control of Refactor Spaces resources that
     * are added to the environment by other accounts that the environment is shared with.
     *
     ...
     *
     * The account owner of the service resource is always the environment owner, regardless of
     * which account creates the route. Routes target a service in the application. If an
     * application does not have any routes, then the first route must be created as a `DEFAULT`
     * `RouteType` .
     *
     * When created, the default route defaults to an active state so state is not a required input.
     * However, like all other state values the state of the default route can be updated after
     * creation, but only when all other routes are also inactive. Conversely, no route can be
     ...
     * properties associated with the `AWS::RefactorSpaces::Route` cannot be updated, even though
     * the property description might indicate otherwise. Updating all other properties will result
     * in the replacement of Route.
     *
     ...
     * Refactor Spaces routes traffic using the API Gateway VPC link. If a service endpoint resolves
     * to a public IP address, Refactor Spaces routes traffic over the public internet. Services can
     * have HTTP or HTTPS URL endpoints. For HTTPS URLs, publicly-signed certificates are supported.
     ...
     * time-to-live (TTL) expires, or every 60 seconds for TTLs less than 60 seconds. This periodic
     ...
     *
     * If the service has an AWS Lambda function endpoint, then Refactor Spaces configures the
     * Lambda function's resource policy to allow the application's API Gateway to invoke the
     * function.
     *
     ...
     * [VPC to VPC connectivity](https://docs.aws.amazon.com/whitepapers/latest/aws-vpc-connectivity-options/amazon-vpc-to-amazon-vpc-connectivity-options.html)
     * between your network and the application proxy VPC. Route creation from the application proxy
     * to service endpoints will fail if your network is not configured to connect to the
     * application proxy VPC. For more information, see
     ...
     */
    public inline fun cfnRouteDefaultRouteInputProperty(
     ...
     */
    public inline fun cfnRouteUriPathRouteInputProperty(
     ...
     *
     * If an AWS resource is launched in a service VPC, and you want it to be accessible to all of
     * an environment’s services with VPCs and routes, apply the `RefactorSpacesSecurityGroup` to
     * the resource. Alternatively, to add more cross-account constraints, apply your own security
     * group.
     *
     * Example:
     * ```
     ...
     */
    public inline fun cfnServiceLambdaEndpointInputProperty(
     ...
     */
    public inline fun cfnServiceUrlEndpointInputProperty(
