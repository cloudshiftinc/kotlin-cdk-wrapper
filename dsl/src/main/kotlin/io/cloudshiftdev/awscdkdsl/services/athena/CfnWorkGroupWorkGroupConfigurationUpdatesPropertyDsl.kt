...
public class CfnWorkGroupWorkGroupConfigurationUpdatesPropertyDsl {
    ...
    /**
     * @param bytesScannedCutoffPerQuery The upper data usage limit (cutoff) for the amount of bytes
     *   a single query in a workgroup is allowed to scan.
     */
     ...
     *   reference Amazon S3 Requester Pays buckets in queries. If set to false, workgroup members
     *   cannot query data from Requester Pays buckets, and queries that retrieve data from
     *   Requester Pays buckets cause an error.
     */
     ...
     *   this workgroup that will be updated. Includes the updated results location and an updated
    ...
    public fun resultConfigurationUpdates(
        resultConfigurationUpdates: CfnWorkGroup.ResultConfigurationUpdatesProperty
    ) {
        cdkBuilder.resultConfigurationUpdates(resultConfigurationUpdates)
    }
