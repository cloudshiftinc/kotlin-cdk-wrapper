...
public class CfnTaskDefinitionPortMappingPropertyDsl {
    ...
    /**
     * @param appProtocol The application protocol that's used for the port mapping. This parameter
     *   only applies to Service Connect. We recommend that you set this parameter to be consistent
     *   with the protocol that your application uses. If you set this parameter, Amazon ECS adds
     ...
     *   automatically assigned host port. If you use containers in a task with the `awsvpc` or
     *   `host` network mode, specify the exposed ports using `containerPort` .
     *
     ...
     * assigned in this way do not count toward the 100 reserved ports limit of a container
     * instance.
     */
     ...
     * [Issue #11185](https://docs.aws.amazon.com/https://github.com/moby/moby/issues/11185) on the
     * Github website.
     *
    ...
    /**
     * @param hostPort The port number on the container instance to reserve for your container. If
     *   you specify a `containerPortRange` , leave this field empty and the value of the `hostPort`
     *   is set as follows:
     * * For containers in a task with the `awsvpc` network mode, the `hostPort` is set to the same
     *   value as the `containerPort` . This is a static mapping strategy.
     * * For containers in a task with the `bridge` network mode, the Amazon ECS agent finds open
     ...
     * set it to `0` ) while specifying a `containerPort` and your container automatically receives
     * a port in the ephemeral port range for your container instance operating system and Docker
     * version.
     *
     ...
     * task is also reserved while the task is running. That is, after a task stops, the host port
     * is released. The current reserved ports are displayed in the `remainingResources` of
     ...
     * includes the default reserved ports. Automatically assigned ports aren't included in the 100
    ...
    /**
     * @param name The name that's used for the port mapping. This parameter only applies to Service
     *   Connect. This parameter is the name that you use in the `serviceConnectConfiguration` of a
     *   service. The name can include up to 64 characters. The characters can include lowercase
