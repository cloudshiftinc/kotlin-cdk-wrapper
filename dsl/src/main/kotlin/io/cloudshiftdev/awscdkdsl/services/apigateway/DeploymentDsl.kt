 ...
 *
 * If you manually define this resource, you will need to know that since deployments are immutable,
 * as long as the resource's logical ID doesn't change, the deployment will represent the snapshot
 * in time in which the resource was created. This means that if you modify the RestApi model (i.e.
 * add methods or resources), these changes will not be reflected unless a new deployment resource
 * is created.
 *
 * To achieve this behavior, the method `addToLogicalId(data)` can be used to augment the logical ID
 * generated for the deployment resource such that it will include arbitrary data. This is done
 * automatically for the `restApi.latestDeployment` deployment.
 *
 * Furthermore, since a deployment does not reference any of the REST API resources and methods,
 * CloudFormation will likely provision it before these resources are created, which means that it
 * will represent a "half-baked" model. Use the `node.addDependency(dep)` method to circumvent that.
...
public class DeploymentDsl(
     ...
     *
     * If this is true, the old API Gateway Deployment resource will not be deleted. This will allow
     * manually reverting back to a previous deployment in case for example
     *
     * Default: false
     *
