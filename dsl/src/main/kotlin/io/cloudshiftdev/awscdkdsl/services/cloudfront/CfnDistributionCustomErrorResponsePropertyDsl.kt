...
public class CfnDistributionCustomErrorResponsePropertyDsl {
     ...
     *   cache the HTTP status code specified in `ErrorCode` . When this time period has elapsed,
     *   CloudFront queries your origin to see whether the problem that caused the error has been
     ...
     *   along with the custom error page. There are a variety of reasons that you might want
     *   CloudFront to return a status code different from the status code that your origin returned
     ...
     *   to a viewer when your origin returns the HTTP status code specified by `ErrorCode` , for
     *   example, `/4xx-errors/403-forbidden.html` . If you want to store your objects and your
     *   custom error pages in different locations, your distribution must include a cache behavior
     ...
     * error pages on an HTTP server and the server starts to return 5xx errors, CloudFront can't
     * get the files that you want to return to viewers because the origin server is unavailable.
     */
