 ...
 * location of a container image in the Amazon ECR registry. For a .zip file deployment package, you
...
public class CfnFunctionCodePropertyDsl {
    ...
    /**
     * @param s3Bucket An Amazon S3 bucket in the same AWS Region as your function. The bucket can
     ...
     *   your function source inline with this parameter, AWS CloudFormation places it in a file
     *   named `index` and zips it to create a
     *   [deployment package](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-package.html)
     *   . This zip file cannot exceed 4MB. For the `Handler` property, the first part of the
     *   handler identifier must be `index` . For example, `index.handler` . For JSON, you must
     *   escape quotes and special characters such as newline ( `\n` ) with a backslash.
     *
