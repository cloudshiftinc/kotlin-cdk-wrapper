...
public class CfnProjectSourcePropertyDsl {
     ...
     *   source code to be built. This information is for the AWS CodeBuild console's use only. Your
     *   code should not get or set `Auth` directly.
     */
    ...
    /**
     * @param buildSpec The build specification for the project. If this value is not provided, then
     *   the source code must contain a buildspec file named `buildspec.yml` at the root level. If
     *   this value is provided, it can be either a single string containing the entire build
     *   specification, or the path to an alternate buildspec file relative to the value of the
     *   built-in environment variable `CODEBUILD_SRC_DIR` . The alternate buildspec file can have a
     *   name other than `buildspec.yml` , for example `myspec.yml` or `build_spec_qa.yml` or
     *   similar. For more information, see the
     *   [Build Spec Reference](https://docs.aws.amazon.com/codebuild/latest/userguide/build-spec-ref.html#build-spec-ref-example)
    ...
    /**
     * @param gitCloneDepth The depth of history to download. Minimum value is 0. If this value is
     *   0, greater than 25, or not provided, then the full history is downloaded with each build
    ...
    /**
     * @param insecureSsl This is used with GitHub Enterprise only. Set to true to ignore SSL
     *   warnings while connecting to your GitHub Enterprise project repository. The default value
     *   is `false` . `InsecureSsl` should be used for testing purposes only. It should not be used
     ...
     *   it. This is because CodePipeline uses the settings in a pipeline's source action instead of
     *   this value.
     ...
     *   the source and the buildspec file. You must connect your AWS account to your GitHub
     *   account. Use the AWS CodeBuild console to start creating a build project. When you use the
     *   console to connect (or reconnect) with GitHub, on the GitHub *Authorize application* page,
     *   for *Organization access* , choose *Request access* next to each repository you want to
     *   allow AWS CodeBuild to have access to, and then choose *Authorize application* . (After you
     *   have connected to your GitHub account, you do not need to finish creating the build
     *   project. You can leave the AWS CodeBuild console.) To instruct AWS CodeBuild to use this
     ...
     *   AWS CodeBuild console.) To instruct AWS CodeBuild to use this connection, in the `source`
     ...
     *   your source provider. This option is valid only when your source provider is GitHub, GitHub
    ...
    /**
     * @param sourceIdentifier An identifier for this project source. The identifier can only
     *   contain alphanumeric characters and underscores, and must be less than 128 characters in
     *   length.
     */
