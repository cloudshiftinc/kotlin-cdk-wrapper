 ...
 * account must have permissions to read the Amazon S3 bucket. The Amazon S3 bucket must contain a
 * valid schema. The schema must refer to simulation assets that are already uploaded to the AWS
 * Cloud . For more information, see the
...
public class CfnSimulationDsl(
     ...
     * equivalent in the other units. The default value is `14D` . A value equivalent to `0` makes
     * the simulation immediately transition to `STOPPING` as soon as it reaches `STARTED` .
     *
