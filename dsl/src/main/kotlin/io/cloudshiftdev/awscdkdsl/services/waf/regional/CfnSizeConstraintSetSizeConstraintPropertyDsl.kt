...
public class CfnSizeConstraintSetSizeConstraintPropertyDsl {
    ...
    /**
     * @param comparisonOperator The type of comparison you want AWS WAF to perform. AWS WAF uses
     *   this in combination with the provided `Size` and `FieldToMatch` to build an expression in
     ...
     *   specified `FieldToMatch` . AWS WAF uses this in combination with `ComparisonOperator` and
     *   `FieldToMatch` to build an expression in the form of " `Size` `ComparisonOperator` size in
     *   bytes of `FieldToMatch` ". If that expression is true, the `SizeConstraint` is considered
     *   to match.
     *
     ...
     *   transformation, AWS WAF performs the transformation on `FieldToMatch` before inspecting a
     *   request for a match.
     *
     * You can only specify a single type of TextTransformation.
     *
     * Note that if you choose `BODY` for the value of `Type` , you must choose `NONE` for
     * `TextTransformation` because the API Gateway API or Application Load Balancer forward only
