 ...
 * for any MySQL-compatible endpoint. For more information, see
...
public class CfnEndpointGcpMySQLSettingsPropertyDsl {
     ...
     *   endpoint. The migration task continues running regardless if the SQL statement succeeds or
     *   fails.
     *
     ...
     *   SQL Server source database that is hosted as part of an Always On availability group
     *   cluster. If you need AWS DMS to poll all the nodes in the Always On cluster for transaction
    ...
    /**
     * @param databaseName Database name for the endpoint. For a MySQL source or target endpoint,
     *   don't explicitly specify the database using the `DatabaseName` request parameter on either
     *   the `CreateEndpoint` or `ModifyEndpoint` API call. Specifying `DatabaseName` when you
     *   create or modify a MySQL endpoint replicates all the task tables to this single database.
     *   For MySQL endpoints, you specify the database only when you specify the schema in the
     ...
     *   to a MySQL-compatible database. Example: `maxFileSize=512`
     */
     ...
     *   target database. Specifies how many threads to use to load the data into the
     *   MySQL-compatible target database. Setting a large number of threads can have an adverse
     *   effect on database performance, because a separate connection is required for each thread.
     *   The default is one.
     *
     ...
     *   time zones in single quotation marks. Example: `serverTimezone=US/Pacific;`
     */
