 ...
 * image's Dockerfile). This configuration maps to the `HEALTHCHECK` parameter of
 ...
 * task definition. Amazon ECS does not monitor Docker health checks that are embedded in a
 * container image and not specified in the container definition. Health check parameters that are
 * specified in a container definition override any Docker health checks that exist in the container
 * image.
 *
 ...
 *   or greater. For more information, see
...
public class CfnTaskDefinitionHealthCheckPropertyDsl {
     ...
     *   if it is healthy. The string array must start with `CMD` to run the command arguments
     ...
     *   considered unhealthy. You may specify between 1 and 10 retries. The default value is 3.
     */
     ...
     *   failed health checks count towards the maximum number of retries. You can specify between 0
     *   and 300 seconds. By default, the `startPeriod` is off.
     *
