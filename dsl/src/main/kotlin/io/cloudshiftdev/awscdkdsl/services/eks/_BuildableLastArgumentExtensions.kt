 ...
 */
public inline fun CfnNodegroup.setScalingConfig(
 ...
 *
 * The nodes will automatically be configured with the right VPC and AMI for the instance type and
 * Kubernetes version.
 *
 * Note that if you specify `updateType: RollingUpdate` or `updateType: ReplacingUpdate`, your nodes
 * might be replaced at deploy time without notice in case the recommended AMI for your machine
 * image type has been updated by AWS. The default behavior for `updateType` is `None`, which means
 * only new instances will be launched using the new AMI.
 *
 ...
 */
public inline fun Cluster.addHelmChart(
    id: String,
