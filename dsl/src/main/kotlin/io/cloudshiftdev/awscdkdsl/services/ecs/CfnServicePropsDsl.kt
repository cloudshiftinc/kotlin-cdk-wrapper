...
public class CfnServicePropsDsl {
    ...
    /**
     * @param deploymentController The deployment controller to use for the service. If no
     ...
     *   and keep running in your service. For new services, if a desired count is not specified, a
     *   default value of `1` is used. When using the `DAEMON` scheduling strategy, the desired
     ...
     *   tasks within the service. For more information, see
     ...
     *   for the service. If `true` , the execute command functionality is turned on for all
    ...
    /**
     * @param loadBalancers A list of load balancer objects to associate with the service. If you
     *   specify the `Role` property, `LoadBalancers` must be specified as well. For information
     *   about the number of load balancers that you can specify per service, see
    ...
    /**
     * @param networkConfiguration The network configuration for the service. This parameter is
     *   required for task definitions that use the `awsvpc` network mode to receive their own
     *   elastic network interface, and it is not supported for other network modes. For more
     *   information, see
     ...
     *   service. You can specify a maximum of 10 constraints for each task. This limit includes
