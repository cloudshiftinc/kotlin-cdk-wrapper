...
public class CfnFirewallPolicyStatefulEngineOptionsPropertyDsl {
     ...
     *   policy. `DEFAULT_ACTION_ORDER` is the default behavior. Stateful rules are provided to the
     *   rule engine as Suricata compatible strings, and Suricata evaluates them based on certain
     ...
     *   connection breaks midstream. Network connections can break due to disruptions in external
     ...
     *   context from traffic before the break. This impacts the behavior of rules that depend on
     *   this context. For example, if you have a stateful rule to `drop http` traffic, Network
     *   Firewall won't match the traffic for this rule because the service won't have the context
     *   from session initialization defining the application layer protocol as HTTP. However, this
     *   behavior is rule dependent—a TCP-layer rule using a `flow:stateless` rule would still
     ...
     *   client can immediately establish a new session. Network Firewall will have context about
