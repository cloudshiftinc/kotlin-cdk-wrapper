...
public class CfnGlobalTableReplicaGlobalSecondaryIndexSpecificationPropertyDsl {
     ...
     *   specific table or index. CloudWatch Contributor Insights for DynamoDB graphs display the
     *   partition key and (if applicable) sort key of frequently accessed items and frequently
     *   throttled items in plaintext. If you require the use of AWS Key Management Service (KMS) to
     *   encrypt this table’s partition key and sort key data with an AWS managed key or customer
     *   managed key, you should not enable CloudWatch Contributor Insights for DynamoDB for this
     *   table.
     */
    ...
    /**
     * @param readProvisionedThroughputSettings Allows you to specify the read capacity settings for
     *   a replica global secondary index when the `BillingMode` is set to `PROVISIONED` .
     */
