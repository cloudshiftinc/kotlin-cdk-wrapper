 ...
 *
 * If you don't want AWS Config to record all current and future supported resource types (excluding
 ...
 *
 * The `AWS::RDS::GlobalCluster` resource type will be recorded in all supported AWS Config Regions
 * where the configuration recorder is enabled.
 *
...
public class CfnConfigurationRecorderRecordingGroupPropertyDsl {
     ...
     *   supported resource types, excluding the global IAM resource types. If you set this field to
     *   `true` , when AWS Config adds support for a new resource type, AWS Config starts recording
     ...
     *   IAM resource types: IAM users, groups, roles, and customer managed policies. These global
     *   IAM resource types can only be recorded by AWS Config in Regions where AWS Config was
     ...
     * set to `false` . The `IncludeGlobalResourceTypes` option is a bundle which only applies to
     ...
     *   supported resource types except the resource types that you specify to exclude from being
     ...
     *   records. For a list of valid `ResourceTypes` values, see the *Resource Type Value* column
     *   in
