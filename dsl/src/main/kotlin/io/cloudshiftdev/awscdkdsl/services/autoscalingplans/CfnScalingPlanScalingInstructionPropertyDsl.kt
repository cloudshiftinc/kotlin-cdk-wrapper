 ...
 * scaling metric, the predictive scaling mode (forecast and scale or forecast only), and the
 * desired behavior when the forecast capacity exceeds the maximum capacity of the resource. With
 * predictive scaling, AWS Auto Scaling generates forecasts with traffic predictions for the two
 * days ahead and schedules scaling actions that proactively add and remove resource capacity to
 * match the forecast.
 *
...
public class CfnScalingPlanScalingInstructionPropertyDsl {
     ...
     *   scaling. This property or a *PredefinedLoadMetricSpecification* is required when
    ...
    public fun customizedLoadMetricSpecification(
        customizedLoadMetricSpecification: CfnScalingPlan.CustomizedLoadMetricSpecificationProperty
    ) {
        cdkBuilder.customizedLoadMetricSpecification(customizedLoadMetricSpecification)
    }
    ...
    /**
     * @param maxCapacity The maximum capacity of the resource. The exception to this upper limit is
     ...
     *   scaling. This property or a *CustomizedLoadMetricSpecification* is required when
     *   configuring predictive scaling, and cannot be used otherwise.
     */
    ...
    public fun predefinedLoadMetricSpecification(
        predefinedLoadMetricSpecification: CfnScalingPlan.PredefinedLoadMetricSpecificationProperty
    ) {
        cdkBuilder.predefinedLoadMetricSpecification(predefinedLoadMetricSpecification)
    }
     ...
     *   the forecast capacity approaches or exceeds the maximum capacity specified for the
     ...
     *   forecast capacity is close to or exceeds the maximum capacity. The value is specified as a
     *   percentage relative to the forecast capacity. For example, if the buffer is 10, this means
     *   a 10 percent buffer. With a 10 percent buffer, if the forecast capacity is 50, and the
     ...
     *   `ForecastAndScale` . Otherwise, AWS Auto Scaling forecasts capacity but does not apply any
     ...
     *   scheduled scaling actions when scaling out. For example, if the forecast says to add
     *   capacity at 10:00 AM, and the buffer time is 5 minutes, then the run time of the
     *   corresponding scheduled scaling action will be 9:55 AM. The intention is to give resources
     *   time to be provisioned. For example, it can take a few minutes to launch an EC2 instance.
     *   The actual amount of time required depends on several factors, such as the size of the
    ...
    /**
     * @param targetTrackingConfigurations The target tracking configurations (up to 10). Each of
     *   these structures must specify a unique scaling metric and a target value for the metric.
     */
