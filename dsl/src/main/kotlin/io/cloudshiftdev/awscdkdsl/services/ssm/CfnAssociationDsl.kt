 ...
 * example, an association can specify that anti-virus software must be installed and running on
 ...
 * Resource Groups or an AWS Auto Scaling Group, State Manager applies the configuration when new
...
public class CfnAssociationDsl(
     ...
     * AWS account by specifying the `InstanceIds` key with a value of `*` . To view a JSON and a
     * YAML example that targets all instances, see "Create an association for all managed instances
