 ...
 * from other dependent resources to point to the replacement DB cluster, and finally deletes the
 * old DB cluster.
 *
 ...
 *   with your DB cluster changes and add the `SnapshotIdentifier` property with the ID of the DB
 *   cluster snapshot that you want to use.
 *
 ...
 * property for an update, the DB cluster is not restored from the DB cluster snapshot again, and
 * the data in the database is not changed. However, if you don't specify the `SnapshotIdentifier`
...
public class CfnDBClusterDsl(
     ...
     *   specify the global cluster ID of the global database cluster.
     */
     ...
     * KMS key is used. If you specify this property, you must set the `StorageEncrypted` property
     * to `true` .
     *
     ...
     * the private IP address from within the DB cluster's virtual private cloud (VPC). It resolves
     * to the public IP address from outside of the DB cluster's VPC. Access to the DB cluster is
