 ...
 * direction, and rule options. For information about the Suricata `Rules` format, see
...
public class CfnRuleGroupStatefulRulePropertyDsl {
     ...
     *   the flow matches the stateful rule criteria. For all actions, Network Firewall performs the
     ...
     *   reset packet back to sender of the packet. A TCP reset packet is a packet with no payload
     *   and a `RST` bit contained in the TCP header flags. `REJECT` is available only for TCP
     *   traffic.
     ...
     *   payload and a `RST` bit contained in the TCP header flags. Also sends an alert log mesage
