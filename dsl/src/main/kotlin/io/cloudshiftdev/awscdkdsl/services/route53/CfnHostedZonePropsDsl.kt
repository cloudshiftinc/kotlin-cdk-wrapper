...
public class CfnHostedZonePropsDsl {
    ...
    /**
     * @param hostedZoneConfig A complex type that contains an optional comment. If you don't want
     *   to specify a comment, omit the `HostedZoneConfig` and `Comment` elements.
     */
    ...
    /**
     * @param hostedZoneTags Adds, edits, or deletes tags for a health check or a hosted zone. For
    ...
    /**
     * @param name The name of the domain. Specify a fully qualified domain name, for example,
     ...
     * name servers for your domain to the set of `NameServers` that are returned by the
    ...
    /**
     * @param queryLoggingConfig Creates a configuration for DNS query logging. After you create a
     ...
     * groups and the associated AWS resources, such as Route 53 hosted zones. There's a limit on
     ...
     *   DNS queries for the specified hosted zone. That log stream is used to log all queries that
     ...
     *   forward to Route 53. If a DNS resolver has already cached the response to a query (such as
     *   the IP address for a load balancer for example.com), the resolver will continue to return
     *   the cached response. It doesn't forward another query to Route 53 until the TTL for the
     *   corresponding resource record set expires. Depending on how many DNS queries are submitted
     *   for a resource record set, and depending on the TTL for that resource record set, query
     *   logs might contain information about only one query out of every several thousand queries
     *   that are submitted to DNS. For more information about how DNS works, see
