...
public class CfnPipelineArtifactStorePropertyDsl {
    ...
    /**
     * @param encryptionKey The encryption key used to encrypt the data in the artifact store, such
     *   as an AWS Key Management Service ( AWS KMS) key. If this is undefined, the default key for
     *   Amazon S3 is used. To see an example artifact store encryption key field, see the example
     *   structure here:
    ...
    /**
     * @param location The S3 bucket used for storing the artifacts for a pipeline. You can specify
     *   the name of an S3 bucket but not a folder in the bucket. A folder to contain the pipeline
     *   artifacts is created for you based on the name of the pipeline. You can use any S3 bucket
