 ...
 *
 * This construct can be used to simplify logic in other constructs which need to create a role but
 * only if certain configurations occur (such as when AutoScaling is configured). The role can be
 * configured in one place, but if it never gets used it doesn't get instantiated and will not be
...
public class LazyRoleDsl(
     ...
     *
     * These policies will be created with the role, whereas those added by `addToPolicy` are added
     * using a separate CloudFormation resource (allowing a way around circular dependencies that
     ...
     *
     * Default: - AWS CloudFormation generates a unique physical ID and uses that ID for the role
     * name.
     *
