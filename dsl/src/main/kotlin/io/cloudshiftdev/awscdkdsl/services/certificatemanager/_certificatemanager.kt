...
public object certificatemanager {
     ...
     * ```
     */
    public inline fun certificateProps(
        block: CertificatePropsDsl.() -> Unit = {}
    ): CertificateProps {
     ...
     * certificate domain is hosted in Amazon Route 53, the domain resides in your AWS account , and
     * you are using DNS validation.
     *
     * However, if the certificate uses email validation, or if the domain is not hosted in Route
     * 53, then the stack will remain in the `CREATE_IN_PROGRESS` state. Further stack operations
     * are delayed until you validate the certificate request, either by acting upon the
     ...
     * Depending on the chosen validation method, ACM checks the domain's DNS record for a
     * validation CNAME, or it attempts to send a validation email message to the domain owner.
     *
     * Example:
     * ```
     * // The code below shows an example of how to instantiate this type.
     ...
     *
     * Will be automatically validated using DNS validation against the specified Route 53 hosted
     * zone.
     *
     * Example:
     * ```
     ...
     * ```
     */
    public inline fun privateCertificateProps(
        block: PrivateCertificatePropsDsl.() -> Unit = {}
