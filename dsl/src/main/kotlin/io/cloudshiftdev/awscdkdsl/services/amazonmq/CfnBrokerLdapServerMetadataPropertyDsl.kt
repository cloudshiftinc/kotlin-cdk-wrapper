...
public class CfnBrokerLdapServerMetadataPropertyDsl {
     ...
     *   role. For example, you can specify `cn` for a group entry's common name. If authentication
     ...
     *   distinguished name of the user matched by userSearchMatching is substituted into the `{0}`
     *   placeholder in the search filter. The client's username is substituted into the `{1}`
     *   placeholder. For example, if you set this option to `(member=uid={1})` for the user
    ...
    /**
     * @param roleSearchSubtree The directory search scope for the role. If set to true, scope is to
    ...
    /**
     * @param serviceAccountPassword Service account password. A service account is an account in
     *   your LDAP server that has access to initiate a connection. For example, `cn=admin` ,
    ...
    /**
     * @param serviceAccountUsername Service account username. A service account is an account in
     *   your LDAP server that has access to initiate a connection. For example, `cn=admin` ,
     ...
     *   for user entries. The subtree is specified by a DN, which specifies the base node of the
     *   subtree. For example, by setting this option to `ou=Users` , `ou=corp` , `dc=corp` ,
     *   `dc=example` , `dc=com` , the search for user entries is restricted to the subtree beneath
     ...
     *   user's group membership. In some cases, user roles may be identified by the value of an
     *   attribute in the user's directory entry. The `UserRoleName` option allows you to provide
     ...
     *   search filter becomes `(uid=janedoe)` after string substitution. It will result in matching
     *   an entry like `uid=janedoe` , `ou=Users` , `ou=corp` , `dc=corp` , `dc=example` , `dc=com`
     *   .
     */
    ...
    /**
     * @param userSearchSubtree The directory search scope for the user. If set to true, scope is to
     *   search the entire subtree.
     */
