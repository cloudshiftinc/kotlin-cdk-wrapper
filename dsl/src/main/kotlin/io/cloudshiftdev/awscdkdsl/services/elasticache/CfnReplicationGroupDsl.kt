 ...
 * ranges between 83 shards (one primary and 5 replicas per shard) and 500 shards (single primary
 * and no replicas). Make sure there are enough available IP addresses to accommodate the increase.
 * Common pitfalls include the subnets in the subnet group have too small a CIDR range or the
 * subnets are shared and heavily used by other clusters. For more information, see
...
public class CfnReplicationGroupDsl(
    ...
    private val cdkBuilder: CfnReplicationGroup.Builder =
        CfnReplicationGroup.Builder.create(scope, id)
     ...
     * enabled and cluster mode disabled. After you migrate all Redis clients to use cluster mode
     * enabled, you can then complete cluster mode configuration and set the cluster mode to
     ...
     * groups (console: shards) specified by the parameter *NumNodeGroups* or the number of node
     * groups configured by *NodeGroupConfiguration* regardless of the number of ARNs specified
     * here.
     *
     ...
     * include multiple tags as shown following: Key= `myKey` , Value= `myKeyValue` Key=
     * `mySecondKey` , Value= `mySecondKeyValue` . Tags on replication groups will be replicated to
     * all nodes.
     *
     ...
     * to `preferred` in the same request, to allow both encrypted and unencrypted connections at
     * the same time. Once you migrate all your Redis clients to use encrypted connections you can
