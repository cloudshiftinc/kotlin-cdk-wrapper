...
public class QueueDsl(
     ...
     * identical content (excluding attributes) as duplicates and delivers only one copy of the
     * message.
     *
     ...
     *
     * Does not wait if set to 0, otherwise waits this amount of seconds by default for messages to
     * arrive.
     *
     ...
     *
     * Even though queues are technically stateful, their contents are transient and it is common to
     * add and remove Queues while rearchitecting your application. The default is therefore
     * `DESTROY`. Change it to `RETAIN` if the messages are so valuable that accidentally losing
     ...
     * the queue before it becomes visible again for dequeueing by another processor.
     *
