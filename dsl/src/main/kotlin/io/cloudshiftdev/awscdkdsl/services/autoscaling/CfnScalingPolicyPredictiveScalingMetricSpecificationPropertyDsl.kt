 ...
 * metrics for a scaling policy. You choose the metric pair, and the policy automatically knows the
 * correct sum and average statistics to use for the load metric and the scaling metric.
 *
 * Example
 ...
 *   metric dimension for the corresponding target group, so you also provide a resource label for
 *   the Application Load Balancer target group that is attached to your Auto Scaling group.
 ...
 *   CloudWatch, this refers to the `RequestCount` and `RequestCountPerTarget` metrics,
 *   respectively.
 ...
 *   number of requests received by each instance is as close to 1000 requests per minute as
