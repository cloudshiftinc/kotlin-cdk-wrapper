...
public class CfnCampaignMessagePropertyDsl {
    ...
    /**
     * @param silentPush Specifies whether the notification is a silent push notification, which is
     *   a push notification that doesn't display on a recipient's device. Silent push notifications
     *   can be used for cases such as updating an app's configuration, displaying messages in an
     *   in-app message center, or supporting phone home functionality.
     */
     ...
     *   message, if the service is unable to deliver the notification the first time. This value is
     *   converted to an expiration value when it's sent to a push notification service. If this
     *   value is `0` , the service treats the notification as if it expires immediately and the
