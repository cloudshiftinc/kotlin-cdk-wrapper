...
public class CfnServerProtocolDetailsPropertyDsl {
    ...
    /**
     * @param passiveIp Indicates passive mode, for FTP and FTPS protocols. Enter a single IPv4
     *   address, such as the public IP address of a firewall, router, or load balancer. For
     *   example:
     *
     ...
     * . This tells the client to use the same IP address as the Control connection and utilize all
     * AZs for their connections. Note, however, that not all FTP clients support the
     * `PassiveIp=0.0.0.0` response. FileZilla and WinSCP do support it. If you are using other
     ...
     *   client attempts to use `SETSTAT` on a file you are uploading to an S3 bucket. Some SFTP
     *   file transfer clients can attempt to change the attributes of remote files, including
     *   timestamp and permissions, using commands, such as `SETSTAT` when uploading the file.
     *   However, these commands are not compatible with object storage systems, such as Amazon S3.
     *   Due to this incompatibility, file uploads from these clients can result in errors even when
     ...
     *   FTPS protocol. TLS Session Resumption provides a mechanism to resume or share a negotiated
     *   secret key between the control and data connection for an FTPS session.
     *   `TlsSessionResumptionMode` determines whether or not the server resumes recent, negotiated
     *   sessions through a unique session ID. This property is available during `CreateServer` and
     ...
     * negotiation. To determine whether or not you can use the `ENFORCED` value, you need to test
     * your clients.
     */
