...
public class CfnLoadBalancerLoadBalancerAttributePropertyDsl {
     ...
     *   balancer and have a bucket policy that grants Elastic Load Balancing permissions to write
     *   to the bucket.
     * * `access_logs.s3.prefix` - The prefix for the location in the S3 bucket for the access logs.
     * * `ipv6.deny_all_igw_traffic` - Blocks internet gateway (IGW) access to the load balancer. It
     *   is set to `false` for internet-facing load balancers and `true` for internal load
     ...
     *   format. The possible values for the attribute are `true` and `false` . The default is
     *   `false` .
     ...
     *   last hop) to the `X-Forwarded-For` header in the HTTP request before it sends it to
     *   targets.
