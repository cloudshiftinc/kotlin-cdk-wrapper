 ...
 * challenge token, AWS WAF applies any custom request handling and labels that you've configured
 * and then allows the web request inspection to proceed to the next rule, similar to a
 * `CountAction` .
 ...
 * the client session for legitimate behavior. The challenge blocks bots or at least increases the
 * cost of operating sophisticated bots.
 *
...
public class CfnWebACLChallengeActionPropertyDsl {
     ...
     *   challenge inspection determines that the request's token is valid and unexpired. For
