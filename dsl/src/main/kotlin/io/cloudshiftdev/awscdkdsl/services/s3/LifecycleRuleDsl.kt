...
public class LifecycleRuleDsl {
     ...
     *   multipart uploads to an Amazon S3 bucket. The AbortIncompleteMultipartUpload property type
     *   creates a lifecycle rule that aborts incomplete multipart uploads to an Amazon S3 bucket.
     *   When Amazon S3 aborts a multipart upload, it deletes all parts associated with the
     *   multipart upload.
     *
     ...
     *   Amazon S3 and Amazon Glacier. If you specify an expiration and transition time, you must
     *   use the same time unit for both properties (either in days or by date). The expiration time
     ...
     *   to the bucket and when old versions of the object expire. For buckets with versioning
     *   enabled (or suspended), specifies the time, in days, between when a new version of the
     *   object is uploaded to the bucket and when old versions of the object expire. When object
     *   versions expire, Amazon S3 permanently deletes them. If you specify a transition and
    ...
    public fun noncurrentVersionTransitions(
        noncurrentVersionTransitions: NoncurrentVersionTransitionDsl.() -> Unit
    ) {
        _noncurrentVersionTransitions.add(
            NoncurrentVersionTransitionDsl().apply(noncurrentVersionTransitions).build()
        )
    }
     ...
     */
    public fun noncurrentVersionTransitions(
        noncurrentVersionTransitions: Collection<NoncurrentVersionTransition>
    ) {
