 ...
 * into the categories. For more information, see
...
public class CfnDocumentClassifierDsl(
     ...
     *
     * The classifier can be trained in multi-class mode, which identifies one and only one class
     * for each document, or multi-label mode, which identifies one or more labels for each
     * document. In multi-label mode, multiple labels for an individual document are separated by a
     ...
     * For example, a tag with "Sales" as the key might be added to a resource to indicate its use
