 ...
 *
 * Security Groups act like a firewall with a set of rules, and are associated with any AWS resource
 * that has or creates Elastic Network Interfaces (ENIs). A typical example of a resource that has a
 * security group is an Instance (or Auto Scaling Group of instances)
 *
 * If you are defining new infrastructure in CDK, there is a good chance you won't have to interact
 * with this class at all. Like IAM Roles, Security Groups need to exist to control access between
 * AWS resources, but CDK will automatically generate and populate them with least-privilege
 * permissions for you so you can concentrate on your business logic.
 *
 * All Constructs that require Security Groups will create one for you if you don't specify one at
 * construction. After construction, you can selectively allow connections to and between constructs
 * via--for example-- the `instance.connections` object. Think of it as "allowing connections to
 * your instance", rather than "adding ingress rules a security group". See the <a
 ...
 *
 * If you have an existing security group you want to use in your CDK application, you would import
 * it like this:
 * ```
...
public class SecurityGroupDsl(
     ...
     * ipv6 traffic. If this is set to false, no outbound traffic will be allowed by default and all
     ...
     *
     * If this is set to true, ingress and egress rules will not be declared under the SecurityGroup
     ...
     *
     * For valid values, see the GroupName parameter of the CreateSecurityGroup action in the Amazon
     * EC2 API Reference.
     *
     * It is not recommended to use an explicit group name.
     *
     * Default: If you don't specify a GroupName, AWS CloudFormation generates a unique physical ID
     * and uses that ID for the group name.
     *
