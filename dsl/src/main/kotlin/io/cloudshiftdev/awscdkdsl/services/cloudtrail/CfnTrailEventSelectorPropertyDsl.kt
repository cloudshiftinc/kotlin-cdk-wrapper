...
public class CfnTrailEventSelectorPropertyDsl {
    ...
    /**
     * @param dataResources CloudTrail supports data event logging for Amazon S3 objects, AWS Lambda
     *   functions, and Amazon DynamoDB tables with basic event selectors. You can specify up to 250
     *   resources for an individual event selector, but the total number of data resources cannot
     *   exceed 250 across all event selectors in a trail. This limit does not apply if you
     ...
     *   do not want management events to be logged on your trail. In this release, the list can be
     *   empty (disables the filter), or it can filter out AWS Key Management Service or Amazon RDS
     *   Data API events by containing `kms.amazonaws.com` or `rdsdata.amazonaws.com` . By default,
     *   `ExcludeManagementEventSources` is empty, and AWS KMS and Amazon RDS Data API events are
     *   logged to your trail. You can exclude management event sources only in Regions that support
     *   the event source.
     */
     ...
     *   events for your trail. For more information, see
     ...
     * information about CloudTrail pricing, see
     ...
     *   events, or all. For example, the EC2 `GetConsoleOutput` is a read-only API operation and
