...
public class CfnAutomationRuleAutomationRulesFindingFiltersPropertyDsl {
     ...
     *   is enabled. This field consists of the resource portion of the Amazon Resource Name (ARN)
     ...
     *   that it was intended to identify. `Confidence` is scored on a 0–100 basis using a ratio
     *   scale. A value of `0` means 0 percent confidence, and a value of `100` means 100 percent
     *   confidence. For example, a data exfiltration detection based on a statistical deviation of
     *   network traffic has low confidence because an actual exfiltration hasn't been verified. For
    ...
    /**
     * @param createdAt A timestamp that indicates when this finding record was created. Uses the
     ...
     *   associated with a finding. `Criticality` is scored on a 0–100 basis, using a ratio scale
     *   that supports only full integers. A score of `0` means that the underlying resources have
     *   no criticality, and a score of `100` is reserved for the most critical resources. For more
     *   information, see
    ...
    /**
     * @param firstObservedAt A timestamp that indicates when the potential security issue captured
     *   by a finding was first observed by the security findings product. Uses the `date-time`
     *   format specified in
     *   [RFC 3339 section 5.6, Internet Date/Time Format](https://docs.aws.amazon.com/https://tools.ietf.org/html/rfc3339#section-5.6)
     *   . The value cannot contain spaces. For example, `2020-03-22T13:22:13.933Z` .
     */
     ...
     *   by a finding was most recently observed by the security findings product. Uses the
     *   `date-time` format specified in
     *   [RFC 3339 section 5.6, Internet Date/Time Format](https://docs.aws.amazon.com/https://tools.ietf.org/html/rfc3339#section-5.6)
     *   . The value cannot contain spaces. For example, `2020-03-22T13:22:13.933Z` .
     */
    ...
    /**
     * @param noteUpdatedAt The timestamp of when the note was updated. Uses the date-time format
     *   specified in
    ...
    /**
     * @param resourceId The identifier for the given resource type. For AWS resources that are
    ...
    public fun build(): CfnAutomationRule.AutomationRulesFindingFiltersProperty {
        ...
        if (_resourceDetailsOther.isNotEmpty())
            cdkBuilder.resourceDetailsOther(_resourceDetailsOther)
