...
public class CfnAutoScalingGroupInstancesDistributionPropertyDsl {
     ...
     *   launches your highest priority instance types first. If all your On-Demand capacity cannot
     *   be fulfilled using your highest priority instance type, then Amazon EC2 Auto Scaling
     *   launches the remaining capacity using the second priority instance type, and so on. This is
     *   the default value for Auto Scaling groups that don't specify `InstanceRequirements` and
     ...
     *   be fulfilled by On-Demand Instances. This base portion is launched first as your group
     *   scales.
     *
     ...
     *   and Spot Instances for your additional capacity beyond `OnDemandBaseCapacity` . Expressed
     *   as a number (for example, 20 specifies 20% On-Demand Instances, 80% Spot Instances). If set
     ...
     *   they are launched. Possible instance types are determined by the launch template overrides
     *   that you specify.
     *
     * The following lists the valid values:
     ...
     *   EC2 Auto Scaling honors the instance type priorities on a best effort basis but optimizes
     *   for capacity first. Note that if the On-Demand allocation strategy is set to `prioritized`
     ...
     *   receive Spot Instances from several pools. This is the default value, but it might lead to
     *   high interruption rates because this strategy only considers instance price and not
     *   available capacity.
     * * **price-capacity-optimized (recommended)** - The price and capacity optimized allocation
     *   strategy looks at both price and capacity to select the Spot Instance pools that are the
     ...
     *   Instances. The Spot pools are determined from the different instance types in the
     *   overrides. Valid only when the `SpotAllocationStrategy` is `lowest-price` . Value must be
     ...
     *   Instance. If your maximum price is lower than the Spot price for the instance types that
     *   you selected, your Spot Instances are not launched. We do not recommend specifying a
     *   maximum price because it can lead to increased interruptions. When Spot Instances launch,
     *   you pay the current Spot price. To remove a maximum price that you previously set, include
