...
public object acmpca {
     ...
     *
     * If conflicting or duplicate certificate information is supplied from other sources, AWS
     * Private CA applies
     ...
     * certificate. Alternatively, you can issue a CA certificate using an on-premises CA, and then
     * use the `AWS::ACMPCA::CertificateAuthorityActivation` resource to import the new CA
     * certificate and activate the CA.
     *
     * Before removing a `AWS::ACMPCA::CertificateAuthority` resource from the CloudFormation stack,
     * disable the affected CA. Otherwise, the action will fail. You can disable the CA by removing
     * its associated `AWS::ACMPCA::CertificateAuthorityActivation` resource from CloudFormation.
     *
     * Example:
     * ```
     ...
     * *nextUpdate* field in the CRL. The CRL is refreshed prior to a certificate's expiration date
     * or when a certificate is revoked. When a certificate is revoked, it appears in the CRL until
     * the certificate expires, and then in one additional CRL after expiration, and it always
     * appears in the audit report.
     *
     * A CRL is typically updated approximately 30 minutes after a certificate is revoked. If for
     * any reason a CRL update fails, AWS Private CA makes further attempts every 15 minutes.
     *
     ...
     * * *Revoked Certificates* : List of revoked certificates. Each list item contains the
     ...
     * * *X509v3 Authority Key Identifier* : Identifies the public key associated with the private
     ...
     * [RFC 5280](https://docs.aws.amazon.com/https://datatracker.ietf.org/doc/html/rfc5280) . Only
     * one of the following naming options should be provided. Providing more than one option
    ...
    /**
     * Defines a custom ASN.1 X.400 `GeneralName` using an object identifier (OID) and value. The
     ...
     * or by the private CA itself, is valid in days, months, or years.
     *
     ...
     * certificates requested through ACM and residing in the same AWS account as the CA.
     *
     ...
     *   to create, retrieve, and list permissions.
     * * If the private CA and the ACM certificates reside in different accounts, then permissions
     *   cannot be used to enable automatic renewals. Instead, the ACM certificate owner must set up
     *   a resource-based policy to enable cross-account issuance and renewals. For more
     *   information, see
     *   [Using a Resource Based Policy with AWS Private CA](https://docs.aws.amazon.com/privateca/latest/userguide/pca-rbp.html)
     *   .
     *
     * To update an `AWS::ACMPCA::Permission` resource, you must first delete the existing
     * permission resource from the CloudFormation stack and then create a new permission resource
