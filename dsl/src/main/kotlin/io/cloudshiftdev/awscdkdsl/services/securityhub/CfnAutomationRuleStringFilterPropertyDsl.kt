...
public class CfnAutomationRuleStringFilterPropertyDsl {
     ...
     *   findings. To search for values that have the filter value, use one of the following
     ...
     *   filter `Title NOT_CONTAINS CloudFront` matches findings that have a `Title` that excludes
     ...
     * by `AND` . A finding matches only if it matches all of those filters. For example, the
     * filters `Title NOT_CONTAINS CloudFront AND Title NOT_CONTAINS CloudWatch` match a finding
     * that excludes both `CloudFront` and `CloudWatch` in the title.
     *
