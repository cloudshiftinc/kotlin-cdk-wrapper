...
public class CodePipelinePropsDsl {
    ...
    /**
     * @param cliVersion CDK CLI version to use in self-mutation and asset publishing steps. If you
     *   want to lock the CDK CLI version used in the pipeline, by steps that are automatically
     ...
     *   deployments. The artifact buckets have to be encrypted to support deploying CDK apps to
     *   another account, so if you want to do that or want to have your artifact buckets encrypted,
     *   be sure to set this value to `true`.
     *
    ...
    /**
     * @param dockerEnabledForSelfMutation Enable Docker for the self-mutate step. Set this to true
     *   if the pipeline itself uses Docker container assets (for example, if you use
    ...
    /**
     * @param dockerEnabledForSynth Enable Docker for the 'synth' step. Set this to true if you are
     *   using file assets that require "bundling" anywhere in your application (meaning an asset
     *   compilation step will be run with the tools provided by a Docker image), both for the
     *   Pipeline stack as well as the application stacks.
     *
     ...
     *   key rotation is disabled, but will add additional costs when enabled.
     */
    ...
    /**
     * @param publishAssetsInParallel Publish assets in multiple CodeBuild projects. If set to
     *   false, use one Project per type to publish all assets.
     *
    ...
    /**
     * @param selfMutation Whether the pipeline will update itself. This needs to be set to `true`
     *   to allow the pipeline to reconfigure itself when assets or stages are being added to it,
     *   and `true` is the recommended setting.
     *
    ...
    /**
     * @param useChangeSets Deploy every stack by creating a change set and executing it. When
     *   enabled, creates a "Prepare" and "Execute" action for each stack. Disable to deploy the
