...
public object awscdk {
     ...
     * construct tree.
     *
     * You would normally define an `App` instance in your program's entrypoint, then define
     * constructs where the app is used as the parent scope.
     *
     ...
     * Cloud assemblies includes artifacts such as CloudFormation templates and assets that are
     * needed to deploy this app into the AWS cloud.
     *
     * Example:
     * ```
     ...
     * ```
     */
    public inline fun assetManifestDockerImageDestination(
     ...
     *
     * This is controlled by the context key 'aws:cdk:asset-staging' and enabled by the CLI by
     * default in order to ensure that when the CDK app exists, all assets are available for
     * deployment. Otherwise, if an app references assets in temporary locations, those will not be
     * available when it exists (see https://github.com/aws/aws-cdk/issues/1716).
     *
     * The `stagedPath` property is a stringified token that represents the location of the file or
     * directory after staging. It will be resolved only during the "prepare" stage and may be
     * either the original path or the staged path depending on the context setting.
     *
     * The file/directory are staged based on their content hash (fingerprint). This means that only
     * if content was changed, copy will happen.
     *
     * Example:
     * ```
     * // The code below shows an example of how to instantiate this type.
     ...
     * ```
     */
    public inline fun assetStagingProps(
        block: AssetStagingPropsDsl.() -> Unit = {}
    ): AssetStagingProps {
     ...
     *
     * A special synthesizer that behaves similarly to `DefaultStackSynthesizer`, but doesn't
     * require bootstrapping the environment it operates in. Instead, it will re-use the Roles that
     * were created for a different region (which is possible because IAM is a global service).
     *
     * However, it will not assume asset buckets or repositories have been created, and therefore
     * does not support assets.
     *
     * The name is poorly chosen -- it does still require bootstrapping, it just does not support
     * assets.
     *
     * Used by the CodePipeline construct for the support stacks needed for cross-region replication
     * S3 buckets. App builders do not need to use this synthesizer directly.
     *
     * Example:
     * ```
     * // The code below shows an example of how to instantiate this type.
     ...
     * ```
     */
    public inline fun bootstraplessSynthesizer(
        block: BootstraplessSynthesizerDsl.() -> Unit = {}
     ...
     * update.
     *
     * During replacement, AWS CloudFormation retains the old group until it finishes creating the
     * new one. If the update fails, AWS CloudFormation can roll back to the old Auto Scaling group
     * and delete the new Auto Scaling group.
     *
     ...
     * defined in the AWS::AutoScaling::AutoScalingGroup resource of your template, even if a
     * scheduled action is in effect.
     *
     * If you do not want AWS CloudFormation to change any of the group size property values when
     * you have a scheduled action in effect, use the AutoScalingScheduledAction update policy to
     * prevent AWS CloudFormation from changing the MinSize, MaxSize, or DesiredCapacity properties
     ...
     * ```
     */
    public inline fun cfnCodeDeployBlueGreenAdditionalOptions(
     ...
     * ```
     */
    public inline fun cfnCodeDeployBlueGreenApplication(
     ...
     * ```
     */
    public inline fun cfnCodeDeployBlueGreenApplicationTarget(
     ...
     * ```
     */
    public inline fun cfnCodeDeployBlueGreenEcsAttributes(
     ...
     * ```
     */
    public inline fun cfnCodeDeployBlueGreenHookProps(
     ...
     * ```
     */
    public inline fun cfnCodeDeployBlueGreenLifecycleEventHooks(
     ...
     * ```
     */
    public inline fun cfnCodeDeployLambdaAliasUpdate(
        block: CfnCodeDeployLambdaAliasUpdateDsl.() -> Unit = {}
     ...
     * ```
     */
    public inline fun cfnConditionProps(
        block: CfnConditionPropsDsl.() -> Unit = {}
    ): CfnConditionProps {
     ...
     * create complete until AWS CloudFormation receives a specified number of success signals or
     ...
     * For a detailed example, see Deploying Applications on Amazon EC2 with AWS CloudFormation.
     *
     * Example:
     * ```
     * // The code below shows an example of how to instantiate this type.
     ...
     * ```
     */
    public inline fun cfnCreationPolicy(
     ...
     * ```
     */
    public inline fun cfnDynamicReferenceProps(
        block: CfnDynamicReferencePropsDsl.() -> Unit = {}
     ...
     *
     * The main use case for this is to overcome a limitation in CloudFormation that does not allow
     * using intrinsic functions as dictionary keys (because dictionary keys in JSON must be
     * strings). Specifically this is common in IAM conditions such as `StringEquals: { lhs: "rhs"
     * }` where you want "lhs" to be a reference.
     *
     ...
     * ```
     */
    public inline fun cfnResourceAutoScalingCreationPolicy(
     ...
     * ```
     */
    public inline fun cfnResourceSignal(
        block: CfnResourceSignalDsl.() -> Unit = {}
    ): CfnResourceSignal {
     ...
     * declared in the AWS CloudFormation template used to create the product they are attempting to
     * use.
     *
     ...
     * rule, you can define only one rule condition; you can define one or more asserts within the
     * Assertions property. You define a rule condition and assertions by using rule-specific
     * intrinsic functions.
     *
     * Example:
     * ```
     ...
     * ```
     */
    public inline fun cfnRuleAssertion(
        block: CfnRuleAssertionDsl.() -> Unit = {}
    ): CfnRuleAssertion {
     ...
     * within the Assertions property. You define a rule condition and assertions by using
     ...
     *
     * Run deployments to nested StackSets from the parent stack, not directly through the StackSet
     * API.
     *
     * Example:
     * ```
     ...
     * ```
     */
    public inline fun cfnTrafficRouting(
        block: CfnTrafficRoutingDsl.() -> Unit = {}
    ): CfnTrafficRouting {
     ...
     * ```
     */
    public inline fun cfnTrafficRoutingConfig(
     ...
     * ```
     */
    public inline fun cfnTrafficRoutingTimeBasedCanary(
     ...
     * ```
     */
    public inline fun cfnTrafficRoutingTimeBasedLinear(
     ...
     * make or whether a scheduled action is associated with the Auto Scaling group.
     *
     * Example:
     * ```
     * // The code below shows an example of how to instantiate this type.
     ...
     *   credentials. It also does not support deploying using **CDK Pipelines**. For either of
     *   those features, use `DefaultStackSynthesizer`.
     * * This synthesizer requires an S3 bucket and ECR repository with well-known names. To not
     *   depend on those, use `LegacyStackSynthesizer`.
     *
     * Be aware that your CLI credentials must be valid for the duration of the entire deployment.
     * If you are using session credentials, make sure the session lifetime is long enough.
     *
     * By default, expects the environment to have been bootstrapped with just the staging resources
     * of the Bootstrap Stack V2 (also known as "modern bootstrap stack"). You can override the
     * default names using the synthesizer's construction properties.
     *
     * Example:
     * ```
     * // The code below shows an example of how to instantiate this type.
     ...
     * ```
     */
    public inline fun cliCredentialsStackSynthesizer(
        block: CliCredentialsStackSynthesizerDsl.() -> Unit = {}
     ...
     * ```
     */
    public inline fun cliCredentialsStackSynthesizerProps(
     ...
     *
     * This class is intended to be used by construct library authors. Application builder should
     * not be able to tell whether or not a construct is backed by a custom resource, and so the use
     * of this class should be invisible.
     *
     * Instead, construct library authors declare a custom construct that hides the choice of
     * provider, and accepts a strongly-typed properties object with the properties your provider
     * accepts.
     *
     * Your custom resource provider (identified by the `serviceToken` property) can be one of 4
     * constructs:
     * * If you are authoring a construct library or application, we recommend you use the
     *   `Provider` class in the `custom-resources` module.
     ...
     * * If you want full control over the provider, you can always directly use a Lambda Function
     *   or SNS Topic by passing the ARN into `serviceToken`.
     *
     * Example:
     * ```
     ...
     * ```
     */
    public inline fun customResourceProps(
        block: CustomResourcePropsDsl.() -> Unit = {}
    ): CustomResourceProps {
     ...
     *
     * This synthesizer:
     * * Supports cross-account deployments (the CLI can have credentials to one account, and you
     *   can still deploy to another account by assuming roles with well-known names in the other
     *   account).
     * * Supports the **CDK Pipelines** library.
     *
     * Requires the environment to have been bootstrapped with Bootstrap Stack V2 (also known as
     * "modern bootstrap stack"). The synthesizer adds a version check to the template, to make sure
     * the bootstrap stack is recent enough to support all features expected by this synthesizer.
     *
     * Example:
     * ```
     * App app;
     ...
     * ```
     */
    public inline fun defaultStackSynthesizer(
        block: DefaultStackSynthesizerDsl.() -> Unit = {}
     ...
     * ```
     */
    public inline fun dockerBuildOptions(
     ...
     * ```
     */
    public inline fun dockerImageAssetLocation(
        block: DockerImageAssetLocationDsl.() -> Unit = {}
     ...
     * ```
     */
    public inline fun dockerImageAssetSource(
     ...
     * ```
     */
    public inline fun dockerRunOptions(
        block: DockerRunOptionsDsl.() -> Unit = {}
    ): DockerRunOptions {
     ...
     * ```
     */
    public inline fun exportValueOptions(
     ...
     * ```
     */
    public inline fun fileAssetLocation(
        block: FileAssetLocationDsl.() -> Unit = {}
    ): FileAssetLocation {
     ...
     * ```
     */
    public inline fun fileFingerprintOptions(
        block: FileFingerprintOptionsDsl.() -> Unit = {}
     ...
     * ```
     */
    public inline fun fingerprintOptions(
     ...
     * ```
     */
    public inline fun getContextKeyOptions(
     ...
     * ```
     */
    public inline fun getContextKeyResult(
        block: GetContextKeyResultDsl.() -> Unit = {}
    ): GetContextKeyResult {
     ...
     * ```
     */
    public inline fun getContextValueOptions(
     ...
     * ```
     */
    public inline fun getContextValueResult(
        block: GetContextValueResultDsl.() -> Unit = {}
     ...
     * ```
     */
    public inline fun lazyAnyValueOptions(
     ...
     * ```
     */
    public inline fun lazyListValueOptions(
     ...
     * ```
     */
    public inline fun lazyStringValueOptions(
     ...
     * When you apply template changes to update a top-level stack, CloudFormation updates the
     * top-level stack and initiates an update to its nested stacks. CloudFormation updates the
     * resources of modified nested stacks, but does not update the resources of unmodified nested
     * stacks.
     *
     * Furthermore, this stack will not be treated as an independent deployment artifact (won't be
     * listed in "cdk list" or deployable through "cdk deploy"), but rather only synthesized as a
     * template and uploaded as an asset to S3.
     *
     * Cross references of resource attributes between the parent stack and the nested stack will
     * automatically be translated to stack parameters and outputs.
     *
     * Example:
     * ```
     * import software.constructs.Construct;
     ...
     * ```
     */
    public inline fun nestedStackProps(
        block: NestedStackPropsDsl.() -> Unit = {}
    ): NestedStackProps {
     ...
     * ```
     */
    public inline fun permissionsBoundaryBindOptions(
        block: PermissionsBoundaryBindOptionsDsl.() -> Unit = {}
     ...
     * ```
     */
    public inline fun policyValidationPluginReportBeta1(
     ...
     * ```
     */
    public inline fun policyViolationBeta1(
        block: PolicyViolationBeta1Dsl.() -> Unit = {}
     ...
     * ```
     */
    public inline fun removalPolicyOptions(
     ...
     *
     * NOT the same as the ResolveContext; ResolveContext is exposed to Token implementors and
     * resolution hooks, whereas this struct is just to bundle a number of things that would
     * otherwise be arguments to resolve() in a readable way.
     *
     * Example:
     * ```
     * // The code below shows an example of how to instantiate this type.
     ...
     * ```
     */
    public inline fun resourceEnvironment(
        block: ResourceEnvironmentDsl.() -> Unit = {}
    ): ResourceEnvironment {
     ...
     *
     * The actual values of these secrets should not be committed to your repository, or even end up
     * in the synthesized CloudFormation template. Instead, you should store them in an external
     * system like AWS Secrets Manager or SSM Parameter Store, and you can reference them by calling
     * `SecretValue.secretsManager()` or `SecretValue.ssmSecure()`.
     *
     * You can use `SecretValue.unsafePlainText()` to construct a `SecretValue` from a literal
     * string, but doing so is highly discouraged.
     *
     * To make sure secret values don't accidentally end up in readable parts of your infrastructure
     * definition (such as the environment variables of an AWS Lambda Function, where everyone who
     * can read the function definition has access to the secret), using secret values directly is
     * not allowed. You must pass them to constructs that accept `SecretValue` properties, which are
     ...
     *
     * (If you are writing something like an AWS Lambda Function and need to access a secret inside
     * it, make the API call to `GetSecretValue` directly inside your Lamba's code, instead of using
     ...
     * ```
     */
    public inline fun sizeConversionOptions(
        block: SizeConversionOptionsDsl.() -> Unit = {}
     ...
     *
     * You can then instantiate your subclass multiple times to model multiple copies of your
     * application which should be be deployed to different environments.
     *
     * Example:
     * ```
     ...
     * ```
     */
    public inline fun stageSynthesisOptions(
        block: StageSynthesisOptionsDsl.() -> Unit = {}
     ...
     * ```
     */
    public inline fun synthesizeStackArtifactOptions(
        block: SynthesizeStackArtifactOptionsDsl.() -> Unit = {}
     ...
     *
     * Normally, you do not need to use this class, as the CloudFormation specification will
     * indicate which resources are taggable. However, sometimes you will need this to make custom
     * resources taggable. Used `tagManager.renderedTags` to obtain a value that will resolve to the
     ...
     * ```
     */
    public inline fun tagManagerOptions(
        block: TagManagerOptionsDsl.() -> Unit = {}
    ): TagManagerOptions {
     ...
     * ```
     */
    public inline fun timeConversionOptions(
        block: TimeConversionOptionsDsl.() -> Unit = {}
     ...
     * ```
     */
    public inline fun uniqueResourceNameOptions(
