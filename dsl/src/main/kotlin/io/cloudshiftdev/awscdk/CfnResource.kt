...
internal constructor(
     ...
     *
     * This can be used for resources across stacks (or nested stack) boundaries and the dependency
     * will automatically be transferred to the relevant scope.
     *
     * @param target
     */
    ...
    /**
     * (deprecated) Indicates that this resource depends on another resource and cannot be
     ...
     *   Note that this is a different set of metadata from CDK node metadata; this metadata ends up
     *   in the stack template under the resource, whereas CDK node metadata ends up in the Cloud
     ...
     *
     * To add a property override, either use `addPropertyOverride` or prefix `path` with
     * "Properties." (i.e. `Properties.TopicName`).
     *
     * If the override is nested, separate each nested level using a dot (.) in the path parameter.
     * If there is an array as part of the nesting, specify the index in the path.
     *
     * To include a literal `.` in the property name, prefix with a `\`. In most programming
     * languages you will need to write this as `"\\."` because the `\` itself will need to be
     * escaped.
     *
     * For example,
     * ```
     ...
     * ```
     *
     * The `value` argument to `addOverride` will not be processed or translated in any way. Pass
     * raw JSON values in here with the correct capitalization for CloudFormation. If you pass CDK
     * classes or structs, they will be rendered with lowercased key names, and CloudFormation will
     * reject the template.
     *
     * @param path * The path of the property, you can use dot notation to override values in
     *   complex types.
     ...
     * CloudFormation, either because you've removed it from the CDK application or because you've
     * made a change that requires the resource to be replaced.
     *
     * The resource can be deleted (`RemovalPolicy.DESTROY`), or left in your AWS account for data
     * recovery and cleanup later (`RemovalPolicy.RETAIN`). In some cases, a snapshot can be taken
     * of the resource prior to deletion (`RemovalPolicy.SNAPSHOT`). A list of resources that
     * support this policy can be found in the following link:
     *
    ...
    public open fun applyRemovalPolicy(policy: RemovalPolicy, options: RemovalPolicyOptions) {
        unwrap(this)
            .applyRemovalPolicy(
                policy.let(RemovalPolicy::unwrap),
    ...
    public open fun applyRemovalPolicy(
        ...
        options: RemovalPolicyOptions.Builder.() -> Unit
    ): Unit = applyRemovalPolicy(policy, RemovalPolicyOptions(options))

    /**
     * Returns a token for an runtime attribute of this resource.
     *
     * Ideally, use generated attribute accessors (e.g. `resource.arn`), but this can be used for
     * future compatibility in case there is no generated attribute.
     *
    ...
    public open fun att(attributeName: String, typeHint: ResolutionTypeHint): Reference =
        unwrap(this)
            .getAtt(attributeName, typeHint.let(ResolutionTypeHint::unwrap))
            .let(Reference::wrap)
    ...
    /**
     * Get a shallow copy of dependencies between this resource and other resources in the same
     * stack.
     */
     ...
     *
     * This can be used for resources across stacks (including nested stacks) and the dependency
     * will automatically be removed from the relevant scope.
     *
     * @param target
     */
    ...
    public open fun replaceDependency(target: CfnResource, newTarget: CfnResource) {
        unwrap(this)
            .replaceDependency(target.let(CfnResource::unwrap), newTarget.let(CfnResource::unwrap))
    }
    ...
    public companion object {
        public fun isCfnResource(x: Any): Boolean =
