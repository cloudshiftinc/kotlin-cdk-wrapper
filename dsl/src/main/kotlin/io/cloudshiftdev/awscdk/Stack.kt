...
internal constructor(
     ...
     * * The value provided to `env.account` when the stack is defined. This can either be a
     *   concrete account (e.g. `585695031111`) or the `Aws.ACCOUNT_ID` token.
     * * `Aws.ACCOUNT_ID`, which represents the CloudFormation intrinsic reference `{ "Ref":
     *   "AWS::AccountId" }` encoded as a string token.
     *
     * Preferably, you should use the return value as an opaque string and not attempt to parse it
     * to implement your logic. If you do, you must first check that it is a concrete value an not
     * an unresolved token. If this value is an unresolved token
     * (`Token.isUnresolved(stack.account)` returns `true`), this implies that the user wishes that
     * this stack will synthesize into a **account-agnostic template**. In this case, your code
     * should either fail (throw an error, emit a synth error using
     * `Annotations.of(construct).addError()`) or implement some other region-agnostic behavior.
     */
     ...
     *
     * This can be used to define dependencies between any two stacks within an app, and also
     ...
     * will return an array with 2 tokens that will resolve at deploy-time to the first two
     * availability zones returned from CloudFormation's `Fn::GetAZs` intrinsic function.
     *
     * If they are not available in the context, returns a set of dummy values and reports them as
     * missing, and let the CLI resolve them by calling EC2 `DescribeAvailabilityZones` on the
     * target environment.
     *
     ...
     *
     * In the form `aws://account/region`. Use `stack.account` and `stack.region` to obtain the
     * specific values, no need to parse.
     *
     * You can use this value to determine if two stacks are targeting the same environment.
     *
     * If either `stack.account` or `stack.region` are not concrete values (e.g. `Aws.ACCOUNT_ID` or
     * `Aws.REGION`) the special strings `unknown-account` and/or `unknown-region` will be used
     * respectively to indicate this stack is region/account-agnostic.
     */
     ...
     *
     * Returns a string list representing the corresponding `Fn.importValue()` expression for this
     * Export. The export expression is automatically wrapped with an `Fn::Join` and the import
     * value with an `Fn::Split`, since CloudFormation can only export strings. You can control the
     * name for the export by passing the `name` option.
     *
     * If you don't supply a value for `name`, the value you're exporting must be a Resource
     * attribute (for example: `bucket.bucketName`) and it will be given the same name as the
     * automatic cross-stack reference that would be created if you used the attribute in another
     * Stack.
     *
     * One of the uses for this method is to *remove* the relationship between two Stacks
     * established by automatic cross-stack references. It will temporarily ensure that the
     * CloudFormation Export still exists while you remove the reference from the consuming stack.
     ...
     * @param options
     */
    public open fun exportStringListValue(
        exportedValue: Any,
        options: ExportValueOptions
    ): List<String> =
        unwrap(this).exportStringListValue(exportedValue, options.let(ExportValueOptions::unwrap))
    ...
    public open fun exportStringListValue(
        exportedValue: Any,
        options: ExportValueOptions.Builder.() -> Unit
    ): List<String> = exportStringListValue(exportedValue, ExportValueOptions(options))

    /**
     * Create a CloudFormation Export for a string value.
     *
     * Returns a string representing the corresponding `Fn.importValue()` expression for this
     * Export. You can control the name for the export by passing the `name` option.
     *
     * If you don't supply a value for `name`, the value you're exporting must be a Resource
     * attribute (for example: `bucket.bucketName`) and it will be given the same name as the
     * automatic cross-stack reference that would be created if you used the attribute in another
     ...
     * <h2>Example</h2>
     *
     * Here is how the process works. Let's say there are two stacks, `producerStack` and
     * `consumerStack`, and `producerStack` has a bucket called `bucket`, which is referenced by
     * `consumerStack` (perhaps because an AWS Lambda Function writes into it, or something like
     * that).
     *
     * It is not safe to remove `producerStack.bucket` because as the bucket is being deleted,
     * `consumerStack` might still be using it.
     *
     ...
     * * In the `ProducerStack` class, call `this.exportValue(this.bucket.bucketName)`. This will
     *   make sure the CloudFormation Export continues to exist while the relationship between the
     *   two stacks is being broken.
     * * Deploy (this will effectively only change the `consumerStack`, but it's safe to deploy
     *   both).
     *
     ...
     * @param options
     */
    public open fun exportValue(exportedValue: Any): String =
        unwrap(this).exportValue(exportedValue)
     ...
     *
     * If `partition`, `region` or `account` are not specified, the stack's partition, region and
     * account will be used.
     *
     * If any component is the empty string, an empty string will be inserted into the generated ARN
     ...
     *
     * The required ARN pieces that are omitted will be taken from the stack that the 'scope' is
     * attached to. If all ARN pieces are supplied, the supplied scope can be 'undefined'.
     *
     * @param components
     */
     ...
     *
     * This method is called when a `CfnElement` is created and used to render the initial logical
     * identity of resources. Logical ID renames are applied at this stage.
     *
     * This method uses the protected method `allocateLogicalId` to render the logical ID for an
     * element. To modify the naming scheme, extend the `Stack` class and override this method.
     *
     ...
     *
     * This value is resolved according to the following rules:
     * * The value provided to `env.region` when the stack is defined. This can either be a concrete
     *   region (e.g. `us-west-2`) or the `Aws.REGION` token.
     * * `Aws.REGION`, which is represents the CloudFormation intrinsic reference `{ "Ref":
     *   "AWS::Region" }` encoded as a string token.
     *
     * Preferably, you should use the return value as an opaque string and not attempt to parse it
     * to implement your logic. If you do, you must first check that it is a concrete value an not
     * an unresolved token. If this value is an unresolved token (`Token.isUnresolved(stack.region)`
     * returns `true`), this implies that the user wishes that this stack will synthesize into a
     * **region-agnostic template**. In this case, your code should either fail (throw an error,
     * emit a synth error using `Annotations.of(construct).addError()`) or implement some other
     * region-agnostic behavior.
     */
     ...
     *
     * Will return a definite value only if the region of the current stack is resolved. If not, a
     * lookup map will be added to the stack and the lookup will be done at CDK deployment time.
     *
     * What regions will be included in the lookup map is controlled by the
     * `&#64;aws-cdk/core:target-partitions` context value: it must be set to a list of partitions,
     * and only regions from the given partitions will be included. If no such context key is set,
     * all regions will be included.
     *
     * This function is intended to be used by construct library authors. Application builders can
     * rely on the abstractions offered by construct libraries and do not have to worry about
     * regional facts.
     *
     ...
     *
     * To modify the naming scheme strategy, extend the `Stack` class and override the
     * `allocateLogicalId` method.
     *
     * @param oldId
     * @param newId
     */
     ...
     *
     * Contains instructions which will be emitted into the cloud assembly on how the key should be
     * supplied.
     *
     ...
     *
     * Works both if 'arn' is a string like 'arn:aws:s3:::bucket', and a Token representing a
     * dynamic CloudFormation expression (in which case the returned components will also be dynamic
     ...
     *
     * This is either the name defined explicitly in the `stackName` prop or allocated based on the
     * stack's location in the construct tree. Stacks that are directly defined under the app use
     * their construct `id` as their stack name. Stacks that are defined deeper within the tree will
     * use a hashed naming scheme based on the construct path to ensure uniqueness.
     *
    ...
    /**
     * The name of the CloudFormation template file emitted to the output directory during
     * synthesis.
     *
     ...
     * @param space
     */
    public open fun toJsonString(obj: Any, space: Number): String =
        unwrap(this).toJsonString(obj, space)
    ...
    public interface Builder {
         ...
         *
         * Enabling this will create a CloudFormation custom resource in both the producing stack
         * and consuming stack in order to perform the export/import
         *
         * This feature is currently experimental
         *
         * Default: false
         *
         * @param crossRegionReferences Enable this flag to allow native cross region stack
         *   references.
         */
         ...
         *
         * Set the `region`/`account` fields of `env` to either a concrete value to select the
         * indicated environment (recommended for production stacks), or to the values of
         * environment variables `CDK_DEFAULT_REGION`/`CDK_DEFAULT_ACCOUNT` to let the target
         * environment depend on the AWS credentials/configuration that the CDK CLI is executed
         * under (recommended for development stacks).
         *
         * If the `Stack` is instantiated inside a `Stage`, any undefined `region`/`account` fields
         * from `env` will default to the same field on the encompassing `Stage`, if configured
         * there.
         *
         ...
         * environment but may not be able to take advantage of all features of the CDK. For
         * example, they will not be able to use environmental context lookups such as
         * `ec2.Vpc.fromLookup` and will not automatically translate Service Principals to the right
         * format based on the environment's AWS partition, and other such enhancements.
         *
         * Default: - The environment of the containing `Stage` if available, otherwise create the
         * stack will be environment-agnostic.
         *
         * Example:
         * ```
        ...
        /**
         * Options for applying a permissions boundary to all IAM Roles and Users created within
         * this Stage.
         *
         * Default: - no permissions boundary is applied
         *
         * @param permissionsBoundary Options for applying a permissions boundary to all IAM Roles
         ...
         * context key will be used. If that is not specified, then the default value `false` will
         * be used.
         *
         * Default: - the value of `@aws-cdk/core:suppressTemplateIndentation`, or `false` if that
         * is not set.
         *
         ...
         * referenced and what IAM roles to use. For more information, see the README of the main
         * CDK package.
         *
         * If not specified, the `defaultStackSynthesizer` from `App` will be used. If that is not
         * specified, `DefaultStackSynthesizer` is used if
         * `&#64;aws-cdk/core:newStyleStackSynthesis` is set to `true` or the CDK major version is
         * v2. In CDK v1 `LegacyStackSynthesizer` is the default if no other synthesizer is
         * specified.
         *
