...
internal constructor(
    ...
    /**
     * The service token to use in order to define custom resources that are backed by this
     * provider.
     */
    ...
    public interface Builder {
         ...
         *
         * This function will be called immediately after `onEvent` and then periodically based on
         * the configured query interval as long as it returns `false`. If the function still
         * returns `false` and the alloted timeout has passed, the operation will fail.
         *
         ...
         * (CREATE/UPDATE/DELETE) and return any additional properties to add to the event, which
         * will later be passed to `isComplete`. The `PhysicalResourceId` property must be included
         * in the response.
         *
         * @param onEventHandler The AWS Lambda function to invoke for all resource lifecycle
         ...
         * `queryInterval` seconds, and until `timeout` has been reached or until `isComplete`
         * returns `true`.
         *
         * Default: Duration.seconds(5)
         *
         * @param queryInterval Time between calls to the `isComplete` handler which determines if
         *   the resource has been stabilized.
         */
         ...
         *
         * Default: - If `vpc` is not supplied, no security groups are attached. Otherwise, a
         * dedicated security group is created for each function.
         *
    ...
    private class BuilderImpl(
        ...
        override fun providerFunctionEnvEncryption(providerFunctionEnvEncryption: IKey) {
            cdkBuilder.providerFunctionEnvEncryption(
