...
internal constructor(
     ...
     *
     * Note that you cannot use this method if `ignoreErrorCodesMatching` is configured for any of
     * the SDK calls. This is because in such a case, the response data might not exist, and will
     * cause a CloudFormation deploy time error.
     *
     * @param dataPath the path to the data.
     */
    public open fun responseField(dataPath: String): String =
        unwrap(this).getResponseField(dataPath)
     ...
     * Example for S3 / listBucket : 'Buckets.0.Name'
     *
     * Use `Token.asXxx` to encode the returned `Reference` as a specific type or use the
     * convenience `getDataString` for string attributes.
     *
     * Note that you cannot use this method if `ignoreErrorCodesMatching` is configured for any of
     * the SDK calls. This is because in such a case, the response data might not exist, and will
     * cause a CloudFormation deploy time error.
    ...
    public interface Builder {
         ...
         *
         * If not specified, this uses whatever JavaScript SDK version is the default in AWS Lambda
        ...
        /**
         * The policy that will be added to the execution role of the Lambda function implementing
         * this custom resource provider.
         *
         * The custom resource also implements `iam.IGrantable`, making it possible to use the
         * `grantXxx()` methods.
         *
         ...
         *
         * Note that a policy must be specified if `role` is not provided, as by default a new role
         * is created which requires policy changes to access resources.
         *
         ...
         *
         * @param removalPolicy The policy to apply when this resource is removed from the
         *   application.
         */
         ...
         * provider.
         *
         * This role will apply to all `AwsCustomResource` instances in the stack. The role must be
         ...
         *
         * Only used if 'vpc' is supplied. Note: internet access for Lambdas requires a NAT gateway,
         * so picking Public subnets is not allowed.
         *
