...
public interface AwsSdkCall {
     ...
     *
     * Can be used primarily for cross-account requests to for example connect hostedzone with a
     * shared vpc. Region controls where assumeRole call is made.
     *
     ...
     *
     * Use this to limit the data returned by the custom resource if working with API calls that
     * could potentially result in custom response objects exceeding the hard limit of 4096 bytes.
     *
     ...
     *
     * **Note: Cross-region operations are generally considered an anti-pattern.** **Consider first
     * deploying a stack in that region.**
     *
    ...
    public interface Builder {
        /**
         * @param action The service action to call. This is the name of an AWS API call, in one of
         *   the following forms:
         * * An API call name as found in the API Reference documentation (`GetObject`)
         * * The API call name starting with a lowercase letter (`getObject`)
         * * The AWS SDK for JavaScript v3 command class name (`GetObjectCommand`)
        ...
        /**
         * @param assumedRoleArn Used for running the SDK calls in underlying lambda with a
         *   different role. Can be used primarily for cross-account requests to for example connect
         *   hostedzone with a shared vpc. Region controls where assumeRole call is made.
         *
         * Example for Route53 / associateVPCWithHostedZone
         */
        ...
        /**
         * @param ignoreErrorCodesMatching The regex pattern to use to catch API errors. The `code`
         *   property of the `Error` object will be tested against this pattern. If there is a match
         *   an error will not be thrown.
         */
        public fun ignoreErrorCodesMatching(ignoreErrorCodesMatching: String)

        /**
         * @param outputPaths Restrict the data returned by the custom resource to specific paths in
         *   the API response. Use this to limit the data returned by the custom resource if working
         *   with API calls that could potentially result in custom response objects exceeding the
         *   hard limit of 4096 bytes.
         *
         * Example for ECS / updateService: ['service.deploymentConfiguration.maximumPercent']
         */
         ...
         *   Mandatory for onCreate call. In onUpdate, you can omit this to passthrough it from
         *   request.
         */
        public fun physicalResourceId(physicalResourceId: PhysicalResourceId)

        /**
         * @param region The region to send service requests to. **Note: Cross-region operations are
         *   generally considered an anti-pattern.** **Consider first deploying a stack in that
         *   region.**
         */
    ...
    private class BuilderImpl : Builder {
         ...
         * Example for ECS / updateService: ['service.deploymentConfiguration.maximumPercent']
         */
        override fun outputPaths(vararg outputPaths: String): Unit =
    ...
    internal constructor(
         ...
         * The regex pattern to use to catch API errors.
         *
         * The `code` property of the `Error` object will be tested against this pattern. If there
         * is a match an error will not be thrown.
         *
         * Default: - do not catch errors
         */
        override fun ignoreErrorCodesMatching(): String? =
         ...
         *
         * Use this to limit the data returned by the custom resource if working with API calls that
         * could potentially result in custom response objects exceeding the hard limit of 4096
         * bytes.
         *
         * Example for ECS / updateService: ['service.deploymentConfiguration.maximumPercent']
         *
         ...
         * The region to send service requests to.
         *
         * **Note: Cross-region operations are generally considered an anti-pattern.** **Consider
         * first deploying a stack in that region.**
         *
         * Default: - the region where this custom resource is deployed
         */
    ...
    public companion object {
        ...
        ): AwsSdkCall = Wrapper(cdkObject)

        internal fun unwrap(
            wrapped: AwsSdkCall
        ): software.amazon.awscdk.customresources.AwsSdkCall = (wrapped as Wrapper).cdkObject
    }
}
