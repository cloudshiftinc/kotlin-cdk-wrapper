...
internal constructor(
     ...
     *
     * Will throw an error if this is accessed before `buildPipeline()` is called, or if
     * selfMutation has been disabled.
     */
    ...
    public interface Builder {
        ...
        public fun assetPublishingCodeBuildDefaults(
            assetPublishingCodeBuildDefaults: CodeBuildOptions.Builder.() -> Unit
        )

        /**
         * CDK CLI version to use in self-mutation and asset publishing steps.
         *
         * If you want to lock the CDK CLI version used in the pipeline, by steps that are
         * automatically generated for you, specify the version here.
         *
         * We recommend you do not specify this value, as not specifying it always uses the latest
         * CLI version which is backwards compatible with old versions.
         *
         * If you do specify it, be aware that this version should always be equal to or higher than
         * the version of the CDK framework used by the CDK app, when the CDK commands are run
         * during your pipeline execution. When you change this version, the *next time* the
         * `SelfMutate` step runs it will still be using the CLI of the the *previous* version that
         * was in this property: it will only start using the new version after `SelfMutate`
         * completes successfully. That means that if you want to update both framework and CLI
         * version, you should update the CLI version first, commit, push and deploy, and only then
         ...
         *
         * The artifact buckets have to be encrypted to support deploying CDK apps to another
         * account, so if you want to do that or want to have your artifact buckets encrypted, be
         * sure to set this value to `true`.
         *
         ...
         * if you don't provide an explicit Bucket for that region using this property, the
         * construct will automatically create a Stack containing an S3 Bucket in that region.
         * Passed directly through to the [cp.Pipeline].
         *
         * Default: - no cross region replication buckets.
         *
         * @param crossRegionReplicationBuckets A map of region to S3 bucket name used for
         *   cross-region CodePipeline.
         */
        public fun crossRegionReplicationBuckets(
         ...
         *
         * Set this to true if the pipeline itself uses Docker container assets (for example, if you
         * use `LinuxBuildImage.fromAsset()` as the build image of a CodeBuild step in the
         * pipeline).
         *
         ...
         * first, commit and allow the pipeline to self-update, and only then use the Docker asset
         * in the pipeline.
         *
         * Default: false
         *
         ...
         *
         * Publishing in parallel improves concurrency and may reduce publishing latency, but may
         * also increase overall provisioning time of the CodeBuild projects.
         *
         ...
         *
         * You can temporarily set this to `false` while you are iterating on the pipeline itself
         * and prefer to deploy changes using `cdk deploy`.
         *
         * Default: true
         *
         ...
         * Default: - Only `codeBuildDefaults` are applied
         *
         * @param selfMutationCodeBuildDefaults Additional customizations to apply to the self
         *   mutation CodeBuild projects.
         */
        ...
        public fun selfMutationCodeBuildDefaults(
            selfMutationCodeBuildDefaults: CodeBuildOptions.Builder.() -> Unit
        )
         ...
         * Default: - Only `codeBuildDefaults` are applied
         *
         * @param synthCodeBuildDefaults Additional customizations to apply to the synthesize
         *   CodeBuild projects.
         */
        ...
        public fun synthCodeBuildDefaults(
            synthCodeBuildDefaults: CodeBuildOptions.Builder.() -> Unit
        )

        /**
         * Deploy every stack by creating a change set and executing it.
         *
         * When enabled, creates a "Prepare" and "Execute" action for each stack. Disable to deploy
         * the stack in one pipeline action.
         *
         * Default: true
         *
    ...
    private class BuilderImpl(
         ...
         *   publishing CodeBuild projects.
         */
        override fun assetPublishingCodeBuildDefaults(
            assetPublishingCodeBuildDefaults: CodeBuildOptions
        ) {
            cdkBuilder.assetPublishingCodeBuildDefaults(
                assetPublishingCodeBuildDefaults.let(CodeBuildOptions::unwrap)
            )
        }
        ...
        override fun assetPublishingCodeBuildDefaults(
            ...
            assetPublishingCodeBuildDefaults(CodeBuildOptions(assetPublishingCodeBuildDefaults))
        ...
        @Suppress("INAPPLICABLE_JVM_NAME")
        @JvmName("92416628ee0d4ef7efc74754c30fcf9c872c992904f5b1db3c94b24ca77cd7ed")
        override fun codeBuildDefaults(
            codeBuildDefaults: CodeBuildOptions.Builder.() -> Unit
         ...
         *   cross-region CodePipeline.
         */
        override fun crossRegionReplicationBuckets(
            crossRegionReplicationBuckets: Map<String, IBucket>
        ) {
            cdkBuilder.crossRegionReplicationBuckets(
                crossRegionReplicationBuckets.mapValues { IBucket.unwrap(it.value) }
            )
        }
         ...
         *   mutation CodeBuild projects.
         */
        override fun selfMutationCodeBuildDefaults(
            selfMutationCodeBuildDefaults: CodeBuildOptions
        ) {
            cdkBuilder.selfMutationCodeBuildDefaults(
                selfMutationCodeBuildDefaults.let(CodeBuildOptions::unwrap)
            )
        }
        ...
        @Suppress("INAPPLICABLE_JVM_NAME")
        @JvmName("8ca7d4625a6b01d9bbb45d464baf8d48c59aaca88a925ec8b7f0ee11d400412d")
        override fun synthCodeBuildDefaults(
            synthCodeBuildDefaults: CodeBuildOptions.Builder.() -> Unit
