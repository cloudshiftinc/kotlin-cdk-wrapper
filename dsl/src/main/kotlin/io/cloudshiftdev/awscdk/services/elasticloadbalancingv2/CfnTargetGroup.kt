...
internal constructor(
     ...
     * check.
     */
    public open fun healthCheckTimeoutSeconds(): Number? =
     ...
     * [io.cloudshiftdev.awscdk.services.elasticloadbalancingv2.CfnTargetGroup].
     */
    @CdkDslMarker
    public interface Builder {
         ...
         * Indicates whether health checks are enabled.
         *
         * If the target type is `lambda` , health checks are disabled by default but can be
         * enabled. If the target type is `instance` , `ip` , or `alb` , health checks are always
         ...
         * target.
         *
         * The range is 5-300. If the target group protocol is TCP, TLS, UDP, TCP_UDP, HTTP or
         * HTTPS, the default is 30 seconds. If the target group protocol is GENEVE, the default is
         ...
         *
         * @param healthCheckPath [HTTP/HTTPS health checks] The destination for health checks on
         *   the targets.
         */
         ...
         * The protocol the load balancer uses when performing health checks on targets.
         *
         * For Application Load Balancers, the default is HTTP. For Network Load Balancers and
         * Gateway Load Balancers, the default is TCP. The TCP protocol is not supported for health
         * checks if the protocol of the target group is HTTP or HTTPS. The GENEVE, TLS, UDP, and
         ...
         *
         * @param healthCheckProtocol The protocol the load balancer uses when performing health
         *   checks on targets.
         */
         ...
         * seconds. For target groups with a protocol of TCP, TLS or HTTPS, the default is 10
         * seconds. For target groups with a protocol of GENEVE, the default is 5 seconds. If the
         ...
         * healthy.
         *
         * The range is 2-10. If the target group protocol is TCP, TCP_UDP, UDP, TLS, HTTP or HTTPS,
         * the default is 5. For target groups with a protocol of GENEVE, the default is 5. If the
         ...
         *
         * @param healthyThresholdCount The number of consecutive health check successes required
         *   before considering a target healthy.
         */
         ...
         * The type of IP address used for this target group.
         *
         * The possible values are `ipv4` and `ipv6` . This is an optional parameter. If not
         ...
         * target groups with a protocol of HTTP or HTTPS, the range is 200-499. For target groups
         * with a protocol of GENEVE, the range is 200-399.
         *
         ...
         *
         * @param matcher [HTTP/HTTPS health checks] The HTTP or gRPC codes to use when checking for
         ...
         * target is a Lambda function, this parameter does not apply. If the protocol is GENEVE,
         * the supported port is 6081.
         *
         ...
         * Load Balancers, the supported protocols are TCP, TLS, UDP, or TCP_UDP. For Gateway Load
         * Balancers, the supported protocol is GENEVE. A TCP_UDP listener must be associated with a
         ...
         *
         * The range is 2-10. If the target group protocol is TCP, TCP_UDP, UDP, TLS, HTTP or HTTPS,
         * the default is 2. For target groups with a protocol of GENEVE, the default is 2. If the
         * target type is `lambda` , the default is 5.
         *
    ...
    private class BuilderImpl(
        ...
        private val cdkBuilder:
            ...
            software.amazon.awscdk.services.elasticloadbalancingv2.CfnTargetGroup.Builder.create(
                scope,
                id
            )
    ...
    public interface MatcherProperty {
         ...
         * value being 200-399. You can specify multiple values (for example, "200,202") or a range
        ...
        public interface Builder {
            /**
             * @param grpcCode You can specify values between 0 and 99. You can specify multiple
             *   values (for example, "0,1") or a range of values (for example, "0-5"). The default
             *   value is 12.
             */
            public fun grpcCode(grpcCode: String)

            /**
             * @param httpCode For Application Load Balancers, you can specify values between 200
             *   and 499, with the default value being 200. You can specify multiple values (for
             *   example, "200,202") or a range of values (for example, "200-299").
             *
             * For Network Load Balancers, you can specify values between 200 and 599, with the
             * default value being 200-399. You can specify multiple values (for example, "200,202")
    ...
    public interface TargetGroupAttributeProperty {
         ...
         *   `draining` to `unused` . The range is 0-3600 seconds. The default value is 300 seconds.
         ...
         *   enabled. The value is `true` , `false` or `use_load_balancer_configuration` . The
         *   default is `use_load_balancer_configuration` .
         * * `target_group_health.dns_failover.minimum_healthy_targets.count` - The minimum number
         *   of targets that must be healthy. If the number of healthy targets is below this value,
         *   mark the zone as unhealthy in DNS, so that traffic is routed only to healthy zones. The
         *   possible values are `off` or an integer from 1 to the maximum number of targets. The
         *   default is `off` .
         * * `target_group_health.dns_failover.minimum_healthy_targets.percentage` - The minimum
         *   percentage of targets that must be healthy. If the percentage of healthy targets is
         *   below this value, mark the zone as unhealthy in DNS, so that traffic is routed only to
         *   healthy zones. The possible values are `off` or an integer from 1 to 100. The default
         *   is `off` .
         ...
         *   below this value, send traffic to all targets, including unhealthy targets. The
         ...
         *   registered target receives an increasing share of the traffic to the target group.
         *   After this time period ends, the target receives its full share of traffic. The range
         *   is 30-900 seconds (15 minutes). The default is 0 seconds (disabled).
         ...
         *   expires, the application-based cookie is considered stale. The range is 1 second to 1
         ...
         *   arrays of values or strings. The value is `true` or `false` . The default is `false` .
         *   If the value is `false` and the request contains a duplicate header field name or query
         ...
         *   balancer terminates connections at the end of the deregistration timeout. The value is
         *   `true` or `false` . For new UDP/TCP_UDP target groups the default is `true` .
         *   Otherwise, the default is `false` .
         * * `preserve_client_ip.enabled` - Indicates whether client IP preservation is enabled. The
         *   value is `true` or `false` . The default is disabled if the target group type is IP
         *   address and the target group protocol is TCP or TLS. Otherwise, the default is enabled.
         *   Client IP preservation cannot be disabled for UDP and TCP_UDP target groups.
         ...
         *   load balancer terminates connections to unhealthy targets. The value is `true` or
         ...
         *   `unhealthy.draining` to `unhealthy` . The range is 0-360000 seconds. The default value
         *   is 0 seconds.
         *
         ...
         *   `no_rebalance` . The default is `no_rebalance` . The two attributes (
         *   `target_failover.on_deregistration` and `target_failover.on_unhealthy` ) cannot be set
         *   independently. The value you set for both attributes must be the same.
         *
        ...
        public interface Builder {
            /**
             * @param key The name of the attribute. The following attributes are supported by all
             *   load balancers:
             ...
             *   from `draining` to `unused` . The range is 0-3600 seconds. The default value is 300
             ...
             *   this value, mark the zone as unhealthy in DNS, so that traffic is routed only to
             *   healthy zones. The possible values are `off` or an integer from 1 to the maximum
             *   number of targets. The default is `off` .
             * * `target_group_health.dns_failover.minimum_healthy_targets.percentage` - The minimum
             *   percentage of targets that must be healthy. If the percentage of healthy targets is
             *   below this value, mark the zone as unhealthy in DNS, so that traffic is routed only
             *   to healthy zones. The possible values are `off` or an integer from 1 to 100. The
             *   default is `off` .
             * * `target_group_health.unhealthy_state_routing.minimum_healthy_targets.count` - The
             *   minimum number of targets that must be healthy. If the number of healthy targets is
             *   below this value, send traffic to all targets, including unhealthy targets. The
             ...
             *   load balancer selects targets when routing requests. The value is `round_robin` ,
             *   `least_outstanding_requests` , or `weighted_random` . The default is `round_robin`
             *   .
             * * `load_balancing.algorithm.anomaly_mitigation` - Only available when
             *   `load_balancing.algorithm.type` is `weighted_random` . Indicates whether anomaly
             ...
             * * `slow_start.duration_seconds` - The time period, in seconds, during which a newly
             *   registered target receives an increasing share of the traffic to the target group.
             *   After this time period ends, the target receives its full share of traffic. The
             ...
             *   period expires, the application-based cookie is considered stale. The range is 1
             *   second to 1 week (604800 seconds). The default value is 1 day (86400 seconds).
             * * `stickiness.lb_cookie.duration_seconds` - The time period, in seconds, during which
             *   requests from a client should be routed to the same target. After this time period
             *   expires, the load balancer-generated cookie is considered stale. The range is 1
             *   second to 1 week (604800 seconds). The default value is 1 day (86400 seconds).
             *
             * The following attribute is supported only if the load balancer is an Application Load
             * Balancer and the target is a Lambda function:
             * * `lambda.multi_value_headers.enabled` - Indicates whether the request and response
             *   headers that are exchanged between the load balancer and the Lambda function
             ...
             *   field name or query parameter key, the load balancer uses the last value sent by
             *   the client.
             *
             * The following attributes are supported only by Network Load Balancers:
             * * `deregistration_delay.connection_termination.enabled` - Indicates whether the load
             *   balancer terminates connections at the end of the deregistration timeout. The value
             *   is `true` or `false` . For new UDP/TCP_UDP target groups the default is `true` .
             *   Otherwise, the default is `false` .
             * * `preserve_client_ip.enabled` - Indicates whether client IP preservation is enabled.
             *   The value is `true` or `false` . The default is disabled if the target group type
             *   is IP address and the target group protocol is TCP or TLS. Otherwise, the default
             *   is enabled. Client IP preservation cannot be disabled for UDP and TCP_UDP target
             *   groups.
             ...
             * * `target_health_state.unhealthy.draining_interval_seconds` - The amount of time for
             *   Elastic Load Balancing to wait before changing the state of an unhealthy target
             *   from `unhealthy.draining` to `unhealthy` . The range is 0-360000 seconds. The
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: TargetGroupAttributeProperty
            ): software.amazon.awscdk.services.elasticloadbalancingv2.CfnTargetGroup.TargetGroupAttributeProperty =
                (wrapped as Wrapper).cdkObject
        }
    }
    ...
    public interface TargetDescriptionProperty {
         ...
         * target type is `ip` and the IP address is outside of the VPC for the target group, this
         * should be an Availability Zone inside the VPC for the target group.
         *
         ...
         * target type is `ip` , specify an IP address. If the target type is `lambda` , specify the
         * ARN of the Lambda function. If the target type is `alb` , specify the ARN of the
         ...
         * matches the target group port. This parameter is not used if the target is a Lambda
         * function.
         *
        ...
        public interface Builder {
            /**
             * @param availabilityZone An Availability Zone or `all` . This determines whether the
             *   target receives traffic from the load balancer nodes in the specified Availability
             *   Zone or from all enabled Availability Zones for the load balancer.
             *
             * For Application Load Balancer target groups, the specified Availability Zone value is
             * only applicable when cross-zone load balancing is off. Otherwise the parameter is
             * ignored and treated as `all` .
             *
             ...
             * the target type is `ip` and the IP address is outside of the VPC for the target
            ...
            /**
             * @param id The ID of the target. If the target type of the target group is `instance`
             *   , specify an instance ID. If the target type is `ip` , specify an IP address. If
             *   the target type is `lambda` , specify the ARN of the Lambda function. If the target
             *   type is `alb` , specify the ARN of the Application Load Balancer target.
             */
            public fun id(id: String)

            /**
             * @param port The port on which the target is listening. If the target group protocol
             *   is GENEVE, the supported port is 6081. If the target type is `alb` , the targeted
        ...
        internal constructor(
             ...
             *
             * If the target type of the target group is `instance` , specify an instance ID. If the
             * target type is `ip` , specify an IP address. If the target type is `lambda` , specify
             * the ARN of the Lambda function. If the target type is `alb` , specify the ARN of the
             * Application Load Balancer target.
             *
             * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-elasticloadbalancingv2-targetgroup-targetdescription.html#cfn-elasticloadbalancingv2-targetgroup-targetdescription-id)
             */
             ...
             * The port on which the target is listening.
             *
             * If the target group protocol is GENEVE, the supported port is 6081. If the target
             * type is `alb` , the targeted Application Load Balancer must have at least one
             * listener whose port matches the target group port. This parameter is not used if the
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: TargetDescriptionProperty
            ): software.amazon.awscdk.services.elasticloadbalancingv2.CfnTargetGroup.TargetDescriptionProperty =
                (wrapped as Wrapper).cdkObject
        }
    }
