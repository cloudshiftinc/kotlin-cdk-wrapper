...
internal constructor(
     ...
     * * Renders the `AWS::CloudFormation::Init` object to the given resource's metadata,
     *   potentially adding a `AWS::CloudFormation::Authentication` object next to it if required.
     * * Updates the instance role policy to be able to call the APIs required for `cfn-init` and
     *   `cfn-signal` to work, and potentially add permissions to download referenced asset and
     *   bucket resources.
    ...
    public open fun attach(attachedResource: CfnResource, attachOptions: AttachInitOptions) {
        unwrap(this)
            .attach(
                attachedResource.let(CfnResource::unwrap),
    ...
    public open fun attach(
        ...
        attachOptions: AttachInitOptions.Builder.() -> Unit
    ): Unit = attach(attachedResource, AttachInitOptions(attachOptions))

    public companion object {
        public fun fromConfig(config: InitConfig): CloudFormationInit =
            software.amazon.awscdk.services.ec2.CloudFormationInit.fromConfig(
                    config.let(InitConfig::unwrap)
                )
                .let(CloudFormationInit::wrap)

        public fun fromConfigSets(props: ConfigSetProps): CloudFormationInit =
            software.amazon.awscdk.services.ec2.CloudFormationInit.fromConfigSets(
                    props.let(ConfigSetProps::unwrap)
                )
                .let(CloudFormationInit::wrap)
        ...
        public fun fromElements(elements: InitElement): CloudFormationInit =
            software.amazon.awscdk.services.ec2.CloudFormationInit.fromElements(
                    elements.let(InitElement::unwrap)
                )
                .let(CloudFormationInit::wrap)
