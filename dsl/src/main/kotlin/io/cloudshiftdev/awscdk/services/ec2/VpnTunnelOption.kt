...
public interface VpnTunnelOption {
     ...
     *
     * Allowed characters are alphanumeric characters period `.` and underscores `_`. Must be
     ...
     * private gateway. A size /30 CIDR block from the 169.254.0.0/16 range.
     *
    ...
    public interface Builder {
         ...
         *   characters period `.` and underscores `_`. Must be between 8 and 64 characters in
         *   length and cannot start with zero (0).
         * @deprecated Use `preSharedKeySecret` instead
         */
        @Deprecated(message = "deprecated in CDK") public fun preSharedKey(preSharedKey: String)
         ...
         *   alphanumeric characters period `.` and underscores `_`. Must be between 8 and 64
         *   characters in length and cannot start with zero (0).
         */
        public fun preSharedKeySecret(preSharedKeySecret: SecretValue)

        /**
         * @param tunnelInsideCidr The range of inside IP addresses for the tunnel. Any specified
         *   CIDR blocks must be unique across all VPN connections that use the same virtual private
         *   gateway. A size /30 CIDR block from the 169.254.0.0/16 range.
         */
    ...
    internal constructor(
        ...
        /**
         * (deprecated) The pre-shared key (PSK) to establish initial authentication between the
         ...
         * virtual private gateway. A size /30 CIDR block from the 169.254.0.0/16 range.
         *
         * Default: an Amazon generated inside IP CIDR
         */
