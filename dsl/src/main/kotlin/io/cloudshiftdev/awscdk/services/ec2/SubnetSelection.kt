...
public interface SubnetSelection {
     ...
     *
     * Select the subnet group with the given name. This only needs to be used if you have multiple
     * subnet groups of the same type and you need to distinguish between them. Otherwise, prefer
     * `subnetType`.
     *
     ...
     *
     * Use this if you don't want to automatically use all subnets in a group, but have a need to
     * control selection down to individual subnets.
     *
    ...
    public fun subnets(): List<ISubnet> =
        unwrap(this).getSubnets()?.map(ISubnet::wrap) ?: emptyList()
    ...
    public interface Builder {
        ...
        /**
         * @param subnetGroupName Select the subnet group with the given name. Select the subnet
         *   group with the given name. This only needs to be used if you have multiple subnet
         *   groups of the same type and you need to distinguish between them. Otherwise, prefer
         *   `subnetType`.
         *
         * This field does not select individual subnets, it selects all subnets that share the
         * given subnet group name. This is the name supplied in `subnetConfiguration`.
         *
         * At most one of `subnetType` and `subnetGroupName` can be supplied.
         */
    ...
    internal constructor(
         ...
         * Select the subnet group with the given name.
         *
         * Select the subnet group with the given name. This only needs to be used if you have
         * multiple subnet groups of the same type and you need to distinguish between them.
         * Otherwise, prefer `subnetType`.
         *
         * This field does not select individual subnets, it selects all subnets that share the
         * given subnet group name. This is the name supplied in `subnetConfiguration`.
         *
         ...
         * Explicitly select individual subnets.
         *
         * Use this if you don't want to automatically use all subnets in a group, but have a need
         * to control selection down to individual subnets.
         *
         * Cannot be specified together with `subnetType` or `subnetGroupName`.
         *
         * Default: - Use all subnets in a selected group (all private subnets by default)
         */
        override fun subnets(): List<ISubnet> =
            unwrap(this).getSubnets()?.map(ISubnet::wrap) ?: emptyList()
    }
