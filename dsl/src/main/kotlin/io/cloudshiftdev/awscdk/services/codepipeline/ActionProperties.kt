...
public interface ActionProperties {
     ...
     *
     * For Actions backed by resources, this is inferred from the Stack `resource` is part of.
     * However, some Actions, like the CloudFormation ones, are not backed by any resource, and they
     * still might want to be cross-account. In general, a concrete Action class should specify
     * either `resource`, or `account` - but not both.
     */
     ...
     *
     * The category defines which action type the owner (the entity that performs the action)
     * performs.
     */
    public fun category(): ActionCategory

    /**  */
    public fun inputs(): List<Artifact> =
        unwrap(this).getInputs()?.map(Artifact::wrap) ?: emptyList()

    /**  */
    public fun outputs(): List<Artifact> =
        unwrap(this).getOutputs()?.map(Artifact::wrap) ?: emptyList()
     ...
     *
     * This is used for automatically handling Actions backed by resources from a different account
     * and/or region.
     */
    ...
    public interface Builder {
        /**
         * @param account The account the Action is supposed to live in. For Actions backed by
         *   resources, this is inferred from the Stack `resource` is part of. However, some
         *   Actions, like the CloudFormation ones, are not backed by any resource, and they still
         *   might want to be cross-account. In general, a concrete Action class should specify
         *   either `resource`, or `account` - but not both.
         */
        ...
        /**
         * @param category The category of the action. The category defines which action type the
         *   owner (the entity that performs the action) performs.
         */
        ...
        /**
         * @param region The AWS region the given Action resides in. Note that a cross-region
         *   Pipeline requires replication buckets to function correctly. You can provide their
         *   names with the `PipelineProps#crossRegionReplicationBuckets` property. If you don't,
         *   the CodePipeline Construct will create new Stacks in your CDK app containing those
         *   buckets, that you will need to `cdk deploy` before deploying the main,
         *   Pipeline-containing Stack.
         */
        public fun region(region: String)

        /**
         * @param resource The optional resource that is backing this Action. This is used for
         *   automatically handling Actions backed by resources from a different account and/or
         *   region.
         */
        ...
        /**
         * @param runOrder The order in which AWS CodePipeline runs this action. For more
    ...
    internal constructor(
         ...
         *
         * For Actions backed by resources, this is inferred from the Stack `resource` is part of.
         * However, some Actions, like the CloudFormation ones, are not backed by any resource, and
         * they still might want to be cross-account. In general, a concrete Action class should
         * specify either `resource`, or `account` - but not both.
         */
         ...
         * performs.
         */
        override fun category(): ActionCategory =
            unwrap(this).getCategory().let(ActionCategory::wrap)

        /**  */
        override fun inputs(): List<Artifact> =
            unwrap(this).getInputs()?.map(Artifact::wrap) ?: emptyList()

        /**  */
        override fun outputs(): List<Artifact> =
            unwrap(this).getOutputs()?.map(Artifact::wrap) ?: emptyList()
         ...
         * The AWS region the given Action resides in.
         *
         * Note that a cross-region Pipeline requires replication buckets to function correctly. You
         * can provide their names with the `PipelineProps#crossRegionReplicationBuckets` property.
         * If you don't, the CodePipeline Construct will create new Stacks in your CDK app
         * containing those buckets, that you will need to `cdk deploy` before deploying the main,
         * Pipeline-containing Stack.
         *
         * Default: the Action resides in the same region as the Pipeline
         */
         ...
         * The optional resource that is backing this Action.
         *
         * This is used for automatically handling Actions backed by resources from a different
