...
internal constructor(
    ...
    public interface Builder {
         ...
         * The number of automated backups that you want to keep.
         *
         * Whenever a new backup is created, AWS OpsWorks CM deletes the oldest backups if this
        ...
        /**
         * Supported on servers running Chef Automate 2.0 only. A PEM-formatted HTTPS certificate.
         * The value can be be a single, self-signed certificate, or a certificate chain. If you
         * specify a custom certificate, you must also specify values for `CustomDomain` and
         ...
         *
         * @param customCertificate Supported on servers running Chef Automate 2.0 only. A
         *   PEM-formatted HTTPS certificate. The value can be be a single, self-signed certificate,
         *   or a certificate chain. If you specify a custom certificate, you must also specify
         *   values for `CustomDomain` and `CustomPrivateKey` . The following are requirements for
         ...
         * server, such as `https://aws.my-company.com` . To access the server, create a CNAME DNS
         * record in your preferred DNS service that points the custom domain to the endpoint that
         * is generated when the server is created (the value of the CreateServer Endpoint
         * attribute). You cannot access the server by using the generated `Endpoint` value if the
         * server is using a custom domain. If you specify a custom domain, you must also specify
         ...
         *
         * @param customDomain Supported on servers running Chef Automate 2.0 only. An optional
         *   public endpoint of a server, such as `https://aws.my-company.com` . To access the
         *   server, create a CNAME DNS record in your preferred DNS service that points the custom
         *   domain to the endpoint that is generated when the server is created (the value of the
         *   CreateServer Endpoint attribute). You cannot access the server by using the generated
         *   `Endpoint` value if the server is using a custom domain. If you specify a custom
         *   domain, you must also specify values for `CustomCertificate` and `CustomPrivateKey` .
         */
        public fun customDomain(customDomain: String)

        /**
         * Supported on servers running Chef Automate 2.0 only. A private key in PEM format for
         * connecting to the server by using HTTPS. The private key must not be encrypted; it cannot
         * be protected by a password or passphrase. If you specify a custom private key, you must
         ...
         *
         * @param customPrivateKey Supported on servers running Chef Automate 2.0 only. A private
         *   key in PEM format for connecting to the server by using HTTPS. The private key must not
         *   be encrypted; it cannot be protected by a password or passphrase. If you specify a
         *   custom private key, you must also specify values for `CustomDomain` and
         *   `CustomCertificate` .
         */
         ...
         * base64-encoded RSA public key. The corresponding private key is required to access the
         * Chef API. When no CHEF_AUTOMATE_PIVOTAL_KEY is set, a private key is generated and
         * returned in the response. When you are specifying the value of CHEF_AUTOMATE_PIVOTAL_KEY
         ...
         *   (!/&#64;#$%^&amp;+=_). The password must contain at least one lower case letter, one
         *   upper case letter, one number, and one special character. When no
         *   CHEF_AUTOMATE_ADMIN_PASSWORD is set, one is generated and returned in the response.
         *
         * **Attributes accepted in a Puppet createServer request:** - `PUPPET_ADMIN_PASSWORD` : To
         * work with the Puppet Enterprise console, a password must use ASCII characters.
         ...
         * The major release version of the engine that you want to use.
         *
         * For a Chef server, the valid value for EngineVersion is currently `2` . For a Puppet
         ...
         *
         * @param instanceProfileArn The ARN of the instance profile that your Amazon EC2 instances
         *   use.
         */
         ...
         *
         * @param preferredBackupWindow The start time for a one-hour period during which AWS
         ...
         * maintenance on the instance.
         *
         * Valid values must be specified in the following format: `DDD:HH:MM` . `MM` must be
         * specified as `00` . The specified time is in coordinated universal time (UTC). The
         ...
         *
         * @param preferredMaintenanceWindow The start time for a one-hour period each week during
         *   which AWS OpsWorks CM performs maintenance on the instance.
         */
         ...
         *
         * @param securityGroupIds A list of security group IDs to attach to the Amazon EC2
         *   instance.
         */
         ...
         *
         * @param serviceRoleArn The service role that the AWS OpsWorks CM service backend uses to
         ...
         *
         * @param tags A map that contains tag keys and tag values to attach to an AWS OpsWorks for
    ...
    private class BuilderImpl(
        ...
        public fun build(): software.amazon.awscdk.services.opsworkscm.CfnServer =
            cdkBuilder.build()
    }
    ...
    public companion object {
        ...
        internal fun unwrap(
            wrapped: CfnServer
        ): software.amazon.awscdk.services.opsworkscm.CfnServer = wrapped.cdkObject
    }
    ...
    public interface EngineAttributeProperty {
         ...
         *   private key is required to access the Chef API. You can generate this key by running
         *   the following [OpenSSL](https://docs.aws.amazon.com/https://www.openssl.org/) command
         ...
         * RSA private key. For more information, see
         ...
         *   Puppet Enterprise console webpage after the server is online. The password must use
         *   between 8 and 32 ASCII characters.
         * * `PUPPET_R10K_REMOTE` : The r10k remote is the URL of your control repository (for
         *   example, ssh://git&#64;your.git-repo.com:user/control-repo.git). Specifying an r10k
         *   remote opens TCP port 8170.
         * * `PUPPET_R10K_PRIVATE_KEY` : If you are using a private Git repository, add
        ...
        public interface Builder {
            /**
             * @param name The name of the engine attribute. *Attribute name for Chef Automate
             *   servers:*
             * * `CHEF_AUTOMATE_ADMIN_PASSWORD`
             *
             * *Attribute names for Puppet Enterprise servers:*
            ...
            /**
             * @param value The value of the engine attribute. *Attribute value for Chef Automate
             *   servers:*
             * * `CHEF_AUTOMATE_PIVOTAL_KEY` : A base64-encoded RSA public key. The corresponding
             *   private key is required to access the Chef API. You can generate this key by
             *   running the following
             *   [OpenSSL](https://docs.aws.amazon.com/https://www.openssl.org/) command on
             *   Linux-based computers.
             *
             * `openssl genrsa -out *pivotal_key_file_name* .pem 2048`
             *
             * On Windows-based computers, you can use the PuTTYgen utility to generate a
             * base64-encoded RSA private key. For more information, see
             * [PuTTYgen - Key Generator for PuTTY on Windows](https://docs.aws.amazon.com/https://www.ssh.com/ssh/putty/windows/puttygen)
             * on SSH.com.
             *
             * *Attribute values for Puppet Enterprise servers:*
             * * `PUPPET_ADMIN_PASSWORD` : An administrator password that you can use to sign in to
             *   the Puppet Enterprise console webpage after the server is online. The password must
             *   use between 8 and 32 ASCII characters.
             * * `PUPPET_R10K_REMOTE` : The r10k remote is the URL of your control repository (for
             *   example, ssh://git&#64;your.git-repo.com:user/control-repo.git). Specifying an r10k
             *   remote opens TCP port 8170.
             * * `PUPPET_R10K_PRIVATE_KEY` : If you are using a private Git repository, add
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: EngineAttributeProperty
            ): software.amazon.awscdk.services.opsworkscm.CfnServer.EngineAttributeProperty =
