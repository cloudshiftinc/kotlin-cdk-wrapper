...
public interface IBucket : IResource {
     ...
     */
    public fun addObjectCreatedNotification(
        dest: IBucketNotificationDestination,
        filters: NotificationKeyFilter
    )
     ...
     * @param filters Filters (see onEvent).
     */
    public fun addObjectRemovedNotification(
        dest: IBucketNotificationDestination,
        filters: NotificationKeyFilter
    )
     ...
     * perform actions on this bucket and/or its contents. Use `bucketArn` and `arnForObjects(keys)`
     ...
     * already has a policy attached, let alone to re-use that policy to add more statements to it.
     * So it's safest to do nothing in these cases.
     *
     * @param permission the policy statement to be added to the bucket's policy.
     * @return metadata about the execution of this method. If the policy was not added, the value
     *   of `statementAdded` will be `false`. You should always check this value to make sure that
     *   the operation was actually carried out. Otherwise, synthesis and deploy will terminate
    ...
    public fun addToResourcePolicy(
        permission: PolicyStatement.Builder.() -> Unit
     ...
     *
     * IMPORTANT: This permission allows anyone to perform actions on S3 objects in this bucket,
     * which is useful for when you configure your bucket as a website and want everyone to be able
     * to read objects in the bucket without needing to authenticate.
     *
     * Without arguments, this method will grant read ("s3:GetObject") access to all objects ("*")
     * in the bucket.
     *
     * The method returns the `iam.Grant` object, which can then be modified as needed. For example,
     * you can add a condition that will restrict access only to an IPv4 range like this:
     * ```
     ...
     *
     * If encryption is used, permission to use the key to encrypt the contents of written files
     ...
     * the objects; in this case, if you need to modify object ACLs, call this method explicitly.
     *
     * @param identity The principal.
     * @param objectsKeyPattern Restrict the permission to a certain key pattern (default '*').
     */
     ...
     * (Role/Group/User).
     *
     * If encryption is used, permission to use the key to decrypt the contents of the bucket will
     * also be granted to the same principal.
     *
     * @param identity The principal.
     * @param objectsKeyPattern Restrict the permission to a certain key pattern (default '*').
     */
     ...
     * (Role/Group/User).
     *
     * If an encryption key is used, permission to use the key for encrypt/decrypt will also be
     * granted.
     *
     * Before CDK version 1.85.0, this method granted the `s3:PutObject*` permission that included
     * `s3:PutObjectAcl`, which could be used to grant read/write object access to IAM principals in
     * other accounts. If you want to get rid of that behavior, update your CDK version to 1.85.0 or
     * later, and make sure the `&#64;aws-cdk/aws-s3:grantWriteWithoutAcl` feature flag is set to
     * `true` in the `context` key of your cdk.json file. If you've already updated, but still need
     * the principal to have permissions to modify the ACLs, use the `grantPutAcl` method.
     *
     * @param identity The principal.
     * @param objectsKeyPattern Restrict the permission to a certain key pattern (default '*').
     */
    ...
    public fun onCloudTrailEvent(
        id: String,
        options: OnCloudTrailBucketEventOptions.Builder.() -> Unit
    ): Rule
     ...
     *
     * Note that some tools like `aws s3 cp` will automatically use either PutObject or the
     * multipart upload API depending on the file size, so using `onCloudTrailWriteObject` may be
     * preferable.
     *
     * Requires that there exists at least one CloudTrail Trail in your account that captures the
     * event. This method will not create the Trail.
    ...
    public fun onCloudTrailPutObject(
        id: String,
        options: OnCloudTrailBucketEventOptions.Builder.() -> Unit
    ): Rule
     ...
     *
     * Note that some tools like `aws s3 cp` will automatically use either PutObject or the
     * multipart upload API depending on the file size, so using this method may be preferable to
    ...
    public fun onCloudTrailWriteObject(
        id: String,
        options: OnCloudTrailBucketEventOptions.Builder.() -> Unit
    ): Rule
     ...
     *
     * If `autoCreatePolicy` is true, a `BucketPolicy` will be created upon the first call to
     * addToResourcePolicy(s).
     */
     ...
     * @return an TransferAccelerationUrl token
     */
    public fun transferAccelerationUrlForObject(
        key: String,
    ...
    public fun transferAccelerationUrlForObject(
        key: String,
        options: TransferAccelerationUrlOptions.Builder.() -> Unit
    ): String
    ...
    public fun virtualHostedUrlForObject(
        key: String,
        options: VirtualHostedStyleUrlOptions.Builder.() -> Unit
    ): String
    ...
    internal constructor(
        ...
        /**
         * Subscribes a destination to receive notifications when an object is created in the
         * bucket.
         *
         * This is identical to calling `onEvent(s3.EventType.OBJECT_CREATED)`.
         *
         ...
         * @param filters Filters (see onEvent).
         */
        override fun addObjectCreatedNotification(
            dest: IBucketNotificationDestination,
            filters: NotificationKeyFilter
        ) {
            unwrap(this)
        ...
        override fun addObjectCreatedNotification(
            dest: IBucketNotificationDestination,
            filters: NotificationKeyFilter.Builder.() -> Unit
        ): Unit = addObjectCreatedNotification(dest, NotificationKeyFilter(filters))

        /**
         * Subscribes a destination to receive notifications when an object is removed from the
         * bucket.
         *
         * This is identical to calling `onEvent(EventType.OBJECT_REMOVED)`.
         *
         ...
         * @param filters Filters (see onEvent).
         */
        override fun addObjectRemovedNotification(
            dest: IBucketNotificationDestination,
            filters: NotificationKeyFilter
        ) {
            unwrap(this)
        ...
        override fun addObjectRemovedNotification(
            dest: IBucketNotificationDestination,
            filters: NotificationKeyFilter.Builder.() -> Unit
        ): Unit = addObjectRemovedNotification(dest, NotificationKeyFilter(filters))

        /**
         * Adds a statement to the resource policy for a principal (i.e. account/role/service) to
         * perform actions on this bucket and/or its contents. Use `bucketArn` and
         * `arnForObjects(keys)` to obtain ARNs for this bucket or objects.
         *
         * Note that the policy statement may or may not be added to the policy. For example, when
         * an `IBucket` is created from an existing bucket, it's not possible to tell whether the
         * bucket already has a policy attached, let alone to re-use that policy to add more
         * statements to it. So it's safest to do nothing in these cases.
         *
         * @param permission the policy statement to be added to the bucket's policy.
         * @return metadata about the execution of this method. If the policy was not added, the
         *   value of `statementAdded` will be `false`. You should always check this value to make
         *   sure that the operation was actually carried out. Otherwise, synthesis and deploy will
         *   terminate silently, which may be confusing.
         */
        override fun addToResourcePolicy(permission: PolicyStatement): AddToResourcePolicyResult =
            unwrap(this)
                .addToResourcePolicy(permission.let(PolicyStatement::unwrap))
        ...
        /** The IPv6 DNS name of the specified bucket. */
        override fun bucketDualStackDomainName(): String =
        ...
        override fun grantDelete(identity: IGrantable, objectsKeyPattern: Any): Grant =
            unwrap(this)
                .grantDelete(identity.let(IGrantable::unwrap), objectsKeyPattern)
                .let(Grant::wrap)
         ...
         *
         * IMPORTANT: This permission allows anyone to perform actions on S3 objects in this bucket,
         * which is useful for when you configure your bucket as a website and want everyone to be
         * able to read objects in the bucket without needing to authenticate.
         *
         * Without arguments, this method will grant read ("s3:GetObject") access to all objects
         * ("*") in the bucket.
         *
         * The method returns the `iam.Grant` object, which can then be modified as needed. For
         * example, you can add a condition that will restrict access only to an IPv4 range like
         * this:
         * ```
         * const grant = bucket.grantPublicAccess();
         * grant.resourceStatement!.addCondition(‘IpAddress’, { “aws:SourceIp”: “54.240.143.0/24” });
        ...
        override fun grantPut(identity: IGrantable, objectsKeyPattern: Any): Grant =
            unwrap(this)
                .grantPut(identity.let(IGrantable::unwrap), objectsKeyPattern)
                .let(Grant::wrap)

        /**
         * Grant the given IAM identity permissions to modify the ACLs of objects in the given
         * Bucket.
         *
         * If your application has the '&#64;aws-cdk/aws-s3:grantWriteWithoutAcl' feature flag set,
         * calling `grantWrite` or `grantReadWrite` no longer grants permissions to modify the ACLs
         * of the objects; in this case, if you need to modify object ACLs, call this method
         * explicitly.
         *
         * @param identity The principal.
         * @param objectsKeyPattern Restrict the permission to a certain key pattern (default '*').
        ...
        override fun grantPutAcl(identity: IGrantable, objectsKeyPattern: String): Grant =
            unwrap(this)
                .grantPutAcl(identity.let(IGrantable::unwrap), objectsKeyPattern)
                .let(Grant::wrap)
         ...
         * (Role/Group/User).
         *
         * If encryption is used, permission to use the key to decrypt the contents of the bucket
         * will also be granted to the same principal.
         *
         * @param identity The principal.
         * @param objectsKeyPattern Restrict the permission to a certain key pattern (default '*').
        ...
        override fun grantRead(identity: IGrantable, objectsKeyPattern: Any): Grant =
            unwrap(this)
                .grantRead(identity.let(IGrantable::unwrap), objectsKeyPattern)
                .let(Grant::wrap)
         ...
         * granted.
         *
         * Before CDK version 1.85.0, this method granted the `s3:PutObject*` permission that
         * included `s3:PutObjectAcl`, which could be used to grant read/write object access to IAM
         * principals in other accounts. If you want to get rid of that behavior, update your CDK
         * version to 1.85.0 or later, and make sure the `&#64;aws-cdk/aws-s3:grantWriteWithoutAcl`
         * feature flag is set to `true` in the `context` key of your cdk.json file. If you've
         * already updated, but still need the principal to have permissions to modify the ACLs, use
         * the `grantPutAcl` method.
         *
         * @param identity The principal.
         * @param objectsKeyPattern Restrict the permission to a certain key pattern (default '*').
         */
        ...
        override fun grantReadWrite(identity: IGrantable, objectsKeyPattern: Any): Grant =
            unwrap(this)
                .grantReadWrite(identity.let(IGrantable::unwrap), objectsKeyPattern)
                .let(Grant::wrap)
        ...
        override fun grantWrite(identity: IGrantable, objectsKeyPattern: Any): Grant =
            unwrap(this)
                .grantWrite(identity.let(IGrantable::unwrap), objectsKeyPattern)
                .let(Grant::wrap)
        ...
        override fun grantWrite(
            ...
            unwrap(this)
                .grantWrite(
                    identity.let(IGrantable::unwrap),
                    objectsKeyPattern,
        ...
        override fun onCloudTrailEvent(id: String, options: OnCloudTrailBucketEventOptions): Rule =
            unwrap(this)
                .onCloudTrailEvent(id, options.let(OnCloudTrailBucketEventOptions::unwrap))
                .let(Rule::wrap)
        ...
        override fun onCloudTrailEvent(
            id: String,
            options: OnCloudTrailBucketEventOptions.Builder.() -> Unit
        ): Rule = onCloudTrailEvent(id, OnCloudTrailBucketEventOptions(options))
         ...
         *
         * Note that some tools like `aws s3 cp` will automatically use either PutObject or the
         * multipart upload API depending on the file size, so using `onCloudTrailWriteObject` may
         * be preferable.
         *
         * Requires that there exists at least one CloudTrail Trail in your account that captures
         * the event. This method will not create the Trail.
         ...
         * @param options Options for adding the rule.
         */
        override fun onCloudTrailPutObject(
            id: String,
            options: OnCloudTrailBucketEventOptions
        ): Rule =
            unwrap(this)
                .onCloudTrailPutObject(id, options.let(OnCloudTrailBucketEventOptions::unwrap))
                .let(Rule::wrap)
        ...
        override fun onCloudTrailPutObject(
            id: String,
            options: OnCloudTrailBucketEventOptions.Builder.() -> Unit
        ): Rule = onCloudTrailPutObject(id, OnCloudTrailBucketEventOptions(options))

        /**
         * Defines an AWS CloudWatch event that triggers when an object at the specified paths
         * (keys) in this bucket are written to.
         *
         * This includes the events PutObject, CopyObject, and CompleteMultipartUpload.
         *
         * Note that some tools like `aws s3 cp` will automatically use either PutObject or the
         * multipart upload API depending on the file size, so using this method may be preferable
         * to `onCloudTrailPutObject`.
         *
         * Requires that there exists at least one CloudTrail Trail in your account that captures
         * the event. This method will not create the Trail.
         *
         ...
         * @param options Options for adding the rule.
         */
        override fun onCloudTrailWriteObject(
            id: String,
            options: OnCloudTrailBucketEventOptions
        ): Rule =
            unwrap(this)
                .onCloudTrailWriteObject(id, options.let(OnCloudTrailBucketEventOptions::unwrap))
                .let(Rule::wrap)
        ...
        override fun onCloudTrailWriteObject(
            id: String,
            options: OnCloudTrailBucketEventOptions.Builder.() -> Unit
        ): Rule = onCloudTrailWriteObject(id, OnCloudTrailBucketEventOptions(options))
         ...
         * The https Transfer Acceleration URL of an S3 object.
         *
         * Specify `dualStack: true` at the options for dual-stack endpoint (connect to the bucket
         ...
         * @return an TransferAccelerationUrl token
         */
        override fun transferAccelerationUrlForObject(
            key: String,
            options: TransferAccelerationUrlOptions
        ): String =
            unwrap(this)
        ...
        override fun transferAccelerationUrlForObject(
            key: String,
            options: TransferAccelerationUrlOptions.Builder.() -> Unit
        ): String = transferAccelerationUrlForObject(key, TransferAccelerationUrlOptions(options))
        ...
        /**
         * The virtual hosted-style URL of an S3 object. Specify `regional: false` at the options
         * for non-regional URL. For example:.
         * * `https://only-bucket.s3.us-west-1.amazonaws.com`
         * * `https://bucket.s3.us-west-1.amazonaws.com/key`
         * * `https://bucket.s3.amazonaws.com/key`
         * * `https://china-bucket.s3.cn-north-1.amazonaws.com.cn/mykey`
         ...
         * @return an ObjectS3Url token
         */
        override fun virtualHostedUrlForObject(
            key: String,
            options: VirtualHostedStyleUrlOptions
        ): String =
            unwrap(this)
                .virtualHostedUrlForObject(key, options.let(VirtualHostedStyleUrlOptions::unwrap))
        ...
        override fun virtualHostedUrlForObject(
            key: String,
            options: VirtualHostedStyleUrlOptions.Builder.() -> Unit
        ): String = virtualHostedUrlForObject(key, VirtualHostedStyleUrlOptions(options))
    }
    ...
    public companion object {
        ...
        internal fun unwrap(wrapped: IBucket): software.amazon.awscdk.services.s3.IBucket =
