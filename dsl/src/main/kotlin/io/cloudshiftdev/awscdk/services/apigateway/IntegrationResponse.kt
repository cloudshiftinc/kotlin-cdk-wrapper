...
public interface IntegrationResponse {
     ...
     *
     * Default: none the request payload is passed through from the method request to the
     ...
     * is an AWS Lambda function, the AWS Lambda function error header is matched. For all other
    ...
    public interface Builder {
        ...
        /**
         * @param responseParameters The response parameters from the backend response that API
         *   Gateway sends to the method response. Use the destination as the key and the source as
         *   the value:
         * * The destination must be an existing response parameter in the MethodResponse property.
        ...
        /**
         * @param responseTemplates The templates that are used to transform the integration
         *   response body. Specify templates as key-value pairs, with a content type as the key and
         *   a template as the value.
         */
        public fun responseTemplates(responseTemplates: Map<String, String>)

        /**
         * @param selectionPattern Specifies the regular expression (regex) pattern used to choose
         *   an integration response based on the response from the back end. For example, if the
         *   success response returns nothing and the error response returns some string, you could
         *   use the `.+` regex to match error response. However, make sure that the error response
        ...
        /**
         * @param statusCode The status code that API Gateway uses to map the integration response
    ...
    internal constructor(
         ...
         * [Documentation](http://docs.aws.amazon.com/apigateway/latest/developerguide/request-response-data-mappings.html)
         */
        override fun responseParameters(): Map<String, String> =
            unwrap(this).getResponseParameters() ?: emptyMap()

        /**
         * The templates that are used to transform the integration response body.
         *
         * Specify templates as key-value pairs, with a content type as the key and a template as
         * the value.
         *
         * [Documentation](http://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-mapping-template-reference.html)
         */
        override fun responseTemplates(): Map<String, String> =
            unwrap(this).getResponseTemplates() ?: emptyMap()
