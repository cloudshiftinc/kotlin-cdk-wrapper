...
public interface ApplyCloudFormationInitOptions {
     ...
     *
     * If `true` (the default), a hash of the config will be embedded into the UserData, so that if
     ...
     *
     * You can use this to prevent CloudFormation from rolling back when instances fail to start up,
     ...
     *
     * By default, the output of running cfn-init is written to a log file on the instance. Set this
     * to `true` to print it to the System Log (visible from the EC2 Console), `false` to not print
     * it.
     *
    ...
    public interface Builder {
         ...
         *   `true` (the default), a hash of the config will be embedded into the UserData, so that
         *   if the config changes, the UserData changes.
         * * If the EC2 instance is instance-store backed or `userDataCausesReplacement` is set,
         *   this will cause the instance to be replaced and the new configuration to be applied.
         * * If the instance is EBS-backed and `userDataCausesReplacement` is not set, the change of
         *   UserData will make the instance restart but not be replaced, and the configuration will
         *   not be applied automatically.
         *
         * If `false`, no hash will be embedded, and if the CloudFormation Init config changes
         * nothing will happen to the running instance. If a config update introduces errors, you
         * will not notice until after the CloudFormation deployment successfully finishes and the
         * next instance fails to launch.
         */
        public fun embedFingerprint(embedFingerprint: Boolean)

        /**
         * @param ignoreFailures Don't fail the instance creation when cfn-init fails. You can use
         *   this to prevent CloudFormation from rolling back when instances fail to start up, to
         *   help in debugging.
         */
        ...
        /**
         * @param printLog Print the results of running cfn-init to the Instance System Log. By
         *   default, the output of running cfn-init is written to a log file on the instance. Set
         *   this to `true` to print it to the System Log (visible from the EC2 Console), `false` to
         *   not print it.
         *
         * (Be aware that the system log is refreshed at certain points in time of the instance life
         * cycle, and successful execution may not always show up).
    ...
    internal constructor(
         ...
         * Force instance replacement by embedding a config fingerprint.
         *
         * If `true` (the default), a hash of the config will be embedded into the UserData, so that
         * if the config changes, the UserData changes.
         * * If the EC2 instance is instance-store backed or `userDataCausesReplacement` is set,
         *   this will cause the instance to be replaced and the new configuration to be applied.
         * * If the instance is EBS-backed and `userDataCausesReplacement` is not set, the change of
         ...
         * Don't fail the instance creation when cfn-init fails.
         *
         * You can use this to prevent CloudFormation from rolling back when instances fail to start
         * up, to help in debugging.
         *
         * Default: false
         */
         ...
         * Print the results of running cfn-init to the Instance System Log.
         *
         * By default, the output of running cfn-init is written to a log file on the instance. Set
         * this to `true` to print it to the System Log (visible from the EC2 Console), `false` to
         * not print it.
         *
         * (Be aware that the system log is refreshed at certain points in time of the instance life
    ...
    public companion object {
        ...
        internal fun unwrap(
            wrapped: ApplyCloudFormationInitOptions
