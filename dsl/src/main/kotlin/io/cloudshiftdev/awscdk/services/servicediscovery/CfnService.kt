...
internal constructor(
    ...
    public open fun healthCheckCustomConfig(`value`: HealthCheckCustomConfigProperty) {
        unwrap(this)
            .setHealthCheckCustomConfig(`value`.let(HealthCheckCustomConfigProperty::unwrap))
    }

    /** A complex type that contains information about an optional custom health check. */
    @Suppress("INAPPLICABLE_JVM_NAME")
    @JvmName("cd5ecf02a06f848a4c7b772a60bf2f9c860dd243a78b0ee5e77c6ec72f54362b")
    public open fun healthCheckCustomConfig(
        `value`: HealthCheckCustomConfigProperty.Builder.() -> Unit
    ): Unit = healthCheckCustomConfig(HealthCheckCustomConfigProperty(`value`))
    ...
    public interface Builder {
         ...
         *
         * @param dnsConfig A complex type that contains information about the Route 53 DNS records
         ...
         *
         * @param healthCheckConfig *Public DNS and HTTP namespaces only.* A complex type that
         *   contains settings for an optional health check. If you specify settings for a health
         *   check, AWS Cloud Map associates the health check with the records that you specify in
         *   `DnsConfig` .
         */
        ...
        public fun healthCheckCustomConfig(
            healthCheckCustomConfig: HealthCheckCustomConfigProperty.Builder.() -> Unit
        )
         ...
         *
         * @param type If present, specifies that the service instances are only discoverable using
    ...
    private class BuilderImpl(
         ...
         *   custom health check.
         */
        override fun healthCheckCustomConfig(
            healthCheckCustomConfig: HealthCheckCustomConfigProperty
        ) {
            cdkBuilder.healthCheckCustomConfig(
                healthCheckCustomConfig.let(HealthCheckCustomConfigProperty::unwrap)
            )
        }
    ...
    public interface HealthCheckConfigProperty {
         ...
         * to change the current status of the endpoint from unhealthy to healthy or the other way
         * around.
         *
         ...
         * format for when the endpoint is healthy. An example file is
         * `/docs/route53-health-check.html` . Route 53 automatically adds the DNS name for the
        ...
        public interface Builder {
             ...
             *   pass or fail for Route 53 to change the current status of the endpoint from
             ...
             *   checks. The path can be any value that your endpoint returns an HTTP status code of
        ...
        internal constructor(
             ...
             * The path that you want Route 53 to request when performing health checks.
             *
             * The path can be any value that your endpoint returns an HTTP status code of a 2xx or
    ...
    public interface DnsConfigProperty {
         ...
         *
         * You must specify a value for `NamespaceId` either for `DnsConfig` or for the
         * [service properties](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-servicediscovery-service.html)
         * . Don't specify a value in both places.
         *
         ...
         * health check. You use the service to register 10 instances. Route 53 responds to DNS
         * queries with IP addresses for up to eight healthy instances. If fewer than eight
         * instances are healthy, Route 53 responds to every DNS query with the IP addresses for all
         ...
         *   Currently, all records have the same weight, so you can't route more or less traffic to
         *   any instances.
         *
         * For example, suppose that the service includes configurations for one `A` record and a
         * health check. You use the service to register 10 instances. Route 53 responds to DNS
        ...
        public interface Builder {
            ...
            /**
             * @param routingPolicy The routing policy that you want to apply to all Route 53 DNS
             *   records that AWS Cloud Map creates when you register an instance and specify this
             *   service.
             *
             ...
             *
             * You can specify the following values:
             * * **MULTIVALUE** - If you define a health check for the service and the health check
             *   is healthy, Route 53 returns the applicable value for up to eight instances.
             *
             * For example, suppose that the service includes configurations for one `A` record and
             * a health check. You use the service to register 10 instances. Route 53 responds to
             * DNS queries with IP addresses for up to eight healthy instances. If fewer than eight
             ...
             * For example, suppose that the service includes configurations for one `A` record and
             * a health check. You use the service to register 10 instances. Route 53 responds to
             * DNS queries with the IP address for one randomly selected instance from among the
    ...
    public interface HealthCheckCustomConfigProperty {
         ...
         * `UpdateInstanceCustomHealthStatus` request before changing the status of the service
         * instance.
         *
         ...
         * value before 30 seconds has passed doesn't accelerate the change. AWS Cloud Map still
        ...
        public interface Builder {
             ...
             * an `UpdateInstanceCustomHealthStatus` request before it changes the health status of
             * a service instance.
             *
             * Sending a second or subsequent `UpdateInstanceCustomHealthStatus` request with the
             * same value before 30 seconds has passed doesn't accelerate the change. AWS Cloud Map
             * still waits `30` seconds after the first request to make the change.
             */
            public fun failureThreshold(failureThreshold: Number)
        }
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: HealthCheckCustomConfigProperty
            ): software.amazon.awscdk.services.servicediscovery.CfnService.HealthCheckCustomConfigProperty =
                (wrapped as Wrapper).cdkObject
        }
    }
    ...
    public interface DnsRecordProperty {
        ...
        public interface Builder {
             ...
             * * **SRV** - Route 53 returns the value for an `SRV` record. The value for an `SRV`
             ...
             * same name as the value of `service-hostname` in the `SRV` record. You can ignore
             * these records.
             */
            public fun type(type: String)
        }
