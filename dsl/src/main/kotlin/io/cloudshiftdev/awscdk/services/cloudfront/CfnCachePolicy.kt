...
internal constructor(
    ...
    public interface Builder {
        ...
        public fun cachePolicyConfig(
            cachePolicyConfig: CachePolicyConfigProperty.Builder.() -> Unit
        )
    }
    ...
    private class BuilderImpl(
        ...
        @Suppress("INAPPLICABLE_JVM_NAME")
        @JvmName("0fda54e25ac114759a639535f6dbaf9669165446bd1b2f9c3b537ceb0eced2e3")
        override fun cachePolicyConfig(
            cachePolicyConfig: CachePolicyConfigProperty.Builder.() -> Unit
    ...
    public interface HeadersConfigProperty {
        ...
        public interface Builder {
            /**
             * @param headerBehavior Determines whether any HTTP headers are included in the cache
    ...
    public interface ParametersInCacheKeyAndForwardedToOriginProperty {
         ...
         *
         * If you set this value to `true` , and this cache behavior also has an origin request
         * policy attached, do not include the `Accept-Encoding` header in the origin request
         * policy. CloudFront always includes the `Accept-Encoding` header in origin requests when
         * the value of this field is `true` , so including this header in an origin request policy
         * has no effect.
         *
         * If both of these fields are `false` , then CloudFront treats the `Accept-Encoding` header
         * the same as any other HTTP header in the viewer request. By default, it's not included in
         * the cache key and it's not included in origin requests. In this case, you can manually
         * add `Accept-Encoding` to the headers whitelist like any other HTTP header.
         *
         ...
         * which query strings) are included in the cache key and in requests that CloudFront sends
         * to the origin.
         *
        ...
        public interface Builder {
            /**
             * @param cookiesConfig An object that determines whether any cookies in viewer requests
             *   (and if so, which cookies) are included in the cache key and in requests that
             *   CloudFront sends to the origin.
             */
            ...
            /**
             * @param enableAcceptEncodingBrotli A flag that can affect whether the
             *   `Accept-Encoding` HTTP header is included in the cache key and included in requests
             *   that CloudFront sends to the origin. This field is related to the
             *   `EnableAcceptEncodingGzip` field. If one or both of these fields is `true` *and*
             *   the viewer request includes the `Accept-Encoding` header, then CloudFront does the
             *   following:
             * * Normalizes the value of the viewer's `Accept-Encoding` header
             * * Includes the normalized header in the cache key
             ...
             * If you set this value to `true` , and this cache behavior also has an origin request
             * policy attached, do not include the `Accept-Encoding` header in the origin request
             * policy. CloudFront always includes the `Accept-Encoding` header in origin requests
             * when the value of this field is `true` , so including this header in an origin
             ...
             * included in the cache key and it's not included in origin requests. In this case, you
             * can manually add `Accept-Encoding` to the headers whitelist like any other HTTP
             * header.
             */
             ...
             *   HTTP header is included in the cache key and included in requests that CloudFront
             *   sends to the origin. This field is related to the `EnableAcceptEncodingBrotli`
             *   field. If one or both of these fields is `true` *and* the viewer request includes
             *   the `Accept-Encoding` header, then CloudFront does the following:
             * * Normalizes the value of the viewer's `Accept-Encoding` header
             * * Includes the normalized header in the cache key
             * * Includes the normalized header in the request to the origin, if a request is
             *   necessary
             ...
             *   which headers) are included in the cache key and in requests that CloudFront sends
             *   to the origin.
             */
            ...
            /**
             * @param queryStringsConfig An object that determines whether any URL query strings in
             *   viewer requests (and if so, which query strings) are included in the cache key and
        ...
        private class BuilderImpl : Builder {
            ...
            override fun enableAcceptEncodingBrotli(enableAcceptEncodingBrotli: IResolvable) {
                cdkBuilder.enableAcceptEncodingBrotli(
                    enableAcceptEncodingBrotli.let(IResolvable::unwrap)
                )
            }
            ...
            override fun enableAcceptEncodingGzip(enableAcceptEncodingGzip: IResolvable) {
                cdkBuilder.enableAcceptEncodingGzip(
                    enableAcceptEncodingGzip.let(IResolvable::unwrap)
                )
            }
        ...
        internal constructor(
            ...
            /**
             * A flag that can affect whether the `Accept-Encoding` HTTP header is included in the
             * cache key and included in requests that CloudFront sends to the origin.
             *
             ...
             * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-cloudfront-cachepolicy-parametersincachekeyandforwardedtoorigin.html#cfn-cloudfront-cachepolicy-parametersincachekeyandforwardedtoorigin-enableacceptencodingbrotli)
             */
            override fun enableAcceptEncodingBrotli(): Any? =
             ...
             * cache key and included in requests that CloudFront sends to the origin.
             *
             * This field is related to the `EnableAcceptEncodingBrotli` field. If one or both of
             * these fields is `true` *and* the viewer request includes the `Accept-Encoding`
             * header, then CloudFront does the following:
             * * Normalizes the value of the viewer's `Accept-Encoding` header
             * * Includes the normalized header in the cache key
             ...
             * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-cloudfront-cachepolicy-parametersincachekeyandforwardedtoorigin.html#cfn-cloudfront-cachepolicy-parametersincachekeyandforwardedtoorigin-enableacceptencodinggzip)
             */
            override fun enableAcceptEncodingGzip(): Any =
             ...
             * so, which query strings) are included in the cache key and in requests that
             * CloudFront sends to the origin.
             *
             * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-cloudfront-cachepolicy-parametersincachekeyandforwardedtoorigin.html#cfn-cloudfront-cachepolicy-parametersincachekeyandforwardedtoorigin-querystringsconfig)
             */
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: ParametersInCacheKeyAndForwardedToOriginProperty
            ): software.amazon.awscdk.services.cloudfront.CfnCachePolicy.ParametersInCacheKeyAndForwardedToOriginProperty =
                (wrapped as Wrapper).cdkObject
        }
    }
    ...
    public interface CookiesConfigProperty {
         ...
         *
         * Valid values are:
         * * `none` – No cookies in viewer requests are included in the cache key or in requests
         *   that CloudFront sends to the origin. Even when this field is set to `none` , any
         ...
         *   requests that CloudFront sends to the origin, **except** for those that are listed in
         *   the `CookieNames` type, which are not included.
         * * `all` – All cookies in viewer requests are included in the cache key and in requests
         *   that CloudFront sends to the origin.
         *
        ...
        public interface Builder {
            /**
             * @param cookieBehavior Determines whether any cookies in viewer requests are included
             *   in the cache key and in requests that CloudFront sends to the origin. Valid values
             *   are:
             * * `none` – No cookies in viewer requests are included in the cache key or in requests
             *   that CloudFront sends to the origin. Even when this field is set to `none` , any
             *   cookies that are listed in an `OriginRequestPolicy` *are* included in origin
             *   requests.
             ...
             *   sends to the origin.
             * * `allExcept` – All cookies in viewer requests are included in the cache key and in
             *   requests that CloudFront sends to the origin, **except** for those that are listed
             *   in the `CookieNames` type, which are not included.
    ...
    public interface QueryStringsConfigProperty {
         ...
         *
         * Valid values are:
         * * `none` – No query strings in viewer requests are included in the cache key or in
         *   requests that CloudFront sends to the origin. Even when this field is set to `none` ,
         *   any query strings that are listed in an `OriginRequestPolicy` *are* included in origin
         *   requests.
         * * `whitelist` – Only the query strings in viewer requests that are listed in the
         *   `QueryStringNames` type are included in the cache key and in requests that CloudFront
         *   sends to the origin.
        ...
        public interface Builder {
             ...
             *   requests are included in the cache key and in requests that CloudFront sends to the
             *   origin. Valid values are:
             * * `none` – No query strings in viewer requests are included in the cache key or in
             *   requests that CloudFront sends to the origin. Even when this field is set to `none`
             *   , any query strings that are listed in an `OriginRequestPolicy` *are* included in
             *   origin requests.
             * * `whitelist` – Only the query strings in viewer requests that are listed in the
             *   `QueryStringNames` type are included in the cache key and in requests that
             *   CloudFront sends to the origin.
             * * `allExcept` – All query strings in viewer requests are included in the cache key
             *   and in requests that CloudFront sends to the origin, **except** those that are
    ...
    public interface CachePolicyConfigProperty {
         ...
         * *not* send `Cache-Control` or `Expires` headers with the object. For more information,
         * see
         ...
         * more than 86400 seconds, then the default value for this field is the same as the value
         * of `MinTTL` .
         *
         ...
         *
         * CloudFront uses this value only when the origin sends `Cache-Control` or `Expires`
         ...
         * or `DefaultTTL` is more than 31536000 seconds, then the default value for this field is
         ...
         *
         * The values included in the cache key are also included in requests that CloudFront sends
         * to the origin.
         *
        ...
        public interface Builder {
            /**
             * @param comment A comment to describe the cache policy. The comment cannot be longer
             *   than 128 characters.
             */
            public fun comment(comment: String)

            /**
             * @param defaultTtl The default amount of time, in seconds, that you want objects to
             *   stay in the CloudFront cache before CloudFront sends another request to the origin
             *   to see if the object has been updated. CloudFront uses this value as the object's
             *   time to live (TTL) only when the origin does *not* send `Cache-Control` or
             *   `Expires` headers with the object. For more information, see
             *   [Managing How Long Content Stays in an Edge Cache (Expiration)](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/Expiration.html)
             *   in the *Amazon CloudFront Developer Guide* .
             *
            ...
            /**
             * @param maxTtl The maximum amount of time, in seconds, that objects stay in the
             *   CloudFront cache before CloudFront sends another request to the origin to see if
             *   the object has been updated. CloudFront uses this value only when the origin sends
             ...
             * `MinTTL` or `DefaultTTL` is more than 31536000 seconds, then the default value for
             * this field is the same as the value of `DefaultTTL` .
             */
            public fun maxTtl(maxTtl: Number)

            /**
             * @param minTtl The minimum amount of time, in seconds, that you want objects to stay
             *   in the CloudFront cache before CloudFront sends another request to the origin to
             *   see if the object has been updated. For more information, see
             *   [Managing How Long Content Stays in an Edge Cache (Expiration)](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/Expiration.html)
             *   in the *Amazon CloudFront Developer Guide* .
             */
            ...
            /**
             * @param parametersInCacheKeyAndForwardedToOrigin The HTTP headers, cookies, and URL
             *   query strings to include in the cache key. The values included in the cache key are
             *   also included in requests that CloudFront sends to the origin.
             */
            ...
            public fun parametersInCacheKeyAndForwardedToOrigin(
                parametersInCacheKeyAndForwardedToOrigin:
                    ParametersInCacheKeyAndForwardedToOriginProperty.Builder.() -> Unit
            )
        }
        ...
        private class BuilderImpl : Builder {
             ...
             *   also included in requests that CloudFront sends to the origin.
             */
            override fun parametersInCacheKeyAndForwardedToOrigin(
                parametersInCacheKeyAndForwardedToOrigin: IResolvable
            ) {
                cdkBuilder.parametersInCacheKeyAndForwardedToOrigin(
                    parametersInCacheKeyAndForwardedToOrigin.let(IResolvable::unwrap)
                )
            }
            ...
            override fun parametersInCacheKeyAndForwardedToOrigin(
                ...
                cdkBuilder.parametersInCacheKeyAndForwardedToOrigin(
                    parametersInCacheKeyAndForwardedToOrigin.let(
                        ParametersInCacheKeyAndForwardedToOriginProperty::unwrap
                    )
                )
            }
        ...
        internal constructor(
             ...
             * CloudFront cache before CloudFront sends another request to the origin to see if the
             * object has been updated.
             *
             * CloudFront uses this value as the object's time to live (TTL) only when the origin
             * does *not* send `Cache-Control` or `Expires` headers with the object. For more
             * information, see
             * [Managing How Long Content Stays in an Edge Cache (Expiration)](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/Expiration.html)
             * in the *Amazon CloudFront Developer Guide* .
             *
             ...
             * The HTTP headers, cookies, and URL query strings to include in the cache key.
             *
             * The values included in the cache key are also included in requests that CloudFront
             * sends to the origin.
             *
             * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-cloudfront-cachepolicy-cachepolicyconfig.html#cfn-cloudfront-cachepolicy-cachepolicyconfig-parametersincachekeyandforwardedtoorigin)
             */
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: CachePolicyConfigProperty
            ): software.amazon.awscdk.services.cloudfront.CfnCachePolicy.CachePolicyConfigProperty =
                (wrapped as Wrapper).cdkObject
        }
    }
