...
internal constructor(
    private val cdkObject: software.amazon.awscdk.services.waf.CfnWebACL,
    ...
    public interface ActivatedRuleProperty {
        /**
         * Specifies the action that Amazon CloudFront or AWS WAF takes when a web request matches
         ...
         * The value must be a unique integer. If you add multiple `Rules` to a `WebACL` , the
         ...
         *
         * You use `RuleId` to get more information about a `Rule` , update a `Rule` , insert a
         * `Rule` into a `WebACL` or delete a one from a `WebACL` , or delete a `Rule` from AWS WAF
         * .
         *
        ...
        public interface Builder {
             ...
             *   request matches the conditions in the `Rule` . Valid values for `Action` include
             *   the following:
             * * `ALLOW` : CloudFront responds with the requested object.
             * * `BLOCK` : CloudFront responds with an HTTP 403 (Forbidden) status code.
             * * `COUNT` : AWS WAF increments a counter of requests that match the conditions in the
             *   rule and then continues to inspect the web request based on the remaining rules in
             *   the web ACL.
             *
             ...
             *   Rules with a lower value for `Priority` are evaluated before `Rules` with a higher
            ...
            /**
             * @param ruleId The `RuleId` for a `Rule` . You use `RuleId` to get more information
             *   about a `Rule` , update a `Rule` , insert a `Rule` into a `WebACL` or delete a one
             *   from a `WebACL` , or delete a `Rule` from AWS WAF .
             *
             * `RuleId` is returned by `CreateRule` and by `ListRules` .
             */
        ...
        internal constructor(
            ...
            /**
             * Specifies the action that Amazon CloudFront or AWS WAF takes when a web request
             ...
             *
             * Rules with a lower value for `Priority` are evaluated before `Rules` with a higher
             * value. The value must be a unique integer. If you add multiple `Rules` to a `WebACL`
             ...
             *
             * You use `RuleId` to get more information about a `Rule` , update a `Rule` , insert a
             * `Rule` into a `WebACL` or delete a one from a `WebACL` , or delete a `Rule` from AWS
             * WAF .
             *
             * `RuleId` is returned by `CreateRule` and by `ListRules` .
             *
    ...
    public interface WafActionProperty {
        ...
        public interface Builder {
            /**
             * @param type Specifies how you want AWS WAF to respond to requests that match the
