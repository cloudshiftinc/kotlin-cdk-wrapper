...
internal constructor(
    ...
    /**
     * (deprecated) The total number of concurrent TCP flows (or connections) from clients to
     * targets.
     *
     * This metric includes connections in the SYN_SENT and ESTABLISHED states. TCP connections are
     * not terminated at the load balancer, so a client opening a TCP connection to a target counts
     * as a single flow.
     *
     * Default: Average over 5 minutes
     *
     ...
     */
    @Deprecated(message = "deprecated in CDK")
    public open fun metricNewFlowCount(): Metric =
        unwrap(this).metricNewFlowCount().let(Metric::wrap)
    ...
    /**
     * After the implementation of `IConnectable` (see https://github.com/aws/aws-cdk/pull/28494),
     * the default value for `securityGroups` is set by the `ec2.Connections` constructor to an
     * empty array. To keep backward compatibility (`securityGroups` is `undefined` if the related
     * property is not specified) a getter has been added.
     */
    public override fun securityGroups(): List<String> =
    ...
    public companion object {
        public fun fromLookup(
            ...
            software.amazon.awscdk.services.elasticloadbalancingv2.NetworkLoadBalancer.fromLookup(
                    scope.let(CloudshiftdevConstructsConstruct::unwrap),
                    id,
                    options.let(NetworkLoadBalancerLookupOptions::unwrap)
                )
                .let(INetworkLoadBalancer::wrap)
        ...
        public fun fromNetworkLoadBalancerAttributes(
            ...
            software.amazon.awscdk.services.elasticloadbalancingv2.NetworkLoadBalancer
                .fromNetworkLoadBalancerAttributes(
                    scope.let(CloudshiftdevConstructsConstruct::unwrap),
                    id,
                    attrs.let(NetworkLoadBalancerAttributes::unwrap)
                )
                .let(INetworkLoadBalancer::wrap)
        ...
        public fun fromNetworkLoadBalancerAttributes(
            ...
            fromNetworkLoadBalancerAttributes(scope, id, NetworkLoadBalancerAttributes(attrs))
