...
internal constructor(
     ...
     * @param _scope
     */
    public override fun bindAsNotificationRuleSource(
        _scope: CloudshiftdevConstructsConstruct
    ): NotificationRuleSourceConfig =
        unwrap(this)
            .bindAsNotificationRuleSource(_scope.let(CloudshiftdevConstructsConstruct::unwrap))
            .let(NotificationRuleSourceConfig::wrap)
     ...
     */
    public open fun bindToCodePipeline(
        _scope: CloudshiftdevConstructsConstruct,
    ...
    public open fun bindToCodePipeline(
        _scope: CloudshiftdevConstructsConstruct,
        options: BindToCodePipelineOptions.Builder.() -> Unit
    ): Unit = bindToCodePipeline(_scope, BindToCodePipelineOptions(options))
     ...
     *
     * You can also use the methods `notifyOnBuildSucceeded` and `notifyOnBuildFailed` to define
     * rules for these specific event emitted.
     *
     * @param id
     * @param target
     * @param options
    ...
    public override fun notifyOn(
        ...
        unwrap(this)
            .notifyOn(
                ...
                options.let(ProjectNotifyOnOptions::unwrap)
            )
    ...
    public override fun notifyOnBuildFailed(
        ...
        unwrap(this)
            .notifyOnBuildFailed(id, target.let(INotificationRuleTarget::unwrap))
            .let(INotificationRule::wrap)
    ...
    public override fun notifyOnBuildFailed(
        ...
        unwrap(this)
            .notifyOnBuildFailed(
                id,
                target.let(INotificationRuleTarget::unwrap),
                options.let(NotificationRuleOptions::unwrap)
            )
            .let(INotificationRule::wrap)
    ...
    public override fun notifyOnBuildSucceeded(
        ...
        unwrap(this)
            .notifyOnBuildSucceeded(id, target.let(INotificationRuleTarget::unwrap))
            .let(INotificationRule::wrap)
    ...
    public override fun onBuildFailed(
        id: String,
        options: OnEventOptions.Builder.() -> Unit
    ): Rule = onBuildFailed(id, OnEventOptions(options))
    ...
    public override fun onBuildStarted(
        id: String,
        options: OnEventOptions.Builder.() -> Unit
    ): Rule = onBuildStarted(id, OnEventOptions(options))
    ...
    public override fun onBuildSucceeded(
        id: String,
        options: OnEventOptions.Builder.() -> Unit
    ): Rule = onBuildSucceeded(id, OnEventOptions(options))
    ...
    public override fun onPhaseChange(
        id: String,
        options: OnEventOptions.Builder.() -> Unit
    ): Rule = onPhaseChange(id, OnEventOptions(options))

    /**
     * Defines a CloudWatch event rule triggered when the build project state changes.
     *
     * You can filter specific build status events using an event pattern filter on the
     * `build-status` detail field:
     *
     * const rule = project.onStateChange('OnBuildStarted', { target }); rule.addEventPattern({
     * detail: { 'build-status': [ "IN_PROGRESS", "SUCCEEDED", "FAILED", "STOPPED" ] } });
     *
    ...
    public override fun onStateChange(
        id: String,
        options: OnEventOptions.Builder.() -> Unit
    ): Rule = onStateChange(id, OnEventOptions(options))
    ...
    public interface Builder {
         ...
         *
         * If set to false, you must individually add traffic rules to allow the CodeBuild project
         * to connect to network targets.
         *
        ...
        /**
         * Indicates whether AWS CodeBuild generates a publicly accessible URL for your project's
         * build badge.
         *
        ...
        /**
         * Whether to check for the presence of any secrets in the environment variables of the
         * default type, BuildEnvironmentVariableType.PLAINTEXT. Since using a secret for the value
         * of that kind of variable would result in it being displayed in plain text in the AWS
         * Console, the construct will throw an exception if it detects a secret was passed there.
         * Pass this property as false if you want to skip this validation, and keep using a secret
         * in a plain text environment variable.
         *
         * Default: true
         *
         * @param checkSecretsInPlainTextEnvVariables Whether to check for the presence of any
         *   secrets in the environment variables of the default type,
         *   BuildEnvironmentVariableType.PLAINTEXT. Since using a secret for the value of that kind
         *   of variable would result in it being displayed in plain text in the AWS Console, the
         *   construct will throw an exception if it detects a secret was passed there. Pass this
         *   property as false if you want to skip this validation, and keep using a secret in a
         ...
         *
         * @param environmentVariables Additional environment variables to add to the build
         *   environment.
         */
         ...
         * harmless, but you can turn these off if you don't plan on using test reports in this
         * project.
         *
         * Default: true
         *
         ...
         *
         * @param grantReportGroupPermissions Add permissions to this project's role to create and
         *   use test report groups with name starting with the name of this project.
         */
         ...
         *
         * @param queuedTimeout The number of minutes after which AWS CodeBuild stops the build if
         *   it's still in queue.
         */
         ...
         *
         * @param securityGroups What security group to associate with the codebuild project's
         *   network interfaces.
         */
         ...
         *   set to true.
         *
         * Then the build container can be paused and inspected using Session Manager by invoking
         * the `codebuild-breakpoint` command somewhere during the build.
         *
         ...
         *
         * To access AWS services, your CodeBuild project needs to be in one of the following types
         * of subnets:
         ...
         *
         * If you don't specify a subnet selection, the default behavior is to use
         * PRIVATE_WITH_EGRESS subnets first if they exist, then PRIVATE_WITHOUT_EGRESS, and finally
         * PUBLIC subnets. If your VPC doesn't have PRIVATE_WITH_EGRESS subnets but you need AWS
         * service access, add VPC Endpoints to your private subnets.
         *
    ...
    private class BuilderImpl(
         ...
         *   plain text environment variable.
         */
        override fun checkSecretsInPlainTextEnvVariables(
        ...
        override fun environmentVariables(
            ...
            cdkBuilder.environmentVariables(
                environmentVariables.mapValues { BuildEnvironmentVariable.unwrap(it.value) }
            )
        }
    ...
    public companion object {
        public fun fromProjectArn(
            ...
            software.amazon.awscdk.services.codebuild.Project.fromProjectArn(
        ...
        public fun fromProjectName(
            ...
            software.amazon.awscdk.services.codebuild.Project.fromProjectName(
                    scope.let(CloudshiftdevConstructsConstruct::unwrap),
                    id,
                    projectName
                )
        ...
        public fun serializeEnvVariables(
            ...
            software.amazon.awscdk.services.codebuild.Project.serializeEnvVariables(
                    environmentVariables.mapValues { BuildEnvironmentVariable.unwrap(it.value) }
                )
                .map(CfnProject.EnvironmentVariableProperty::wrap)
        ...
        public fun serializeEnvVariables(
            ...
            software.amazon.awscdk.services.codebuild.Project.serializeEnvVariables(
                    environmentVariables.mapValues { BuildEnvironmentVariable.unwrap(it.value) },
                    validateNoPlainTextSecrets,
                    principal.let(IGrantable::unwrap)
                )
                .map(CfnProject.EnvironmentVariableProperty::wrap)
