...
internal constructor(
    ...
    public interface Builder {
         ...
         * will cache authorizer responses. If this field is not set, the default value is 300. The
         ...
         * is required when authorization caching is enabled. The value is a comma-separated string
         * of one or more mapping expressions of the specified request parameters. For example, if
         * an `Auth` header, a `Name` query string parameter are defined as identity sources, this
         * value is `method.request.header.Auth, method.request.querystring.Name` . These parameters
         * will be used to derive the authorization caching key and to perform runtime validation of
         ...
         *
         * @param providerArns A list of the Amazon Cognito user pool ARNs for the
         ...
         * The authorizer type.
         *
         * Valid values are `TOKEN` for a Lambda function using a single authorization token
         * submitted in a custom header, `REQUEST` for a Lambda function using incoming request
