...
internal constructor(
    ...
    public interface Builder {
         ...
         * .
         *
         * CloudFormation automatically shifts traffic from the version an alias currently points
         * to, to a new state machine version that you specify.
         *
         * `RoutingConfiguration` and `DeploymentPreference` are mutually exclusive properties. You
         * must define only one of these properties.
         *
         * Based on the type of deployment you want to perform, you can specify one of the following
         * settings:
         * * `LINEAR` - Shifts traffic to the new version in equal increments with an equal number
         ...
         * version receives 100 percent of the traffic. This deployment immediately rolls back the
         * new version if any Amazon CloudWatch alarms are triggered.
         ...
         * to the new version. In the second increment, before a specified time interval in seconds
         * gets over, the remaining traffic is shifted to the new version. The shift to the new
         * version for the remaining traffic takes place only if no CloudWatch alarms are triggered
         ...
         * The name of the state machine alias.
         *
         * If you don't provide a name, it uses an automatically generated name based on the logical
         * ID.
         *
    ...
    private class BuilderImpl(
        ...
        private val cdkBuilder:
            ...
            software.amazon.awscdk.services.stepfunctions.CfnStateMachineAlias.Builder.create(
                scope,
                id
            )
    ...
    public interface RoutingConfigurationVersionProperty {
        ...
        public interface Builder {
            /**
             * @param stateMachineVersionArn The Amazon Resource Name (ARN) that identifies one or
             *   two state machine versions defined in the routing configuration. If you specify the
             *   ARN of a second version, it must belong to the same state machine as the first
             *   version.
             */
            public fun stateMachineVersionArn(stateMachineVersionArn: String)

            /**
             * @param weight The percentage of traffic you want to route to the state machine
             *   version. The sum of the weights in the routing configuration must be equal to 100.
             */
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: RoutingConfigurationVersionProperty
            ): software.amazon.awscdk.services.stepfunctions.CfnStateMachineAlias.RoutingConfigurationVersionProperty =
    ...
    public interface DeploymentPreferenceProperty {
         ...
         * resource that will be the final version to which the alias points to when the traffic
         ...
         * minutes, this deployment increases traffic by 20 percent every 600 minutes until the new
         * version receives 100 percent of the traffic. This deployment immediately rolls back the
         * new version if any CloudWatch alarms are triggered.
         * * `ALL_AT_ONCE` - Shifts 100 percent of traffic to the new version immediately.
         *   CloudFormation monitors the new version and rolls it back automatically to the previous
         *   version if any CloudWatch alarms are triggered.
         * * `CANARY` - Shifts traffic in two increments.
        ...
        public interface Builder {
            /**
             * @param alarms A list of Amazon CloudWatch alarms to be monitored during the
             *   deployment. The deployment fails and rolls back if any of these alarms go into the
             *   `ALARM` state.
             */
            ...
            /**
             * @param percentage The percentage of traffic to shift to the new version in each
             *   increment.
             */
             ...
             *   [`AWS::StepFunctions::StateMachineVersion`](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-stepfunctions-statemachineversion.html)
             *   resource that will be the final version to which the alias points to when the
             *   traffic shifting is complete. While performing gradual deployments, you can only
             *   provide a single state machine version ARN. To explicitly set version weights in a
             *   CloudFormation template, use `RoutingConfiguration` instead.
             */
            public fun stateMachineVersionArn(stateMachineVersionArn: String)
             ...
             *   following types:.
             * * `LINEAR` - Shifts traffic to the new version in equal increments with an equal
             *   number of minutes between each increment.
             *
             * For example, if you specify the increment percent as `20` with an interval of `600`
             * minutes, this deployment increases traffic by 20 percent every 600 minutes until the
             * new version receives 100 percent of the traffic. This deployment immediately rolls
             * back the new version if any CloudWatch alarms are triggered.
             * * `ALL_AT_ONCE` - Shifts 100 percent of traffic to the new version immediately.
             *   CloudFormation monitors the new version and rolls it back automatically to the
             *   previous version if any CloudWatch alarms are triggered.
             * * `CANARY` - Shifts traffic in two increments.
             *
             * In the first increment, a small percentage of traffic, for example, 10 percent is
             * shifted to the new version. In the second increment, before a specified time interval
             * in seconds gets over, the remaining traffic is shifted to the new version. The shift
             * to the new version for the remaining traffic takes place only if no CloudWatch alarms
             * are triggered during the specified time interval.
             */
        ...
        internal constructor(
             ...
             * shifting is complete.
             *
             * While performing gradual deployments, you can only provide a single state machine
             * version ARN. To explicitly set version weights in a CloudFormation template, use
             * `RoutingConfiguration` instead.
             *
             * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-stepfunctions-statemachinealias-deploymentpreference.html#cfn-stepfunctions-statemachinealias-deploymentpreference-statemachineversionarn)
             */
            ...
            /**
             * The type of deployment you want to perform. You can specify one of the following
             * types:.
             * * `LINEAR` - Shifts traffic to the new version in equal increments with an equal
             *   number of minutes between each increment.
             *
