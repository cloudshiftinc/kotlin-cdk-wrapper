...
public interface BuildEnvironment {
    ...
    public fun environmentVariables(): Map<String, BuildEnvironmentVariable> =
        unwrap(this).getEnvironmentVariables()?.mapValues {
            BuildEnvironmentVariable.wrap(it.value)
        } ?: emptyMap()

    /**
     * Indicates how the project builds Docker images.
     *
     * Specify true to enable running the Docker daemon inside a Docker container. This value must
     * be set to true only if this build project will be used to build Docker images, and the
     * specified build environment image is not one provided by AWS CodeBuild with Docker support.
    ...
    public interface Builder {
        ...
        /**
         * @param computeType The type of compute to use for this build. See the `ComputeType` enum
         ...
         *   running the Docker daemon inside a Docker container. This value must be set to true
         *   only if this build project will be used to build Docker images, and the specified build
         *   environment image is not one provided by AWS CodeBuild with Docker support. Otherwise,
         *   all associated builds that attempt to interact with the Docker daemon will fail.
         */
    ...
    private class BuilderImpl : Builder {
        ...
        override fun certificate(
            certificate: BuildEnvironmentCertificate.Builder.() -> Unit
    ...
    internal constructor(
         ...
         * Default: LinuxBuildImage.STANDARD_1_0
         */
        override fun buildImage(): IBuildImage? =
         ...
         * Default: taken from `#buildImage#defaultComputeType`
         */
        override fun computeType(): ComputeType? =
         ...
         * Indicates how the project builds Docker images.
         *
         * Specify true to enable running the Docker daemon inside a Docker container. This value
         * must be set to true only if this build project will be used to build Docker images, and
         * the specified build environment image is not one provided by AWS CodeBuild with Docker
         * support. Otherwise, all associated builds that attempt to interact with the Docker daemon
         * will fail.
         *
         * Default: false
         */
