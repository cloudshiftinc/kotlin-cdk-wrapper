...
internal constructor(
    ...
    public open fun criteria(
        `value`: AutomationRulesFindingFiltersProperty.Builder.() -> Unit
    ): Unit = criteria(AutomationRulesFindingFiltersProperty(`value`))
    ...
    public interface Builder {
         ...
         * A set of
         * [AWS Security Finding Format (ASFF)](https://docs.aws.amazon.com/securityhub/latest/userguide/securityhub-findings-format.html)
         * finding field attributes and corresponding expected values that Security Hub uses to
         * filter findings. If a rule is enabled and a finding matches the criteria specified in
        ...
        /**
         * Specifies whether a rule is the last to be applied with respect to a finding that matches
         * the rule criteria.
         *
         * This is useful when a finding matches the criteria for multiple rules, and each rule has
         * different actions. If a rule is terminal, Security Hub applies the rule action to a
         * finding that matches the rule criteria and doesn't evaluate other rules for the finding.
         * By default, a rule isn't terminal.
         *
    ...
    private class BuilderImpl(
        ...
        @Suppress("INAPPLICABLE_JVM_NAME")
        @JvmName("135935e2b9aeab7dd2476700f8acf596a520fc2014f76071e71ae8c8b1b8754b")
        override fun criteria(
            criteria: AutomationRulesFindingFiltersProperty.Builder.() -> Unit
        ): Unit = criteria(AutomationRulesFindingFiltersProperty(criteria))
    ...
    public interface SeverityUpdateProperty {
        ...
        public interface Builder {
             ...
             *   deprecated in favor of `Label` . If you provide `Normalized` and do not provide
            ...
            /**
             * @param product The native severity as defined by the AWS service or integrated
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: SeverityUpdateProperty
            ): software.amazon.awscdk.services.securityhub.CfnAutomationRule.SeverityUpdateProperty =
    ...
    public interface MapFilterProperty {
         ...
         * operators:
         * * To search for values that include the filter value, use `CONTAINS` . For example, for
         *   the `ResourceTags` field, the filter `Department CONTAINS Security` matches findings
         ...
         *   for the `ResourceTags` field, the filter `Department EQUALS Security` matches findings
         *   that have the value `Security` for the `Department` tag.
         *
         * `CONTAINS` and `EQUALS` filters on the same field are joined by `OR` . A finding matches
         * if it matches any one of those filters. For example, the filters `Department CONTAINS
         * Security OR Department CONTAINS Finance` match a finding that includes either `Security`
         * , `Finance` , or both values.
         *
         * To search for values that don't have the filter value, use one of the following
         * comparison operators:
         * * To search for values that exclude the filter value, use `NOT_CONTAINS` . For example,
         *   for the `ResourceTags` field, the filter `Department NOT_CONTAINS Finance` matches
         *   findings that exclude the value `Finance` for the `Department` tag.
         * * To search for values other than the filter value, use `NOT_EQUALS` . For example, for
         *   the `ResourceTags` field, the filter `Department NOT_EQUALS Finance` matches findings
         ...
         * NOT_CONTAINS Security AND Department NOT_CONTAINS Finance` match a finding that excludes
         ...
         * `Department` might be `Security` . If you provide `security` as the filter value, then
         * there's no match.
         *
        ...
        public interface Builder {
             ...
             *   findings with a map filter. To search for values that have the filter value, use
             *   one of the following comparison operators:
             * * To search for values that include the filter value, use `CONTAINS` . For example,
             *   for the `ResourceTags` field, the filter `Department CONTAINS Security` matches
             *   findings that include the value `Security` for the `Department` tag. In the same
             *   example, a finding with a value of `Security team` for the `Department` tag is a
             *   match.
             ...
             * matches if it matches any one of those filters. For example, the filters `Department
             * CONTAINS Security OR Department CONTAINS Finance` match a finding that includes
             * either `Security` , `Finance` , or both values.
             *
             * To search for values that don't have the filter value, use one of the following
             * comparison operators:
             * * To search for values that exclude the filter value, use `NOT_CONTAINS` . For
             *   example, for the `ResourceTags` field, the filter `Department NOT_CONTAINS Finance`
             *   matches findings that exclude the value `Finance` for the `Department` tag.
             ...
             *   findings that don’t have the value `Finance` for the `Department` tag.
             *
             ...
             * `Department NOT_CONTAINS Security AND Department NOT_CONTAINS Finance` match a
             * finding that excludes both the `Security` and `Finance` values.
             *
             ...
             * field. Similarly, you can’t have both an `EQUALS` filter and a `NOT_EQUALS` filter on
            ...
            /**
             * @param key The key of the map filter. For example, for `ResourceTags` , `Key`
             *   identifies the name of the tag. For `UserDefinedFields` , `Key` is the name of the
             *   field.
             */
            public fun key(key: String)

            /**
             * @param value The value for the key in the map filter. Filter values are case
             *   sensitive. For example, one of the values for a tag called `Department` might be
             *   `Security` . If you provide `security` as the filter value, then there's no match.
             */
        ...
        internal constructor(
            ...
            /**
             * The condition to apply to the key value when filtering Security Hub findings with a
             * map filter.
             *
             * To search for values that have the filter value, use one of the following comparison
             * operators:
             ...
             *
             * Filter values are case sensitive. For example, one of the values for a tag called
             * `Department` might be `Security` . If you provide `security` as the filter value,
    ...
    public interface WorkflowUpdateProperty {
         ...
         *
         * The workflow status is specific to an individual finding. It does not affect the
         * generation of new findings. For example, setting the workflow status to `SUPPRESSED` or
         ...
         *   Used when the initial reviewer is not the resource owner, and needs intervention from
         *   the resource owner.
         * * `RESOLVED` - The finding was reviewed and remediated and is now considered resolved.
         * * `SUPPRESSED` - Indicates that you reviewed the finding and do not believe that any
        ...
        public interface Builder {
            /**
             * @param status The status of the investigation into the finding. The workflow status
             *   is specific to an individual finding. It does not affect the generation of new
             ...
             * * `NEW` - The initial state of a finding, before it is reviewed.
             *
             * Security Hub also resets `WorkFlowStatus` from `NOTIFIED` or `RESOLVED` to `NEW` in
             * the following cases:
             * * The record state changes from `ARCHIVED` to `ACTIVE` .
             * * The compliance status changes from `PASSED` to either `WARNING` , `FAILED` , or
             *   `NOT_AVAILABLE` .
        ...
        internal constructor(
             ...
             *
             * The workflow status is specific to an individual finding. It does not affect the
             * generation of new findings. For example, setting the workflow status to `SUPPRESSED`
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: WorkflowUpdateProperty
            ): software.amazon.awscdk.services.securityhub.CfnAutomationRule.WorkflowUpdateProperty =
                (wrapped as Wrapper).cdkObject
        }
    }
    ...
    public interface AutomationRulesFindingFiltersProperty {
         ...
         *
         * This field consists of the resource portion of the Amazon Resource Name (ARN) returned
         * for a standard in the
         ...
         */
        public fun complianceSecurityControlId(): Any? =
         ...
         * percent confidence, and a value of `100` means 100 percent confidence. For example, a
         * data exfiltration detection based on a statistical deviation of network traffic has low
         * confidence because an actual exfiltration hasn't been verified. For more information, see
         ...
         * score of `100` is reserved for the most critical resources. For more information, see
        ...
        /**
         * A timestamp that indicates when the potential security issue captured by a finding was
         * most recently observed by the security findings product.
         *
         * Uses the `date-time` format specified in
         * [RFC 3339 section 5.6, Internet Date/Time Format](https://docs.aws.amazon.com/https://tools.ietf.org/html/rfc3339#section-5.6)
         * . The value cannot contain spaces. For example, `2020-03-22T13:22:13.933Z` .
         ...
         * For AWS resources that lack ARNs, this is the identifier as defined by the AWS service
         * that created the resource. For non- AWS resources, this is a unique identifier that is
         ...
         * finding.
         *
         * For a list of namespaces, classifiers, and categories, see
        ...
        public interface Builder {
            /**
             * @param awsAccountId The AWS account ID in which a finding was generated. Array
            ...
            /**
             * @param companyName The name of the company for the product that generated the
             *   finding. For control-based findings, the company is AWS .
             *
             * Array Members: Minimum number of 1 item. Maximum number of 20 items.
             */
             ...
             *   control is enabled. This field consists of the resource portion of the Amazon
             *   Resource Name (ARN) returned for a standard in the
             *   [DescribeStandards](https://docs.aws.amazon.com/securityhub/1.0/APIReference/API_DescribeStandards.html)
             *   API response.
             *
             * Array Members: Minimum number of 1 item. Maximum number of 20 items.
             ...
             *   generated. Security control IDs are the same across standards. Array Members:
             ...
             *   findings generated from controls. Array Members: Minimum number of 1 item. Maximum
             *   number of 20 items.
             */
            ...
            /**
             * @param confidence The likelihood that a finding accurately identifies the behavior or
             *   issue that it was intended to identify. `Confidence` is scored on a 0–100 basis
             *   using a ratio scale. A value of `0` means 0 percent confidence, and a value of
             *   `100` means 100 percent confidence. For example, a data exfiltration detection
             ...
             *   associated with a finding. `Criticality` is scored on a 0–100 basis, using a ratio
             *   scale that supports only full integers. A score of `0` means that the underlying
             *   resources have no criticality, and a score of `100` is reserved for the most
             *   critical resources. For more information, see
             *   [Criticality](https://docs.aws.amazon.com/securityhub/latest/userguide/asff-top-level-attributes.html#asff-criticality)
             *   in the *AWS Security Hub User Guide* .
             *
             * Array Members: Minimum number of 1 item. Maximum number of 20 items.
            ...
            /**
             * @param description A finding's description. Array Members: Minimum number of 1 item.
             ...
             *   captured by a finding was first observed by the security findings product. Uses the
            ...
            /**
             * @param generatorId The identifier for the solution-specific component that generated
             *   a finding. Array Members: Minimum number of 1 item. Maximum number of 100 items.
             */
            ...
            /**
             * @param id The product-specific identifier for a finding. Array Members: Minimum
            ...
            /**
             * @param lastObservedAt A timestamp that indicates when the potential security issue
             *   captured by a finding was most recently observed by the security findings product.
             *   Uses the `date-time` format specified in
             *   [RFC 3339 section 5.6, Internet Date/Time Format](https://docs.aws.amazon.com/https://tools.ietf.org/html/rfc3339#section-5.6)
             *   . The value cannot contain spaces. For example, `2020-03-22T13:22:13.933Z` .
             *
            ...
            /**
             * @param noteText The text of a user-defined note that's added to a finding. Array
            ...
            /**
             * @param noteUpdatedAt The timestamp of when the note was updated. Uses the date-time
             *   format specified in
             *   [RFC 3339 section 5.6, Internet Date/Time Format](https://docs.aws.amazon.com/https://www.rfc-editor.org/rfc/rfc3339#section-5.6)
             *   . The value cannot contain spaces. For example, `2020-03-22T13:22:13.933Z` .
             *
            ...
            /**
             * @param noteUpdatedBy The principal that created a note. Array Members: Minimum number
             *   of 1 item. Maximum number of 20 items.
             */
            ...
            /**
             * @param productArn The Amazon Resource Name (ARN) for a third-party product that
             *   generated a finding in Security Hub. Array Members: Minimum number of 1 item.
             *   Maximum number of 20 items.
             */
             ...
             *   control-based findings, the product name is Security Hub. Array Members: Minimum
             *   number of 1 item. Maximum number of 20 items.
             */
            ...
            /**
             * @param recordState Provides the current state of a finding. Array Members: Minimum
             *   number of 1 item. Maximum number of 20 items.
             */
            ...
            /**
             * @param relatedFindingsProductArn The ARN for the product that generated a related
            ...
            /**
             * @param resourceDetailsOther Custom fields and values about the resource that a
             *   finding pertains to. Array Members: Minimum number of 1 item. Maximum number of 20
             *   items.
             */
            ...
            /**
             * @param resourceId The identifier for the given resource type. For AWS resources that
             *   are identified by Amazon Resource Names (ARNs), this is the ARN. For AWS resources
             *   that lack ARNs, this is the identifier as defined by the AWS service that created
             *   the resource. For non- AWS resources, this is a unique identifier that is
             *   associated with the resource.
             *
             * Array Members: Minimum number of 1 item. Maximum number of 100 items.
             */
            ...
            /**
             * @param resourcePartition The partition in which the resource that the finding
             *   pertains to is located. A partition is a group of AWS Regions . Each AWS account is
            ...
            /**
             * @param resourceTags A list of AWS tags associated with a resource at the time the
             *   finding was processed. Array Members: Minimum number of 1 item. Maximum number of
             *   20 items.
             */
            ...
            /**
             * @param resourceType A finding's title. Array Members: Minimum number of 1 item.
            ...
            /**
             * @param severityLabel The severity value of the finding. Array Members: Minimum number
             *   of 1 item. Maximum number of 20 items.
             */
             ...
             *   finding product. Array Members: Minimum number of 1 item. Maximum number of 20
             *   items.
             */
            ...
            /**
             * @param title A finding's title. Array Members: Minimum number of 1 item. Maximum
             *   number of 100 items.
             */
            ...
            /**
             * @param type One or more finding types in the format of namespace/category/classifier
             *   that classify a finding. For a list of namespaces, classifiers, and categories, see
             *   [Types taxonomy for ASFF](https://docs.aws.amazon.com/securityhub/latest/userguide/securityhub-findings-format-type-taxonomy.html)
             *   in the *AWS Security Hub User Guide* .
             *
             * Array Members: Minimum number of 1 item. Maximum number of 20 items.
            ...
            /**
             * @param userDefinedFields A list of user-defined name and value string pairs added to
             *   a finding. Array Members: Minimum number of 1 item. Maximum number of 20 items.
             */
            ...
            /**
             * @param verificationState Provides the veracity of a finding. Array Members: Minimum
             *   number of 1 item. Maximum number of 20 items.
             */
            ...
            /**
             * @param workflowStatus Provides information about the status of the investigation into
             *   a finding. Array Members: Minimum number of 1 item. Maximum number of 20 items.
             */
        ...
        private class BuilderImpl : Builder {
             ...
             * Array Members: Minimum number of 1 item. Maximum number of 20 items.
             */
            override fun companyName(vararg companyName: Any): Unit =
             ...
             * Array Members: Minimum number of 1 item. Maximum number of 20 items.
             */
            override fun complianceAssociatedStandardsId(
                complianceAssociatedStandardsId: IResolvable
            ) {
                cdkBuilder.complianceAssociatedStandardsId(
                    complianceAssociatedStandardsId.let(IResolvable::unwrap)
                )
            }
             ...
             */
            override fun complianceAssociatedStandardsId(
                complianceAssociatedStandardsId: List<Any>
            ) {
             ...
             */
            override fun complianceAssociatedStandardsId(
                vararg complianceAssociatedStandardsId: Any
            ...
            override fun complianceSecurityControlId(complianceSecurityControlId: IResolvable) {
                cdkBuilder.complianceSecurityControlId(
                    complianceSecurityControlId.let(IResolvable::unwrap)
                )
            }
             ...
             *   Minimum number of 1 item. Maximum number of 20 items.
             */
            override fun complianceSecurityControlId(
                vararg complianceSecurityControlId: Any
            ): Unit = complianceSecurityControlId(complianceSecurityControlId.toList())
             ...
             * Array Members: Minimum number of 1 item. Maximum number of 20 items.
             */
            override fun criticality(vararg criticality: Any): Unit =
                criticality(criticality.toList())
             ...
             *   Maximum number of 20 items.
             */
            override fun description(vararg description: Any): Unit =
             ...
             *   a finding. Array Members: Minimum number of 1 item. Maximum number of 100 items.
             */
            override fun generatorId(vararg generatorId: Any): Unit =
                generatorId(generatorId.toList())
             ...
             *   number of 1 item. Maximum number of 20 items.
             */
            override fun productName(vararg productName: Any): Unit =
             ...
             *   number of 1 item. Maximum number of 20 items.
             */
            override fun recordState(vararg recordState: Any): Unit =
                recordState(recordState.toList())
            ...
            override fun relatedFindingsProductArn(relatedFindingsProductArn: IResolvable) {
                cdkBuilder.relatedFindingsProductArn(
                    relatedFindingsProductArn.let(IResolvable::unwrap)
                )
            }
        ...
        internal constructor(
             ...
             * The unique identifier of a standard in which a control is enabled.
             *
             * This field consists of the resource portion of the Amazon Resource Name (ARN)
             ...
             * low confidence because an actual exfiltration hasn't been verified. For more
             * information, see
             * [Confidence](https://docs.aws.amazon.com/securityhub/latest/userguide/asff-top-level-attributes.html#asff-confidence)
             * in the *AWS Security Hub User Guide* .
             *
             ...
             *
             * `Criticality` is scored on a 0–100 basis, using a ratio scale that supports only full
             * integers. A score of `0` means that the underlying resources have no criticality, and
             * a score of `100` is reserved for the most critical resources. For more information,
             * see
             * [Criticality](https://docs.aws.amazon.com/securityhub/latest/userguide/asff-top-level-attributes.html#asff-criticality)
             * in the *AWS Security Hub User Guide* .
             *
             ...
             * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-securityhub-automationrule-automationrulesfindingfilters.html#cfn-securityhub-automationrule-automationrulesfindingfilters-relatedfindingsproductarn)
             */
            override fun relatedFindingsProductArn(): Any? =
             ...
             * ARN. For AWS resources that lack ARNs, this is the identifier as defined by the AWS
             * service that created the resource. For non- AWS resources, this is a unique
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: AutomationRulesFindingFiltersProperty
            ): software.amazon.awscdk.services.securityhub.CfnAutomationRule.AutomationRulesFindingFiltersProperty =
                (wrapped as Wrapper).cdkObject
        }
    }
    ...
    public interface RelatedFindingProperty {
        ...
        public interface Builder {
            /**
             * @param id The product-generated identifier for a related finding. Array Members:
             *   Minimum number of 1 item. Maximum number of 20 items.
             */
            public fun id(id: Any)

            /**
             * @param productArn The Amazon Resource Name (ARN) for the product that generated a
             *   related finding.
             */
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: RelatedFindingProperty
            ): software.amazon.awscdk.services.securityhub.CfnAutomationRule.RelatedFindingProperty =
                (wrapped as Wrapper).cdkObject
        }
    }
    ...
    public interface DateFilterProperty {
        ...
        public interface Builder {
            ...
            /**
             * @param end A timestamp that provides the end date for the date filter. A correctly
             *   formatted example is `2020-05-21T20:16:34.724Z` . The value cannot contain spaces,
             *   and date and time should be separated by `T` . For more information, see
             *   [RFC 3339 section 5.6, Internet Date/Time Format](https://docs.aws.amazon.com/https://www.rfc-editor.org/rfc/rfc3339#section-5.6)
             *   .
             */
            ...
            /**
             * @param start A timestamp that provides the start date for the date filter. A
    ...
    public interface AutomationRulesFindingFieldsUpdateProperty {
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: AutomationRulesFindingFieldsUpdateProperty
            ): software.amazon.awscdk.services.securityhub.CfnAutomationRule.AutomationRulesFindingFieldsUpdateProperty =
                (wrapped as Wrapper).cdkObject
        }
    }
    ...
    public interface NumberFilterProperty {
        ...
        public interface Builder {
            ...
            /**
             * @param gte The greater-than-equal condition to be applied to a single field when
             *   querying for findings.
             */
            public fun gte(gte: Number)

            /**
             * @param lte The less-than-equal condition to be applied to a single field when
             *   querying for findings.
             */
    ...
    public interface AutomationRulesActionProperty {
         ...
         * namespace/category/classifier. For more information, see
         * [Types taxonomy for ASFF](https://docs.aws.amazon.com/securityhub/latest/userguide/securityhub-findings-format-type-taxonomy.html)
         * in the *AWS Security Hub User Guide* .
         *
        ...
        public interface Builder {
            /**
             * @param findingFieldsUpdate Specifies that the automation rule action is an update to
             *   a finding field.
             */
            ...
            public fun findingFieldsUpdate(
                findingFieldsUpdate: AutomationRulesFindingFieldsUpdateProperty.Builder.() -> Unit
            )
        ...
        private class BuilderImpl : Builder {
            ...
            override fun findingFieldsUpdate(
                ...
                cdkBuilder.findingFieldsUpdate(
                    findingFieldsUpdate.let(AutomationRulesFindingFieldsUpdateProperty::unwrap)
                )
            }
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: AutomationRulesActionProperty
            ): software.amazon.awscdk.services.securityhub.CfnAutomationRule.AutomationRulesActionProperty =
                (wrapped as Wrapper).cdkObject
        }
    }
    ...
    public interface StringFilterProperty {
         ...
         *   filter `Title CONTAINS CloudFront` matches findings that have a `Title` that includes
         *   the string CloudFront.
         * * To search for values that exactly match the filter value, use `EQUALS` . For example,
         *   the filter `AwsAccountId EQUALS 123456789012` only matches findings that have an
         *   account ID of `123456789012` .
         * * To search for values that start with the filter value, use `PREFIX` . For example, the
         *   filter `ResourceRegion PREFIX us` matches findings that have a `ResourceRegion` that
         *   starts with `us` . A `ResourceRegion` that starts with a different value, such as `af`
         ...
         * comparison operators:
         * * To search for values that exclude the filter value, use `NOT_CONTAINS` . For example,
         *   the filter `Title NOT_CONTAINS CloudFront` matches findings that have a `Title` that
         *   excludes the string CloudFront.
         * * To search for values other than the filter value, use `NOT_EQUALS` . For example, the
         *   filter `AwsAccountId NOT_EQUALS 123456789012` only matches findings that have an
         *   account ID other than `123456789012` .
         ...
         * joined by `AND` . A finding matches only if it matches all of those filters. For example,
         * the filters `Title NOT_CONTAINS CloudFront AND Title NOT_CONTAINS CloudWatch` match a
         ...
         * `PREFIX_NOT_EQUALS` filter on the same field. Combining filters in this way returns an
         * error. `CONTAINS` filters can only be used with other `CONTAINS` filters. `NOT_CONTAINS`
         * filters can only be used with other `NOT_CONTAINS` filters.
         *
         * You can combine `PREFIX` filters with `NOT_EQUALS` or `PREFIX_NOT_EQUALS` filters for the
         * same field. Security Hub first processes the `PREFIX` filters, and then the `NOT_EQUALS`
         * or `PREFIX_NOT_EQUALS` filters.
         *
         * For example, for the following filters, Security Hub first identifies findings that have
         * resource types that start with either `AwsIam` or `AwsEc2` . It then excludes findings
         * that have a resource type of `AwsIamPolicy` and findings that have a resource type of
         ...
         *
         * Filter values are case sensitive. For example, the product name for control-based
         * findings is `Security Hub` . If you provide `security hub` as the filter value, there's
         * no match.
         *
        ...
        public interface Builder {
            /**
             * @param comparison The condition to apply to a string value when filtering Security
             *   Hub findings. To search for values that have the filter value, use one of the
             *   following comparison operators:
             * * To search for values that include the filter value, use `CONTAINS` . For example,
             *   the filter `Title CONTAINS CloudFront` matches findings that have a `Title` that
             *   includes the string CloudFront.
             * * To search for values that exactly match the filter value, use `EQUALS` . For
             *   example, the filter `AwsAccountId EQUALS 123456789012` only matches findings that
             *   have an account ID of `123456789012` .
             * * To search for values that start with the filter value, use `PREFIX` . For example,
             *   the filter `ResourceRegion PREFIX us` matches findings that have a `ResourceRegion`
             *   that starts with `us` . A `ResourceRegion` that starts with a different value, such
             ...
             * * To search for values other than the filter value, use `NOT_EQUALS` . For example,
             *   the filter `AwsAccountId NOT_EQUALS 123456789012` only matches findings that have
             *   an account ID other than `123456789012` .
             * * To search for values that don't start with the filter value, use
             *   `PREFIX_NOT_EQUALS` . For example, the filter `ResourceRegion PREFIX_NOT_EQUALS us`
             *   matches findings with a `ResourceRegion` that starts with a value other than `us` .
             *
             * `NOT_CONTAINS` , `NOT_EQUALS` , and `PREFIX_NOT_EQUALS` filters on the same field are
             ...
             * `PREFIX_NOT_EQUALS` filter on the same field. Combining filters in this way returns
             * an error. `CONTAINS` filters can only be used with other `CONTAINS` filters.
             ...
             * have resource types that start with either `AwsIam` or `AwsEc2` . It then excludes
             * findings that have a resource type of `AwsIamPolicy` and findings that have a
             * resource type of `AwsEc2NetworkInterface` .
             * * `ResourceType PREFIX AwsIam`
             * * `ResourceType PREFIX AwsEc2`
             * * `ResourceType NOT_EQUALS AwsIamPolicy`
             * * `ResourceType NOT_EQUALS AwsEc2NetworkInterface`
            ...
            /**
             * @param value The string filter value. Filter values are case sensitive. For example,
             *   the product name for control-based findings is `Security Hub` . If you provide
        ...
        internal constructor(
             ...
             *
             * Filter values are case sensitive. For example, the product name for control-based
             * findings is `Security Hub` . If you provide `security hub` as the filter value,
             * there's no match.
             *
             * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-securityhub-automationrule-stringfilter.html#cfn-securityhub-automationrule-stringfilter-value)
             */
