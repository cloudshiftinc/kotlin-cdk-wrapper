...
public interface RuleTargetConfig {
     ...
     * target (for example, start a build for a CodeBuild project). We need it to determine whether
     * the rule belongs to a different account than the target - if so, we generate a more complex
     * setup, including an additional stack containing the EventBusPolicy.
     *
     ...
     */
    public fun targetResource(): IConstruct? =
    ...
    public interface Builder {
        ...
        public fun batchParameters(
            batchParameters: CfnRule.BatchParametersProperty.Builder.() -> Unit
        )
        ...
        public fun deadLetterConfig(
            deadLetterConfig: CfnRule.DeadLetterConfigProperty.Builder.() -> Unit
        )
         ...
         *   Gateway REST endpoint or EventBridge API destination.
         */
        ...
        /**
         * @param kinesisParameters Settings that control shard assignment, when the target is a
         *   Kinesis stream. If you don't include this parameter, eventId is used as the partition
         *   key.
         */
        ...
        public fun kinesisParameters(
            kinesisParameters: CfnRule.KinesisParametersProperty.Builder.() -> Unit
        )
        ...
        /**
         * @param runCommandParameters Parameters used when the rule invokes Amazon EC2 Systems
         *   Manager Run Command.
         */
        ...
        public fun runCommandParameters(
            runCommandParameters: CfnRule.RunCommandParametersProperty.Builder.() -> Unit
        )

        /**
         * @param sqsParameters Parameters used when the FIFO sqs queue is used an event target by
         *   the rule.
         */
        ...
        /**
         * @param targetResource The resource that is backing this target. This is the resource that
         *   will actually have some action performed on it when used as a target (for example,
         *   start a build for a CodeBuild project). We need it to determine whether the rule
         *   belongs to a different account than the target - if so, we generate a more complex
         *   setup, including an additional stack containing the EventBusPolicy.
         */
    ...
    private class BuilderImpl : Builder {
        ...
        override fun deadLetterConfig(deadLetterConfig: CfnRule.DeadLetterConfigProperty) {
            cdkBuilder.deadLetterConfig(
                deadLetterConfig.let(CfnRule.DeadLetterConfigProperty::unwrap)
            )
        }
        ...
        override fun ecsParameters(
            ecsParameters: CfnRule.EcsParametersProperty.Builder.() -> Unit
        ...
        override fun httpParameters(
            httpParameters: CfnRule.HttpParametersProperty.Builder.() -> Unit
        ...
        override fun kinesisParameters(kinesisParameters: CfnRule.KinesisParametersProperty) {
            cdkBuilder.kinesisParameters(
                kinesisParameters.let(CfnRule.KinesisParametersProperty::unwrap)
            )
        }
        ...
        override fun retryPolicy(
            retryPolicy: CfnRule.RetryPolicyProperty.Builder.() -> Unit
        ...
        override fun runCommandParameters(
            ...
            cdkBuilder.runCommandParameters(
                runCommandParameters.let(CfnRule.RunCommandParametersProperty::unwrap)
            )
        }
        ...
        override fun sqsParameters(
            sqsParameters: CfnRule.SqsParametersProperty.Builder.() -> Unit
        ...
        public fun build(): software.amazon.awscdk.services.events.RuleTargetConfig =
            cdkBuilder.build()
    }
    ...
    internal constructor(
         ...
         *
         * This is the resource that will actually have some action performed on it when used as a
         * target (for example, start a build for a CodeBuild project). We need it to determine
         * whether the rule belongs to a different account than the target - if so, we generate a
         * more complex setup, including an additional stack containing the EventBusPolicy.
         *
         * Default: the target is not backed by any resource
         *
         * [Documentation](https://docs.aws.amazon.com/eventbridge/latest/userguide/eventbridge-cross-account-event-delivery.html)
