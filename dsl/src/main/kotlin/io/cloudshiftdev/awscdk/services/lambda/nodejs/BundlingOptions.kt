...
public interface BundlingOptions : DockerRunOptions {
     ...
     *
     * For consistency, this custom hash will be SHA256 hashed and encoded as hex. The resulting
     * hash will be the asset hash.
     *
     ...
     *
     * Default: - false if `true` the `aws-sdk` will be included in the asset bundle and not be
     ...
     *
     * By default esbuild's output is ASCII-only. Any non-ASCII characters are escaped using
     * backslash escape sequences. Using escape sequences makes the generated output slightly
     * bigger, and also makes it harder to read. If you would like for esbuild to print the original
     * characters without using escape sequences, use `Charset.UTF8`.
     *
     ...
     *
     * This image should have esbuild installed globally. If you plan to use `nodeModules` it should
     * also have `npm`, `yarn` or `pnpm` depending on the lock file you're using.
     *
     * See
     ...
     *
     * In JavaScript the `name` property on functions and classes defaults to a nearby identifier in
     * the source code.
     *
    ...
    public interface Builder {
        /**
         * @param assetHash Specify a custom hash for this asset. For consistency, this custom hash
         *   will be SHA256 hashed and encoded as hex. The resulting hash will be the asset hash.
         *
        ...
        /**
         * @param banner Use this to insert an arbitrary string at the beginning of generated
         *   JavaScript files. This is similar to footer which inserts at the end instead of the
         *   beginning.
         *
         * This is commonly used to insert comments:
         */
         ...
         *   ASCII-only. Any non-ASCII characters are escaped using backslash escape sequences.
         *   Using escape sequences makes the generated output slightly bigger, and also makes it
         *   harder to read. If you would like for esbuild to print the original characters without
         *   using escape sequences, use `Charset.UTF8`.
         */
        ...
        /**
         * @param dockerImage A custom bundling Docker image. This image should have esbuild
         *   installed globally. If you plan to use `nodeModules` it should also have `npm`, `yarn`
         *   or `pnpm` depending on the lock file you're using.
         *
         * See
         * https://github.com/aws/aws-cdk/blob/main/packages/aws-cdk-lib/aws-lambda-nodejs/lib/Dockerfile
         * for the default image provided by aws-cdk-lib/aws-lambda-nodejs.
         ...
         *   possible. This is useful if your function relies on node modules that should be
         *   installed (`nodeModules`) in a Lambda compatible environment.
         */
         ...
         *   JavaScript the `name` property on functions and classes defaults to a nearby identifier
         *   in the source code.
         *
         * However, minification renames symbols to reduce code size and bundling sometimes need to
         * rename symbols to avoid collisions. That changes value of the `name` property for many of
         * these cases. This is usually fine because the `name` property is normally only used for
         * debugging. However, some frameworks rely on the `name` property for registration and
         * binding purposes. If this is the case, you can enable this option to preserve the
         * original `name` values even in minified code.
         */
         ...
         *   loader for a given file type lets you load that file type with an `import` statement or
         *   a `require` call.
         *
         * For example, `{ '.png': 'dataurl' }`.
         */
         ...
         *   directory with metadata about the build. The metadata in this JSON file follows this
         *   schema (specified using TypeScript syntax):
         * ```
         * {
         * outputs: {
         * [path: string]: {
         ...
         *
         * This data can then be analyzed by other tools. For example, bundle buddy can consume
         * esbuild's metadata format and generates a treemap visualization of the modules in your
         * bundle and how much space each one takes up.
         */
        ...
        /**
         * @param nodeModules A list of modules that should be installed instead of bundled. Modules
         *   are installed in a Lambda compatible environment only when bundling runs in Docker.
         */
        ...
        /**
         * @param platform Set platform if server is multi-platform capable. *Requires Docker Engine
         *   API v1.38+*. Example value: `linux/amd64`
         */
        public fun platform(platform: String)

        /**
         * @param preCompilation Run compilation using tsc before running file through bundling
         *   step. This usually is not required unless you are using new experimental features that
         *   are only supported by typescript's `tsc` compiler. One example of such feature is
         *   `emitDecoratorMetadata`.
         */
        ...
        /**
         * @param sourcesContent Whether to include original source code in source maps when
         *   bundling.
         */
        ...
        /**
         * @param tsconfig Normally the esbuild automatically discovers `tsconfig.json` files and
         *   reads their contents during a build. However, you can also configure a custom
         *   `tsconfig.json` file to use instead.
         *
         * This is similar to entry path, you need to provide path to your custom `tsconfig.json`.
         *
         * This can be useful if you need to do multiple builds of the same code with different
    ...
    private class BuilderImpl : Builder {
         ...
         *   are installed in a Lambda compatible environment only when bundling runs in Docker.
         */
        override fun nodeModules(vararg nodeModules: String): Unit =
            nodeModules(nodeModules.toList())
        ...
        /** @param volumesFrom Where to mount the specified volumes from. */
        override fun volumesFrom(vararg volumesFrom: String): Unit =
    ...
    internal constructor(
        ...
        /**
         * Which option to use to copy the source files to the docker container and output files
         * back.
         *
         * Default: - BundlingFileAccess.BIND_MOUNT
         */
         ...
         * By default esbuild's output is ASCII-only. Any non-ASCII characters are escaped using
         * backslash escape sequences. Using escape sequences makes the generated output slightly
         * bigger, and also makes it harder to read. If you would like for esbuild to print the
         * original characters without using escape sequences, use `Charset.UTF8`.
         *
         * Default: Charset.ASCII
         *
         * [Documentation](https://esbuild.github.io/api/#charset)
         ...
         * Default: - use the Docker image provided by aws-cdk-lib/aws-lambda-nodejs
         */
        override fun dockerImage(): DockerImage? =
        ...
        /**
         * A list of modules that should be considered as externals (already available in the
         * runtime).
         *
         * Default: - no replacements are made
         */
        override fun externalModules(): List<String> =
            unwrap(this).getExternalModules() ?: emptyList()
        ...
        /**
         * This option allows you to automatically replace a global variable with an import from
         * another file.
         *
         * Default: - no code is injected
         *
         ...
         * Whether to preserve the original `name` values even in minified code.
         *
         * In JavaScript the `name` property on functions and classes defaults to a nearby
         ...
         * Log level for esbuild.
         *
         * This is also propagated to the package manager and applies to its specific install
         * command.
         *
         * Default: LogLevel.WARNING
         */
        ...
        /**
         * This option tells esbuild to write out a JSON file relative to output directory with
         ...
         * A list of modules that should be installed instead of bundled.
         *
         * Modules are installed in a Lambda compatible environment only when bundling runs in
         * Docker.
         *
         * Default: - all modules are bundled
         */
        ...
        /**
         * Normally the esbuild automatically discovers `tsconfig.json` files and reads their
         ...
         */
        override fun volumes(): List<DockerVolume> =
