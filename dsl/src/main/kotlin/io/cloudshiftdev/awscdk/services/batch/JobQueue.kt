...
internal constructor(
     ...
     * @param order
     */
    public override fun addComputeEnvironment(
        computeEnvironment: IComputeEnvironment,
        order: Number
    ) {
        unwrap(this)
            .addComputeEnvironment(computeEnvironment.let(IComputeEnvironment::unwrap), order)
    }
     ...
     * priority value of 10 is given scheduling preference over a job queue with a priority value
     * of 1.
     */
    ...
    public interface Builder {
         ...
         * other.
         *
         * The job scheduler uses this parameter to determine which compute environment runs a
         * specific job. Compute environments must be in the VALID state before you can associate
         * them with a job queue. You can associate up to three compute environments with a job
         * queue. All of the compute environments must be either EC2 (EC2 or SPOT) or Fargate
         ...
         * Default: none
         *
         * @param computeEnvironments The set of compute environments mapped to a job queue and
         ...
         * If the job queue is enabled, it is able to accept jobs.
         *
         * Otherwise, new jobs can't be added to the queue, but jobs already in the queue can
         * finish.
         *
         * Default: true
         *
         ...
         * The priority of the job queue.
         *
         * Job queues with a higher priority are evaluated first when associated with the same
         * compute environment. Priority is determined in descending order. For example, a job queue
         * with a priority of 10 is given scheduling preference over a job queue with a priority
         * of 1.
         *
         * Default: 1
         *
    ...
    private class BuilderImpl(
        ...
        override fun computeEnvironments(computeEnvironments: List<OrderedComputeEnvironment>) {
            cdkBuilder.computeEnvironments(
                computeEnvironments.map(OrderedComputeEnvironment::unwrap)
            )
        }
         ...
         *   their order relative to each other.
         */
        override fun computeEnvironments(
            vararg computeEnvironments: OrderedComputeEnvironment
        ): Unit = computeEnvironments(computeEnvironments.toList())
    ...
    public companion object {
        public fun fromJobQueueArn(
            ...
            software.amazon.awscdk.services.batch.JobQueue.fromJobQueueArn(
