...
public interface IPlacementGroup : IResource {
    ...
    internal constructor(
         ...
         * spread level placement group reduces the risk of simultaneous failures that might occur
         * when instances share the same equipment. Spread level placement groups provide access to
         * distinct hardware, and are therefore suitable for mixing instance types or launching
         * instances over time. If you start or launch an instance in a spread placement group and
         * there is insufficient unique hardware to fulfill the request, the request fails. Amazon
         * EC2 makes more distinct hardware available over time, so you can try your request again
         * later. Placement groups can spread instances across racks or hosts. You can use host
         * level spread placement groups only with AWS Outposts.
         *
         * Default: - no spread level
         */
         ...
         * Which strategy to use when launching instances.
         *
         * Default: - `PlacementGroupStrategy.PARTITION` if `partitions` is defined, `CLUSTER`
         * otherwise
         */
