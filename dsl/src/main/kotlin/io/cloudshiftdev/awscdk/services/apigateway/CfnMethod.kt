...
internal constructor(
    ...
    public open fun authorizationScopes(): List<String> =
    ...
    public interface Builder {
         ...
         *
         * @param apiKeyRequired A boolean flag specifying whether a valid ApiKey is required to
         *   invoke this method.
         */
         ...
         * if any method scopes matches a claimed scope in the access token. Otherwise, the
         ...
         *
         * @param requestModels A key-value map specifying data schemas, represented by Model
         *   resources, (as the mapped value) of the request payloads of given content types (as the
         *   mapping key).
         */
    ...
    private class BuilderImpl(
        ...
        public fun build(): software.amazon.awscdk.services.apigateway.CfnMethod =
            cdkBuilder.build()
    }
    ...
    public companion object {
        ...
        ): CfnMethod = CfnMethod(cdkObject)

        internal fun unwrap(
            wrapped: CfnMethod
        ): software.amazon.awscdk.services.apigateway.CfnMethod = wrapped.cdkObject
    }
    ...
    public interface MethodResponseProperty {
         ...
         * method response header is required or not. The expression of the key must match the
         * pattern `method.response.header.{name}` , where `name` is a valid and unique header name.
         * API Gateway passes certain integration response data to the method response headers
         * specified here according to the mapping you prescribe in the API's IntegrationResponse.
         * The integration response data that can be mapped include an integration response header
         * expressed in `integration.response.header.{name}` , a static value enclosed within a pair
         * of single quotes (e.g., `'application/json'` ), or a JSON expression from the back-end
        ...
        public interface Builder {
            /**
             * @param responseModels Specifies the Model resources used for the response's
             ...
             *   response header and the value specifies whether the associated method response
        ...
        internal constructor(
             ...
             * Specifies the Model resources used for the response's content-type.
             *
             * Response models are represented as a key/value map, with a content-type as the key
             ...
             * can send back to the caller.
             *
             * A key defines a method response header and the value specifies whether the associated
             * method response header is required or not. The expression of the key must match the
             * pattern `method.response.header.{name}` , where `name` is a valid and unique header
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: MethodResponseProperty
            ): software.amazon.awscdk.services.apigateway.CfnMethod.MethodResponseProperty =
    ...
    public interface IntegrationProperty {
         ...
         */
        public fun cacheKeyParameters(): List<String> =
            unwrap(this).getCacheKeyParameters() ?: emptyList()

        /**
         * Specifies a group of related cached parameters.
         *
         * By default, API Gateway uses the resource ID as the `cacheNamespace` . You can specify
         * the same `cacheNamespace` across resources to return the same cached data for requests to
         * different resources.
         *
         ...
         * identity be passed through from the request, specify the string `arn:aws:iam::\*:user/\*`
         ...
         * integrations, you must set the integration method to `POST` . For all other types, you
         ...
         * transformation when the method request content type does not match any content type
         ...
         * the back end without transformation when no mapping template is defined in the
         * integration request. If a template is defined when this option is selected, the method
         * request of an unmapped content-type will be rejected with an HTTP 415 Unsupported Media
         * Type response. `NEVER` : rejects the method request with an HTTP 415 Unsupported Media
         * Type response when either the method request content type does not match any content type
         ...
         * pre-encoded as required by the back end. The method request parameter value must match
         ...
         *
         * The content type value is the key in this map, and the template (as a String) is the
         * value.
         *
         ...
         * URL according to the RFC-3986 specification for standard integrations. If
         * `connectionType` is `VPC_LINK` specify the Network Load Balancer DNS name. For `AWS` or
         ...
         * of the integrated AWS service, if applicable. For example, for integration with the S3
        ...
        public interface Builder {
            /**
             * @param cacheKeyParameters A list of request parameters whose values API Gateway
             *   caches. To be valid values for `cacheKeyParameters` , these parameters must also be
            ...
            /**
             * @param cacheNamespace Specifies a group of related cached parameters. By default, API
             ...
             *   The valid value is `INTERNET` for connections through the public routable internet
             *   or `VPC_LINK` for private connections between API Gateway and a network load
             *   balancer in a VPC. The default value is `INTERNET` .
             */
            public fun connectionType(connectionType: String)

            /**
             * @param contentHandling Specifies how to handle request payload content type
             *   conversions. Supported values are `CONVERT_TO_BINARY` and `CONVERT_TO_TEXT` , with
             ...
             *   For AWS integrations, three options are available. To specify an IAM Role for API
             *   Gateway to assume, use the role's Amazon Resource Name (ARN). To require that the
            ...
            /**
             * @param integrationHttpMethod Specifies the integration's HTTP method type. For the
             *   Type property, if you specify `MOCK` , this property is optional. For Lambda
             *   integrations, you must set the integration method to `POST` . For all other types,
            ...
            /**
             * @param passthroughBehavior Specifies how the method request body of an unmapped
             *   content type will be passed through the integration request to the back end without
             *   transformation. A content type is unmapped if no mapping template is defined in the
             *   integration or the content type does not match any of the mapped content types, as
             *   specified in `requestTemplates` . The valid value is one of the following:
             *   `WHEN_NO_MATCH` : passes the method request body through the integration request to
             *   the back end without transformation when the method request content type does not
             ...
             *   template is defined in the integration request. If a template is defined when this
             *   option is selected, the method request of an unmapped content-type will be rejected
             *   with an HTTP 415 Unsupported Media Type response. `NEVER` : rejects the method
             *   request with an HTTP 415 Unsupported Media Type response when either the method
            ...
            /**
             * @param requestParameters A key-value map specifying request parameters that are
             *   passed from the method request to the back end. The key is an integration request
             *   parameter name and the associated value is a method request parameter value or
             *   static value that must be enclosed within single quotes and pre-encoded as required
            ...
            /**
             * @param requestTemplates Represents a map of Velocity templates that are applied on
             *   the request payload based on the value of the Content-Type header sent by the
             *   client. The content type value is the key in this map, and the template (as a
             *   String) is the value.
             */
            ...
            /**
             * @param timeoutInMillis Custom timeout between 50 and 29,000 milliseconds. The default
             ...
             *   following:. For the HTTP and HTTP proxy integrations, each integration can specify
             *   a protocol ( `http/https` ), port and path. Standard 80 and 443 ports are supported
        ...
        internal constructor(
             ...
             * A list of request parameters whose values API Gateway caches.
             *
             * To be valid values for `cacheKeyParameters` , these parameters must also be specified
             * for Method `requestParameters` .
             *
             * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-apigateway-method-integration.html#cfn-apigateway-method-integration-cachekeyparameters)
             */
            override fun cacheKeyParameters(): List<String> =
                unwrap(this).getCacheKeyParameters() ?: emptyList()

            /**
             * Specifies a group of related cached parameters.
             *
             * By default, API Gateway uses the resource ID as the `cacheNamespace` . You can
             * specify the same `cacheNamespace` across resources to return the same cached data for
             ...
             *
             * The valid value is `INTERNET` for connections through the public routable internet or
             * `VPC_LINK` for private connections between API Gateway and a network load balancer in
             ...
             *
             * A content type is unmapped if no mapping template is defined in the integration or
             ...
             * request. `WHEN_NO_TEMPLATES` : passes the method request body through the integration
             ...
             * Unsupported Media Type response. `NEVER` : rejects the method request with an HTTP
             * 415 Unsupported Media Type response when either the method request content type does
             * not match any content type associated with the mapping templates defined in the
             ...
             *
             * The key is an integration request parameter name and the associated value is a method
             * request parameter value or static value that must be enclosed within single quotes
             * and pre-encoded as required by the back end. The method request parameter value must
             * match the pattern of `method.request.{location}.{name}` , where `location` is
             ...
             * on the value of the Content-Type header sent by the client.
             *
             * The content type value is the key in this map, and the template (as a String) is the
             * value.
             *
             ...
             * custom ports above 1024. An HTTP or HTTP proxy integration with a `connectionType` of
             ...
             * For `HTTP` or `HTTP_PROXY` integrations, the URI must be a fully formed, encoded
             * HTTP(S) URL according to the RFC-3986 specification for standard integrations. If
             * `connectionType` is `VPC_LINK` specify the Network Load Balancer DNS name. For `AWS`
             ...
             * supported by certain AWS service for fast host-name lookup. action can be used for an
    ...
    public interface IntegrationResponseProperty {
         ...
         * end is an AWS Lambda function, the AWS Lambda function error header is matched. For all
        ...
        public interface Builder {
            /**
             * @param contentHandling Specifies how to handle response payload content type
             *   conversions. Supported values are `CONVERT_TO_BINARY` and `CONVERT_TO_TEXT` , with
             *   the following behaviors:
             *
             * If this property is not defined, the response payload will be passed through from the
            ...
            /**
             * @param responseParameters A key-value map specifying response parameters that are
             *   passed to the method response from the back end. The key is a method response
             *   header parameter name and the mapped value is an integration response header value,
             *   a static value enclosed within a pair of single quotes, or a JSON expression from
            ...
            /**
             * @param responseTemplates Specifies the templates used to transform the integration
             *   response body. Response templates are represented as a key/value map, with a
            ...
            /**
             * @param selectionPattern Specifies the regular expression (regex) pattern used to
             *   choose an integration response based on the response from the back end. For
             *   example, if the success response returns nothing and the error response returns
             *   some string, you could use the `.+` regex to match error response. However, make
             *   sure that the error response does not contain any newline ( `\n` ) character in
             *   such cases. If the back end is an AWS Lambda function, the AWS Lambda function
             *   error header is matched. For all other HTTP and AWS back ends, the HTTP status code
             *   is matched.
             */
            public fun selectionPattern(selectionPattern: String)

            /**
             * @param statusCode Specifies the status code that is used to map the integration
        ...
        internal constructor(
             ...
             * from the back end.
             *
             * The key is a method response header parameter name and the mapped value is an
             * integration response header value, a static value enclosed within a pair of single
             ...
             * Specifies the templates used to transform the integration response body.
             *
             * Response templates are represented as a key/value map, with a content-type as the key
             ...
             *
             * For example, if the success response returns nothing and the error response returns
             * some string, you could use the `.+` regex to match error response. However, make sure
             * that the error response does not contain any newline ( `\n` ) character in such
             * cases. If the back end is an AWS Lambda function, the AWS Lambda function error
             * header is matched. For all other HTTP and AWS back ends, the HTTP status code is
             * matched.
             *
             * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-apigateway-method-integrationresponse.html#cfn-apigateway-method-integrationresponse-selectionpattern)
             */
        ...
        public companion object {
            ...
            internal fun unwrap(
                wrapped: IntegrationResponseProperty
            ): software.amazon.awscdk.services.apigateway.CfnMethod.IntegrationResponseProperty =
                (wrapped as Wrapper).cdkObject
        }
    }
