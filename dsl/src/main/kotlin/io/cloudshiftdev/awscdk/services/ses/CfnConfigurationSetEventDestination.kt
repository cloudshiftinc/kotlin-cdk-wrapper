...
internal constructor(
    ...
    private class BuilderImpl(
        ...
        private val cdkBuilder:
            ...
            software.amazon.awscdk.services.ses.CfnConfigurationSetEventDestination.Builder.create(
                scope,
                id
            )
    ...
    public interface KinesisFirehoseDestinationProperty {
        ...
        public interface Builder {
            /**
             * @param deliveryStreamArn The ARN of the Amazon Kinesis Firehose stream that email
             *   sending events should be published to.
             */
            public fun deliveryStreamArn(deliveryStreamArn: String)

            /**
             * @param iamRoleArn The ARN of the IAM role under which Amazon SES publishes email
    ...
    public interface DimensionConfigurationProperty {
        ...
        public interface Builder {
             ...
             *   Amazon CloudWatch if you do not provide the value of the dimension when you send an
             *   email. The default value must meet the following requirements:
             ...
             *   email sending metric. The name must meet the following requirements:
             * * Contain only ASCII letters (a-z, A-Z), numbers (0-9), underscores (_), dashes (-),
             *   or colons (:).
             * * Contain 256 characters or fewer.
             */
             ...
             *   to publish to Amazon CloudWatch. To use the message tags that you specify using an
             *   `X-SES-MESSAGE-TAGS` header or a parameter to the `SendEmail` / `SendRawEmail` API,
             *   specify `messageTag` . To use your own email headers, specify `emailHeader` . To
        ...
        internal constructor(
             ...
             *
             * To use the message tags that you specify using an `X-SES-MESSAGE-TAGS` header or a
             * parameter to the `SendEmail` / `SendRawEmail` API, specify `messageTag` . To use your
             * own email headers, specify `emailHeader` . To put a custom tag on any link included
             * in your email, specify `linkTag` .
             *
             * [Documentation](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-ses-configurationseteventdestination-dimensionconfiguration.html#cfn-ses-configurationseteventdestination-dimensionconfiguration-dimensionvaluesource)
             */
    ...
    public interface EventDestinationProperty {
         ...
         *   the recipient’s mail server. (If account-level or global suppression is being used, SES
         *   will still count it as a send, but delivery is suppressed.)
         ...
         *   template parameters and data. (This event type only occurs when you send email using
         *   the
         ...
         *   API operations.)
         * * `deliveryDelay` - The email couldn't be delivered to the recipient’s mail server
        ...
        public interface Builder {
            /**
             * @param cloudWatchDestination An object that contains the names, default values, and
             *   sources of the dimensions associated with an Amazon CloudWatch event destination.
             */
            ...
            /**
             * @param enabled Sets whether Amazon SES publishes events to this destination when you
             *   send an email with the associated configuration set. Set to `true` to enable
             *   publishing to this destination; set to `false` to prevent publishing to this
            ...
            /**
             * @param kinesisFirehoseDestination An object that contains the delivery stream ARN and
             *   the IAM role ARN associated with an Amazon Kinesis Firehose event destination.
             */
             ...
             * * `reject` - SES accepted the email, but determined that it contained a virus and
             *   didn’t attempt to deliver it to the recipient’s mail server.
             * * `bounce` - ( *Hard bounce* ) The recipient's mail server permanently rejected the
             *   email. ( *Soft bounces* are only included when SES fails to deliver the email after
             *   retrying for a period of time.)
             * * `complaint` - The email was successfully delivered to the recipient’s mail server,
             *   but the recipient marked it as spam.
             * * `delivery` - SES successfully delivered the email to the recipient's mail server.
             * * `open` - The recipient received the message and opened it in their email client.
             * * `click` - The recipient clicked one or more links in the email.
             * * `renderingFailure` - The email wasn't sent because of a template rendering issue.
             *   This event type can occur when template data is missing, or when there is a
             *   mismatch between template parameters and data. (This event type only occurs when
             *   you send email using the
             *   [`SendTemplatedEmail`](https://docs.aws.amazon.com/ses/latest/APIReference/API_SendTemplatedEmail.html)
             *   or
             *   [`SendBulkTemplatedEmail`](https://docs.aws.amazon.com/ses/latest/APIReference/API_SendBulkTemplatedEmail.html)
             *   API operations.)
             * * `deliveryDelay` - The email couldn't be delivered to the recipient’s mail server
             *   because a temporary issue occurred. Delivery delays can occur, for example, when
             *   the recipient's inbox is full, or when the receiving email server experiences a
             *   transient issue.
             * * `subscription` - The email was successfully delivered, but the recipient updated
             *   their subscription preferences by clicking on an *unsubscribe* link as part of your
             *   [subscription management](https://docs.aws.amazon.com/ses/latest/dg/sending-email-subscription-management.html)
             *   .
             */
    ...
    public interface SnsDestinationProperty {
        ...
        public interface Builder {
            /**
             * @param topicArn The ARN of the Amazon SNS topic for email sending events. You can
    ...
    public interface CloudWatchDestinationProperty {
        ...
        public interface Builder {
            /**
             * @param dimensionConfigurations A list of dimensions upon which to categorize your
             *   emails when you publish email sending events to Amazon CloudWatch.
             */
