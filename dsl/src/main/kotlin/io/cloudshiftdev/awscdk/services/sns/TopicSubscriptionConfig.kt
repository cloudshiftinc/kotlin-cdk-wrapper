...
public interface TopicSubscriptionConfig : SubscriptionOptions {
     ...
     *
     * Normally you'd want the subscription to be created on the consuming stack because the topic
     * is usually referenced by the consumer's resource policy (e.g. SQS queue policy). Otherwise,
     * it will cause a cyclic reference.
     *
     ...
     * be defined.
     */
    public fun subscriberScope(): Construct? =
     ...
     * queue policy in order for the subscription to successfully deliver messages.
     *
     * Default: - empty list
     */
    ...
    public interface Builder {
        /**
         * @param deadLetterQueue Queue to be used as dead letter queue. If not passed no dead
         *   letter queue is enabled.
         */
        public fun deadLetterQueue(deadLetterQueue: IQueue)

        /**
         * @param endpoint The subscription endpoint. The meaning of this value depends on the value
         *   for 'protocol'.
         */
         ...
         *   To apply a filter policy to the message attributes, use `filterPolicy`. A maximum of
         *   one of `filterPolicyWithMessageBody` and `filterPolicy` may be used.
         */
         ...
         *   Raw messages are free of JSON formatting and can be sent to HTTP/S and Amazon SQS
         *   endpoints. For more information, see GetSubscriptionAttributes in the Amazon Simple
         *   Notification Service API Reference.
         */
        public fun rawMessageDelivery(rawMessageDelivery: Boolean)

        /**
         * @param region The region where the topic resides, in the case of cross-region
         *   subscriptions.
         */
        public fun region(region: String)

        /**
         * @param subscriberId The id of the SNS subscription resource created under `scope`. In
         *   most cases, it is recommended to use the `uniqueId` of the topic you are subscribing
         *   to.
         */
         ...
         *   Normally you'd want the subscription to be created on the consuming stack because the
         *   topic is usually referenced by the consumer's resource policy (e.g. SQS queue policy).
         *   Otherwise, it will cause a cyclic reference.
         *
         * If this is undefined, the subscription will be created on the topic's stack.
         */
        ...
        /**
         * @param subscriptionDependency The resources that need to be created before the
         *   subscription can be safely created. For example for SQS subscription, the subscription
         *   needs to have a dependency on the SQS queue policy in order for the subscription to
    ...
    private class BuilderImpl : Builder {
         ...
         *   one of `filterPolicyWithMessageBody` and `filterPolicy` may be used.
         */
        override fun filterPolicyWithMessageBody(
            filterPolicyWithMessageBody: Map<String, FilterOrPolicy>
        ) {
            cdkBuilder.filterPolicyWithMessageBody(
                filterPolicyWithMessageBody.mapValues { FilterOrPolicy.unwrap(it.value) }
            )
        }
    ...
    internal constructor(
        internal val cdkObject: software.amazon.awscdk.services.sns.TopicSubscriptionConfig,
         ...
         */
        override fun deadLetterQueue(): IQueue? =
        ...
        override fun filterPolicy(): Map<String, SubscriptionFilter> =
            unwrap(this).getFilterPolicy()?.mapValues { SubscriptionFilter.wrap(it.value) }
                ?: emptyMap()

        /**
         * The filter policy that is applied on the message body.
         *
         * To apply a filter policy to the message attributes, use `filterPolicy`. A maximum of one
         * of `filterPolicyWithMessageBody` and `filterPolicy` may be used.
         *
         * Default: - all messages are delivered
         */
        override fun filterPolicyWithMessageBody(): Map<String, FilterOrPolicy> =
            unwrap(this).getFilterPolicyWithMessageBody()?.mapValues {
                FilterOrPolicy.wrap(it.value)
            } ?: emptyMap()
         ...
         * The id of the SNS subscription resource created under `scope`.
         *
         * In most cases, it is recommended to use the `uniqueId` of the topic you are subscribing
         * to.
         */
    ...
    public companion object {
        ...
        internal fun unwrap(
            wrapped: TopicSubscriptionConfig
