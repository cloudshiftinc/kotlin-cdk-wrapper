...
internal constructor(
     ...
     *
     * This allows full control of the default action of the load balancer, including Action
     * chaining, fixed responses and redirect responses. See the `ListenerAction` class for all
     * options.
     *
     * It's possible to add routing conditions to the Action added in this way. At least one Action
     * must be added without conditions (which becomes the default Action).
     *
     * @param id
     * @param props
     */
    ...
    public override fun addAction(
        id: String,
        props: AddApplicationActionProps.Builder.() -> Unit
    ): Unit = addAction(id, AddApplicationActionProps(props))

    /**
     * Add one or more certificates to this listener.
     *
     * After the first certificate, this creates ApplicationListenerCertificates resources since
     * cloudformation requires the certificates array on the listener resource to have a length
     * of 1.
     *
     * @param id
     * @param certificates
     ...
     *
     * All target groups will be load balanced to with equal weight and without stickiness. For a
     * more complex configuration than that, use `addAction()`.
     *
    ...
    public override fun addTargetGroups(
        id: String,
        props: AddApplicationTargetGroupsProps.Builder.() -> Unit
    ): Unit = addTargetGroups(id, AddApplicationTargetGroupsProps(props))
     ...
     */
    public override fun addTargets(
        id: String,
        props: AddApplicationTargetsProps
    ): ApplicationTargetGroup =
        unwrap(this)
            .addTargets(id, props.let(AddApplicationTargetsProps::unwrap))
            .let(ApplicationTargetGroup::wrap)
    ...
    public override fun registerConnectable(connectable: IConnectable, portRange: Port) {
        unwrap(this)
            .registerConnectable(connectable.let(IConnectable::unwrap), portRange.let(Port::unwrap))
    }
    ...
    public override fun registerConnectable(
        connectable: IConnectable,
        portRange: Port.Builder.() -> Unit
    ...
    public interface Builder {
         ...
         *
         * This allows full control of the default action of the load balancer, including Action
         * chaining, fixed responses and redirect responses.
         *
         ...
         *
         * All target groups will be load balanced to with equal weight and without stickiness. For
         * a more complex configuration than that, use either `defaultAction` or `addAction()`.
         *
         ...
         *
         * If this is specified, the load balancer will be opened up to anyone who can reach it. For
         * internal load balancers this is anyone in the same VPC. For public load balancers, this
         * is anyone on the internet.
         *
         * If you want to be more selective about who can access this load balancer, set this to
         * `false` and use the listener's `connections` object to selectively grant access to the
    ...
    private class BuilderImpl(
         ...
         * @param defaultTargetGroups Default target groups to load balance to.
         */
        override fun defaultTargetGroups(
            vararg defaultTargetGroups: IApplicationTargetGroup
        ): Unit = defaultTargetGroups(defaultTargetGroups.toList())
    ...
    public companion object {
        public fun fromApplicationListenerAttributes(
            ...
            software.amazon.awscdk.services.elasticloadbalancingv2.ApplicationListener
                .fromApplicationListenerAttributes(
                    ...
                    id,
                    attrs.let(ApplicationListenerAttributes::unwrap)
                )
                .let(IApplicationListener::wrap)
        ...
        public fun fromApplicationListenerAttributes(
            ...
            fromApplicationListenerAttributes(scope, id, ApplicationListenerAttributes(attrs))
        ...
        public fun fromLookup(
            ...
            software.amazon.awscdk.services.elasticloadbalancingv2.ApplicationListener.fromLookup(
                    scope.let(CloudshiftdevConstructsConstruct::unwrap),
                    id,
                    options.let(ApplicationListenerLookupOptions::unwrap)
                )
                .let(IApplicationListener::wrap)
