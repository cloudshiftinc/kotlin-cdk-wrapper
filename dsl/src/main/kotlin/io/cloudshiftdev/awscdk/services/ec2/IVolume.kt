...
public interface IVolume : IResource {
     ...
     * an unresolvable circular reference between the instance role and the instance. Use
     * `IVolume.grantAttachVolumeToSelf` to grant an instance permission to attach this volume to
     * itself.
     *
     ...
     *
     * If you are looking to grant an Instance, AutoScalingGroup, EC2-Fleet, SpotFleet, ECS host,
     * etc the ability to attach this volume to **itself** then this is the method you want to use.
     *
     ...
     *
     * Use `IVolume.grantDetachVolumeFromSelf` to grant an instance permission to detach this volume
     * from itself.
     *
     * @param grantee the principal being granted permission.
     ...
     *
     * This is implemented via the same mechanism as `IVolume.grantAttachVolumeByResourceTag`, and
    ...
    internal constructor(
         ...
         * Grants permission to attach this Volume to an instance.
         *
         * CAUTION: Granting an instance permission to attach to itself using this method will lead
         * to an unresolvable circular reference between the instance role and the instance. Use
         * `IVolume.grantAttachVolumeToSelf` to grant an instance permission to attach this volume
         * to itself.
         *
         * @param grantee the principal being granted permission.
         * @param instances the instances to which permission is being granted to attach this volume
         *   to.
         */
        ...
        override fun grantAttachVolume(grantee: IGrantable, instances: List<IInstance>): Grant =
            unwrap(this)
                .grantAttachVolume(
                    grantee.let(IGrantable::unwrap),
                    instances.map(IInstance::unwrap)
                )
                .let(Grant::wrap)
         ...
         * Grants permission to attach the Volume by a ResourceTag condition.
         *
         * If you are looking to grant an Instance, AutoScalingGroup, EC2-Fleet, SpotFleet, ECS
         * host, etc the ability to attach this volume to **itself** then this is the method you
         * want to use.
         *
         * This is implemented by adding a Tag with key `VolumeGrantAttach-&lt;suffix&gt;` to the
         * given constructs and this Volume, and then conditioning the Grant such that the grantee
         * is only given the ability to AttachVolume if both the Volume and the destination Instance
         * have that tag applied to them.
         *
         * @param grantee the principal being granted permission.
         * @param constructs The list of constructs that will have the generated resource tag
         *   applied to them.
         * @param tagKeySuffix A suffix to use on the generated Tag key in place of the generated
         *   hash value.
         */
        override fun grantAttachVolumeByResourceTag(
            ...
            unwrap(this)
                .grantAttachVolumeByResourceTag(
                    grantee.let(IGrantable::unwrap),
                    constructs.map(Construct::unwrap)
                )
                .let(Grant::wrap)
    ...
    public companion object {
        ...
        internal fun unwrap(wrapped: IVolume): software.amazon.awscdk.services.ec2.IVolume =
