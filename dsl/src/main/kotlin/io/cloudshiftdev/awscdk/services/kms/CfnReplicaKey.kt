...
internal constructor(
     ...
     * about the key ARNs of multi-Region keys, see
     * [How multi-Region keys work](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html#mrk-how-it-works)
    ...
    public interface Builder {
         ...
         * keys. AWS Key Management Service does not synchronize this property.
         *
         ...
         * operations. Also, while the replica key is being created, its key state is `Creating` .
         ...
         *
         * @param enabled Specifies whether the replica key is enabled. Disabled KMS keys cannot be
         ...
         *   permission on the KMS key. This reduces the risk that the KMS key becomes unmanageable.
        ...
        /**
         * Specifies the number of days in the waiting period before AWS KMS deletes a replica key
         * that has been removed from a CloudFormation stack.
         *
         * Enter a value between 7 and 30 days. The default value is 30 days.
         *
         * When you remove a replica key from a CloudFormation stack, AWS KMS schedules the replica
         * key for deletion and starts the mandatory waiting period. The `PendingWindowInDays`
         * property determines the length of waiting period. During the waiting period, the key
         * state of replica key is `Pending Deletion` , which prevents it from being used in
         * cryptographic operations. When the waiting period expires, AWS KMS permanently deletes
         * the replica key.
         *
         ...
         * remove it from the stack, regardless of the waiting period. However, if you specify a
         * replica key in your template that is based on the same primary key as the original
         * replica key, CloudFormation creates a new replica key with the same key ID, key material,
         * and other shared properties of the original replica key. This new replica key can decrypt
         * ciphertext that was encrypted under the original replica key, or any related multi-Region
         * key.
         *
         * For detailed information about deleting multi-Region keys, see
         * [Deleting multi-Region keys](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-delete.html)
         * in the *AWS Key Management Service Developer Guide* .
         *
         * For information about the `PendingDeletion` key state, see
         * [Key state: Effect on your KMS key](https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html)
         ...
         *   KMS deletes a replica key that has been removed from a CloudFormation stack.
         */
         ...
         * Specifies the multi-Region primary key to replicate.
         *
         * The primary key must be in a different AWS Region of the same AWS partition. You can
         ...
         * scheduled for deletion and a new replica key is created based on the specified primary
         * key. While it is scheduled for deletion, the existing replica key becomes unusable. You
         * can cancel the scheduled deletion of the key outside of CloudFormation.
         *
         ...
         * required, but the tag value can be an empty (null) string. You cannot have more than one
         ...
         * usage and costs aggregated by tags. Tags can also be used to control access to a KMS key.
         * For details, see
    ...
    public companion object {
        ...
        ): CfnReplicaKey = CfnReplicaKey(cdkObject)

        internal fun unwrap(
            wrapped: CfnReplicaKey
        ): software.amazon.awscdk.services.kms.CfnReplicaKey = wrapped.cdkObject
    }
}
