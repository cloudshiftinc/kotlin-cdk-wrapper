...
public interface DockerImageAssetSource {
     ...
     */
    public fun dockerBuildArgs(): Map<String, String> =
        unwrap(this).getDockerBuildArgs() ?: emptyMap()

    /**
     * Build secrets to pass to the `docker build` command.
     *
     * Since Docker build secrets are resolved before deployment, keys and values cannot refer to
     * unresolved tokens (such as `lambda.functionArn` or `queue.queueUrl`).
     *
     * Only allowed when `directoryName` is specified.
     *
     ...
     */
    public fun dockerBuildSecrets(): Map<String, String> =
     ...
     *
     * This hash is used throughout the system to identify this image and avoid duplicate work in
     * case the source did not change.
     *
    ...
    public interface Builder {
        ...
        /**
         * @param directoryName The directory where the Dockerfile is stored, must be relative to
         *   the cloud assembly root.
         */
        public fun directoryName(directoryName: String)

        /**
         * @param dockerBuildArgs Build args to pass to the `docker build` command. Since Docker
        ...
        /**
         * @param dockerBuildSecrets Build secrets to pass to the `docker build` command. Since
         *   Docker build secrets are resolved before deployment, keys and values cannot refer to
         *   unresolved tokens (such as `lambda.functionArn` or `queue.queueUrl`).
         *
         * Only allowed when `directoryName` is specified.
         */
        ...
        /**
         * @param networkMode Networking mode for the RUN commands during build. *Requires Docker
         *   Engine API v1.25+*. Specify this property to build images on a specific networking
         *   mode.
         */
         ...
         *   throughout the system to identify this image and avoid duplicate work in case the
         *   source did not change.
         *
         * NOTE: this means that if you wish to update your docker image, you must make a
         * modification to the source (e.g. add some metadata to your Dockerfile).
         */
    ...
    internal constructor(
        ...
        /**
         * The directory where the Dockerfile is stored, must be relative to the cloud assembly
         * root.
         *
         * Default: - Exactly one of `directoryName` and `executable` is required
         */
         ...
         * Default: - no build args are passed
         */
        override fun dockerBuildArgs(): Map<String, String> =
            unwrap(this).getDockerBuildArgs() ?: emptyMap()

        /**
         * Build secrets to pass to the `docker build` command.
         *
         * Since Docker build secrets are resolved before deployment, keys and values cannot refer
         * to unresolved tokens (such as `lambda.functionArn` or `queue.queueUrl`).
         *
         * Only allowed when `directoryName` is specified.
         *
         * Default: - no build secrets are passed
         */
        override fun dockerBuildSecrets(): Map<String, String> =
            unwrap(this).getDockerBuildSecrets() ?: emptyMap()
         ...
         * The hash of the contents of the docker build context.
         *
         * This hash is used throughout the system to identify this image and avoid duplicate work
    ...
    public companion object {
        ...
        internal fun unwrap(
            wrapped: DockerImageAssetSource
