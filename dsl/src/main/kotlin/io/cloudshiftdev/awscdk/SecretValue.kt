...
internal constructor(
     ...
     * Resolve the secret.
     *
     * If the feature flag is not set, resolve as normal. Otherwise, throw a descriptive error that
     * the usage guard is missing.
     *
     * @param context
     */
     ...
     *
     * Call this to indicate that you want to use the secret value held by this object in an
     * unchecked way. If you don't call this method, using the secret value directly in a string
     * context or as a property value somewhere will produce an error.
     *
     * This method has 'unsafe' in the name on purpose! Make sure that the construct property you
     * are using the returned value in is does not end up in a place in your AWS infrastructure
     * where it could be read by anyone unexpected.
     *
     * When in doubt, don't call this method and only pass the object to constructs that accept
     * `SecretValue` parameters.
     */
    ...
    public companion object {
        public fun cfnDynamicReference(ref: CfnDynamicReference): SecretValue =
            software.amazon.awscdk.SecretValue.cfnDynamicReference(
                    ref.let(CfnDynamicReference::unwrap)
                )
                .let(SecretValue::wrap)

        public fun cfnParameter(`param`: CfnParameter): SecretValue =
            software.amazon.awscdk.SecretValue.cfnParameter(`param`.let(CfnParameter::unwrap))
                .let(SecretValue::wrap)

        public fun isSecretValue(x: Any): Boolean =
        ...
        public fun secretsManager(
            ...
            software.amazon.awscdk.SecretValue.secretsManager(
                    secretId,
                    options.let(SecretsManagerSecretOptions::unwrap)
                )
                .let(SecretValue::wrap)
        ...
        public fun secretsManager(
            secretId: String,
            options: SecretsManagerSecretOptions.Builder.() -> Unit
        ): SecretValue = secretsManager(secretId, SecretsManagerSecretOptions(options))
        ...
        public fun ssmSecure(parameterName: String, version: String): SecretValue =
            software.amazon.awscdk.SecretValue.ssmSecure(parameterName, version)
                .let(SecretValue::wrap)
