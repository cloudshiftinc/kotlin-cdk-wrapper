...
public interface CfnJobProps {
   ...
   * available only for AWS Glue version 3.0 or later Spark ETL jobs in the following AWS Regions: US
   * East (Ohio), US East (N. Virginia), US West (N. California), US West (Oregon), Asia Pacific
   * (Mumbai), Asia Pacific (Seoul), Asia Pacific (Singapore), Asia Pacific (Sydney), Asia Pacific
   * (Tokyo), Canada (Central), Europe (Frankfurt), Europe (Ireland), Europe (London), Europe (Spain),
   ...
   * disk, and provides 1 executor per worker. We recommend this worker type for jobs whose workloads
   * contain your most demanding transforms, aggregations, joins, and queries. This worker type is
   * available only for AWS Glue version 3.0 or later Spark ETL jobs, in the same AWS Regions as
