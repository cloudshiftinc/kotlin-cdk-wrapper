 ...
 * instance to have a public IPv4 address. For more information, see [Connect to your instances using
 * EC2 Instance Connect
 * Endpoint](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/Connect-using-EC2-Instance-Connect-Endpoint.html)
 * in the *Amazon EC2 User Guide* .
 *
 * With the replacement update behavior, AWS CloudFormation usually creates the new resource first,
 * changes references to point to the new resource, and then deletes the old resource. However, you can
 * create only one EC2 Instance Connect Endpoint per VPC, so the replacement process fails. If you need
 * to modify an EC2 Instance Connect Endpoint, you must replace the resource manually.
 *
 * Example:
 *
 * ```
...
public open class CfnInstanceConnectEndpoint(
  ...
  public constructor(
    ...
    props: CfnInstanceConnectEndpointProps,
      ...
      this(software.amazon.awscdk.services.ec2.CfnInstanceConnectEndpoint(scope.let(CloudshiftdevConstructsConstruct.Companion::unwrap),
      id, props.let(CfnInstanceConnectEndpointProps.Companion::unwrap))
  )
  ...
  public open fun preserveClientIp(`value`: IResolvable) {
    unwrap(this).setPreserveClientIp(`value`.let(IResolvable.Companion::unwrap))
  }
