...
public open class CfnInstance internal constructor(
  ...
  public companion object {
    public val CFN_RESOURCE_TYPE_NAME: String =
        software.amazon.awscdk.services.lightsail.CfnInstance.CFN_RESOURCE_TYPE_NAME
  ...
  public interface PortProperty {
    ...
    /**
     * The IPv4 address, or range of IPv4 addresses (in CIDR notation) that are allowed to connect
     * to an instance through the ports, and the protocol.
     *
     *
    ...
    /**
     * The IPv6 address, or range of IPv6 addresses (in CIDR notation) that are allowed to connect
     * to an instance through the ports, and the protocol.
     *
     ...
     * delivery of streamed data between applications running on hosts communicating by an IP network.
     * If you have an application that doesn't require reliable data stream service, use UDP instead.
     ...
     * data stream service can use UDP, which provides a connectionless datagram service that
     * emphasizes reduced latency over reliability. If you do require reliable data stream service, use
     * TCP instead.
    ...
    public interface Builder {
       ...
       *
       * * `tcp` - Transmission Control Protocol (TCP) provides reliable, ordered, and error-checked
       * delivery of streamed data between applications running on hosts communicating by an IP
       * network. If you have an application that doesn't require reliable data stream service, use UDP
       * instead.
       * * `all` - All transport layer protocol types.
       * * `udp` - With User Datagram Protocol (UDP), computer applications can send messages (or
       * datagrams) to other hosts on an Internet Protocol (IP) network. Prior communications are not
       * required to set up transmission channels or data paths. Applications that don't require
       * reliable data stream service can use UDP, which provides a connectionless datagram service
       * that emphasizes reduced latency over reliability. If you do require reliable data stream
       * service, use TCP instead.
       * * `icmp` - Internet Control Message Protocol (ICMP) is used to send error messages and
       * operational information indicating success or failure when communicating with an instance. For
       * example, an error is indicated when an instance could not be reached. When you specify `icmp`
       * as the `protocol` , you must specify the ICMP type using the `fromPort` parameter, and ICMP
    ...
    public companion object {
      ...
      internal fun unwrap(wrapped: PortProperty):
          ...
          CdkObject).cdkObject as software.amazon.awscdk.services.lightsail.CfnInstance.PortProperty
    }
  }
